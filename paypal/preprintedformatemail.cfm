<cfif form.billname eq ''>
<h3>Please Select a billformat.</h3>
<cfabort>
</cfif>
<cfset dts = 'Netiquette2_i'>
<cfset target_arcust = 'Netiquette2_a.arcust'>
<cfset target_icagent = 'Netiquette2_a.icagent'>
<cfset target_currency = 'Netiquette2_a.currency'>
...
<cfswitch expression="#tran#">
	<cfcase value="RC"><cfset ptype=target_apvend><cfset fname=dts&"CBIL_"&tran&".cfr"></cfcase>
	<cfcase value="PR"><cfset ptype=target_apvend><cfset fname=dts&"CBIL_"&tran&".cfr"></cfcase>
	<cfcase value="INV">
		...
		<!---<cfif BilType eq "TAXINV">
			<cfset fname=dts&"CBIL_TAXINV.cfr">
          
          <cfelseif BilType eq 'PDFFC'>--->
          <cfset fname=dts&"CBIL_TAXINV(FC).cfr">
		<!---<cfelse>--->
			<cfset fname=dts&"CBIL_TAXINV2.cfr">
		<!---</cfif>--->
	</cfcase>
	<cfcase value="DO">
		<cfset ptype=target_arcust>
		<!---<cfif BilType eq "DO">---><cfset fname=dts&"CBIL_"&tran&".cfr">
        
        
       <!--- <cfelseif BilType eq "DO2">--->
        <cfset fname=dts&"CBIL_DO2.cfr">
        <!---<cfelse>---> 
		
		
		<cfset fname=dts&"CBil_PLIST.cfr"><!---</cfif>--->
	</cfcase>
	<cfcase value="CS"><cfset ptype=target_arcust><cfset fname=dts&"CBIL_"&tran&".cfr"></cfcase>
	<cfcase value="CN"><cfset ptype=target_arcust><cfset fname=dts&"CBIL_"&tran&".cfr"></cfcase>
	<cfcase value="DN"><cfset ptype=target_arcust><cfset fname=dts&"CBIL_"&tran&".cfr"></cfcase>
	<cfcase value="QUO">
	<cfif isdefined('BilType')>
	<cfset ptype=target_arcust><cfset fname=dts&"CBIL_"&tran&"2.cfr">
    <cfelse>
    <cfset ptype=target_arcust><cfset fname=dts&"CBIL_"&tran&".cfr">
    </cfif>
    </cfcase>
	<cfcase value="PO"><cfset ptype=target_apvend><cfset fname=dts&"CBIL_"&tran&".cfr"></cfcase>
	<cfcase value="SO">
<cfif isdefined('BilType')>
	<cfset ptype=target_arcust><cfset fname=dts&"CBIL_"&tran&"2.cfr">
...
<cfquery datasource='#dts#' name="getHeaderInfo">
	...
	ag.desp as agen_desp,ag.HP as agentHP,ag.commsion1 as agent_email,curr.currency,curr.currcode
...
<cfloop query="getHeaderInfo">
		...
		<cfif getHeaderInfo.rem0 eq "Profile">
			<cfif tran eq 'PR' or tran eq 'PO' or tran eq 'RC'>
				<cfquery datasource='#dts#' name="getCustAdd">
					select name,name2,add1,add2,add3,add4,attn,phone,fax,e_mail,contact from #target_apvend# where custno = '#getheaderInfo.custno#' 
				</cfquery>
			<cfelse>
				<cfquery datasource='#dts#' name="getCustAdd">
					select name,name2,add1,add2,add3,add4,attn,phone,fax,e_mail,contact from #target_arcust# where custno = '#getheaderInfo.custno#' 
				</cfquery>
			</cfif>
		<cfelse>
			<cfquery datasource='#dts#' name="getCustAdd">
				select b.name,b.name2,b.e_mail,b.contact,a.add1,a.add2,a.add3,a.add4, a.attn, a.phone, a.fax 
	...
	<cfif tran eq "PO">
		<cfquery name="getclient" datasource="#dts#">
			select custno,name,add1,add2,add3,add4,attn,contact,phone,fax,e_mail,business
			from #target_arcust#
			where custno='#getheaderinfo.rem7#'
		</cfquery>
		<cfif getclient.recordcount eq 0>
			<cfset getclient.custno="">
			<cfset getclient.name="">
			<cfset getclient.add1="">
			<cfset getclient.add2="">
			<cfset getclient.add3="">
			<cfset getclient.add4="">
			<cfset getclient.attn="">
			<cfset getclient.contact="">
			<cfset getclient.phone="">
			<cfset getclient.fax="">
			<cfset getclient.e_mail="">
			<cfset getclient.business="">
		</cfif>
		<cfquery name="InsertICBil_M" datasource="#dts#">
		  	Insert into r_icbil_m (B_Name, B_Name2, B_Add1, B_Add2, B_Add3, B_Add4, B_Attn, B_Tel, B_Fax, B_Email,B_Contact,
		  	D_Name, D_Add1, D_Add2, D_Add3, D_Add4, D_Attn, D_Tel, D_Fax,D_Email,D_Contact,
		  	...
		  	Net_bil, Taxp1, Taxamt_bil, CurrCode, CurrRate, Gross_bil, Deposit,
		  	C_Custno,C_Name,C_Add1,C_Add2,C_Add3,C_Add4,C_Email,C_Business,C_Contact,C_Tel,C_Department,C_Fax,C_NoOfEmployee,AgentEmail,AgentHP)
				
		  	values 
			(<cfqueryparam cfsqltype="cf_sql_char" value="#getCustAdd.name#">,<cfqueryparam cfsqltype="cf_sql_char" value="#getCustAdd.name2#">,'#jsstringformat(getCustAdd.add1)#','#jsstringformat(getCustAdd.add2)#',
		  	'#jsstringformat(getCustAdd.add3)#','#jsstringformat(getCustAdd.add4)#',<cfqueryparam cfsqltype="cf_sql_char" value="#getCustAdd.attn#">,
		  	'#jsstringformat(getCustAdd.phone)#', '#jsstringformat(getCustAdd.fax)#','#jsstringformat(getCustAdd.e_mail)#','#jsstringformat(getCustAdd.contact)#',
		  	'#jsstringformat(getDeliveryAdd.name)#','#jsstringformat(getDeliveryAdd.add1)#', '#jsstringformat(getDeliveryAdd.add2)#',
		  	'#jsstringformat(getDeliveryAdd.add3)#','#jsstringformat(getDeliveryAdd.add4)#','#jsstringformat(getDeliveryAdd.attn)#',
		  	'#jsstringformat(getDeliveryAdd.phone)#','#jsstringformat(getDeliveryAdd.fax)#','#jsstringformat(getDeliveryAdd.e_mail)#','#jsstringformat(getDeliveryAdd.contact)#',
			...
			'#jsstringformat(getheaderinfo.term)#','#jsstringformat(getheaderinfo.agenno)#',
		  	'#jsstringformat(getheaderinfo.rem1)#','#jsstringformat(getheaderinfo.rem2)#','#jsstringformat(getheaderinfo.currency)#','#jsstringformat(getheaderinfo.rem4)#',
			'#jsstringformat(getheaderinfo.rem5)#','#jsstringformat(getheaderinfo.rem6)#','#jsstringformat(getheaderinfo.rem7)#','#jsstringformat(getheaderinfo.rem8)#','#jsstringformat(getheaderinfo.rem9)#',
		  	'#jsstringformat(getheaderinfo.rem10)#','#jsstringformat(getheaderinfo.rem11)#','#jsstringformat(getheaderinfo.cs_pm_debt)#','#jsstringformat(getheaderinfo.grand_bil)#','#jsstringformat(getheaderinfo.disp1)#', 
			...
			'#jsstringformat(getheaderinfo.net_bil)#','#jsstringformat(xtaxp1)#','#jsstringformat(getheaderinfo.tax_bil)#','#jsstringformat(getheaderinfo.currcode)#', 
		  	...
		  	'#jsstringformat(getclient.custno)#','#jsstringformat(getclient.name)#','#jsstringformat(getclient.add1)#','#jsstringformat(getclient.add2)#',
		  	'#jsstringformat(getclient.add3)#','#jsstringformat(getclient.add4)#',<cfqueryparam cfsqltype="cf_sql_varchar" value="#getclient.e_mail#">,
		  	<cfqueryparam cfsqltype="cf_sql_varchar" value="#getclient.business#">,<cfqueryparam cfsqltype="cf_sql_varchar" value="#getclient.contact#">,
		  	<cfqueryparam cfsqltype="cf_sql_varchar" value="#getclient.phone#">,'#jsstringformat(getheaderinfo.rem11)#',<cfqueryparam cfsqltype="cf_sql_varchar" value="#getclient.fax#">,'#jsstringformat(getheaderinfo.rem10)#',
		  	'#getheaderinfo.agent_email#','#getheaderinfo.agentHP#'
		 	)
		</cfquery>
	<cfelse>
		<cfquery name="InsertICBil_M" datasource="#dts#">
		  	Insert into r_icbil_m (B_Name, B_Name2, B_Add1, B_Add2, B_Add3, B_Add4, B_Attn, B_Tel, B_Fax, B_Email,
		  	D_Name, D_Add1, D_Add2, D_Add3, D_Add4, D_Attn, D_Tel, D_Fax,D_Email,
		  	Custno, Refno,PONO,DONO,
		  	Date, Terms, Agent, Rem1, Rem2, Rem3, Rem4, 
		  	Rem5, Rem6, Rem7, Rem8, Rem9, Rem10, Rem11, Rem12, Total, Disp1, Disp2, Disp3, Disamt_bil, 
		  	Net_bil, Taxp1, Taxamt_bil, CurrCode, CurrRate, Gross_bil, Deposit,AgentHP,AgentDesp)
				
		  	values 
			(<cfqueryparam cfsqltype="cf_sql_char" value="#getCustAdd.name#">,<cfqueryparam cfsqltype="cf_sql_char" value="#getCustAdd.name2#">,'#jsstringformat(getCustAdd.add1)#','#jsstringformat(getCustAdd.add2)#',
		  	'#jsstringformat(getCustAdd.add3)#','#jsstringformat(getCustAdd.add4)#',<cfqueryparam cfsqltype="cf_sql_char" value="#getCustAdd.attn#">,
		  	'#jsstringformat(getCustAdd.phone)#', '#jsstringformat(getCustAdd.fax)#','#jsstringformat(getCustAdd.e_mail)#',
		  	'#jsstringformat(getDeliveryAdd.name)#','#jsstringformat(getDeliveryAdd.add1)#', '#jsstringformat(getDeliveryAdd.add2)#',
		  	'#jsstringformat(getDeliveryAdd.add3)#','#jsstringformat(getDeliveryAdd.add4)#','#jsstringformat(getDeliveryAdd.attn)#',
		  	'#jsstringformat(getDeliveryAdd.phone)#','#jsstringformat(getDeliveryAdd.fax)#','#jsstringformat(getDeliveryAdd.e_mail)#',
		  	'#jsstringformat(getheaderinfo.custno)#','#jsstringformat(getheaderinfo.refno)#','#jsstringformat(getheaderinfo.pono)#','#jsstringformat(getheaderinfo.dono)#','#jsstringformat(dateformat(getheaderinfo.wos_date,"yyyy-mm-dd"))#',
			'#jsstringformat(getheaderinfo.term)#','#jsstringformat(getheaderinfo.agenno)#',
		  	'#jsstringformat(getheaderinfo.rem1)#','#jsstringformat(getheaderinfo.rem2)#','#jsstringformat(getheaderinfo.currcode)#','#jsstringformat(getheaderinfo.rem4)#',
			'#jsstringformat(getheaderinfo.rem5)#','#jsstringformat(getheaderinfo.rem6)#','#jsstringformat(getheaderinfo.rem7)#','#jsstringformat(getheaderinfo.rem8)#','#jsstringformat(getheaderinfo.rem9)#',
		  	'#jsstringformat(getheaderinfo.rem10)#','#jsstringformat(getheaderinfo.rem11)#','#jsstringformat(getheaderinfo.cs_pm_debt)#','#jsstringformat(getheaderinfo.grand_bil)#','#jsstringformat(getheaderinfo.disp1)#', 
		  	'#jsstringformat(getheaderinfo.disp2)#','#jsstringformat(getheaderinfo.disp3)#','#jsstringformat(thisdisamt_bil)#',
			'#jsstringformat(getheaderinfo.net_bil)#','#jsstringformat(xtaxp1)#','#jsstringformat(getheaderinfo.tax_bil)#','#jsstringformat(getheaderinfo.currcode)#', 
		  	'#jsstringformat(getheaderinfo.currrate)#','#jsstringformat(getheaderinfo.gross_bil)#','#val(getheaderinfo.deposit)#','#getheaderinfo.agentHP#','#getheaderinfo.agen_desp#')
		</cfquery>
	</cfif>
	...
	<cfloop query="getbodyinfo">
		...
		<cfif tran neq "PO" and mylist1 neq "">
			<cfset iteminfo[4] = "* S/N "&mylist1>
		<cfelse>
			<cfset iteminfo[4] ="">
		</cfif>
		...
		<cfif tran eq "PO">
			<cfloop index="a" from="1" to="3">
				<cfif iteminfo[a] neq "" and iteminfo[a] neq "XCOST">
					<cfif info eq "">
						<cfset info = trim(wrap(iteminfo[a],18))>
					<cfelse>
						<cfset info = info&chr(10)&trim(wrap(iteminfo[a],18))>
					</cfif>
				</cfif>
			</cfloop>
		<cfelse>
		...
		<cfif info eq "">
			<cfset info=trim(wrap(getbodyinfo.desp,18))>
		</cfif>
		<cfset str = ListGetAt(info,1,chr(13)&chr(10))>
		<cfset recordcnt = ListLen(info,chr(13)&chr(10))>
		
		<cfif tran eq "PO">
			<cfset infob = trim(wrap(getbodyinfo.desp,18))>
			<cfif infob eq "">
				<cfset strb = "">
				<cfset recordcntb = 0>
			<cfelse>
				<cfset strb = ListGetAt(infob,1,chr(13)&chr(10))>
				<cfset recordcntb = ListLen(infob,chr(13)&chr(10))>
			</cfif>
			
			<cfquery name="InsertICBil_S" datasource='#dts#'>
		 		Insert into r_icbil_s (SRefno, No, ItemNo, Desp, Despa, SN_No, Unit, Qty, Price, Amt,dispec1, brem1,brem3,brem4)
				values ('#jsstringformat(getbodyinfo.refno)#','#j#','#jsstringformat(getbodyinfo.itemno)#',<cfqueryparam cfsqltype="cf_sql_char" value="#str#">,<cfqueryparam cfsqltype="cf_sql_char" value="#strb#">,'#getbodyinfo.brem1#',
				'#getbodyinfo.unit_bil#','#jsstringformat(getbodyinfo.qty_bil)#','#jsstringformat(getbodyinfo.price_bil)#','#jsstringformat(getbodyinfo.amt_bil)#',
				'#jsstringformat(getbodyinfo.disamt_bil)#','#jsstringformat(getbodyinfo.currentrow)#','#getbodyinfo.brem2#','#getbodyinfo.brem1#')
		  	</cfquery>
		  	<cfset j = j + 1>
		  	
		  	<cfif (recordcnt gt 1) or (recordcntb gt 1)>
				<cfif recordcnt gt recordcntb>
					<cfloop from="2" to="#recordcnt#" index="i">
						...
						<cfif recordcntb gte i>
							<cfset strb = ListGetAt(infob,i,chr(13)&chr(10))>
						<cfelse>
							<cfset strb ="">
						</cfif>
						...
						<cfquery name="InsertICBil_S" datasource='#dts#'>
			 				Insert into r_icbil_s (SRefno, No, Desp, Despa)
							values ('#jsstringformat(getbodyinfo.refno)#','#j#','#jsstringformat(str)#','#jsstringformat(strb)#')
			  			</cfquery>
						<cfset j = j + 1>
					</cfloop>
				<cfelseif recordcntb gt recordcnt>
					<cfloop from="2" to="#recordcntb#" index="i">
						<cfif recordcnt gte i>
							<cfset str = ListGetAt(info,i,chr(13)&chr(10))>
				            <cfset str1 = Replace(Left(str,1)," ","")>
		...
		<cfelse>
			<cfquery name="InsertICBil_S" datasource='#dts#'>
		 		Insert into r_icbil_s (SRefno, No, ItemNo, Desp, SN_No, Unit, Qty, Price, Amt,dispec1, brem1,brem4)
				values ('#jsstringformat(getbodyinfo.refno)#','#j#','#jsstringformat(getbodyinfo.itemno)#',<cfqueryparam cfsqltype="cf_sql_char" value="#str#">,'#mylist1#',
				...
				'#jsstringformat(getbodyinfo.disamt_bil)#','#jsstringformat(getbodyinfo.currentrow)#','#getbodyinfo.brem1#')
		  	</cfquery>
		  	<cfset j = j + 1>
			...
			<cfif recordcnt gt 1>
				<cfloop from="2" to="#recordcnt#" index="i">
					...
					<cfquery name="InsertICBil_S" datasource='#dts#'>
						...
						values ('#jsstringformat(getbodyinfo.refno)#','#j#',<cfqueryparam cfsqltype="cf_sql_char" value="#str#">)
		  			</cfquery>
					<cfset j = j + 1>
				</cfloop>
			</cfif>			  
		</cfif>
		<cfif getbodyinfo.recordcount neq getbodyinfo.currentrow and tran neq "PO">
			<cfquery name="InsertICBil_S" datasource='#dts#'>
...
<cfif tran eq "INV">
            ...
            <cfif getheaderinfo.rem8 neq '' and getheaderinfo.rem9 neq ''>
            <cfquery name="InsertICBil_S" datasource='#dts#'>
			 	Insert into r_icbil_s (SRefno, No,desp)
				values ('#jsstringformat(getbodyinfo.refno)#','#j#','Period From #getheaderinfo.rem8# To #getheaderinfo.rem9#')
			</cfquery>
			<cfset j = j + 1>
            </cfif>
		</cfif>
...
<cfreport template="#fname#" format="PDF"  query="MyQuery" filename="#filename1#" overwrite="yes"><!--- or "FlashPaper" or "Excel" or "RTF" --->
    ...
    <cfreportparam name="type" value="#getheaderinfo.type#">
</cfreport>

<!--- <cfif getheaderinfo.e_mail neq ""> --->
   <cfmail from="noreply@mynetiquette.com" to="#form.email#" type="html"
			subject="Payment Confirmation from Netiquette #getheaderinfo.type#-#getheaderinfo.refno#"
		> <cfmailparam file = "#expandpath('\paypal\#filename1#')#" >
Hi #getId.company#,       

	Thank you for your subscription!
Check the attachment for your Invoice.

<br />

 <p>&nbsp;</p>
            <p>If you require any assistance from our customer service center, you may call the following number:</p>
            <TABLE>
            <TR>
<TD style="text-align:right"><b>Singapore</b> :</TD><TD>  &nbsp;(+65) 6223 1157</TD>
</TR>
<tr>
<td style="text-align:right"><b>Malaysia</b>  :</td><td> (+603) 5632 2508</td>
</tr>
<tr>
<td style="text-align:right"><b>Hong Kong</b> :</td><td> (+852) 2242 6111</td>
</TR>
</TABLE>

<p>&nbsp;</p>
<p><img src="https://crm.netiquette.com.sg/upload/net_c/emailmarketing/campaign/Netiquette-transparrent1.png" height="35" width="150"/></p>
<p>Warmest Regards,</p><p><h4>Netiquette Software team<h4></p>
</cfmail>

   <cfmail from="noreply@mynetiquette.com" to="chonghow@mynetiquette.com;<!--- edwin@netiquette.com.sg --->" 
			subject="Paypal Payment #getheaderinfo.type#-#getheaderinfo.refno#"
		> <cfmailparam file = "#expandpath('\paypal\#filename1#')#" >
...
<cfoutput>
    <p>Thank you #getGSetup.compro# for registering.
    We have just sent you an email.</p>
</cfoutput>
<!--- <cfelse>
<cfform name="email" method="post" action="preprintedformatemail.cfm?tran=#url.tran#&nexttranno=#url.nexttranno#&BillName=#BillName#&doption=#doption#" >

Enter Email: <cfinput type="text" name="email" value="" maxlength="20" required="yes" message="Please Enter The Email">
<cfoutput>
