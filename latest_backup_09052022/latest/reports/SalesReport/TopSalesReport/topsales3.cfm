...
<cfswitch expression="#form.result#">
	<cfcase value="HTML">
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="2">
			...
			<cfquery name="getareadata" datasource="#dts#">
				...
				left join
				(select area,sum(invgross)-sum(discount)as suminvamt from artran where type = 'INV' and (void = '' or void is null)
				<cfif form.areafrom neq "" and form.areato neq "">
				...
				left join
				(select area,sum(invgross)-sum(discount)as sumcsamt from artran where type = 'CS' and (void = '' or void is null)
				<cfif form.areafrom neq "" and form.areato neq "">
					and area >='#form.areafrom#' and area <='#form.areato#'
				</cfif>
				<cfif trim(form.customerfrom) neq "" and trim(form.customerto) neq "">
				...
				left join
				(select area,sum(invgross)-sum(discount)as sumdnamt from artran where type = 'DN' and (void = '' or void is null)
				<cfif form.areafrom neq "" and form.areato neq "">
					and area >='#form.areafrom#' and area <='#form.areato#'
				</cfif>
				<cfif trim(form.customerfrom) neq "" and trim(form.customerto) neq "">
				...
				left join
				(select area,sum(invgross)-sum(discount)as sumcnamt from artran where type = 'CN' and (void = '' or void is null)
				<cfif form.areafrom neq "" and form.areato neq "">
					and area >='#form.areafrom#' and area <='#form.areato#'
				</cfif>
				<cfif trim(form.customerfrom) neq "" and trim(form.customerto) neq "">
	...
	<cfcase value="EXCEL">
		...
		<cffile action="write" nameconflict="overwrite" file="#HRootPath#\Excel_Report\#dts#\Area_Sales_Report_By-Type_#huserid#.xls" output="#tostring(data)#">
        <cfheader name="Content-Disposition" value="inline; filename=Area_Sales_Report_By-Type_#huserid#.xls">
		<cfcontent type="application/vnd.ms-excel" deletefile="yes" file="#HRootPath#\Excel_Report\#dts#\Area_Sales_Report_By-Type_#huserid#.xls">
	</cfcase>
</cfswitch>