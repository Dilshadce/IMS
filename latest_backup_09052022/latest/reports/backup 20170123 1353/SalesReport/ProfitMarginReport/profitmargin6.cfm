	...
	<cfcase value="HTML">
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="2">
			...
			<cfquery name="getsales" datasource="#dts#">
				select custno,name,sum(if(taxincl="T",amt-taxamt,amt)) as sumamt,sum(if(taxincl="T",amt_bil-taxamt_bil,amt_bil)) as sumamt_bil, sum(it_cos) as sumcost,(sum(if(taxincl="T",amt-taxamt,amt)) - sum(it_cos)) as profit, (((sum(if(taxincl="T",amt-taxamt,amt)) - sum(it_cos))/sum(if(taxincl="T",amt-taxamt,amt)))*100) as margin
				from ictran
	...
	<cfcase value="EXCEL">
		...
		<Worksheet ss:Name="Customer_Profit_Margin_Report">
  		<Table ss:ExpandedColumnCount="7" x:FullColumns="1" x:FullRows="1">
			...
			<cfquery name="getsales" datasource="#dts#">
				select custno,name,sum(if(taxincl="T",amt-taxamt,amt)) as sumamt, sum(it_cos) as sumcost,sum(if(taxincl="T",amt_bil-taxamt_bil,amt_bil)) as sumamt_bil,(sum(if(taxincl="T",amt-taxamt,amt)) - sum(it_cos)) as profit, (((sum(if(taxincl="T",amt-taxamt,amt)) - sum(it_cos))/sum(if(taxincl="T",amt-taxamt,amt)))*100) as margin
				from ictran
				where (type = 'inv' or type = 'cs' or type = 'dn') and (void = '' or void is null)
                 <cfif form.radio1 eq 'item'>
                and (linecode ='' or linecode is null)
       ...
       <!---Agent from Bill--->
        ...
        <cfheader name="Content-Disposition" value="inline; filename=Customer_Profit_Margin_#huserid#.xls">
	...
	<cfcase value="EXCEL WITH ADDITIONAL COST">
		...
		<Styles>
		 ...
		 <Table ss:ExpandedColumnCount="20" x:FullColumns="1" x:FullRows="1">
			...
			<cfquery name="getsales" datasource="#dts#">
				select custno,name,sum(if(taxincl="T",amt-taxamt,amt)) as sumamt,sum(if(taxincl="T",amt_bil-taxamt_bil,amt_bil)) as sumamt_bil, sum(it_cos) as sumcost,(sum(m_charge1+m_charge2+m_charge3+m_charge4+m_charge5+m_charge6+m_charge7)) as sumxcost,
...
<cfelse>
       <!---Agent from Bill--->
        ...
        <cfheader name="Content-Disposition" value="inline; filename=Customer_Xcost_Profit_Margin_#huserid#.xls">
