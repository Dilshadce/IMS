  ...
  <cfoutput>
    <table width="100%" border="0" cellspacing="0" cellpadding="2">
      ...
      <cfquery name="getsales" datasource="#dts#">
				select a.refno,a.type,a.fperiod,a.custno,<cfif isdefined('form.cbdiscount')>ifnull(sum(if(taxincl="T",amt-taxamt,amt)),0)-(select discount from artran where type=a.type and refno=a.refno) as sumamt<cfelse>sum(if(taxincl="T",amt-taxamt,amt)) as sumamt</cfif>,<cfif isdefined('form.cbdiscount')>ifnull(sum(if(taxincl="T",amt_bil-taxamt_bil,amt_bil)),0)-(select disc_bil from artran where type=a.type and refno=a.refno) as sumamt_bil<cfelse>sum(if(taxincl="T",amt_bil-taxamt_bil,amt_bil)) as sumamt_bil</cfif>, sum(it_cos) as sumcost,agenno from ictran as a
...
<cfelse>
    ...
    <Worksheet ss:Name="Bill_Profit_Margin_Report">
      <Table ss:ExpandedColumnCount="10" x:FullColumns="1" x:FullRows="1">
        ...
        <cfoutput>
          ...
          <cfset totalprofit = 0>
          <cfquery name="getsales" datasource="#dts#">
				select a.refno,a.type,a.fperiod,a.custno,<cfif isdefined('form.cbdiscount')>ifnull(sum(if(taxincl="T",amt-taxamt,amt)),0)-(select discount from artran where type=a.type and refno=a.refno) as sumamt<cfelse>sum(if(taxincl="T",amt-taxamt,amt)) as sumamt</cfif>,<cfif isdefined('form.cbdiscount')>ifnull(sum(if(taxincl="T",amt_bil-taxamt_bil,amt_bil)),0)-(select disc_bil from artran where type=a.type and refno=a.refno) as sumamt_bil<cfelse>sum(if(taxincl="T",amt_bil-taxamt_bil,amt_bil)) as sumamt_bil</cfif>, sum(it_cos) as sumcost,name from ictran a
				where (type = 'inv' or type = 'cs' or type = 'dn') and (void = '' or void is null)
                <cfif form.radio1 eq 'item'>
                and (linecode ='' or linecode is null)
                <cfelseif form.radio1 eq 'serv'>
  ...
  <cfheader name="Content-Disposition" value="inline; filename=Bill_Profit_Margin_#huserid#.xls">
  <cfcontent type="application/vnd.ms-excel" deletefile="yes" file="#HRootPath#\Excel_Report\#dts#\Bill_Profit_Margin_#huserid#.xls">
  </cfcase>
  <cfcase value="EXCEL AFTER PROVOSION DISCOUNT">
  <cfxml variable="data">
  <?mso-application progid="Excel.Sheet"?>
  <Workbook xmlns="urn:schemas-microsoft-com:office:spreadsheet" xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:x="urn:schemas-microsoft-com:office:excel" xmlns:ss="urn:schemas-microsoft-com:office:spreadsheet" xmlns:html="http://www.w3.org/TR/REC-html40">
    <Styles>
  ...
  <cfheader name="Content-Disposition" value="inline; filename=Bill_Profit_Margin-Discount_#huserid#.xls">
    ...
    <Worksheet ss:Name="Bill_Profit_Margin_Report">
      <Table ss:ExpandedColumnCount="10" x:FullColumns="1" x:FullRows="1">
        ...
        <cfoutput>
          ...
          <cfset totalxcost = 0>
          <cfquery name="getsales" datasource="#dts#">
				select refno,type,fperiod,custno,sum(if(taxincl="T",amt-taxamt,amt)) as sumamt, sum(it_cos) as sumcost,name,
  ...
  <cfheader name="Content-Disposition" value="inline; filename=Bill_Profit_Margin_Additional_#huserid#.xls">
