...
<cfquery name="getallservice" datasource="#dts#">
  select servi as servi 
  from icservi 
  where 0=0
  <cfif form.servicefrom neq "" and form.serviceto neq "">
	and servi >= '#form.servicefrom#' and servi <= '#form.serviceto#'
  </cfif>
  union all
  select itemno as servi 
  from icitem 
  where 0=0 
  and itemtype='SV'
  <cfif form.serviceItemFrom neq "" and form.serviceItemTo neq "">
	and itemno >= '#form.serviceItemFrom#' and itemno <= '#form.serviceItemTo#'
  </cfif>
</cfquery>
...
<cfquery name="getservice" datasource="#dts#">
  select * from ictran
  where fperiod >= '#form.periodfrom#' and fperiod <= '#form.periodto#' <cfif IsDefined('marktype')>and type in (#ListQualify(marktype,"'")#)</cfif> 
  <cfif itemlist neq ''>
