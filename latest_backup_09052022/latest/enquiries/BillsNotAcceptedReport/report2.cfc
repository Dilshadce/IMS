...
<cffunction name="listAccount" access="remote" returntype="struct">
	...
	<cfif IsDefined("form.sSearch") AND form.sSearch NEQ "">
		...
		<cfloop from="0" to="#form.iColumns-1#" index="i" step="1">	
			<cfif Evaluate('form.bSearchable_'&i) EQ "true">
				<cfset sWhere=sWhere&"aa."&Evaluate('form.mDataProp_'&i)&" LIKE ""%"&trim(form.sSearch)&"%"" OR ">
			</cfif>
		</cfloop>
	...
	<cfquery name="getFilteredDataSet" datasource="#dts#">
		SELECT SQL_CALC_FOUND_ROWS *
       	FROM ( 	SELECT a.frtype,a.frrefno,a.itemno, IFNULL (b.refno,'NOT FOUND') AS existbill 
				FROM iclink AS a
				LEFT JOIN (	SELECT refno,type,itemno
							FROM ictran) AS b 
				ON a.itemno = b.itemno
				AND a.type = b.type
				AND a.refno = b.refno
	 		  ) AS aa
        WHERE existbill = 'not found'
		#sWhere#
        #sOrder#
        #sLimit#
	</cfquery>
	...
	<cfquery name="getTotalDataSetLength" datasource="#dts#">
		...
		<cfloop query="getFilteredDataSet">	
			...
			<cfset data["frtype"]=frtype>
   			<cfset data["frrefno"]=frrefno>
