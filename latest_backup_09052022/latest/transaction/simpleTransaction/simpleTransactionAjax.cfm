...
<cfif IsDefined("form.action")>
	...
	<cfelseif action EQ "getItemInfo">
		...
		<cfset custno=trim(URLDecode(form.custno))>

		<cfquery name="getItemInfo" datasource="#dts#">
			SELECT itemno,desp,despa,comment,#itemPriceType# AS itemprice,price2,price3,price4,price5,price6,unit,'' AS lineCode,<cfif itemPriceType EQ 'price'>taxcode<cfelse>pur_taxcode</cfif> AS taxcode
			FROM icitem 
        ...
        <cfif getItemInfo.recordcount EQ 0>
            ...
            <cfquery name="getItemInfo" datasource="#dts#">
                SELECT servi AS itemno,desp,despa,'' AS comment,#tempItemPriceType# AS itemprice,#tempItemPriceType# as price2,#tempItemPriceType# as price3,#tempItemPriceType# as price4,#tempItemPriceType# as price5,#tempItemPriceType# as price6,'' AS unit,'SV' AS lineCode,'' as taxcode
                FROM icservi
        ...
        <cfset price = getItemInfo.price2>
        <cfelseif getpricelvl.pricelvl eq 3>
        <cfset price = getItemInfo.price3>
        <cfelseif getpricelvl.pricelvl eq 4>
        <cfset price = getItemInfo.price4>
        <cfelseif getpricelvl.pricelvl eq 5>
        <cfset price = getItemInfo.price5>
        <cfelseif getpricelvl.pricelvl eq 6>
        <cfset price = getItemInfo.price6>
        <cfelse>
        <cfset price = getItemInfo.itemprice>
        </cfif>
        <cfelse>
        <cfset price = getItemInfo.itemprice>
        </cfif>
        
        <cfset getItemInfo.itemprice = price >
        </cfif>
	...
	<cfelseif action EQ "updateItem">
        ...
        <cfset itemTaxIncluded_input = trim(form.itemTaxIncluded_input)>
		...
		<cfif trancode GT getLastTrancode.LastTrancode>
			<cfquery name="insertItem" datasource="#dts#">
				INSERT INTO ictrantemp (
                                         ...
                                         amt_bil,amt1_bil,userID,taxincl)
				VALUES
				(
                    ...
                    <cfqueryparam cfsqltype="cf_sql_varchar" value="#itemTaxIncluded_input#">
				)
			</cfquery>
		<cfelse>
			<cfquery name="updateItem" datasource="#dts#">
				...
				SET
                    ...
                    taxincl=<cfqueryparam cfsqltype="cf_sql_varchar" value="#itemTaxIncluded_input#">	
	...
	<cfelseif action EQ "updateTrancode">
		...
		<cfquery name="getItemList" datasource="#dts#">
        	SELECT trancode,linecode,itemno,desp,location,qty_bil,unit_bil,factor1,factor2,price_bil,dispec1,note_a,taxpec1,taxAmt_bil,amt_bil,taxincl
            FROM ictrantemp
            WHERE uuid = <cfqueryparam cfsqltype="cf_sql_varchar" value="#uuid#">
            ORDER BY trancode;
        </cfquery>
		<cfoutput>
        	<cfloop query="getItemList">
                ...
                <tr id="#trancode#" class="edit_tr last_edit_tr">
                    ...
                    <td class="itemTaxTD">
                    	<input type="checkbox" id="itemTaxIncluded_input_#trancode#" name="itemTaxIncluded_input_#trancode#" value="T" <cfif getItemList.taxincl eq 'T'>checked</cfif>>
		...
		<cftry>
            ...
            <cfoutput>
                ...
                <tr id="#trancode#" class="edit_tr last_edit_tr">
                    ...
                    <td class="itemTaxTD">
                    	<input type="checkbox" id="itemTaxIncluded_input_#trancode#" name="itemTaxIncluded_input_#trancode#" value="T">
                        <select id="itemTaxCode_input_#trancode#" name="itemTaxCode_input_#trancode#" class="form-control input-sm itemTaxCode">
                            <cfloop query="getTax">
	...
	<cfelseif action EQ "getReferenceNo">
        ...
        <cfif getRefNoSet.refnoused eq "1">
	...
	<cfelseif action EQ "getBillDetails">
		...
		<cfquery name="getItemList" datasource="#dts#">
			SELECT	trancode,linecode,itemno,desp,despa,comment,brem1,brem2,brem3,brem4,
                   	...
                   	dispec1,disamt,disamt_bil,
                   	note_a,taxpec1,taxamt_bil,
                   	amt,amt_bil,amt1,amt1_bil,taxincl
			FROM ictran
			WHERE type = <cfqueryparam cfsqltype="cf_sql_varchar" value="#type#">
            AND refno = <cfqueryparam cfsqltype="cf_sql_varchar" value="#refno#">
            ORDER BY trancode ;
