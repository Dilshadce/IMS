		...
		ajax:{
			...
			data:function(term,page){
				return{
					...
					term:term,
					viewownagent:viewownagent,
					huserid:huserid,
					target_icagent:target_icagent,
	...
	*/
	...
	$("form").on("select2-close",".itemno_input",function(e){
		...
		if(itemno_input=="createNewItem"){
			...
			dataString=dataString+"custno="+escape(encodeURIComponent(custno));
	...
	$("form").on("change",".itemTaxIncluded ",function(e){
		var trancode = $(this).closest("tr").attr("id");
		itemCalculation(trancode);
		totalCalculation();
		updateItem(trancode,'');	
...
function itemCalculation(trancode){
	...
	if($("#itemTaxIncluded_input_"+trancode).is(':checked')){
	var taxAmtBil_input=Number(amt_input*(taxpec1_input/(100+(taxpec1_input*1)))).toFixed(totalDecimalPoint);
	}
	else{
...
function totalCalculation(){
	...
	$(".edit_tr").each(function(index, element) {
		...
		if($("#itemno_label_"+trancode).val() != ''){
			...
			if(itemTax == 1 && !$("#itemTaxIncluded_input_"+trancode).is(':checked')){
				totalitemtaxadded=(parseFloat(totalitemtaxadded)+parseFloat($("#taxAmtBil_input_"+trancode).val())).toFixed(totalDecimalPoint)
			};
	...
	if(itemTax == 1){
		...
		totalTax=parseFloat(totalitemtaxadded);
	}
	else{
...
function updateItem(trancode,field){
	...
	if(itemTax == 1){
		...
		if($("#itemTaxIncluded_input_"+trancode).is(':checked')){
		var itemTaxIncluded_input='T';
		}
		else{
		var itemTaxIncluded_input='';
		}
	}
	else{
		var itemTaxIncluded_input='';
	...
	dataString=dataString+"qty_input="+qty_input+"&";
	dataString=dataString+"unitOfMeasurement_input="+escape(encodeURIComponent(unitOfMeasurement_input))+"&";
	...
	dataString=dataString+"itemTaxIncluded_input="+itemTaxIncluded_input+"&";
