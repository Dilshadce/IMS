...
<cfquery name="getPlacementDetailsQuery" datasource="#dts#">
    SELECT
        ...
        (SELECT attn FROM manpower_i.arcust WHERE custno = p.custno) as `Billing name`,
        ...
        DATE_ADD(a.assignmentslipdate,INTERVAL (SELECT term FROM manpower_i.artran WHERE a.invoiceno = artran.refno LIMIT 1) DAY) as `invoiceDue`,
    ...
    ORDER by a.empno,i.refno ASC,i.trancode ASC;
...
<cfswitch expression="#form.result#">
	<cfcase value="EXCEL">
		...
		<cfset tempCandidate = getPlacementDetailsQuery['candidate'][1] >
		<cfset grandBillAmount = 0>
		<cfset grandPayAmount = 0>
		<cfset grandGSTAmount = 0>
		<cfset grandTotalWithGst = 0>
				...
				<Worksheet ss:Name="View Bill Listing Report">
					<Table x:FullColumns="1" x:FullRows="1" ss:DefaultColumnWidth="54" ss:DefaultRowHeight="11.25">
						...
						<cfloop query="getPlacementDetailsQuery" >
							...
							<cfif tempCandidate neq #getPlacementDetailsQuery["candidate"][getPlacementDetailsQuery.currentRow]# >
								<Row>
									<Cell>
									</Cell>
									<Cell>
								...
								<cfset tempCandidate = #getPlacementDetailsQuery["candidate"][getPlacementDetailsQuery.currentRow ]# >

								<cfset grandBillAmount = grandBillAmount + totalBillAmount>
								<cfset grandPayAmount = grandPayAmount + totalPayAmount>
								<cfset grandGSTAmount = grandGSTAmount + totalGSTAmount>
								<cfset grandTotalWithGst = grandTotalWithGst + totalWithGST>
