...
<cfswitch expression="#form.result#">
	<cfcase value="EXCEL">
	   ...
	   <cfif form.periodfrom neq "" and form.periodto neq "">
		...
		<cfquery name="getagent" datasource="#dts#">
			select wos_date,refno,counter,sum(invgross) as invgross,sum(discount) as discount,sum(tax) as tax,sum(grand) as grand,sum(CS_PM_cash) as CS_PM_cash,sum(CS_PM_crcd) as CS_PM_crcd,sum(CS_PM_crc2) as CS_PM_crc2,sum(CS_PM_cheq) as  CS_PM_cheq,sum(CS_PM_vouc) as CS_PM_vouc, sum(deposit) as deposit,sum(CS_PM_dbcd) as CS_PM_dbcd,sum(CS_PM_cashcd) as CS_PM_cashcd,sum(roundadj) as roundadj,sum(m_charge1) as m_charge1 from artran
		...
		<cfloop query="getagent">
				...
				<Row ss:AutoFitHeight="0">
                    ...
                    <Cell ss:StyleID="s32"><Data ss:Type="Number">#val(getagent.deposit)#</Data></Cell>
								...
								<cfset totalmcharge = totalmcharge + val(getagent.m_charge1)>
	...
	<cfcase value="HTML">
		...
		<cfquery name="getagent" datasource="#dts#">
			select wos_date,refno,counter,sum(invgross) as invgross,sum(discount) as discount,sum(tax) as tax,sum(grand) as grand,sum(CS_PM_cash) as CS_PM_cash,sum(CS_PM_crcd) as CS_PM_crcd,sum(CS_PM_crc2) as CS_PM_crc2,sum(CS_PM_cheq) as  CS_PM_cheq,sum(CS_PM_vouc) as CS_PM_vouc,sum(CS_PM_dbcd) as CS_PM_dbcd,sum(CS_PM_cashcd) as CS_PM_cashcd, sum(deposit) as deposit,sum(roundadj) as roundadj,sum(m_charge1) as m_charge1 from artran
			where type='CS' and (void = '' or void is null)
			<cfif form.agentfrom neq "" and form.agentto neq "">
			and agenno >='#form.agentfrom#' and agenno <= '#form.agentto#'
			</cfif>
