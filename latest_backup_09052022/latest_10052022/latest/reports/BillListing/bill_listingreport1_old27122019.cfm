...
SELECT art.*,cust.arrem5,inv.sstno,inv.gstno,cust.arrem6,cust.invoiceformat,assign.branch,assign.batches,art.name,cust.arrem1 as location,
...
LEFT JOIN (SELECT * FROM ictran WHERE fperiod<>99 AND (void='' or void is null) AND brem3<>'' and type='#url.trancode#' GROUP BY brem6) ict
...
LEFT JOIN (
    ...
    WHERE 1=1
    <!---<cfif form.periodfrom neq "" and form.periodto neq "">
        AND payrollperiod between 
        <cfif left(form.periodfrom,1) eq 0>'#replace(form.periodfrom,'0','')#'<cfelse>'#form.periodfrom#'</cfif> 
        AND <cfif left(form.periodto,1) eq 0>'#replace(form.periodto,'0','')#'<cfelse>'#form.periodto#'</cfif> 
    </cfif>--->
    AND assignmentslipdate > #getgeneral.lastaccyear#
    ) assign
ON ict.brem6=assign.refno
LEFT JOIN placement p
ON p.placementno=assign.placementno
LEFT JOIN #replace(dts,'_i','_p')#.pmast pms
ON pms.empno=ict.brem5
LEFT JOIN (SELECT * FROM invaddress GROUP BY shortcode) inv
ON assign.branch=inv.shortcode
