...
<cfquery datasource="#dts#" name="gettran">
	select 
	a.*
	from artran as a<cfif (form.locationfrom neq "" and form.locationto neq "") or url.trancode eq "TR">, ictran as b</cfif>
	where 
	a.type='#url.trancode#' 
	and (a.void='' or a.void is null)
	<cfif (form.locationfrom neq "" and form.locationto neq "") or url.trancode eq "TR">
	...
	order by a.refno
		...
		<cfoutput>
			...
			<cfif form.refNoFrom neq "" and form.refNoTo neq "">
				<tr>
					<td colspan="11"><div align="center"><font size="1.5" face="Arial, Helvetica, sans-serif">Ref No From #form.refNoFrom# To #form.refNoTo#</font></div></td>
				</tr>
			</cfif>
			<cfif ndatefrom neq "" and ndateto neq "">
				<tr>
