	...
	<cffunction name="calculate_moving_average_cost">
		...
		<cfloop query="getitem">
			...
			<cfquery name="getictran" datasource="#arguments.dts#">
				select type,refno,(date_format(wos_date,'%Y-%m-%d')) as wos_date,trancode,itemcount,itemno,ifnull(qty,0) as qty,ifnull(if(taxincl="T",amt-taxamt,amt),0) as amt ,location
				from ictran 
			...
			<cfloop query="getictran">
				<cfswitch expression="#getictran.type#">
					...
					<cfdefaultcase>
                        ...
                        <cfoutput>
                        #location# #cumamt# - #cumqty# - #cost# <Br>
                        </cfoutput>
						
						<!--- Update Cost --->
                        <cfif getictran.type eq "TROU">
                        <cfquery name="update_cost" datasource="#arguments.dts#">
							update ictran set it_cos=(qty*#cost#)
							where type='TRIN' and refno='#getictran.refno#' and itemno='#getictran.itemno#' 
							and wos_date='#getictran.wos_date#' and itemcount='#getictran.itemcount#';
						</cfquery>
                        <cfelse>
						<cfquery name="update_cost" datasource="#arguments.dts#">
							update ictran set it_cos=(qty*#cost#)
		...
		<cfloop query="getitem">
			...
			<!--- Get Relevent Cost --->
			<cfquery name="getictran" datasource="#arguments.dts#">
				select type,refno,(date_format(wos_date,'%Y-%m-%d')) as wos_date,trancode,itemno,ifnull(qty,0) as qty,ifnull(if(taxincl="T",amt-taxamt,amt),0) as amt 
				from ictran 
