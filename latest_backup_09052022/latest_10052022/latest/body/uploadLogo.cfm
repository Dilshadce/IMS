<cfif IsDefined("url.action")>
	<cftry>
			<cfset thisImage = ExpandPath("/billformat/#LCASE(dts)#/companyLogo.jpg")>            
			<cffile 
            		action = "delete" 
                    file = "#thisImage#">
    <cfcatch type="any">
	...
	<script type="text/javascript">
        ...
        function confirmDelete(){
            if(confirm("Are you sure want to delete the company logo?")){
                document.getElementById('form').action = "/latest/body/uploadLogo.cfm?action=delete";
...
<cfoutput>
    ...
    <form role="form" id="form" name="form" action="/latest/body/uploadLogo.cfm" method="post" enctype="multipart/form-data" onSubmit="return validate();">
        <div class="container">
                    ...
                    <div class="col-sm-4">
                    	<div class="companyLogoImage">
                        	<cfif FileExists(thisImage)>
                    			<img src="/billformat/#dts#/companyLogo.jpg" alt="Company Logo" onDblClick="confirmDelete();"/>
                            </cfif>
                        </div>
                    </div>      
                </div>
