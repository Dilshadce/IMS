<cfcomponent>
    ...
    <cfset dts = trim(getdts.dts)>
        ...
        <cfquery name="listMatchedAccount" datasource="#dts#">
            SELECT companyID
			FROM main.useraccountlimit
			WHERE remark = 'ASIA'
			AND	companyID LIKE <cfqueryparam cfsqltype="cf_sql_varchar" value="%#term#%" />
            ORDER BY companyID
            LIMIT #start#,#limit#;
        </cfquery>
    ...
    <cfset dts = trim(getdts.dts)>
        ...
        <cfquery name="getSelectedAccount" datasource="#dts#">
            SELECT companyID
			FROM main.useraccountlimit
			WHERE remark = 'LIVE-ASIA'
            AND companyID=<cfqueryparam cfsqltype="cf_sql_varchar" value="#value#" />;
        </cfquery>
