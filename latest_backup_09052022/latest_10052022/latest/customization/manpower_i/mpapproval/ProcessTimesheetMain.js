// JavaScript Document
function declineremark(tmonth, placementno, start, end)
{
	var remarks = '';
	remarks = prompt('Please key in reason for cancelling.');
	if(remarks != null){ window.open('/latest/customization/manpower_i/mpapproval/TimesheetApprovalProcess.cfm?type=dec&pno='+escape(encodeURIComponent(placementno))+'&remarks='+remarks+'&id='+escape(encodeURIComponent(tmonth))+'&startdate='+escape(encodeURIComponent(start))+'&enddate='+escape(encodeURIComponent(end)),'_self');
...
$(document).ready(function(e) {
	...
	var resultTable=$('#resultTable')
		.dataTable({
				...
				{'aTargets':[13],'sTitle':'ACTION','mData':'empno','bSortable':false,'bVisible':true,'sWidth':'15%',				
					'mRender': function(data,type,full){
						...
						return '<span class="glyphicon glyphicon-ok btn btn-link" title="Process" '+
									...
									'onclick=" declineremark('+full.tmonth1+','+full.placementno+',\''+full.start+'\',\''+full.end+'\') ;"></span>';
					}
			...
			'sAjaxSource':'/latest/customization/manpower_i/mpapproval/ProcessTimesheetMain.cfc',
			'sServerMethod':'POST',
        	'sScrollX':'100%'
		})
		.fadeIn();
