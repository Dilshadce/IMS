...
<cfquery name="getForeignerEPFType" datasource="#dts#">
	SELECT id,description,
 CASE
 WHEN EEContributionType = "as per schedule" THEN EEContributionType
 WHEN EEContributionType = 'percent' THEN CONCAT(ROUND(EEContribution,1),"%")
 ELSE ROUND(EEContribution,1) END as EEContribution,

CASE WHEN ERContributionType = 'as per schedule' THEN ERContributionType
WHEN ERContributionType ='percent' THEN CONCAT(ROUND(ERContribution,1),'%')
else ROUND(ERContribution,1) END AS ERContribution,

CASE WHEN AmtBillToClientType = 'as per schedule' THEN AmtBillToClientType
WHEN AmtBillToClientType = 'percent' THEN CONCAT(ROUND(AmtBillToClient,1),"%")
ELSE ROUND(AmtBillToClient,1) END AS amtbilltoclient
 FROM manpower_i.foreignerEPFType;
</cfquery>
<cfif IsDefined("url.action")>
	...
	<cfelseif url.action EQ "update">
		...
		<cfset priceid = selectpricematrix.priceid>
		<cfset pricename = selectpricematrix.pricename>
		<cfset statutoryBillingType = selectpricematrix.statutoryBillingType >
	</cfif>
</cfif>
								...
								<table width="100%">
									<tr>
										...
										<td>
											...
											<input type='checkbox' name='statutoryBillingType'
											<cfif #statutoryBillingType# eq 'Y'>
												<cfoutput>
													CHECKED
												</cfoutput>
											</cfif>
											/> &nbsp; Include statutory Billing
										</td>
									</tr>
									...
									<cfloop query="getForeignerEPFType">
										<tr>
											<td align='right'>
												<input type="radio" name="foreignerEPFType" value="#getForeignerEPFType.id#" #getForeignerEPFType.id eq selectpricematrix.foreignerEPFType ? 'checked' : ''#>
											</td>
											<td>
												#getForeignerEPFType.description#
											</td>
											<td align='right'>
												#getForeignerEPFType.EEContribution#
											</td>
											<td align='right'>
												#getForeignerEPFType.ERContribution#
											</td>
											<td align='right'>
												#getForeignerEPFType.AmtBillToClient#
											</td>
										</tr>
									</cfloop>
									<tr><td colspan='4'>&nbsp;</td></tr>
									<tr>
										<td colspan="100%">
                                            ...
                                            concat("A-",shelf) as itemno, desp from icshelf order by length(shelf),shelf
													...
													<td>
														<select name="itemname" id="itemname" onchange="document.getElementById('itemname2').value=this.options[this.selectedIndex].id" style="width:150px">
															...
															<cfloop query="getallitem">
																<option value="#getallitem.itemno#" id="#getallitem.desp#">
																	#getallitem.itemno# - #getallitem.desp#
																</option>
															</cfloop>
															<option value="ALLAWEXC" id="All AW EXCEPT 0%">
																All AW EXCEPT 0%
															</option>
														</select>
													...
													<input type="hidden" class="pull-right form-control input-sm" style="text-align:right" id="payadminfee" name="payadminfee" value="" placeholder="Pay Admin Fee" />
													<input type="hidden" class="pull-right form-control input-sm" style="text-align:right" id="payadminfee" name="payadminfee" value="" placeholder="Pay Admin Fee" />
													<td>
									...
									<div class="row">
										<div class="col-sm-12">
											<div class="form-group">
												<label for="itemname" class="col-sm-1 pull-left">
													Item Name
												</label>
												<label for="itemname" class="col-sm-1 control-label">
													Payable
												</label>
