<cfcomponent>
    ...
    <!---	<cfset sWhere="">
        ...
        <cfquery name="getFilteredDataSet" datasource="#dts#">
            SELECT SQL_CALC_FOUND_ROWS id, doctype, client, associate, expirydate
            FROM #targetTable#
            <cfif IsDefined("form.sSearch") AND form.sSearch NEQ "">
                WHERE (
                    <cfloop from="0" to="#form.iColumns-1#" index="i" step="1"> 
        ...
        <cfloop query="getFilteredDataSet">	
			...
			<cfset data["id"]=id>
            <cfset data["docType"]=doctype>
            <cfset data["client"]=client>
            <cfset data["associate"]=associate>
            <cfset data["expirydate"]=DateFormat(expirydate, 'dd/mm/yyyy')>
