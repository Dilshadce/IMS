<cfif url.type1 eq "Add">
	...
	<cfquery name="getitembal" datasource="#dts#">
		select desp,despa,comment,ucost,price,price2,price3,price4,barcode,qtybf,minimum,fcurrcode,fucost,fprice<cfloop from="2" to="10" index="i">,fcurrcode#i#,fucost#i#,fprice#i#</cfloop>,category,packing,qty2,
		unit,graded,remark1,wpformula,wqformula,taxcode,itemtype
		<cfif lcase(hcomid) eq "ge_i" or lcase(hcomid) eq "gecn_i" or lcase(hcomid) eq "idi_i" or lcase(hcomid) eq "gwa_i" or lcase(hcomid) eq "gwachina_i" or lcase(hcomid) eq "saehan_i">
		,aitemno as productcode
 ...
 <cfquery name="getgsetuplocation" datasource="#dts#">
    ...
    <cfif getitembal.fcurrcode neq "" and getartran.currcode eq getitembal.fcurrcode>
        ...
        <cfif evaluate('getitembal.fcurrcode#i#') neq "" and getartran.currcode eq evaluate('getitembal.fcurrcode#i#')>
        <cfif tran eq "INV" or tran eq "DO" or tran eq "QUO" or tran eq "SO" or tran eq "CS">
        <cfset price = evaluate('getitembal.fprice#i#')>
		<cfelse>
        <cfset price = evaluate('getitembal.fucost#i#')>
		</cfif>
        </cfif>
