...
function generateSerialNo(){
	...
	if(document.itemform2.serialno.value == ""){
		...
		DWREngine._execute(_tranflocation, null, 'generateserialno', startserialno, encodeURIComponent(escape(itemno)), location, wos_date, qty, reply_generateSerialNo);
	}
}

function reply_generateSerialNo(status){
	var itemno = document.itemform.itemno.value;
	window.location = 'dsp_serialno_opening.cfm?itemno=' + encodeURIComponent(escape(itemno));
}
