...
<!---	<cfset sWhere="">
	...
	<cfquery name="getFilteredDataSet" datasource="#dts#">
		SELECT SQL_CALC_FOUND_ROWS attentionno,name,customerno,(select concat(name,name2) as name from #replace(dts,'_i','_a','all')#.arcust where custno=a.customerno) as custname
		FROM #targetTable# as a
		<cfif IsDefined("form.sSearch") AND form.sSearch NEQ "">
            WHERE (
                <cfloop from="0" to="#form.iColumns-1#" index="i" step="1"> 
                    <cfif Evaluate('form.bSearchable_'&i) EQ "true">
    ...
    <cfcatch>
    <cfquery name="getFilteredDataSet" datasource="#dts#">
		SELECT SQL_CALC_FOUND_ROWS attentionno,name,customerno,(select concat(name,name2) as name from arcust where custno=a.customerno) as custname
		FROM #targetTable# as a
		<cfif IsDefined("form.sSearch") AND form.sSearch NEQ "">
            WHERE (
                <cfloop from="0" to="#form.iColumns-1#" index="i" step="1"> 
	...
	<cfquery name="getTotalDataSetLength" datasource="#dts#">
		...
		<cfloop query="getFilteredDataSet">	
            ...
            <cfset data["customerno"]=customerno&" - "&custname>     
