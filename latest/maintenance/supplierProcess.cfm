...
<cfset words_id_list = "100,104,23,101">
...
<cfif IsDefined("url.action")>
			...
			<cftry>
				<cfquery name="createSupplier" datasource="#dts#">
					INSERT INTO #target_apvend# (custno,name,name2,comuen,agent,groupto,status,
                                                 ...
                                                 business,area,end_user,date,gst_act_date,gstappno,gst_app_datefrom,gst_app_dateto,gst_valid_period)
					VALUES
					(
	...
	<cfelseif url.action EQ "update">
		<cftry>
            ...
            <cfif hlinkams EQ "Y">
                ...
                <cfif checkgldata.recordcount NEQ 0>
                    <cfquery name="checkgldata" datasource="#dts1#">
                    	UPDATE gldata
                        SET
                        		desp = <cfqueryparam cfsqltype="cf_sql_char" value="#trim(form.name)#">, 
								desp2 = <cfqueryparam cfsqltype="cf_sql_char" value="#trim(form.name2)#">,
                                currcode = <cfqueryparam cfsqltype="cf_sql_varchar" value="#trim(form.currcode)#">,
                    			groupto = <cfqueryparam cfsqltype="cf_sql_char" value="#trim(form.groupto)#">
						WHERE accno = <cfqueryparam cfsqltype="cf_sql_varchar" value="#trim(form.custno)#">;
                    </cfquery>
                </cfif>
            </cfif>
			...
			<cfcatch type="any">
				<script type="text/javascript">
					alert('Failed to update #trim(form.custno)#!\nError Message: #cfcatch.message# : #cfcatch.Detail#');
