...
<cffunction name="listAccount" access="remote" returntype="struct">
    ...
    <cfset AMS_bills=valuelist(getamsbills.reference)>
	
	<cfquery name="getFilteredDataSet" datasource="#dts#">
		SELECT SQL_CALC_FOUND_ROWS wos_date,type,refno,fperiod,grand
        FROM artran 
        WHERE posted='P' 
        AND fperiod<>'99' 
        AND refno NOT IN (<cfqueryparam cfsqltype="cf_sql_varchar" list="yes" separator="," value="#AMS_bills#">)
        #sWhere#
        #sOrder#        
        #sLimit#
	</cfquery>
