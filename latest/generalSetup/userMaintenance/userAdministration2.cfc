...
<cffunction name="listAccount" access="remote" returntype="struct">
	...
	<cfquery name="getFilteredDataSet" datasource="main">  	
        ...
        <cfelseif husergrpid EQ "admin">
            SELECT * 
            FROM users 
            WHERE (userbranch = '#trim(dts)#' OR userID IN (SELECT userID FROM multicomusers WHERE FIND_IN_SET('#dts#',comlist)))
            	  AND usergrpid != 'super' 
    ...
    <cfloop query="getFilteredDataSet">	
        ...
        <cfset data["portalaccess"]=portalaccess> 
