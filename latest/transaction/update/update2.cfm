...
if(ckbx_arr[i].checked == true)
{  
qtychecked=(qtychecked*1)+1;
} 
}
document.getElementById('totalqtychecked').value=qtychecked;

}

function validateasaiki()
{
var ckbx_arr=document.getElementsByName('checkbox'); 
var ckbx_arr_ln=ckbx_arr.length;
...
if((document.updatepage.totalqty.value*1) < (qtychecked*1))
{
	alert("Serial No Selected more than Qty");
	return false;
}
else{
for(var i=0;i<ckbx_arr_ln;i++){ 
			...
			<cfelse>	
				...
				<cfloop from="1" to="#cnt#" index="i">
					...
					<tr onMouseOut="javascript:this.style.backgroundColor='';" onMouseOver="javascript:this.style.backgroundColor='99FF00';"> 			 
                        ...
                        <cftry>
							<cfset qtytoful = qtytoful* val(getupdate.factor2) / val(getupdate.factor1)>
                            <cfcatch>
                            </cfcatch>
                            </cftry>			
						...
						<td><input type="checkbox" name="checkbox" id="checkbox" value=";#refno#;#convertquote(xitemno)#;#trancode#" onClick="calculatechecked();"></td>
                ...
                <td colspan="2"><strong>Item Selected :</strong></td>					
                <cfif getdisplaysetup.update_unit eq 'Y'>
                    	<td></td>
                </cfif>
                <td><input type="text" name="totalqtychecked" id="totalqtychecked" value="" size="2" readonly></td>
                </tr>
				<tr>             
...
<cfelseif url.t2 eq "DO">
	...
	<cfif url.t1 eq "SO" or url.t1 eq "QUO" or url.t1 eq "SAM">
        ...
        <!---ASAIKI UPDATE SAM--->
        <cfif lcase(hcomid) eq "asaiki_i" and url.t1 eq "SO">
		<cfquery name="checkasaikipackinglist" datasource="#dts#">
        select * from ictran where category="HPM" and type='#t1#' and (<cfloop from="1" to="#cnt#" index="i"><cfif i neq 1> or </cfif>refno='#listgetat(mylist,i,";")#'</cfloop>)  and (shipped+writeoff) < qty order by trancode				
        </cfquery>
		<cfif checkasaikipackinglist.recordcount neq 0>
		<cfoutput><form action="asaikiupdate3.cfm?t1=#URLEncodedFormat(t1)#&t2=#URLEncodedFormat(t2)#&trancode=#URLEncodedFormat(trancode)#&invset=#URLEncodedFormat(invset)#" method="post" name="updatepage" onsubmit="return validateasaiki();">
		<cfset session.formName="updatepage"></cfoutput>
		<cfset totalserialrecord = 0>
        <cfset lastlist=0>
			<cfoutput><p>Last Delivery Order No : <font size="2">#getGeneralInfo.tranno#</font></p></cfoutput>
				...
				<cfloop from="1" to="#cnt#" index="i">
                    ...
                    <cfquery datasource="#dts#" name="gettotalqty">
                    Select ifnull(sum(qty),0)-ifnull(sum(shipped),0)-ifnull(sum(writeoff),0) as totalqty from ictran 
						where refno='#listgetat(mylist,i,";")#' and type='#t1#' and (shipped+writeoff) < qty 
                    </cfquery>
					
					<cfoutput query="getupdate">
                    <cfquery name="getserial" datasource="#dts#">
                    select serialno,ctgno,refno from iserial where refno='#getupdate.brem5#' and type='SAM' and location='#getupdate.location#' and itemno='#getupdate.itemno#' <!---and trancode='#getupdate.brem6#'---> and generated='' order by ctgno
                    </cfquery>
                    <tr>
                    <th colspan="3"><div align="left">Item No : #getupdate.itemno#</div></th>
                    </tr>
                    <cfloop query="getserial">						
						<tr> 			 
                            <td>#getserial.ctgno#</td>
	           				<td>#getserial.serialno#</td>
	                    	<td><input type="checkbox" name="checkbox" value=";#getupdate.refno#;#convertquote(getupdate.itemno)#;#getupdate.trancode#;#getserial.serialno#;#getserial.refno#"></td>
						</tr>	
					<cfset totalserialrecord = totalserialrecord+1>
                    </cfloop>
                    <tr>
                    <th colspan="3"><div align="center"><input type="button" name="checkallitem" value="Check all Above Item" onClick="javascript:for (i = #lastlist#; i < #totalserialrecord#; i++)
document.updatepage.checkbox[i].checked=true;"> Outstanding Qty : #getupdate.qty#</div></th>
                    </tr>
                    <tr>
                    <td colspan="3">&nbsp;</td>
                    </tr>
                    <cfset lastlist=totalserialrecord>
					</cfoutput> 
				</cfloop>
                <tr>
                        <th>Date</th>
                        <td colspan="4">
                            <cfoutput>
                            <input type="text" name="totalqty" id="totalqty" value="#gettotalqty.totalqty#" size="2" readonly>
                            <input style="background-color:##FFFF99" type="text" name="f_cdate" size="10" value="#dateformat(now(),"dd/mm/yyyy")#" maxlength="10"> </cfoutput>
                            <img src="/images/cal.gif" width=17 height=15 border=0 onClick="showCalendarControl(f_cdate);">(DD/MM/YYYY)
                        </td>
						</tr>
                        <tr>
                        <th>Total Serial</th>
                        <td colspan="4"><cfoutput>#totalserialrecord#</cfoutput></td>
                        </tr>
