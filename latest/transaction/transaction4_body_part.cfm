...
<cfoutput>
	<form name='form1' method='post' action='transaction4_insert_update_item.cfm' target="_blank" enctype="multipart/form-data" <cfif mode neq 'delete'>onsubmit='return validate()'</cfif>>
...
<input type='hidden' name='type1' value='#listfirst(mode)#'>
<input type='hidden' name='hmode' value='#listfirst(mode)#'>
...
<cfif (lcase(hcomid) eq "weikeninv_i" or lcase(hcomid) eq "weikenint_i" or lcase(hcomid) eq "weikenid_i" or lcase(hcomid) eq "futurehome_i" or lcase(hcomid) eq "weikenbuilder_i" or lcase(hcomid) eq "weikendecor_i" or lcase(hcomid) eq "netilung_i")>
    <cfquery datasource="#dts#" name="weikenprojectso">
    select sum(grand) as grand from artran where type="SO" and (void="" or void is null) and source="#xsource#"
    </cfquery>
    
    <cfquery datasource="#dts#" name="weikenprojectinv">
    select sum(grand) as grand from artran where type="INV" and (void="" or void is null) and source="#xsource#" and refno <>"#listfirst(nexttranno)#"
    </cfquery>
    
    <cfquery datasource="#dts#" name="weikenprojectinv2">
    select sum(amt) as amt from ictran where type="INV" and (void="" or void is null) and source="#xsource#" and refno ="#listfirst(nexttranno)#"
    </cfquery>
    
    <input type='hidden' name='weikensoproject' id="weikensoproject" value='#val(weikenprojectso.grand)#' readonly>
    <input type='hidden' name='weikeninvproject' id="weikeninvproject" value='#val(weikenprojectinv.grand)+val(weikenprojectinv2.amt)#' readonly>
</cfif>

<table align='center' class='data' width='85%' cellspacing="0">
	<tr>
		<th colspan='6'>Item Detail</th>
    </tr>
    <tr>
    <td colspan="4">
    Item Code<br />
    ...
    <cfelseif lcase(hcomid) eq "hodaka_i" and (tran eq "INV" or tran eq "CS" )>
    <input type='text' name='itemno' id="itm6" value='#convertquote(itemno)#' readonly>
    ...
    <tr>
    <td><cftooltip autodismissdelay="60000" sourcefortooltip="/default/transaction/itembal2.cfm?itemno=#URLENCODEDFORMAT(itemno)#&project=&job=&batchcode=">Balance on Hand</cftooltip></td>
    ...
    <tr>
    <td><cftooltip autodismissdelay="60000" sourcefortooltip="/default/transaction/reservedqty.cfm?itemno=#URLENCODEDFORMAT(itemno)#&project=&job=&batchcode=">Available Qty</cftooltip></td>
    <td><input name='reserveqty2' id="reserveqty2" type='text' value="#listfirst(balonhand)-getreserveqty.reserveqty#" size='10' maxlength='10' readonly>
            <input name='reserveqty' id="reserveqty" type='hidden' value="#getreserveqty.reserveqty#" size='10' maxlength='10' readonly></td>
    </tr>
    <tr>
    <td><cfif lcase(hcomid) eq "marquis_i" or lcase(HcomID) eq "hempel_i">PKG<cfelse>Qty</cfif>
    ...
    <!---check minimum selling price--->
        ...
        <cfif checkcustom.minimum_selling_price eq "Y" and (tran neq 'PO' and tran neq 'RC' and tran neq 'RQ' and tran neq 'PR') and (checkcustom.minimum_selling_price1 eq "" or ListFindNoCase(checkcustom.minimum_selling_price1,tran))>
		...
		<cfelse>
            ...
            <input name='batchqty' type="hidden" size='10' maxlength='10' value=''>
			<input name='oldqty' id="oldqty" type='hidden' size='10' maxlength='10' value=''>
				...
				<cfelse>
                    ...
                    <cfif getmodule.batchcode eq 1>
                        ...
                        <cfif lcase(hcomid) neq "visionlaw_i" and lcase(hcomid) neq "mastercare_i" and lcase(hcomid) neq "gorgeous_i">
                            <input type="button" name="submit" value="Select #getGeneralInfo.lbatch# Code" onclick="selectBatch();">
                            </cfif>
                        </cfif>
            </cfif>
    ...
    <tr>
    <td><cfif lcase(hcomid) eq "marquis_i" or lcase(HcomID) eq "hempel_i">Qty<cfelse>Amount</cfif></td>
    <td>
    ...
    <td></td>
    <td>
        <input name='dispec3' id="dis36" tabindex="10" type='text' value='#numberformat(dispec3,stDecl_Discount)#' validate='float' size='10' maxlength='7' required='yes' message='Please input a value for Discount 2(%)(0-100).' <cfif lcase(hcomid) eq "gel_i" and husergrpid neq "Admin" and husergrpid neq "Super">readonly</cfif> <cfif getGeneralInfo.itemdiscmethod eq 'byprice'>onKeyUp="getDiscountControl3();<cfelse>onKeyUp="getDiscountControl();</cfif>">
    </td>
    </tr>
    ...
    <td>Disc(Amount)</td>
    <td>
        <input name='discamt' id="discamt_id" tabindex="10" type='text' value='#numberformat(discamt,stDecl_Discount)#' validate='float' size='10' maxlength='7' required='yes' message='Please input a value for Discount 2(%)(0-100).' <cfif lcase(hcomid) eq "gel_i" and husergrpid neq "Admin" and husergrpid neq "Super">readonly</cfif> <cfif getGeneralInfo.itemdiscmethod eq 'byprice'>onKeyUp="getDiscountControl3();<cfelse>onKeyUp="getDiscountControl();</cfif>">
    </td>
    </tr>
    <cfif wpitemtax eq "Y">
    <tr>
	...
	Tax included <input type="checkbox" name="taxincl" id="taxincl" value="T" onclick="getTax1();" <cfif isdefined('getitem.taxincl')><cfif getitem.taxincl eq "T"> checked="checked"</cfif></cfif>  />
    </td>
    </tr>
    <tr>
            <th>Tax (%)</th>
            <td><cfquery name="select_tax" datasource="#dts#">
               ...
               <input name="taxamt_bil" id="taxamt_bil" tabindex="24" type="text" value="#numberformat(taxamt_bil,stDecl_Discount)#" size="10" maxlength="10">
               </td>
    ...
    <tr>
    <td colspan="4">
	<cfif lcase(HcomID) eq "asaiki_i">Supplier Part Number<cfelse>#getgenerallayer.ldescription#</cfif><br />
    <input name='desp' id="dep5" tabindex="1" type='text' value="#convertquote(desp)#" size='60' maxlength='60'><br />
	...
	<tr>
        ...
        <tr>
			...
			<select name='sv_part' id="sv5">
          	...
          	<td><input type='text' name='sercost' id="sc5" maxlength='14' value='#numberformat(sercost,"_.__")#' size='14'></td>
        </tr>
	<cfelse>
    ...
    select fcurrcode from icitem where itemno='#itemno#'
        ...
        <td colspan="4">Currency Code: #getfcurrcode.fcurrcode#</td>
    </tr>
	</cfif>
    <tr>
    	<td>U.O.M.<br />
    ...
    <tr>
    <td colspan="4"><input type="hidden" name="oldlocation" value="#xlocation#">
    <cfif lcase(hcomid) eq "mhca_i">Marketer<cfelseif lcase(hcomid) eq "nikbra_i">Dept Code<cfelse>Location</cfif><br />
            ...
            <cfif (lcase(hcomid) eq "kjcpl_i" or lcase(hcomid) eq "mlpl_i" or lcase(hcomid) eq "viva_i" or lcase(hcomid) eq "kjctrial_i" or lcase(hcomid) eq "tlxil_i") and (HUserGrpID neq 'Cust Svr' and HUserGrpID neq 'Van Sales')>
            <cfelse>
			...
			</cfif>&nbsp;&nbsp;<a onClick="ColdFusion.Window.show('createlocationAjax');" onMouseOver="this.style.cursor='hand';" >Create New Location</a>
    </td>
    </tr>
    </cfif>
    ...
    <cfif lcase(hcomid) eq "glenn_i" or lcase(hcomid) eq "glenndemo_i">
        ...
        <cfif trim(getGeneralInfo.bodyremark1list) eq ''>
            <input type='text'  name='requestdate' id="req3" maxlength='40' value='#convertquote(brem1)#' size='20' <cfif lcase(Hcomid) eq "probulk_i">onKeyUp="calculateAmt(this.value);"</cfif> <cfif getGeneralInfo.datepickerbrem1 eq "Y">readonly</cfif>>
            
            <cfif getGeneralInfo.datepickerbrem1 eq "Y">
            <img src="/images/cal.gif" width=17 height=15 border=0 onClick="showCalendarControl(requestdate);">
            </cfif>
            <cfelse>
            <select name="requestdate" id="req3">
    ...
    <cfelseif lcase(hcomid) eq "atc2005_i">
        ...
        <cfelseif lcase(hcomid) eq "bsssb_i" or lcase(hcomid) eq "puss_i" or lcase(hcomid) eq "rvsb_i" or lcase(hcomid) eq "hnm_i">
        <input type='text' onkeyup="checkqtybsssb();" name='crequestdate' id="creq4"  maxlength='40' value='#convertquote(brem2)#' size='20'>
        <cfelse>
        <cfif trim(getGeneralInfo.bodyremark2list) eq ''>
            <input type='text' <cfif lcase(hcomid) eq "asiasoft_i">onkeyup="getProfit();"</cfif> name='crequestdate' id="creq4"  maxlength='40' value='<cfif (lcase(hcomid) eq "marquis_i" or lcase(HcomID) eq "hempel_i") and isdefined('form.importpermit')>#form.importpermit#<cfelseif lcase(hcomid) eq 'fixics_i'>#getitembal.qty2#<cfelse>#convertquote(brem2)#</cfif>' <cfif getGeneralInfo.datepickerbrem2 eq "Y">readonly</cfif> size='20'>
            
            <cfif getGeneralInfo.datepickerbrem2 eq "Y">
            <img src="/images/cal.gif" width=17 height=15 border=0 onClick="showCalendarControl(crequestdate);">
            </cfif>
    ...
    <cfif lcase(hcomid) eq "aimpest_i"><textarea name='brem3' id="br36" size='20' rows="3">#convertquote(brem3)#</textarea> 
        ...
        <cfelse> 
          ...
          <cfif trim(getGeneralInfo.bodyremark3list) eq ''>
            <input type='text' name='brem3' id="br36" maxlength='40' value='#convertquote(brem3)#' size='20'<cfif lcase(hcomid) eq "accord_i">onblur='JavaScript:changebrem4(this.value);'</cfif> <cfif getGeneralInfo.datepickerbrem3 eq "Y">readonly</cfif>>
            <cfif getGeneralInfo.datepickerbrem3 eq "Y">
            <img src="/images/cal.gif" width=17 height=15 border=0 onClick="showCalendarControl(brem3);">
            </cfif>
    ...
    <!---Body Remark 4--->
    <td>
    <cfif lcase(hcomid) eq "aimpest_i"><textarea name='brem4' id="br46" size='20' rows="3">#convertquote(brem4)#</textarea>
        <cfelse>
        
        <cfif trim(getGeneralInfo.bodyremark4list) eq ''>
            <input type='text' name='brem4' id="br46" maxlength='40' value='#convertquote(brem4)#' size='20' <cfif lcase(hcomid) eq "litelab_i" and husergrpid neq 'admin'>readonly</cfif> <cfif getGeneralInfo.datepickerbrem4 eq "Y">readonly</cfif>>
            <cfif getGeneralInfo.datepickerbrem4 eq "Y">
            <img src="/images/cal.gif" width=17 height=15 border=0 onClick="showCalendarControl(brem4);">
            </cfif>
            <cfelse>
    ...
    <tr>
			...
			<td>
            Packing<br />
            <input type='text' name='packing' id="packing" maxlength='12' value='#convertquote(packing)#' size='20'></td>
            <cfif tran eq "CN" or (lcase(hcomid) eq "mastercare_i" or lcase(hcomid) eq "gorgeous_i")>
            <td <cfif getdisplaysetup2.body_initial neq "Y">style="visibility:hidden"</cfif>>
            Initial Cost<br />
            <input type="text" name="it_cos" id="it_cos" value="#it_cos#" />
            ...
            <td>
            <cfif lcase(hcomid) eq "ltm_i">Supplier<cfelse>Why Free?</cfif>
    ...
    <tr>
        <th <cfif getdisplaysetup2.body_nodisplay neq "Y">style="visibility:hidden"</cfif>><input type='checkbox' name='nodisplay' id="nodisplay" value='Y' <cfif lcase(hcomid) eq "bsssb_i" or lcase(hcomid) eq "puss_i" or lcase(hcomid) eq "rvsb_i" or lcase(hcomid) eq "hnm_i">onclick="bsssbnodisplay();"</cfif> <cfif nodisplay eq "Y">checked</cfif>> No Display </th>
        <input type="hidden" name="supp" value="#supp#" />
		<input type='hidden' name='shelf' id="shelf" value='#convertquote(shelf)#'>
        <th>
        <input type='checkbox' name='totalupdisplay' id="totalupdisplay" value='Y' <cfif totalupdisplay eq "Y">checked</cfif>> Total Up
        </th>
        <th><input type='checkbox' name='deductableitem' id="deductableitem" value='Y' <cfif deductableitem eq "Y">checked</cfif>> Deductable Item</th>
    </tr>
	<tr>
    <th>DO NO<br />
    <input type="text" name="dono" id="dono" value="#dono#" />
    </th>
	<th <cfif getdisplaysetup2.body_pono neq "Y">style="visibility:hidden"</cfif>>PO No.<br />
    <input type='text' name='pono' id="pono" maxlength='40' value="#pono#" size='20'>
    </th>
    
    <cfif tran eq 'PO'>
	        <th <cfif getdisplaysetup2.body_requiredate neq "Y">style="visibility:hidden"</cfif>>Require Date<br />
            <input type='text' name='requiredate' id="requiredate" maxlength='40' value="#requiredate#" size='10' onKeyUp="DateFormat(this,this.value,event,false,'3');" onBlur="DateFormat(this,this.value,event,true,'3');">	<img src="/images/cal.gif" width=17 height=15 border=0 onClick="showCalendarControl(requiredate);"></th>
			<th <cfif getdisplaysetup2.body_replydate neq "Y">style="visibility:hidden"</cfif>>Reply Date<br />
            <input type='text' name='replydate' id="replydate" maxlength='40' value="#replydate#" size='10' onKeyUp="DateFormat(this,this.value,event,false,'3');" onBlur="DateFormat(this,this.value,event,true,'3');"> <img src="/images/cal.gif" width=17 height=15 border=0 onClick="showCalendarControl(replydate);">		</th>
    <cfelse>
    ...
    <cfif tran eq 'SO'>
	        <th <cfif getdisplaysetup2.body_deliverydate neq "Y">style="visibility:hidden"</cfif>>Delivery Date<br />
            <input type='text' name='deliverydate' id="deliverydate" maxlength='40' value="#deliverydate#" size='10' onKeyUp="DateFormat(this,this.value,event,false,'3');" onBlur="DateFormat(this,this.value,event,true,'3');"> <img src="/images/cal.gif" width=17 height=15 border=0 onClick="showCalendarControl(deliverydate);">
            </th>
     <cfelse>
    ...
    <tr>
        ...
        <th>
           	<cfif getmodule.job eq "1">#getGeneralInfo.lJOB#<br /></cfif>
            <cfif getmodule.job neq "1"><div style="visibility:hidden"></cfif>
            <cfif getGeneralInfo.jobbyitem eq "Y" >
		...
		<a onClick="ColdFusion.Window.show('createJobAjax');" onMouseOver="this.style.cursor='hand';" >Create New Job</a>
            </cfif>
    		</th>
        </cfif>	
    </tr>
    <tr <cfif getdisplaysetup2.body_title neq "Y">style="visibility:hidden"</cfif>>
    <td colspan="4">
    Title <br />
    ...
    <tr>
    <td align="right"><cfif mode eq 'Edit'><input type='submit' tabindex="16" name='Submit' value='  Save  '><cfelse> <input type='submit' tabindex="16" name='Submit' value='  #mode#  '> </cfif></td>
    </tr>
</table>
</form>
...
<script type="text/javascript">
<cfif lcase(hcomid) eq "topsteelhol_i" or lcase(hcomid) eq "topsteel_i">checkqtytopsteel();<cfelseif lcase(hcomid) eq "bsssb_i" or lcase(hcomid) eq "puss_i" or lcase(hcomid) eq "rvsb_i" or lcase(hcomid) eq "hnm_i">checkqtybsssb();<cfelse><cfif getGeneralInfo.editamount eq '1' or getGeneralInfo.itemdiscmethod eq 'byprice'><cfelse><cfif url.type1 eq "Add" and checkpromotion.recordcount neq 0 and checkpromotion.type eq "buy">buypromo();</cfif>checkqty();</cfif></cfif>
