    ...
    <cfquery name="gettaxmethod" datasource="#dts#">
     ...
     <cfif gettaxmethod.billtaxable eq "Y">
            ...
            <cfquery datasource='#dts#' name="updateictran">
                ...
                taxamt=(round((round((coalesce(amt_bil,'0')*-1)+0.000001,2) * #form.taxper#/100)+0.000001,2)*-1),
                taxamt_bil=(round((round((coalesce(amt_bil,'0')*-1)+0.000001,2) * #form.taxper#/100)+0.000001,2)*-1)
                <cfelse>
                taxamt=round((round(coalesce(amt_bil,'0')+0.000001,2) * #form.taxper#/100)+0.000001,2),
                taxamt_bil=round((round(coalesce(amt_bil,'0')+0.000001,2) * #form.taxper#/100)+0.000001,2)
                </cfif>                    
                where type='#tran#'
                and uuid="#uuid#"
            </cfquery>
