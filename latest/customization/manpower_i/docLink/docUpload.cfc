<cfcomponent>
    ...
    <!---	<cfset sWhere="">
        ...
        <cfquery name="getFilteredDataSet" datasource="#dts#">
            SELECT SQL_CALC_FOUND_ROWS id, name, desp, path, uploaded_by, uploaded_on
            FROM #targetTable#
            WHERE uuid = "#form.docUuid#"
            <cfif IsDefined("form.sSearch") AND form.sSearch NEQ "">
                AND (
                    <cfloop from="0" to="#form.iColumns-1#" index="i" step="1"> 
                        <cfif Evaluate('form.bSearchable_'&i) EQ "true">
        ...
        <cfloop query="getFilteredDataSet">	
            ...
            <cfset data["path"]=path>
            <cfset data["uploaded_by"]=uploaded_by>
            <cfset data["uploaded_on"]=DateTimeFormat(uploaded_on, 'dd/mm/yyyy HH:nn:ss')>
