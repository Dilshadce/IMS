...
<cfswitch expression="#form.result#">
	<cfcase value="EXCEL">
        ...
        <cfif isdefined('form.fieldglass') eq true>
			...
			"EPF","SOCSO","EIS","Medical","Equipment Changes","Sub-total","",
			"","","",
			"OT1.5(QTY)","OT1.5(RM)",
			"OT2.0(QTY)","OT2.0(RM)",
			"OT3.0(QTY)","OT3.0(RM)",
			...
			<Table x:FullColumns="1" x:FullRows="1" ss:DefaultColumnWidth="54" ss:DefaultRowHeight="11.25">
				...
				<cfloop query="getCostingQuery" >
                    ...
                    <cfif isdefined('PUIG') eq true>
                        <cfset subtotal1 = getCostingQuery.custsalary + getCostingQuery.custcpf + getCostingQuery.custsdf + getCostingQuery.custeis + getCostingQuery.medical />
                    <cfelse>
                        <cfset subtotal1 = getCostingQuery.custsalary + getCostingQuery.adminfee + getCostingQuery.custcpf + getCostingQuery.custsdf + getCostingQuery.custeis + getCostingQuery.medical+ getCostingQuery.equip />
                    </cfif>					
					<cfscript>
								...
								if(getCostingQuery.arrem5 == "GSTITEMIZED" || (year(getCostingQuery.completedate)>2018 && getCostingQuery.arrem5 == "GSTBILLRATE" && getCostingQuery.arrem7 == 1) || (month(getCostingQuery.completedate)>=9 && year(getCostingQuery.completedate)==2018 && getCostingQuery.arrem5 == "GSTBILLRATE" && getCostingQuery.arrem7 == 1)){
                                    ...
                                    if(isdefined('PUIG') eq true){
                                        nonTaxable = subtotal1 + OTTotal - getCostingQuery.medical + val(getCostingQuery.lvltotaler1) + val(getCostingQuery.lvltotaler2);
                                    }else{
                                        nonTaxable = subtotal1 + OTTotal - getCostingQuery.adminfee - getCostingQuery.medical + val(getCostingQuery.lvltotaler1) + val(getCostingQuery.lvltotaler2);
                                    }
