...
<cfswitch expression="#form.result#">
	<cfcase value="EXCEL">
		...
		<Table ss:ExpandedColumnCount="25" x:FullColumns="1" x:FullRows="1">
			...
			<cfquery name="getactualsales" datasource="#dts#">
				...
				left join
				(select itemno,sum(qty)-sum(cnqty) as sumqty,sum(if(taxincl="T",amt-taxamt,amt))-sum(cnamt) as sumamt,sum(it_cos) as sumcost,wos_group
				...
				<cfif trim(form.productfrom) neq "" and form.productto neq "">
        ...
        <cfheader name="Content-Disposition" value="inline; filename=Product_Profit_Margin_#huserid#.xls">
	...
	<cfcase value="EXCEL BY GROUP">
		...
		<Table ss:ExpandedColumnCount="10" x:FullColumns="1" x:FullRows="1">
			...
			<Row ss:AutoFitHeight="0" ss:Height="23.0625">
				<Cell ss:StyleID="s32"><Data ss:Type="String">Product No.</Data></Cell>
                <cfif getdisplaydetail.report_aitemno eq 'Y'>
                <Cell ss:StyleID="s32"><Data ss:Type="String">Product Code.</Data></Cell>
                </cfif>
                
				<Cell ss:StyleID="s32"><Data ss:Type="String">Product Description</Data></Cell>
			...
			<cfloop query="getgroup">
				...
				<cfloop query="getitem">
					<cfquery name="getactualsales" datasource="#dts#">
						select sum(qty)-sum(cnqty) as sumqty, sum(if(taxincl="T",amt-taxamt,amt))-sum(cnamt) as sumamt, sum(it_cos) as sumcost,(sum(if(taxincl="T",amt-taxamt,amt))-sum(it_cos)) as profit,
						(((sum(if(taxincl="T",amt-taxamt,amt))-sum(it_cos))/sum(if(taxincl="T",amt-taxamt,amt)))*100) as margin
        ...
        <cfheader name="Content-Disposition" value="inline; filename=Product_Profit_Margin-By_Group_#huserid#.xls">
	...
	<cfcase value="EXCEL WITHOUT SALES DISCOUNT">
		<cfxml variable="data">
		<?mso-application progid="Excel.Sheet"?>
		<Workbook xmlns="urn:schemas-microsoft-com:office:spreadsheet" xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:x="urn:schemas-microsoft-com:office:excel" xmlns:ss="urn:schemas-microsoft-com:office:spreadsheet" xmlns:html="http://www.w3.org/TR/REC-html40">
		<Styles>
		...
		<Worksheet ss:Name="Product_Profit_Margin_Report">
  		<Table ss:ExpandedColumnCount="10" x:FullColumns="1" x:FullRows="1">
			...
			<cfquery name="getactualsales" datasource="#dts#">
				...
				(((ifnull(b.sumamt,0)-ifnull(b.sumcost,0))/ifnull(b.sumamt,0))*100) as margin from icitem as a
				left join
				(select itemno,sum(qty)-sum(cnqty) as sumqty,sum(if(taxincl="T",amt-taxamt,amt))-sum(cnamt) as sumamt,sum(it_cos) as sumcost
        ...
        <cfheader name="Content-Disposition" value="inline; filename=Product_Profit_Margin-Discount_#huserid#.xls">
	...
	<cfcase value="EXCEL WITH ADDITIONAL COST">
		<cfxml variable="data">
		<?mso-application progid="Excel.Sheet"?>
		<Workbook xmlns="urn:schemas-microsoft-com:office:spreadsheet" xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:x="urn:schemas-microsoft-com:office:excel" xmlns:ss="urn:schemas-microsoft-com:office:spreadsheet" xmlns:html="http://www.w3.org/TR/REC-html40">
		<Styles>
        ...
        <cfheader name="Content-Disposition" value="inline; filename=Product_Profit_Margin-Additional_#huserid#.xls">
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="2">
			...
			<cfquery name="getitem" datasource="#dts#">
				...
				(
					...
					from iccate
					<cfif trim(form.categoryfrom) neq "" and trim(form.categoryto) neq "">
						where cate >= '#form.categoryfrom#' and cate <= '#form.categoryto#'
					</cfif>
				...
				(
					...
					from icgroup
					<cfif trim(form.groupfrom) neq "" and trim(form.groupto) neq "">
						where wos_group >= '#form.categoryfrom#' and wos_group <= '#form.categoryto#'
					</cfif>
				)as c on a.wos_group=c.wos_group
				
				left join
				(
					select sum(qty)-sum(cnqty) as sumqty,sum(if(taxincl="T",amt-taxamt,amt))-sum(cnamt) as sumamt, sum(it_cos) as sumcost,itemno,wos_group
					from ictran 
