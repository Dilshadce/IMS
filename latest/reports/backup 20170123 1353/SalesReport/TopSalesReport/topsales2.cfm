...
<cfswitch expression="#form.result#">
	<cfcase value="HTML">
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="2">
			<tr>
				<td colspan="9"><div align="center"><font size="3" face="Times New Roman, Times, serif"><strong>TOP 50 SALES REPORT BY #trantype#</strong></font></div></td>
			</tr>
            ...
            <cfquery name="getagentdata" datasource="#dts#">
				...
				left join
                (select desp,agent from #target_icagent# )as f on a.agent=f.agent
				<cfif form.agentfrom neq "" and form.agentto neq "">
			...
			<cfquery name="getagentdata" datasource="#dts#">
				...
				left join
				(select agenno,sum(invgross)-sum(discount)as suminvamt from artran where type = 'INV' and (void = '' or void is null)
				<cfif form.areafrom neq "" and form.areato neq "">
				and area >='#form.areafrom#' and area <='#form.areato#'
				</cfif>
				<cfif trim(form.customerfrom) neq "" and trim(form.customerto) neq "">
				...
				left join
				(select agenno,sum(invgross)-sum(discount)as sumcsamt from artran where type = 'CS' and (void = '' or void is null)
				<cfif form.areafrom neq "" and form.areato neq "">
				and area >='#form.areafrom#' and area <='#form.areato#'
				</cfif>
				<cfif trim(form.customerfrom) neq "" and trim(form.customerto) neq "">
				...
				left join
				(select agenno,sum(invgross)-sum(discount)as sumdnamt from artran where type = 'DN' and (void = '' or void is null)
				<cfif form.areafrom neq "" and form.areato neq "">
				and area >='#form.areafrom#' and area <='#form.areato#'
				</cfif>
				<cfif trim(form.customerfrom) neq "" and trim(form.customerto) neq "">
				...
				left join
				(select agenno,sum(invgross)-sum(discount)as sumcnamt from artran where type = 'CN' and (void = '' or void is null)
				<cfif form.areafrom neq "" and form.areato neq "">
				and area >='#form.areafrom#' and area <='#form.areato#'
				</cfif>
				<cfif trim(form.customerfrom) neq "" and trim(form.customerto) neq "">
	...
	<cfcase value="EXCEL">
        ...
        <!---Agent from Customer Profile--->
            ...
            <cfquery name="getagentdata" datasource="#dts#">
				...
				group by custno) as d on a.agent=d.custno
				...
				group by custno) as e on a.agent=e.custno
				left join
                (select desp,agent from #target_icagent# )as f on a.agent=f.agent
				<cfif form.agentfrom neq "" and form.agentto neq "">
				where a.agent >='#form.agentfrom#' and a.agent <= '#form.agentto#'
