...
<cfif type eq "ref">
  <cfquery name="getdata" datasource="#dts#">
		...
		and c.location >= '#form.locationFrom#' and c.location <= '#form.locationTo#'
		</cfif>
  ...
  <cfquery name="getdata" datasource="#dts#">
		select a.itemno,a.desp,b.*,c.* 
        from icitem a,ictran b,iserial c where a.itemno = b.itemno
        and b.itemno = c.itemno	and b.refno = c.refno 
        ...
        <cfif form.billType neq "">
			and c.type = '#form.billType#'
        		...
        		<cfif form.datefrom neq "" and form.dateto neq "">
			and c.wos_date >= '#ndatefrom#' and c.wos_date <= '#ndateto#'
		</cfif>
		<cfif trim(form.productfrom) neq "" and trim(form.productto) neq "">
		and c.itemno >='#form.productfrom#' and c.itemno <='#form.productto#'
		</cfif>
		...
		and b.custno >= '#form.customerFrom#' and b.custno <='#form.customerTo#'
		</cfif>
        and (c.void='' or c.void is null)
        group by b.type,b.refno,c.serialno
    ...
    <Worksheet ss:Name="Others Report"><cfoutput>
        ...
        <cfif form.locationFrom NEQ "" AND form.locationTo NEQ "">
        	AND location BETWEEN "#form.locationFrom#" AND "#form.locationTo#"
              ...
              <cfelse>
              	<cfwddx action = "cfml2wddx" input = "#getlastseriallocation.location#" output = "wddxText6">
              </cfif>
              ...
              <Row ss:AutoFitHeight="0" ss:Height="12">
                ...
                <Cell ss:StyleID="s28">
                  <Data ss:Type="String"><cfif outqty gt 0>-</cfif>#wddxText4#</Data>
                </Cell>
                <Cell ss:StyleID="s28">
                  <Data ss:Type="String">#wddxText5#</Data>
                </Cell>
  ...
  <cfif type eq "ref">
    ...
    <table width="100%" border="0" align="center">
      ...
      <cfoutput query="getdata">
		...
		select <!--- a.itemno,b.itemno, --->c.* from <!--- icitem a,ictran b, ---> iserial c where <!--- a.itemno = b.itemno  --->
		<!--- and b.itemno = c.itemno and b.refno = c.refno and --->
		c.serialno = '#serialno#' and (c.type = 'RC' or c.type = 'CN' or c.type = 'TR' or c.type='OAI' or c.type='ADD')
		and c.itemno ='#getdata.itemno#'
		<cfif form.locationFrom neq "" and form.locationTo neq "">
		and c.location >= '#form.locationFrom#' and c.location <= '#form.locationTo#'
		</cfif>
		...
		and b.itemno = c.itemno and b.refno = c.refno and --->
		c.serialno = '#serialno#' and (c.type = 'CS'
		or c.type = 'DN' or c.type = 'INV' or c.type = 'PR' or c.type = 'TR' or c.type = 'ISS' or c.type = 'OAR')
		and c.itemno ='#getdata.itemno#'
		<cfif form.locationFrom neq "" and form.locationTo neq "">
		and c.location >= '#form.locationFrom#' and c.location <='#form.locationTo#'
		</cfif>
        ...
        <cfif xitemno neq "" and xitemno neq itemno and subtotalqty neq 0>
        <tr><td colspan="100%"><hr></td></tr>
        <tr><th colspan="5"><div align="right">Item Total : </div></th>
        <td size="2" face="Times New Roman, Times, serif"><div align="right"><strong>#subtotalqty#</strong></div></td></tr>
        ...
        <cfset subtotalqty=subtotalqty+bal>
