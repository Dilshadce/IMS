<cfcomponent>
    <cffunction name="listAccount" access="remote" returntype="struct">
        ...
        <cfquery name="listMatchedAccount" datasource="#dts#">
            SELECT cashierid
            FROM cashier
            WHERE cashierid LIKE <cfqueryparam cfsqltype="cf_sql_varchar" value="%#term#%" />
            ORDER BY cashierid
            LIMIT #start#,#limit#;
        </cfquery>
        ...
        <cfloop query="listMatchedAccount">
            ...
            <cfset matchedAccount["id"]=evaluate('listMatchedAccount.cashierid')>
            <cfset matchedAccount["cashierid"]=evaluate('listMatchedAccount.cashierid')>
    ...
    <cffunction name="getSelectedAccount" access="remote" returntype="struct">
        ...
        <cfquery name="getSelectedAccount" datasource="#dts#">
            SELECT cashierid
            FROM cashier
            WHERE cashierid=<cfqueryparam cfsqltype="cf_sql_varchar" value="#value#" />;
        </cfquery>
        
        <cfset selectedAccount=StructNew()>
        <cfset matchedAccount["id"]=evaluate('listMatchedAccount.cashierid')>
        <cfset selectedAccount["cashierid"]=evaluate('getSelectedAccount.cashierid')>
