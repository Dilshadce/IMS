...
<cfquery dbtype="query" name="queryData">
    SELECT col_2 AS client_ref_id, col_4 AS client_date, col_5 AS time_in, col_6 AS time_type FROM excelData 
</cfquery>
...
<cfloop query="totalemp">
    ...
    <cfloop from="1" to="#monthdays#" index="a">
        ...
        <cfquery name="getin" dbtype="query">
            SELECT client_date, client_ref_id, time_in, time_type FROM queryData
        ...
        <cfquery name="getout" dbtype="query">
            SELECT client_date, client_ref_id, time_in, time_type FROM queryData
            WHERE client_ref_id = '#totalemp.client_ref_id#'
            AND client_date = '#DateFormat(currentdate, 'd')# #DateFormat(currentdate, 'mmm')# #DateFormat(currentdate, 'yyyy')# (#DateFormat(currentdate, 'ddd')#)'
            AND time_type = 'Out'
        ...
        <cfif "#getin.recordcount#" NEQ "0">   #placementno# - #currentdate# - #getin.time_in#<br>
            <cfset timestart = "#Timeformat(CreateTime(Left(Right(getin.time_in, 4), 2), Right(getin.time_in, 2), '00'), 'HH:nn:ss')#">
        </cfif>
        <cfif "#getout.recordcount#" NEQ "0">  
            <cfset timeend = "#Timeformat(CreateTime(Left(Right(getout.time_in, 4), 2), Right(getout.time_in, 2), '00'), 'HH:nn:ss')#">
        </cfif>
       
        <cfif "#timestart#" NEQ "" AND "#timeend#" NEQ "">
            <cfset starttime = "#timestart#">    
            <cfset endtime = "#timeend#">   
            <cfset breaktime = "1">
            <cfif "#stcol#" EQ "RD" OR "#stcol#" EQ "OD" OR "#stcol#" EQ "PH">
                <cfset breaktime = "0">
            </cfif>
            <cfif "#stcol#" EQ "NPL">
                <cfset stcol = "">
