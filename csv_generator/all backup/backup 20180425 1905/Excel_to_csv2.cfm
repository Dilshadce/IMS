...
<cfloop query="totalemp">
    ...
    <cfloop from="1" to="#monthdays#" index="a">
        ...
        <cfloop query="gettime">
            <cfif "#gettime.time_in#" NEQ "" AND #Len(Replace(gettime.time_in, "'", "", "ALL"))# EQ "4"> 
                <cfset timestart = #Replace(gettime.time_in, "'", "", "ALL")#>     
                <cfset timestart = "#Timeformat(CreateTime(Left(timestart, 2), Right(timestart, 2), '00'), 'HH:nn:ss')#">
            </cfif>
            <cfif "#gettime.time_out#" NEQ "" AND #Len(Replace(gettime.time_out, "'", "", "ALL"))# EQ "4">
                <cfset timeend = #Replace(gettime.time_out, "'", "", "ALL")#>    
                <cfset timeend = "#Timeformat(CreateTime(Left(timeend, 2), Right(timeend, 2), '00'), 'HH:nn:ss')#">
            </cfif>
        </cfloop>
