...
<cfloop list="#SpreadSheetInfo(SpreadsheetRead(srcFile)).SHEETNAMES#" index="a">
	...
	<cfset lastRowYear = "#year(monthData.col_1)#">

	<cfloop query="queryData" startrow="#startrow#" endrow="#val(lastrow)#">
        
        <cfquery name="getplacementbasedondate" datasource="#dts#">
            SELECT placementno FROM placement
            WHERE "#LsDateFormat(queryData.col_1, 'yyyy-mm-dd', 'English (Australian)')#" BETWEEN startdate AND completedate
            AND empno = "#empno#" AND jobstatus = '2'
        </cfquery>
        
        <cfif "#getplacementbasedondate.recordcount#" NEQ 0>
            <cfset placementno = "#getplacementbasedondate.placementno#">
        </cfif>
		...
		<cfelse>
            ...
            <cfset workhours = "#ListFirst(queryData.col_10, ':')#.#ListFirst(Right(queryData.col_10, 2)/60*100, '.')#"><!---changed the workhours from minutes to decimal format--->
			<cfset breaktime = "0">
		</cfif>
		...
		<cffile action="#writeAppend#" file="#outputfile#"
				output="#placementno#,#empno#,#stcol#,#tsrowcount#,#LsDateFormat(queryData.col_1, 'yyyy-mm-dd', 'English (Australian)')#,#starttime#,#endtime#,#workhours#,#breaktime#,0,0,0,0,0,0,0,0,""#remarks#"",#ampm#,#lastRowMonth#,#lastRowYear#"
				addnewline="yes">
