<cfsetting showdebugoutput="true" requesttimeout="600">
<cfoutput>
    ...
    <cfset headerlist = "Placement No,Employee Name,Leave Type,Days Taken,Leave Start Date,Leave Start Time,Leave End Date,Leave End Time,Leave Status,Employee Remarks,Manager Remarks,Document Name,Updated On,Updated By,Submitted On,Employee No,Client No,Client Name,Contract Start Date,Contract End Date">
    ...
    <cfloop query = "getleavetype">
        ...
        <cfset headerlist = ListAppend(headerlist, '#UCase(getleavetype.costcode)# (#UCase(getleavetype.desp)#) Earned Leave', ',')>
        <cfset headerlist = ListAppend(headerlist, '#UCase(getleavetype.costcode)# (#UCase(getleavetype.desp)#) Earned Leave Rounding', ',')>
    </cfloop>
    
    <!---<cfset headerlist = ListAppend(headerlist, '#headerlist2#', ',')>--->
    
    <cfquery name="getleave" datasource="#dts#" cachedwithin="#createtimespan(0,0,10,0)#">                                                                    <!---main query--->
        SELECT 
        pl.placementno,
        pl.empname,
        ll.leavetype,
        ...
        pl.custno,
        pl.custname,
        pl.startdate AS begindate,
        CASE WHEN pl.completedate = "0000-00-00" THEN pl.startdate ELSE pl.completedate END AS completedate
        <cfloop query="getleavetype">
            ,pl.#getleavetype.costcode#entitle, COALESCE(pl.#getleavetype.costcode#totaldays, 0) AS "#getleavetype.costcode#totaldays"
            ,pl.#getleavetype.costcode#earndays ,pl.#getleavetype.costcode#earntype
        </cfloop>
        FROM placement pl
        LEFT JOIN leavelist ll ON ll.placementno = pl.placementno
        WHERE 1=1
    ...
    <cfquery name="filteredleave" dbtype="query">
       SELECT * FROM getleave WHERE startdate IS NOT NULL
    </cfquery>
    ...
    <cfloop query = "getleavetype">
        ...
        <cfif "#getleavetype.costcode#" EQ "AL">                                                                    <!---earned leave for AL--->
            <cfset balancelist = ListAppend(balancelist, '#UCase(getleavetype.costcode)# Leave Earned As Of #DateFormat(now(), "yyyy-mm-dd")#', ',')>
        </cfif>
    </cfloop>
    ...
    <cfif "#form.leave_report#" NEQ "Leave Taken Only">
        ...
        <cfloop query="groupPno">                                                                                       <!---calculate leave balance and write it--->
            ...
            <cfloop query="getleavetype">
                ...
                <cfif "#getleavetype.costcode#" EQ "AL">                                                                <!---earned leave calculation--->
                    <cfquery name="getearnleave" dbtype="query">
                        SELECT * FROM getleave
                        WHERE placementno = '#groupPno.placementno#'
                    </cfquery>
                    <cfif "#Evaluate('getearnleave.#getleavetype.costcode#earndays')#" EQ "Y">
                        <cfset contractdays = #DateDiff('d', "#DateFormat(getearnleave.begindate, 'yyyy-mm-dd')#", "#DateFormat(getearnleave.completedate, 'yyyy-mm-dd')#")# + 1>
                        <cfset leavedays = #Val(Evaluate('getearnleave.#getleavetype.costcode#totaldays'))#>
                        <cfset currentdays = #DateDiff('d', "#DateFormat(getearnleave.begindate, 'yyyy-mm-dd')#", "#DateFormat(now(), 'yyyy-mm-dd')#")# + 1>
                        
                        <cfif "#currentdays#" LTE "#contractdays#">
                            <cfif "#Evaluate('getearnleave.#getleavetype.costcode#earntype')#" EQ "Up">
                        ...
                        <cfset realbalancelist = ListAppend(realbalancelist,  '#totalearned#', ',')>
                    <cfelse>
                        <cfset realbalancelist = ListAppend(realbalancelist,  'Entitled Leave', ',')>
                    </cfif>
                </cfif>                                                                                                 <!---earned leave calculation--->
    ...
    <cfif "#form.leave_report#" NEQ "Leave Balance Only">
        <cfset SpreadSheetAddRow(excel,"#headerlist#")>                                                                  <!---Write leave record header--->
        <cfset SpreadSheetFormatRow(excel, s23, rowcount)>
        <cfset rowcount += 1>
            
        <cfset SpreadSheetAddRows(excel, filteredleave)>                                                                 <!---Write the leave record--->
    </cfif>
    ...
    <cfif #IsDefined('Leave_Report')# AND "#form.Leave_Report#" EQ "Leave Report with attachment">
        ...
        <cfif #attachmentfound# EQ true>
            ...
            <cffile action = "delete" file = "#HRootPath#\Excel_Report\LeaveReport_#timenow#.xlsx">
        </cfif>
    </cfif>
