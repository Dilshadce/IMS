...
<cfquery name="gettimelist" datasource="#dts#">
    SELECT * FROM
    (
        SELECT * FROM 
        (
            ...
            FROM placement aa
            LEFT JOIN #dsname#.timesheet b ON aa.placementno = b.placementno
            WHERE b.tmonth = '#form.monthselected#'
            AND b.tyear = '#form.yearselected#'
            
            <cfif #IsDefined('form.status')#>
                AND b.status IN (<cfqueryparam list="true" separator="," value="#form.status#">)
            </cfif>
            
            <cfif (#IsDefined('form.fromclient')# AND "#form.fromclient#" NEQ "") AND (#IsDefined('form.toclient')# AND "#form.toclient#" NEQ "")>
                AND aa.custno BETWEEN <cfqueryparam cfsqltype="cf_sql_varchar" value="#form.fromclient#"> 
                AND <cfqueryparam cfsqltype="cf_sql_varchar" value="#form.toclient#">
            <cfelse>
                <cfif "#form.fromclient#" NEQ "">
                    AND aa.custno = <cfqueryparam cfsqltype="cf_sql_varchar" value="#form.fromclient#">
                <cfelseif "#form.toclient#" NEQ "">
                    AND aa.custno = <cfqueryparam cfsqltype="cf_sql_varchar" value="#form.toclient#">
                </cfif>
            </cfif>
                    
            <cfif (#IsDefined('form.fromemp')# AND "#form.fromemp#" NEQ "") AND (#IsDefined('form.toemp')# AND "#form.toemp#" NEQ "")>
                AND aa.empno BETWEEN <cfqueryparam cfsqltype="cf_sql_varchar" value="#form.fromemp#"> 
                AND <cfqueryparam cfsqltype="cf_sql_varchar" value="#form.toemp#">
            <cfelse>
                <cfif "#form.fromemp#" NEQ "">
                    AND aa.empno = <cfqueryparam cfsqltype="cf_sql_varchar" value="#form.fromemp#">
                <cfelseif "#form.toemp#" NEQ "">
                    AND aa.empno = <cfqueryparam cfsqltype="cf_sql_varchar" value="#form.toemp#">
                </cfif>
            </cfif>

            <cfif #isDefined('form.custlist')# AND "#form.custlist#" NEQ "">
                AND aa.custno IN (<cfqueryparam list="Yes" separator="," value="#Replace(form.custlist, ' ', '', 'ALL')#">)
            </cfif>

            <cfif #isDefined('form.emplist')# AND "#form.emplist#" NEQ "">
                AND aa.empno IN (<cfqueryparam list="Yes" separator="," value="#Replace(form.emplist, ' ', '', 'ALL')#">)
            </cfif>

            <cfif #isDefined('form.filteremplist')# AND "#form.filteremplist#" NEQ "">
                AND aa.empno NOT IN (<cfqueryparam list="Yes" separator="," value="#Replace(form.filteremplist, ' ', '', 'ALL')#">)
            </cfif>
        )a
        LEFT JOIN 
        (
        ...
        ) d ON d.entryid = a.hrmgr
        LEFT JOIN iccolorid e ON a.timesheet = e.colorid
        LEFT JOIN
        (
...
<cfquery name="placementcount" dbtype="query">
    SELECT placementno FROM gettimelist GROUP BY placementno
</cfquery>

<cfif "#gettimelist.recordcount#" EQ 0>
    <script>
        alert('No timesheet found for the selected filter.');
        window.close();
    </script>  
    <cfabort>
</cfif>
        ...
        <html xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:x="urn:schemas-microsoft-com:office:excel" xmlns="http://www.w3.org/TR/REC-html40">
            ...
            <body>
                ...
                <div id="Book1_8828" align=center x:publishsource="Excel">
                    ...
                    <cfloop query="placementcount">
                        
                        <cfquery name="gettimesheet" dbtype="query">
                            SELECT * FROM gettimelist
                            WHERE placementno = '#placementcount.placementno#'
                            ORDER BY pdate
                        </cfquery>
                        
                        <cfquery name="getstartend" dbtype="query">
                            SELECT MIN(pdate) AS monthstart, MAX(pdate) AS monthend FROM gettimelist
                            WHERE placementno = '#placementcount.placementno#'
                            ORDER BY pdate
                        </cfquery>
                        ...
                        <table border=0 cellpadding=0 cellspacing=0 width=800 id="titlec" style='border-collapse:collapse;table-layout:fixed;width:800pt'>
                            ...
                            <col width=32 span=2 style='width:32pt'>
                            ...
                            <tr height=26 style='height:19.5pt'>
                                <td colspan=25 class=xl1138828 width=669 style='width:502pt'>
                                    <cfif month(getstartend.monthstart) eq month(getstartend.monthend)>
                                        #dateformat(getstartend.monthstart,'Mmm yyyy')#
                                    <cfelse>
                                        #dateformat(getstartend.monthstart,'Mmm yyyy')# to #dateformat(getstartend.monthend,'Mmm yyyy')#
                                    </cfif>
                                </td>
                            </tr>
                            <tr height=26 style='height:19.5pt'>
                                <td colspan=25 class=xl1138828 width=669 style='width:502pt'>
                                    #counter#: MONTHLY TIMESHEET (<cfif "#gettimesheet.status#" EQ "">SAVED<cfelse>#UCase(gettimesheet.status)#</cfif>)
                                </td>
                            </tr>
                            ...
                            <tr height=21 style='mso-height-source:userset;height:15.75pt'>
                                ...
                                <td colspan=9 class=xl1248828 width=320 style='border-right:1pt solid black;border-left:none;width:240pt'>Name</td>
                                <td colspan=8 class=xl1248828 width=128 style='border-right:1pt solid black;border-left:none;width:96pt'>Email</td>
                                <td colspan="5" class=xl898828 style='border-top:none;border-left:none'>Signature:</td>
                            </tr>
                            <tr height=20 style='height:15.0pt'>
                                <td colspan=9 height=20 class=xl1258828 style='border-right:1pt solid black;height:15.0pt;border-left:none'>
                                    #gettimesheet.username#
                                </td>
                                <td colspan=8 class=xl1288828 width=128 style='border-left:none;width:96pt'>#gettimesheet.useremail#</td> 
                                <td colspan="5" rowspan="5" height=22 class=xl748828 style='height:15.0pt;border-top:none'></td>
                            </tr>
                            <tr height=21 style='mso-height-source:userset;height:15.75pt'>
                                <td colspan=2 rowspan=2 height=41 class=xl1188828 style='border-right:1pt solid black;border-bottom:1pt solid black;height:30.75pt'>
                                ...
                                <td colspan=8 class=xl1248828 width=128 style='border-right:1pt solid black;border-left:none;width:96pt'>Updated On</td>
                            </tr>
                            <tr height=20 style='height:15.0pt'>
                                <td colspan=9 height=20 class=xl1258828 style='border-right:1pt solid black;height:15.0pt;border-left:none'>
                                    #DateTimeFormat(gettimesheet.created_on, 'yyyy-mm-dd hh:nn:ss')#
                                </td>
                                <td colspan=8 class=xl1288828 width=128 style='border-left:none;width:96pt'>
                            ...
                            <tr height=20 style='height:15.0pt'>
                                <td colspan=19 rowspan=2 height=40 class=xl1118828 width=576 style='height:30.0pt;width:432pt'>
                            ...
                            <cfloop query="gettimesheet">
                                ...
                                <tr onMouseOut="javascript:this.style.backgroundColor='';" onMouseOver="javascript:this.style.backgroundColor='##dce3e5';" height=20 style='height:12.0pt'>
                            ...
                            <tr>
                                <td colspan=24 class=xl658828 width=735 style='border-right:1pt solid black;border-bottom:1pt solid black;height:30.0pt;width:552pt'>
                                    #gettimesheet.term#
                                </td>
                            </tr>
                        </table>
