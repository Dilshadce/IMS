...
<cfoutput>
		...
		<form name="form" action="servicereportresult.cfm?type=#url.type#" method="post" target="_blank">
	
</cfoutput>
<body>
<h3>
	<a href="servicereportmenu.cfm"><cfoutput>Service</cfoutput> Report</a> >> 
	<a><font size="2"><cfoutput>Service Listing Report</cfoutput></font></a>
</h3>
<table width="65%" border="0" cellspacing="0" cellpadding="3" class="data" align="center">
<cfoutput><input type="hidden" name="tran" id="tran" value="#target_arcust#" /><input type="hidden" name="fromto" id="fromto" value="" /></cfoutput>
	...
	<tr>
      	...
      	<td width="75%">
			...
			<cfif getgeneral.filterall eq "1">
				<input type="button" size="10" value="Ajax Search" onClick="document.getElementById('fromto').value='from';ColdFusion.Window.show('findCustomer');" />&nbsp;<input type="text" name="searchcustomerfr" onKeyUp="getcustomer('customerfrom');">
			</cfif>		</td>
    </tr>
    <tr>
      	<th>Cust/Supp No To</th>
      	<td>
			<select name="customerto">
          		<option value="">Choose a customer</option>
          		<cfoutput query="getcust">
            		<option value="#convertquote(custno)#">#custno# - #name#</option>
          		</cfoutput>
			...
			<cfif getgeneral.filterall eq "1">
				<input type="button" size="10" value="Ajax Search" onClick="document.getElementById('fromto').value='to';ColdFusion.Window.show('findCustomer');" />&nbsp;<input type="text" name="searchcustomerto" onKeyUp="getcustomer('customerto');">
			</cfif>		</td>
    </tr>
	<tr>
      	<td colspan="100%"><hr></td>
