...
function getproject(type){
	if(type == 'projectto'){
		var inputtext = document.form.searchprojectto.value;
		DWREngine._execute(_reportflocation, null, 'projectlookup', inputtext, getprojectResult);
		
	}else{
		var inputtext = document.form.searchprojectfr.value;
		DWREngine._execute(_reportflocation, null, 'projectlookup', inputtext, getprojectResult2);
	}
}

function getprojectResult(projectArray){
	DWRUtil.removeAllOptions("projectto");
	DWRUtil.addOptions("projectto", projectArray,"KEY", "VALUE");
}

function getprojectResult2(projectArray){
	DWRUtil.removeAllOptions("projectfrom");
	DWRUtil.addOptions("projectfrom", projectArray,"KEY", "VALUE");
}
// end: project search
...
<cfoutput>
		...
		<form name="form" action="servicereportresult4.cfm" method="post" target="_blank">
	
</cfoutput>
<body>
<h3>
	<a href="servicereportmenu.cfm"><cfoutput>Service</cfoutput> Report</a> >> 
	<a><font size="2"><cfoutput>Supplier - Service Report</cfoutput></font></a>
</h3>
...
<tr>
		...
		<tr> <input type="hidden" name="tran" id="tran" value="#target_apvend#" /><input type="hidden" name="fromto" id="fromto" value="" />
	        ...
	        <td width="75%"><select name="Servicefrom">
					...
					<cfoutput query="getservice">
						<option value="#servi#">#servi# - #desp#</option>
					</cfoutput>
				</select>			</td>
	    </tr>
	...
	<tr>
      	<th width="25%">Supplier From</th>
      	<td width="75%">
			<select name="supplierfrom">
			...
			<cfif getgeneral.filterall eq "1">
				<input type="button" size="10" value="Ajax Search" onClick="document.getElementById('fromto').value='from';ColdFusion.Window.show('findCustomer');" />&nbsp;<input type="text" name="searchsupplierfr" onKeyUp="getsupplier('supplierfrom');">
			</cfif>		</td>
    </tr>
    <tr>
      	<th>supplier To</th>
      	<td>
			<select name="supplierto">
          		<option value="">Choose a supplier</option>
          		<cfoutput query="getsupplier">
            		<option value="#convertquote(custno)#">#custno# - #name#</option>
          		</cfoutput>
			...
			<cfif getgeneral.filterall eq "1">
				<input type="button" size="10" value="Ajax Search" onClick="document.getElementById('fromto').value='to';ColdFusion.Window.show('findCustomer');" />&nbsp;<input type="text" name="searchsupplierto" onKeyUp="getsupplier('supplierto');">
			</cfif>		</td>
    </tr>
	<tr>
      	<td colspan="100%"><hr></td>
