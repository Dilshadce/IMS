    ...
    <cfif lcase(hcomid) neq "acht_i">
    <td><a onMouseOver="JavaScript:this.style.cursor='hand';" onClick="document.getElementById('expressservicelist').value = unescape('#getitem1.itemno#');ajaxFunction(window.document.getElementById('itemDetail'),'/default/report-stock/expressadjustment/addItemAjax.cfm?itemno=#getitem1.itemno#'+'&date='+document.getElementById('wos_date').value);setTimeout('updateVal();',750);document.getElementById('expressservicelist').focus();ColdFusion.Window.hide('searchitem');" >SELECT</a></td>
    </cfif>
    </tr>
    </cfloop>
