...
<cfset xwos_date="#dateformat(now(),'DD/MM/YYYY')#">
<cfset xcustno="">
<cfset xterm="">
<cfset xcurrcode="">
<cfset xcurrrate="">
...
<cfset xtax=getgsetup.df_purchasetax>
<cfelse>
<cfset xtax=getgsetup.df_salestax>
</cfif>

<cfquery name="getterms" datasource="#dts#">
...
function getSupp(type,option){
		if(document.getElementById('searchsuppfr').value!='')
		{
...
document.getElementById("hiddentext").value = comid;
	...
	function changecust(){
	var ajaxurl6 = '/default/transaction/matrixexpressbill/customerajax.cfm?type='+document.getElementById('tran').value+'&refno='+document.getElementById('refno').value;
	new Ajax.Request(ajaxurl6,
      {
        method:'get',
        onSuccess: function(getdetailback){
		document.getElementById('ajaxfielda6').innerHTML = trim(getdetailback.responseText);
        },
        onFailure: function(){ 
		alert('Error'); },		
	...
	var ajaxurl5 = '/default/transaction/matrixexpressbill/locationajax.cfm?type='+document.getElementById('tran').value+'&refno='+document.getElementById('refno').value;
	new Ajax.Request(ajaxurl5,
      {
        method:'get',
        onSuccess: function(getdetailback){
		document.getElementById('ajaxField5').innerHTML = trim(getdetailback.responseText);
        },
        onFailure: function(){ 
		alert('Error1'); },		
	...
	setTimeout('changecust();',500);
	
	
	
	if(document.getElementById('tran').value =="TR"){
	<cfif (lcase(hcomid) eq "aipl_i" or lcase(hcomid) eq "gramas_i" or lcase(hcomid) eq "belco_i" or lcase(hcomid) eq "kjpe_i")>
	<cfelse>
	document.getElementById('agentdisplay').style.visibility="hidden";
	document.getElementById('agentdisplay2').style.visibility="hidden";
	</cfif>
	document.getElementById('currdisplay').style.visibility="hidden";
	document.getElementById('currdisplay2').style.visibility="hidden";
	document.getElementById('bcode').style.visibility="hidden";
	document.getElementById('add2display').style.visibility="hidden";
	document.getElementById('add3display').style.visibility="hidden";
	document.getElementById('add4display').style.visibility="hidden";
	document.getElementById('attndisplay').style.visibility="hidden";
	document.getElementById('attndisplay2').style.visibility="hidden";
	document.getElementById('phonedisplay').style.visibility="hidden";
	document.getElementById('phoneadisplay').style.visibility="hidden";
	<cfif not isdefined('url.mode')>
	setTimeout('document.getElementById("custno").value="";',1500);
	</cfif>
	document.getElementById('custdisplay').value="Authorised By";
	document.getElementById('namedisplayfield').value="Reason for transfer";
	document.getElementById('name2displayfield').value="Description";
	document.getElementById('billtodisplayfield').value="Ref. Doc. No.";
	}
	else{
	<cfif (lcase(hcomid) eq "aipl_i" or lcase(hcomid) eq "gramas_i" or lcase(hcomid) eq "belco_i" or lcase(hcomid) eq "kjpe_i")>
	<cfelse>	
	...
	document.getElementById('currdisplay').style.visibility="visible";
	document.getElementById('currdisplay2').style.visibility="visible";
	document.getElementById('bcode').style.visibility="visible";

	document.getElementById('add2display').style.visibility="visible";
	document.getElementById('add3display').style.visibility="visible";
	document.getElementById('add4display').style.visibility="visible";
	document.getElementById('attndisplay').style.visibility="visible";
	document.getElementById('attndisplay2').style.visibility="visible";
	document.getElementById('phonedisplay').style.visibility="visible";
	document.getElementById('phoneadisplay').style.visibility="visible";
	document.getElementById('custdisplay').value="Cust / Supp Code";
	document.getElementById('namedisplayfield').value="Name";
	document.getElementById('name2displayfield').value="";
	document.getElementById('billtodisplayfield').value="Bill To";
	}
	...
	select * from artran
	where type=<cfqueryparam cfsqltype="cf_sql_varchar" value="#url.tran#">
	and refno = <cfqueryparam cfsqltype="cf_sql_varchar" value="#url.refno#">
	</cfquery>
	
	<cfset xwos_date="#dateformat(getartran.wos_date,'DD/MM/YYYY')#">
	<cfset xcustno=getartran.custno>
	<cfset xtax=getartran.note>
	<cfset xterm=getartran.term>
	<cfset xcurrcode=getartran.currcode>
	<cfset xcurrrate=getartran.currrate>
	<cfset xagent=getartran.agenno>
	<cfset xpono=getartran.pono>
	<cfset xdono=getartran.dono>
	<cfset xname=getartran.name>
	<cfset xname2=getartran.frem1>
	<cfset xadd1=getartran.frem1>
	<cfset xadd2=getartran.frem2>
	<cfset xadd3=getartran.frem3>
	<cfset xadd4=getartran.frem4>
	<cfset xattn=getartran.rem2>
	<cfset xphone=getartran.rem4>
	<cfset xphonea=getartran.phonea>
	<cfset xfax=getartran.frem6>
	<cfset xdadd1=getartran.comm0>
	<cfset xdadd2=getartran.comm1>
	<cfset xdadd3=getartran.comm2>
	<cfset xdadd4=getartran.comm3>
	<cfset xdattn=getartran.rem3>
	<cfset xdphone=getartran.rem12>
	<cfset xdphonea=getartran.d_phone2>
	<cfset xrem1=getartran.rem1>
	<cfset xrem2=getartran.rem2>
	<cfset xrem5=getartran.rem5>
	<cfset xrem6=getartran.rem6>
	<cfset xrem7=getartran.rem7>
	<cfset xrem8=getartran.rem8>
	<cfset xrem9=getartran.rem9>
	<cfset xrem10=getartran.rem10>
	<cfset xrem11=getartran.rem11>
	
	
	<cfquery name="insertbackictrantemp" datasource="#dts#">
		INSERT INTO ictrantemp
		(REFNO,TYPE, REFNO2, TRANCODE, CUSTNO, FPERIOD, WOS_DATE, CURRRATE, ITEMCOUNT, LINECODE, ITEMNO, DESP, DESPA, AGENNO, LOCATION, SOURCE, JOB, SIGN, QTY_BIL, PRICE_BIL, UNIT_BIL, AMT1_BIL, DISPEC1, DISPEC2, DISPEC3, DISAMT_BIL, AMT_BIL, TAXPEC1, TAXPEC2, TAXPEC3, TAXAMT_BIL, NOTE_A, IMPSTAGE, QTY, PRICE, UNIT, AMT1, DISAMT, AMT, TAXAMT, FACTOR1, FACTOR2, DONO, DODATE, SODATE, BREM1, BREM2, BREM3, BREM4, PACKING, NOTE1, NOTE2, GLTRADAC, UPDCOST, GST_ITEM, TOTALUP, WITHSN, NODISPLAY, GRADE, PUR_PRICE, QTY1, QTY2, QTY3, QTY4, QTY5, QTY6, QTY7, QTY_RET, TEMPFIGI, SERCOST, M_CHARGE1, M_CHARGE2, ADTCOST1, ADTCOST2, IT_COS, AV_COST, BATCHCODE, EXPDATE, POINT, INV_DISC, INV_TAX, SUPP, EDI_COU1, WRITEOFF, TOSHIP, SHIPPED, NAME, DEL_BY, VAN, GENERATED, UD_QTY, TOINV, EXPORTED, EXPORTED1, EXPORTED2, EXPORTED3, BRK_TO, SV_PART, LAST_YEAR, VOID, SONO, MC1_BIL, MC2_BIL, USERID, DAMT, OLDBILL, WOS_GROUP, CATEGORY, AREA, SHELF, TEMP, TEMP1, BODY, TOTALGROUP, MARK, TYPE_SEQ, PROMOTER, TABLENO, MEMBER, TOURGROUP, TRDATETIME, TIME, BOMNO, COMMENT, DEFECTIVE, M_CHARGE3, M_CHARGE4, M_CHARGE5, M_CHARGE6, M_CHARGE7, MC3_BIL, MC4_BIL, MC5_BIL, MC6_BIL, MC7_BIL, taxincl,UUID)
		...
		FROM ictran
		where 
		type=<cfif url.tran eq "TR">"TROU"<cfelse><cfqueryparam cfsqltype="cf_sql_varchar" value="#url.tran#"></cfif>
		and refno = <cfqueryparam cfsqltype="cf_sql_varchar" value="#url.refno#">
	...
	{
	document.getElementById('d_add1').value=document.getElementById('hiddadd1').value;
	document.getElementById('d_add2').value=document.getElementById('hiddadd2').value;
	document.getElementById('d_add3').value=document.getElementById('hiddadd3').value;
	document.getElementById('d_add4').value=document.getElementById('hiddadd4').value;
	document.getElementById('d_attn').value=document.getElementById('hiddattn').value;
	document.getElementById('d_phone').value=document.getElementById('hiddphone').value;
	document.getElementById('d_phone2').value=document.getElementById('hiddphone2').value;
	...
	document.getElementById('hiddadd1').value=document.getElementById('d_add1').value;
	}
	if (document.getElementById('d_add2').value !=''){
	document.getElementById('hiddadd2').value=document.getElementById('d_add2').value;
	}
	if (document.getElementById('d_add3').value !=''){
	document.getElementById('hiddadd3').value=document.getElementById('d_add3').value;
	}
	if (document.getElementById('d_add4').value !=''){
	document.getElementById('hiddadd4').value=document.getElementById('d_add4').value;
	}
	if (document.getElementById('d_attn').value !=''){
	document.getElementById('hiddattn').value=document.getElementById('d_attn').value;
	}
	if (document.getElementById('d_phone').value !=''){
	document.getElementById('hiddphone').value=document.getElementById('d_phone').value;
	}
	if (document.getElementById('d_phone2').value !=''){
	document.getElementById('hiddphone2').value=document.getElementById('d_phone2').value;
	}
	...
	var itemlisting=itemlisting+"&servicecode"+k+"="+document.getElementById('additem_'+k).value+"&qty"+k+"="+document.getElementById('additemqty_'+k).value+"&price"+k+"="+escape(document.getElementById('additemprice_'+k).value)+"&disc"+k+"="+escape(document.getElementById('additemdisc_'+k).value)+"&disca"+k+"="+escape(document.getElementById('additemdisc2_'+k).value)+"&discb"+k+"="+escape(document.getElementById('additemdisc3_'+k).value)+"&location"+k+"="+escape(document.getElementById('additemloc_'+k).value);
	<cfelse>
	var itemlisting=itemlisting+"&servicecode"+k+"="+document.getElementById('additem_'+k).value+"&qty"+k+"="+document.getElementById('additemqty_'+k).value+"&price"+k+"="+escape(document.getElementById('additemprice_'+k).value)+"&disc"+k+"="+escape(document.getElementById('additemdisc_'+k).value)+"&location"+k+"="+escape(document.getElementById('additemloc_'+k).value);
	</cfif>
	...
	var ajaxurl2 = '/default/transaction/matrixexpressbill/addmultiproductsAjax.cfm?tran='+escape(tran)+'&tranno='+refno+'&uuid=#URLEncodedFormat(uuid)#&custno='+escape(custno)+'&locationfr='+escape(locationfr)+'&locationto='+escape(locationto)+itemlisting;
	}
	else
	{
	...
	{
	ajaxFunction(window.document.getElementById('itemDetail'),'/default/transaction/matrixexpressbill/addItemAjax.cfm?tran='+document.getElementById('tran').value+'&itemno='+document.getElementById('expressservicelist').value+'&custno='+document.getElementById('custno').value);
	...
	<cfoutput>
	var customerurl = '/default/transaction/matrixexpressbill/memberdetailajax.cfm?detail=1&type='+document.getElementById('tran').value+'&custno='+columnvalue;
	...
	document.getElementById('term').value=document.getElementById('termajaxhid').value;
	document.getElementById('agent').value=document.getElementById('agentajaxhid').value;
	
	document.getElementById('b_name').value=document.getElementById('nameajaxhid').value;
	document.getElementById('b_name2').value=document.getElementById('nameajaxhid').value;
	document.getElementById('b_add1').value=document.getElementById('add1ajaxhid').value;
	document.getElementById('b_add2').value=document.getElementById('add2ajaxhid').value;
	document.getElementById('b_add3').value=document.getElementById('add3ajaxhid').value;
	document.getElementById('b_add4').value=document.getElementById('add4ajaxhid').value;
	document.getElementById('b_attn').value=document.getElementById('attnajaxhid').value;
	
	document.getElementById('b_phone').value=document.getElementById('phoneajaxhid').value;
	document.getElementById('b_phone2').value=document.getElementById('phoneaajaxhid').value;
	document.getElementById('b_fax').value=document.getElementById('faxajaxhid').value;
	document.getElementById('billtodisplayfield').value="Profile";
	...
	selectOptionByValue(document.getElementById('driver'),document.getElementById('driverhid').value);
			...
			{
				<cfoutput>document.getElementById('taxper').value='#val(getgsetup.gst)#';</cfoutput>
				if(document.getElementById('tran').value=='PO' || document.getElementById('tran').value=='RC' || document.getElementById('tran').value=='PR')
				{
	...
	temp=((subtotal)-((subtotal)*((100-disamt1)/100)).toFixed(fixnumdisc));
	totaldiscount=temp;
	temp=(subtotal-totaldiscount).toFixed(fixnumdisc);
	temp=((temp)-(temp*((100-disamt2)/100)).toFixed(fixnumdisc));
	totaldiscount=(parseFloat(totaldiscount)+parseFloat(temp)).toFixed(fixnumdisc);
	temp=(subtotal-totaldiscount).toFixed(fixnumdisc);
	temp=((temp)-(temp*((100-disamt3)/100)).toFixed(fixnumdisc));
	 ...
	 new Ajax.Request(ajaxurl2,
      {
        method:'get',
        onSuccess: function(getdetailback){
		document.getElementById('ajaxField1').innerHTML = trim(getdetailback.responseText);
        },
        onFailure: function(){ 
		alert('Error Retriving Item'); },		
	...
	if (document.getElementById('tran').value == "TR"){
	alert('Please Key in authorised By')
	}
	else{
	...
	new Ajax.Request(ajaxurl,
      {
        ...
        onSuccess: function(flyback){
			document.getElementById('ajaxFieldPro2').innerHTML = "";
...
<cfoutput>
<br>
<h3 style=" font-size:36px">Period is over 18!! Kindly Do Year End Processing</h3><br>
<input type="button" name="periodoverclose" style="font: large bolder" id="periodoverclose" value="Close" onClick="window.close();">
</cfoutput>
<cfabort>
</cfif>
...
<td width="12%">
<cfif isdefined('url.mode')>
<input type="text" name="tran" id="tran" value="#url.tran#" readonly>
<cfelse>
...
<td width="3%" <cfif isdefined('url.mode')>style="visibility:hidden"</cfif>><cfselect name="refnoset" id="refnoset" bind="cfc:refnobill.getrefnoset({tran},'#dts#')" bindonload="yes" display="lastno" value="counter" /></td>
...
<cfelse>
<cfinput type="text" name="refno" id="refno" bind="cfc:refnobill.getrefno({tran},'#dts#',{refnoset})" bindonload="yes" required="yes" message="Please Key In Reference Number" onKeyUp="nextIndex('refno','custno');" >
</cfif>
</td>
...
<tr>
<th><input type="text" style="background:none;border: none; text-align:right;font-weight: bold;" name="custdisplay" id="custdisplay" value="Cust / Supp Code"></th>
<td colspan="2">
<div id="ajaxfielda6">
...
<input type="text" name="searchsuppfr" id="searchsuppfr" onKeyUp="getSupp('custno',document.getElementById('title').value);">
</div>
</td>
<th>Date</th>
<td colspan="2"><input type="text" name="wos_date" id="wos_date" value="#xwos_date#" onKeyUp="nextIndex('wos_date','expressservicelist');" readonly /><img src="/images/cal.gif" width=17 height=15 border=0 onClick="showCalendarControl(wos_date);">&nbsp;(DD/MM/YYYY)</td>

</tr>

<tr  id="namedisplay">
<th>
<input type="text" style="background:none;border: none; text-align:right;font-weight: bold;" name="namedisplayfield" id="namedisplayfield" value="Name">&nbsp;&nbsp; </th>
<td><cfinput type="text" name="b_name" id="b_name" size="40" value="#xname#" maxlength="35" /> </td>
<td></td>
<th>Terms</th>
<td colspan="2">
<cfselect name="term" id="term">
<cfloop query="getterms">
<option value="#getterms.term#" <cfif xterm eq getterms.term>selected</cfif>>#getterms.termdesp#</option>
</cfloop>
</cfselect>
<input type="hidden" name="termhid" id="termhid" value=""></td>
</tr>
<tr>
<td><input type="text" name="bcode" id="bcode" value="Profile" size="15"/><input type="text" style="background:none;border: none; text-align:right;font-weight: bold;" name="name2displayfield" id="name2displayfield" value=""></td>
<td><cfinput type="text" name="b_name2" id="b_name2" value="#xname2#" size="40" maxlength="35" /></td>
<td></td>
<th  id="agentdisplay">Agent</th>
<td colspan="2"  id="agentdisplay2"><cfselect name="agent" id="agent" selected="#xagent#" bind="cfc:custsupp.getagent('#dts#','#Hlinkams#')" bindonload="yes" display="agentdesp" value="agent" /><input type="hidden" name="agenthid" id="agenthid" value=""></td>
</tr>
<tr >
<th><input type="text" style="background:none;border: none; text-align:right;font-weight: bold;" name="billtodisplayfield" id="billtodisplayfield" value="Bill To"></th>
<td>
<cfinput type="text" name="b_add1" id="b_add1" value="#xadd1#" size="40" maxlength="35" /></td>
<td></td>
<th id="currdisplay">Currency</th>
<td colspan="2" id="currdisplay2"><cfinput type="text" name="currcode" id="currcode" value="#xcurrcode#" size="10"  /><input type="hidden" name="currcodehid" id="currcodehid" value="">&nbsp;<cfinput type="text" name="currrate" id="currrate" value="#xcurrrate#" bind="cfc:custsupp.getcurrrate('#dts#',{currcode},'#target_currency#',{wos_date})" bindonload="yes" size="5" /></td>
</tr>
<tr id="add2display">
<td></td>
<td><cfinput type="text" name="b_add2" id="b_add2" value="#xadd2#" size="40" maxlength="35" /></td>
<td></td>
<th>PONO</th>
<td colspan="2"><cfinput type="text" name="PONO" id="PONO" value="#xpono#" size="30" /></td>
</tr>
<tr id="add3display">
<td></td>
<td><cfinput type="text" name="b_add3" id="b_add3" value="#xadd3#" size="40" maxlength="35" /></td>
<td></td>
<th>DONO</th>
<td colspan="2"><cfinput type="text" name="DONO" id="DONO" value="#xdono#" size="30" /></td>

</tr>
<tr id="add4display">
<td></td>
<td><cfinput type="text" name="b_add4" id="b_add4" value="#xadd4#" size="40" maxlength="35" /></td>
<td></td>
<th style="visibility:hidden">#getgsetup.rem5#</th>
  <cfquery name="getbrand" datasource="#dts#">
  select * from brand
  </cfquery>
<td colspan="2" style="visibility:hidden"><input type="text" name="rem5" id="rem5" value="#xrem5#"></td>
</tr>
<tr>
<th id="attndisplay">Attn</th>
<td id="attndisplay2"><cfinput type="text" name="b_attn" id="b_attn" value="#xattn#" size="40" maxlength="35" /></td>
<td></td>
  ...
  <cfquery name="getlocation" datasource="#dts#">
    ...
    <div id="ajaxField5">
        ...
        <cfelse>
        	Location &nbsp;&nbsp;&nbsp;&nbsp;
            <select name="locationfr" id="locationfr">
            	<cfif LCASE(hcomid) EQ 'aipl_i' and url.tran EQ 'SO'>
...
<tr id="phonedisplay">
<th>Telephone</th>
<td><cfinput type="text" name="b_phone" id="b_phone" value="#xphone#" size="40" maxlength="35" /></td>

</tr>
<tr id="phoneadisplay">
<th>Telephone 2</th>
<td><cfinput type="text" name="b_phone2" id="b_phone2" value="#xphonea#" size="40" maxlength="35" /></td>
<td></td>
<th>Fax</th>
<td colspan="2"><cfinput type="text" name="b_fax" id="b_fax" value="#xfax#" size="40" maxlength="35" />
...
<td style="visibility:hidden"><cfinput type="text" name="rem6" id="rem6" value="#xrem6#" size="40" maxlength="100" /></td>
<td>&nbsp;</td>
<td>&nbsp;</td>
...
<td style="visibility:hidden"><cfinput type="hidden" name="rem7" id="rem7" value="#xrem7#" size="40" maxlength="100" />
<cfinput type="hidden" name="rem8" id="rem8" value="#xrem8#" size="40" maxlength="100" />
<cfinput type="hidden" name="rem9" id="rem9" value="#xrem9#" size="40" maxlength="100" />
<cfinput type="hidden" name="rem10" id="rem10" value="#xrem10#" size="40" maxlength="100" />
<cfinput type="hidden" name="rem11" id="rem11" value="#xrem11#" size="40" maxlength="100" />
</td>
<td>&nbsp;</td>
<td>&nbsp;</td>
<td width="8%">&nbsp;</td>
...
<tr>
  ...
  <input type="hidden" name="disbil1" id="disbil1" value="0" />
  ...
  <input type="hidden" name="disbil2" id="disbil2" value="0" />%
  <cfinput type="text" size="5" name="dispec3" id="dispec3" value="0" validate="float" validateat="onblur" message="Please key in numeric value" onKeyUp="calcdisc();caltax();calcfoot();"/><input type="hidden" name="disbil3" id="disbil3" value="0" />%<br>
...
<tr>
  <th>Included Tax</th>
  <td><input type="checkbox" name="taxincl" id="taxincl" value="T" <cfif getgsetup.taxincluded eq 'Y'>checked</cfif> onClick="caltax()" /></td>
  ...
  <td><cfselect name="taxcode" id="taxcode"  bind="CFC:tax.getTaxQry('#dts#','#target_taxtable#',{tran})" value="code" selected="#xtax#" display="code" onChange="setTimeout('caltax()',500);" bindonload="yes"/>&nbsp;&nbsp;<cfinput type="text" name="taxper" id="taxper" value="0" size="8" bind="CFC:tax.getTax('#dts#','#target_taxtable#',{taxcode})" onKeyUp="caltax()" bindonload="yes"  />&nbsp;&nbsp;&nbsp;
...
<tr>
  ...
  <cfif isdefined('url.mode')>
  <cfinput type="button" name="Submit" id="Submit" value="Accept" onClick="validformfield();"/>
  <cfelse>
  <cfinput type="button" name="Submit" id="Submit" value="Accept" onClick="validformfield();" disabled/>
  </cfif>
  &nbsp;&nbsp;&nbsp;<cfinput type="button" name="close" id="Close" value="Close" onClick="closetran();" ></div> </th>
  <td colspan="2">&nbsp;</td>
  <td>&nbsp;</td>
  <td>&nbsp;</td>
</tr>
...
<th>Delivery To</th>
<td>
<cfinput type="text" name="d_add1" id="d_add1" value="#xdadd1#" size="40" maxlength="35" /></td>
<td>&nbsp;</td>
<th>Job</th>
...
<tr style="visibility:hidden">
<td></td>
<td><cfinput type="text" name="d_add2" id="d_add2" value="#xdadd2#" size="40" maxlength="35" /></td>
<td>&nbsp;</td>
<th>End User</th>
...
<td></td>
<td><cfinput type="text" name="d_add3" id="d_add3" value="#xdadd3#" size="40" maxlength="35" /></td>
<td>&nbsp;</td>
</tr>
<tr style="visibility:hidden">
<td></td>
<td><cfinput type="text" name="d_add4" id="d_add4" value="#xdadd4#" size="40" maxlength="35" /></td>
<td>&nbsp;</td>
<th>Del Attn</th>
<td colspan="2"><cfinput type="text" name="d_attn" id="d_attn" value="#xdattn#" size="40" maxlength="35" /></td>
<th>Del Telephone</th>
<td><cfinput type="text" name="d_phone" id="d_phone" value="#xdphone#" size="40" maxlength="35" /><cfinput type="text" name="d_phone2" id="d_phone2" value="#xdphonea#" size="40" maxlength="35" /></td>
<td width="5%">&nbsp;</td>
...
<cfwindow center="true" width="1000" height="500" name="searchitem" refreshOnShow="true" closable="true" modal="false" title="Search Item" initshow="false"
        source="/default/transaction/matrixexpressbill/searchitem.cfm?reftype={tran}&custno={custno}" />
</cfif>
...
<cfif isdefined('url.mode')>
<cfwindow name="processing" width="300" height="300" initshow="true" center="true" closable="false" draggable="false" title="Processing....Please Wait" modal="true" resizable="false" >
