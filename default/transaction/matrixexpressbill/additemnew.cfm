...
SELECT price,price2,price3,ucost,desp,despa,unit,fcurrcode,fucost,fprice<cfloop from="2" to="10" index="i">,fcurrcode#i#,fucost#i#,fprice#i#</cfloop> from icitem WHERE itemno = <cfqueryparam cfsqltype="cf_sql_varchar" value="#itemno#"> 
</cfquery>
...
select business,dispec1,dispec2,dispec3,currcode from <cfif tran eq "INV" or tran eq "DO" or tran eq "QUO" or tran eq "SO" or tran eq "CS">#target_arcust#<cfelse>#target_apvend#</cfif> where custno=<cfqueryparam cfsqltype="cf_sql_varchar" value="#url.custno#"> 
</cfquery>
...
<cfset qty = 1>
<cfif tran eq 'PO' or tran eq 'PR' or tran eq 'RC' or (tran eq 'TR' and getgsetup.df_trprice neq "Price")>
...
<cfif isdefined('validitem.fcurrcode')>
    <cfif validitem.fcurrcode neq "" and getcustomerbusiness.currcode eq validitem.fcurrcode>
        <cfif tran eq "INV" or tran eq "DO" or tran eq "QUO" or tran eq "SO" or tran eq "CS">
        <cfset price = validitem.fprice>
		<cfelse>
        <cfset price = validitem.fucost>
		</cfif>
        </cfif>
        <cfloop from="2" to="10" index="i">
        <cfif evaluate('validitem.fcurrcode#i#') neq "" and getcustomerbusiness.currcode eq evaluate('validitem.fcurrcode#i#')>
        <cfif tran eq "INV" or tran eq "DO" or tran eq "QUO" or tran eq "SO" or tran eq "CS">
        <cfset price = evaluate('validitem.fprice#i#')>
		<cfelse>
        <cfset price = evaluate('validitem.fucost#i#')>
		</cfif>
        </cfif>
