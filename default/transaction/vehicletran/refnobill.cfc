<cfcomponent>
    ...
    <cffunction name="getrefnoset" access="remote" returntype="query">
		<cfargument name="tran" type="string" required="yes">
        <cfargument name="dts" type="string" required="yes">
        <cfargument name="husergrpid" type="string" required="no">
        ...
        <cfelseif lcase(dts) eq "sosbat_i">
        
        <cfquery datasource="#dts#" name="getset">
			select counter,concat(counter," - ",refnocode,lastusedno,refnocode2) as lastno
            from refnoset
			where type = '#tran#'
            <cfif lcase(husergrpid) eq 'sales'>
            and counter in ('1','2','3','4','5','6')
            <cfelseif lcase(husergrpid) eq 'workshop'>
			and counter in ('7','8','9','10','11','12')
			</cfif>
             order by counter
		</cfquery>
