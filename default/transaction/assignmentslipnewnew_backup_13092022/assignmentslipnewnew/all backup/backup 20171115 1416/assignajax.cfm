...
<!---proration rate for 2 month if timesheet cycle is not 01-31, [20170219, Alvin]--->
	<cfif month(lsdateformat(realstartdate, 'yyyy-mm-dd', "en_AU")) neq month(lsdateformat(realenddate, 'yyyy-mm-dd', "en_AU"))>
        ...
        <cfif val(noofdays) neq 0> <!---if eq 0 means work for 0 days--->
			<cfset lvleerate2 = numberformat(employeepay/val(noofdays),'.__')*-1>
            <cfset lvlerrate2 = numberformat(employerpay/val(noofdays),'.__')*-1>
        <cfelse>
			<cfset lvleerate2 = 0>
            <cfset lvlerrate2 = 0>
        </cfif>
...
<cfset govlist = "15,16,42">
      ...
      <tr>
            <td colspan="2">
            <cfset awnowdesp = "">
            <select style="width:200px" name="allowance#a#" id="allowance#a#" onChange="document.getElementById('allowancedesp#a#').value=this.options[this.selectedIndex].id;document.getElementById('allowancedesp#a#').readOnly = false;document.getElementById('awee#a#').readOnly = false;document.getElementById('awer#a#').readOnly = false;">
...
<!---             <cfif awnowdesp neq "">
            ...
            <td>
             <input type="text" name="awee#a#" id="awee#a#" size="10" value="#numberformat(evaluate('awee#a#'),'.__')#" onKeyUp="document.getElementById('awer#a#').value=this.value;calselfallow();calcustallow();" onclick="if(document.getElementById('allowancedesp#a#').value == ''){alert('Please select an Allowance to edit this field')}" <cfif evaluate('allowancedesp#a#') eq ''>readonly</cfif>>
            </td>
             <td style="background-color:#bgcolor#">
             <input type="text" name="awer#a#" id="awer#a#" size="10" value="#numberformat(evaluate('awer#a#'),'.__')#" onkeyup="calcustallow();" onclick="if(document.getElementById('allowancedesp#a#').value == ''){alert('Please select an Allowance to edit this field')}" <cfif evaluate('allowancedesp#a#') eq ''>readonly</cfif>>
            </td>
      </tr>
      </cfloop>
      <tr>
