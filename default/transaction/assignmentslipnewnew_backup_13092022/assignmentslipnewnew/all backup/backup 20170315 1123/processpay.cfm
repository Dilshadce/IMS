...
<cfset bonuspay = 0.00>
<cfloop index='a' from='1' to='18'>
  <cfif evaluate('getawbonus.allowance#a#') eq 9>
     <cfset bonuspay += val(evaluate('getawbonus.awee#a#'))>
  </cfif>
</cfloop>

<cfloop index='a' from='1' to='6'>
  <cfif (evaluate('getfixawbonus.fixawcode#a#') eq 9)>
    <cfset bonuspay += val(evaluate('getfixawbonus.fixawee#a#'))>
  </cfif>
</cfloop>

<cfloop index="a" from="1" to="18">
  <cfif evaluate('url.allowance#a#') eq 9 >
     <cfset bonuspay += val(evaluate('url.aweetax#a#'))>
  </cfif>
</cfloop>

<cfloop index="a" from="1" to="6">
  <cfif (evaluate('url.fixawcode#a#') eq 9) >
    <cfset bonuspay += val(evaluate('url.fixawee#a#'))>
  </cfif>
</cfloop>
...
<cfset basicpay = val(url.basicpay)+val(url.selfphnlsalary) >
           ...
           <cfset OT6 = roundno(OT6,'#get_now_month.sud_otrnd#','#get_now_month.sud_otpsu#')>
            ...
            SELECT refno,emppaymenttype FROM assignmentslip
            WHERE empno="#empno#"
	        ...
	        <cfif epfno neq "" and epfcat neq "X" and fix_cpf eq 0 and epf_selected neq 0>
                ...
                <cfset choosepayin = val(PAYIN) + val(comm_qry.basicpay)>
				
				<!---<cfquery name="awqry_epf" datasource="#db#">
				...
				<cfif isdefined('url.fixawcode#a#') and evaluate('url.fixawee#a#') neq 0.00>

					<cfif listfind(valuelist(awqry_epf.aw_cou),evaluate('url.fixawcode#a#'))>

						<cfset choosepayin = evaluate("#choosepayin# + val(url.fixawee#a#)")>
				...
				<!---EPF of Multiple Assignments--->
           		...
           		<cfloop query="getcurrentassign">
					...
					<cfif weekpay neq '#getcurrentassign.emppaymenttype#'>
...
<!--- 		        <cfif epf1hd eq "Y">
                ...
                <cfif getcurrentassign.recordcount gt 0>
                ...
                <!---<cfoutput>
                <br>gettotalepf: <cfdump var="#gettotalepf#"><br>
                EPFW:<cfdump var="#EPFW#"><br>
                </cfoutput>--->
            ...
            <cfelse>
                ...
                SELECT * FROM awtable where AW_HRD = 1 ORDER BY AW_cou 
                </cfquery>
                ...
                <cfset socso_rate += val(totalsocsowage)>
                <!--- Socso calculation for Multiple Assignments--->
                </cfif>
                ...
                <cfloop query="getcurrentassign">
                    ...
                    <cfset socso_yee-= val(totalsocsoamt.socsoww)>
                    <cfset socso_yer-= val(totalsocsoamt.socsocc)>
                </cfloop>
                <!--- Socso calculation for Multiple Assignments--->
				<cfoutput>
				<br>SOCSO YER: #socso_yer#<br>
				</cfoutput>
				</cfif>
...
<!---            <cfif select_empdata.itaxno neq "" and select_empdata.itaxcat neq "X">--->
            ...
            <cfelse>
                ...
                SELECT 
                <cfloop index="a" from="101" to="115">
                dbded#a#<cfif a neq 115>,</cfif>
                </cfloop> 
                FROM pmast
                WHERE empno="#empno#"
                </cfquery>      
			...
			<!---<cfloop index="a" from="1" to="18">
       <cfif listfind(valuelist(awqry.aw_cou),evaluate('url.allowance#a#')) and evaluate('url.aweetax#a#') neq 0.00>
       		<cfset pcbgrosspay = evaluate("#pcbgrosspay# - val(url.aweetax#a#)")>
            <cfset weekpcbgross = evaluate("#weekpcbgross# - val(url.aweetax#a#)")> 
       </cfif>
       </cfloop>
       
       <cfloop index="a" from="101" to="115">
		   <cfset pcbgrosspay = evaluate("#pcbgrosspay# - val(dbdedqry.dbded#a#)")>
           <cfset weekpcbgross = evaluate("#weekpcbgross# - val(dbdedqry.dbded#a#)")>
       </cfloop>
			
		--->
		
		<cfloop index="a" from="1" to="18">
		<cfif (evaluate('url.allowance#a#') eq 9 and evaluate('url.aweetax#a#') neq 0.00)>
      		<cfset pcbgrosspay = evaluate("#pcbgrosspay# - val(url.aweetax#a#)")>
            <cfset weekpcbgross = evaluate("#weekpcbgross# - val(url.aweetax#a#)")>
		</cfif>
		</cfloop>
	  ...
	  <cfquery name="awqry" datasource="#db#">
		...
		<!---<cfloop index="a" from="1" to="6">
		<cfif isdefined('url.fixawcode#a#') and evaluate('url.fixawee#a#') neq 0.00>
			<cfif listfind(valuelist(awqry.aw_cou),evaluate('url.fixawcode#a#'))>
				<cfset pcbgrosspay = evaluate("#pcbgrosspay# + val(url.fixawee#a#)")>
				<cfset weekpcbgross = evaluate("#weekpcbgross# + val(url.fixawee#a#)")> 
			</cfif>
		</cfif>
		</cfloop>--->
...
<cfabort>   
            	...
            	<cfset currentpcb = numberformat(pcbbonus - currentpcb12 + currentpcb,'.__')>
            </cfif>            
			...
			AND batchno IN (SELECT batches FROM assignmentslip WHERE payrollperiod="#get_now_month.mmonth#" GROUP BY batches)
			AND empno = "#empno#"
			GROUP BY empno
			</cfquery>
			...
			<cfif currentpcb lt 0>
				<cfset currentpcb =0.00>
			</cfif>
...
<!--- </cfif> --->
            ...
            <cfif select_empdata.expattbl neq "" and select_empdata.expatdate neq "" and select_empdata.expatdate neq ''>
                ...
                <cfset currentpcb = 0>
                <cfset expatgross = val(grosspay) + val(pbonus)>
...
<!---            #currentpcb#<br>
			...
			UPDATE #weekpay# 
			SET DED115 = #currentpcb#, 
			itaxpcb = #weekpcbgross#, 
			DED114 = #expatded# 
			WHERE empno = "#empno#"
			</cfquery>
	
			<cfquery name="update_pcb_to_paytra" datasource="#db#">
                  SELECT DED115,itaxpcb,DED114, paydate 
                  FROM #weekpay#
                  WHERE empno = "#empno#"  
                  AND paydate <> ""
			...
			<cfloop query="update_pcb_to_paytra">
				...
				<br><br>
				Resident Tax Amount = #currentpcb#<br> 
				<cfif select_empdata.expattbl neq "" and select_empdata.expatdate neq "" and select_empdata.expatdate neq ''>
					Taxable Gross = #expatgross#<br>
                <cfelse>
					Taxable Gross = #pcbgrosspay#<br>
				</cfif>
				Non - Resident Tax Amount = #expatded#<br>
				Bonus Taxable Gross: #pbonus#
				<br>
				</cfoutput>
...
<cfset totherded=0>
            ...
            <cfset tDW +=selectdw.DW>
            </cfif>
            <cfif selectdw.BASICPAY neq ''>
			<cfset tBASICPAY +=selectdw.BASICPAY>
            </cfif>
            <cfif selectdw.NPLPAY neq ''>
            <cfset ttotalNPL+=selectdw.NPLPAY>
            </cfif>
            <cfif selectdw.OT1 neq ''>
            <cfset tOT1+=selectdw.OT1>
            </cfif>
            <cfif selectdw.OT2 neq ''>
            <cfset tOT2+=selectdw.OT2>
            </cfif>
            <cfif selectdw.OT3 neq ''>
            <cfset tOT3+=selectdw.OT3>
            </cfif>
            <cfif selectdw.OT4 neq ''>
            <cfset tOT4+=selectdw.OT4>
            </cfif>
			<cfif selectdw.OT5 neq ''>
            <cfset tOT5+=selectdw.OT5>
            </cfif>
            <cfif selectdw.OT6 neq ''>
            <cfset tOT6+=selectdw.OT6>
            </cfif>
            <cfif selectdw.OTPay neq ''>
            <cfset tOTtotal+=selectdw.OTPay>
            </cfif>
            <cfif selectdw.TAW neq ''>
            <cfset tTAW+=selectdw.TAW>
            </cfif>
            <cfif selectdw.grosspay neq ''>
            <cfset tgrosspay+=val(selectdw.grosspay)>
            </cfif>
            <cfif selectdw.EPFWW neq ''>
            <cfset tEPFEE+=selectdw.EPFWW>
            <cfoutput>
            <br> tEPFEE: #tEPFEE#
            </cfoutput>
            </cfif>
            <cfif selectdw.EPFCC neq ''>
            <cfset tEPFER+=selectdw.EPFCC>
            <cfoutput>
            <br> tEPFER: #tEPFER#
            </cfoutput>
            </cfif>
            <cfif selectdw.epf_pay neq ''>
            <cfset tepf_pay+=selectdw.epf_pay>
            </cfif>
            <cfif selectdw.TDED neq ''>
            <cfset tTDED +=selectdw.TDED>
            </cfif>
            <cfif selectdw.TDEDU neq ''>
            <cfset tTDEDU+=selectdw.TDEDU>
            </cfif>
            <cfif selectdw.NETPAY neq ''>
            <cfset tNETPAY+=NETPAY>
            </cfif>
            <cfif selectdw.epf_pay_a neq ''>
            <cfset tepf_pay_a+=selectdw.epf_pay_a>
            <cfoutput>
            <br>#getcurrentassign.emppaymenttype#. #selectdw.epf_pay_a#
            </cfoutput>
            </cfif> 
            <cfif selectdw.cpf_amt neq ''>
            <cfset tcpf_amt+=selectdw.cpf_amt>
            </cfif>
            <cfif isdefined('additionalwages')>
				<cfif selectdw.additionalwages neq ''>
					<cfset tadditionalwages+=selectdw.additionalwages>
				</cfif>
            </cfif>
            <cfif isdefined('hourrate')>
				<cfif selectdw.total_late_h neq ''>
					<cfset ttotal_late_h+=selectdw.total_late_h>
				</cfif>
				<cfif selectdw.total_earlyD_h neq ''>
					<cfset ttotal_earlyD_h+=selectdw.total_earlyD_h>
				</cfif>
				<cfif selectdw.total_noP_h neq ''>
					<cfset ttotal_noP_h+=selectdw.total_noP_h>
				</cfif>
				<cfif selectdw.total_work_h neq ''>
					<cfset ttotal_work_h+=selectdw.total_work_h>
				</cfif>
				<cfif selectdw.hourrate neq ''>
				<cfset thourrate+=selectdw.hourrate>
				</cfif>
            </cfif>
            <cfif selectdw.nsded neq ''>
            <cfset tnsded +=selectdw.nsded>
            </cfif>
            <cfif selectdw.otherded neq ''>
            <cfset totherded+=selectdw.otherded>
            </cfif>
            </cfloop>
            

			<cfquery name="updatedw" datasource="#db#">
		        UPDATE #url.paydate# SET DW = #val(tdW)# , 
				BASICPAY = #tbasicpay#, 
				NPLPAY = #ttotalNPL#, 
				OT1 = #tOT1#, 
				OT2 = #tOT2# , OT3 = #tOT3# , OT4 = #tOT4# , OT5 = #tOT5# , OT6 = #tOT6# , 
				OTPay = #tOTtotal# , TAW = #ttaw# , grosspay=#tgrosspay#, EPFWW=#tEPFEE#, 
				EPFCC=#tEPFER#, 
				epf_pay = #tepf_pay#, TDED=#ttded# , 
				TDEDU = #tTDEDU#, NETPAY = #tnetpay#, 
				epf_pay_a = #tepf_pay_a# , cpf_amt="#payin_2nd#"
                <cfif isdefined('additionalwages')>
