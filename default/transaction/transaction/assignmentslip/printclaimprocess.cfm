...
SELECT * FROM (
select empno,coalesce(addchargeself,0)+coalesce(addchargeself2,0)+coalesce(addchargeself3,0)+coalesce(addchargeself4,0)+coalesce(addchargeself5,0)+coalesce(addchargeself6,0) as totalamt, concat(
...
) as chargeamt,refno,placementno
...
LEFT JOIN
(
SELECT nricn,bankaccno,name, empno as emp FROM beps_p.PMAST
) as b
on a.empno = b.emp
LEFT JOIN
(
...
LEFT JOIN
(
SELECT area,desp from #target_icarea#
) as d
