...
SELECT * FROM (
select empno,coalesce(addchargeself,0)+coalesce(addchargeself2,0)+coalesce(addchargeself3,0)+coalesce(addchargeself4,0)+coalesce(addchargeself5,0)+coalesce(addchargeself6,0) as totalamt from assignmentslip where (
claimadd1 = "Y"
or claimadd2 = "Y"
or claimadd3 = "Y"
or claimadd4 = "Y"
...
LEFT JOIN
(
SELECT nricn,bankaccno,name, empno as emp,bankcode FROM beps_p.PMAST
) as b
on a.empno = b.emp
</cfquery>
