...
<cfif type neq "Delete">
	...
	<head>
		...
		<script language="javascript" type="text/javascript">
			...
			function getResult(result){
				if(result>0){
					<cfif sign eq -1>
					change('stock_in','block');change('stock_out','none');getSerialnoSelect();<cfif lcase(hcomid) eq "dental_i" or lcase(hcomid) eq "dental10_i" or lcase(hcomid) eq "mfss_i" or lcase(hcomid) eq "hcss_i" or lcase(hcomid) eq "iaf_i">document.serialForm.serialnoselect.value="";document.serialForm.serialnoselect.focus();<cfelse>document.serialForm.select_serialno.focus();</cfif>
					<cfelse>
			...
			function addNewSerialno(){
				...
				<cfif (lcase(hcomid) eq "dental_i" or lcase(hcomid) eq "dental10_i" or lcase(hcomid) eq "mfss_i" or lcase(hcomid) eq "hcss_i" or lcase(hcomid) eq "iaf_i") and sign neq "-1">
				...
				<cfif (lcase(hcomid) eq "dental_i" or lcase(hcomid) eq "dental10_i" or lcase(hcomid) eq "mfss_i" or lcase(hcomid) eq "hcss_i" or lcase(hcomid) eq "iaf_i") and sign eq "-1">
				...
				<cfif lcase(hcomid) eq "dental_i" or lcase(hcomid) eq "dental10_i" or lcase(hcomid) eq "mfss_i" or lcase(hcomid) eq "hcss_i" or lcase(hcomid) eq "iaf_i">
				try{
	...
	<body onLoad="init()">
		...
		<br>
        <cfif getgsetup.serialnorun eq 'Y' and form.tran eq "INV" or form.tran eq "DO" or form.tran eq "CN" or form.tran eq "DN" or form.tran eq "ISS">
        <table class="data">
            <tr>
        ...
        <cfif getgsetup.serialnorun eq 'Y' and (form.tran eq 'RC' or form.tran eq "OAI")>
          <table class="data">
            <tr>
		...
		<tr id="stock_out">
			<td>New</td>
			<td align="right"><input type="text" name="text_serialno" maxlength="<cfif lcase(hcomid) neq "dental_i" or lcase(hcomid) neq "dental10_i" or lcase(hcomid) neq "mfss_i" or lcase(hcomid) neq "hcss_i" or lcase(hcomid) neq "iaf_i">100<cfelse>100</cfif>" size="30" onKeyPress="return handleEnter(event)"></td>
