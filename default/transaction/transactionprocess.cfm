...
<cfif lcase(hcomid) eq "bsssb_i" or lcase(hcomid) eq "puss_i" or lcase(hcomid) eq "rvsb_i" or lcase(hcomid) eq "hnm_i">
	
	<cfset amt1_bil=(val(form.crequestdate) * val(form.price))+0.000001>
	<cfset amt1_bil=numberformat((fix(amt1_bil*100)/100),".__")>	
    
<cfelse>
<cfif val(form.price) eq 0 and getGeneralInfo.editamount eq "1">
	<cfset amt1_bil=val(form.amt)>
<cfelseif val(form.qty) eq 0>
	<cfset amt1_bil=val(form.amt)>
<cfelseif lcase(hcomid) eq "thats_i">
	<cfset amt1_bil=val(form.amt)>
<cfelseif lcase(hcomid) eq "bsssb_i">
	<cfset amt1_bil=(val(form.qty) * val(form.price))+0.000001>
	<cfset amt1_bil=numberformat((fix(amt1_bil*100)/100),".__")>	
<cfelse>
	<cfset amt1_bil=(val(form.qty) * val(form.price))+0.000001>
	<cfset amt1_bil=numberformat(amt1_bil,".__")>	
...
<cfquery datasource="#dts#" name="getgsetup2">
	select * from gsetup2
        ...
        <cfset realamt1 = numberformat(((100-val(form.dispec1)) / 100)*val(form.price),stDecl_UPrice)* val(form.qty)>
		<cfset disamt_bil1 = numberformat((val(form.price)*val(form.qty))-realamt1,stDecl_UPrice)>
        <cfset netamttemp = amt1_bil - disamt_bil1>
        
        <cfset realamt2 = numberformat(((100-val(form.dispec2)) / 100)*(netamttemp/val(form.qty)),stDecl_UPrice)* val(form.qty)>
        <cfset disamt_bil2 = numberformat(netamttemp-realamt2,stDecl_UPrice)>
        <cfset netamttemp = netamttemp - disamt_bil2>
        
        <cfset realamt3 = numberformat(((100-val(form.dispec3)) / 100)*(netamttemp/val(form.qty)),stDecl_UPrice)* val(form.qty)>
        ...
        <!---
		<cfset disamt_bilnew1 = numberformat(((round(val(((val(form.dispec1) / 100) * val(form.price) * -1*10000)))/10000)
        *-1),stDecl_UPrice)>
        <cfset disamt_bil1 = numberformat(((round(val(((val(form.dispec1) / 100) * val(form.price) * -1*10000)))/10000)
        *-1),stDecl_UPrice)* val(form.qty)>
        <cfset disamt_bilnew2 = (numberformat((val(form.dispec2) / 10000) * (val(form.price)-((val(form.dispec1) / 100) * val(form.price)))))>
        <cfset disamt_bil2 = (numberformat((val(form.dispec2) / 10000) * (val(form.price)-((val(form.dispec1) / 100) * val(form.price)))))* val(form.qty)>
        <cfset disamt_bilnew3 = (numberformat((val(form.dispec3) / 100) * (val(form.price)-((val(form.dispec1) / 100) * val(form.price))-((val(form.dispec2) / 100) * (val(form.price)-((val(form.dispec1) / 100) * val(form.price)))))))>
        ...
        <cfset dispriceno = val(form.price)- (val(disamt_bilnew1) + val(disamt_bilnew2) + val(disamt_bilnew3))>
        <cfset dispriceno = numberformat(dispriceno,stDecl_UPrice)>
        <cfset netamttemp = numberformat(dispriceno * val(form.qty),stDecl_UPrice)>
        <cfset disamt_bil = numberformat(val(form.price) * val(form.qty),stDecl_UPrice) - netamttemp>
		--->
...
<cfset amt_bil = numberformat(val(netamt),'.__')>
...
<cfif tran eq 'CN'>
<cftry>
<cfinclude template = "transactionprocess_cn_cal_it_cos.cfm">
...
<cfif getitem.wserialno eq "T" and (tran neq "QUO" and tran neq "SO" and tran neq "PO" and tran neq "RQ")>
	<cfif tran eq "SAM">
