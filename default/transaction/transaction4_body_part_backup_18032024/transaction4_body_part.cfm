...
<cfif lcase(hcomid) eq "mika_i" and (tran eq "SO" or tran eq "CS" or tran eq "DO" or tran eq "INV" or tran eq "QUO")>
<cfquery name="checkitemoriprice" datasource="#dts#">
	SELECT price from icitem where itemno=<cfqueryparam cfsqltype="cf_sql_varchar" value="#itemno#">
</cfquery>

<cfif price lt checkitemoriprice.price>
	<cfset getGeneralInfo.disclimit=0>
</cfif>

</cfif>

<cfif dispec1 gt getGeneralInfo.disclimit and getGeneralInfo.disclimit neq 0>
<input type="hidden" name="discountlimit" id="discountlimit" value="#dispec1#">
<cfelse>
<input type="hidden" name="discountlimit" id="discountlimit" value="#getGeneralInfo.disclimit#">
</cfif>
    
<cfif isdefined('form.placementno')>
    <input type="hidden" name="placementno" id="placementno" value="#form.placementno#">
</cfif>
...
function settaxcode()
	...
	function calcDiscLimit(){
		var discAmt = document.getElementById("discamt_id").value; 
		var ttlAmt = document.getElementById("amt").value;
		var discPerc = 0;
		
		if(isNaN(discAmt)){
			alert("The value is not a number. Please try again");
			document.getElementById("discamt_id").value = "0.00";
		}
		else{
			discPerc = discAmt/ttlAmt*100;
			if(discPerc>document.getElementById("discountlimit").value && document.getElementById('discountlimitcontrol').value=="0"){
...
<table align='center' class='data' width='85%' cellspacing="0">
    ...
    <tr>
        <th width="35%">#words[121]#<cfif lcase(hcomid) eq "kjcpl_i" or lcase(hcomid) eq "mlpl_i" or lcase(hcomid) eq "kjctrial_i" or lcase(hcomid) eq "viva_i"><br />Product Code</cfif></th>
        <td colspan='3'>
        ...
        <th <cfif getdisplaysetup2.body_bal_on_hand neq "Y">style="visibility:hidden"</cfif>><cftooltip autodismissdelay="60000" sourcefortooltip="/default/transaction/itembal2.cfm?itemno=#URLENCODEDFORMAT(itemno)#&project=&job=&batchcode=">#words[1113]#</cftooltip></th>
    	<td <cfif getdisplaysetup2.body_bal_on_hand neq "Y">style="visibility:hidden" <cfelseif getservice.itemtype eq "SV">style="visibility:hidden"</cfif>><input name='balance' id='balance' type='text' size='10' maxlength='10'value='#listfirst(balonhand)#' readonly>
        <div id="balanceAjax"></div>
        </td>
  
        
    </tr>
    <tr>
        <th rowspan='2'><cfif lcase(HcomID) eq "asaiki_i">Supplier Part Number<cfelse>#words[65]#</cfif></th>
        ...
        <cfelse>
			<td colspan='5' nowrap><input name='desp' id="dep5" tabindex="1" type='text' value="#convertquote(desp)#" size='60' maxlength='450'></td>
		</cfif>
    </tr>
    ...
    <tr>
        <td colspan='3' nowrap>
			...
			<cfelse>
				<input name='despa' id="dep6" type='text' tabindex="2" value="#convertquote(despa)#" size='60' maxlength='450'>
			</cfif>		</td>
    ...
    <tr>
        <td style="background-color:##D6ECFC;font-size:13px;padding:0px 10px 0px 10px;" align="right"><strong>#words[127]#</strong></td>
        <td colspan='3'>
			...
			<cfelse>
				...
				<cfelse>
                    ...
                    <cfelseif lcase(hcomid) eq "d72ipl_i">
                    <textarea name='comment' id="comm5" tabindex="3"  cols='100' rows='5' wrap="soft" style="font-family:'Courier New', Courier, monospace">#convertquote(comment)#</textarea>
                    <cfelse>
					<textarea name='comment' id="comm5" tabindex="3" <cfif lcase(hcomid) eq "agape_i"> cols='70'<cfelse> cols='#getGeneralInfo.commentlimit#'</cfif> rows='5' <cfif lcase(hcomid) eq "spcivil_i" >wrap="soft"<cfelse>wrap="hard"</cfif> style="font-family:'Courier New', Courier, monospace">#convertquote(comment)#</textarea>
					</cfif>
                </cfif>
        ...
        <td colspan='2'>
            ...
            <cfif lcase(hcomid) neq "mastercare_i" and lcase(hcomid) neq "gorgeous_i">
				<div <cfif getdisplaysetup2.body_searchcomment neq "Y">style="visibility:hidden"</cfif>><input type="button" name="searchComment" style="width: 100px; height: 20px; font-size:9px" value="#words[1291]#" onClick="javascript:ColdFusion.Window.show('findComment');"></div>
                ...
                <div <cfif getdisplaysetup2.body_createnewcomment neq "Y">style="visibility:hidden"</cfif>><input type="button" name="createComment" style="width: 100px; height: 20px; font-size:9px" value="#words[464]#" onClick="window.open('/default/maintenance/commenttable2.cfm?type=Create')"></div>
            ...
            <cfif lcase(hcomid) neq "mastercare_i" and lcase(hcomid) neq "gorgeous_i">
            <div <cfif getdisplaysetup2.body_uploadimage neq "Y">style="visibility:hidden"</cfif>><input type="button" name="uploadimage" style="width: 100px; height: 20px; font-size:9px" value="#words[135]#" onClick="javascript:ColdFusion.Window.show('uploadimage');"></div>
            </cfif>
	...
	<cfif isservice eq 1 and getGeneralInfo.showservicepart eq 'Y'>
		...
		<tr>
        	...
        	<th <cfif getdisplaysetup2.body_service neq "Y">style="visibility:hidden"</cfif>>#words[321]#</th>
        ...
        <tr>
          	<th <cfif getdisplaysetup2.body_service neq "Y">style="visibility:hidden"</cfif>>#words[296]#</th>
	...
	<cfif lcase(hcomid) eq "sdc_i" or lcase(hcomid) eq "sdab_i" or lcase(hcomid) eq "chequer_i" or lcase(hcomid) eq "demoindo_i">
    <tr>
	...
	<tr>
        <th rowspan="2" <cfif getdisplaysetup2.body_uom neq "Y">style="visibility:hidden"</cfif>>#words[2180]#</th>
        <!--- REMARK ON 190908 --->
        ...
        <th nowrap><cfif lcase(hcomid) eq "marquis_i" or lcase(HcomID) eq "hempel_i">PKG<cfelse>#words[1121]#</cfif></th>
		<!--- ADD ON 05-06-2009 --->
        ...
        <input type="hidden" name="discountlimitcontrol" id="discountlimitcontrol" value="0" />
        
        <!---check minimum selling price--->
        
        <cfquery name="checkminimumsellingprice" datasource="#dts#">
			select
			price,ucost,fcurrcode,fprice,fucost<cfloop from="2" to="10" index="i">,fcurrcode#i#,fprice#i#,fucost#i#</cfloop>
			from icitem 
			where itemno='#itemno#'
		</cfquery>
        
        <cfif getbillcurrcode.currcode eq checkminimumsellingprice.fcurrcode>
		<cfset checkminimumsellingprice.price=checkminimumsellingprice.fprice>
        <cfset checkminimumsellingprice.ucost=checkminimumsellingprice.fucost>
        </cfif>
        
        <cfloop from="2" to="10" index="i">
        <cfif getbillcurrcode.currcode eq evaluate('checkminimumsellingprice.fcurrcode#i#')>
        <cfset checkminimumsellingprice.price= evaluate('checkminimumsellingprice.fprice#i#')>
        <cfset checkminimumsellingprice.ucost= evaluate('checkminimumsellingprice.fucost#i#')>
        </cfif>
        </cfloop>
        ...
        <cfset newdate = "0000-00-00">
			...
			<cfif checkcustom.customcompany eq "Y">
				...
				<cfquery name="insert" datasource="#dts#">
					...
					<cfqueryparam cfsqltype="cf_sql_char" value="#trim(form.items)#">)
				</cfquery>
			</cfif>
		...
		<cfquery name="checkbatch" datasource="#dts#">
			...
			<cfif multilocation eq "Y" and is_service neq 1>
				<input name='qty' id="qt6" tabindex="4" type='text' size='10' maxlength='10' value='#batchqty#' onkeyup="<cfif url.type1 eq "Add" and checkpromotion.recordcount neq 0 and checkpromotion.type eq "buy" and lcase(hcomid) neq "mika_i">buypromo();</cfif><cfif lcase(hcomid) eq "topsteelhol_i" or lcase(hcomid) eq "topsteel_i">checkqtytopsteel();<cfelseif lcase(hcomid) eq "bsssb_i" or lcase(hcomid) eq "puss_i" or lcase(hcomid) eq "rvsb_i" or lcase(hcomid) eq "hnm_i">checkqtybsssb();<cfelse>checkqty();</cfif>" readonly>
			<cfelse>
				<cfif graded neq "" and graded eq "Y">
					<input name='qty' id="qt6" tabindex="4" type='text' size='10' maxlength='10' value='#batchqty#' onkeyup="<cfif url.type1 eq "Add" and checkpromotion.recordcount neq 0 and checkpromotion.type eq "buy" and lcase(hcomid) neq "mika_i">buypromo();</cfif><cfif lcase(hcomid) eq "topsteelhol_i" or lcase(hcomid) eq "topsteel_i">checkqtytopsteel();<cfelseif lcase(hcomid) eq "bsssb_i" or lcase(hcomid) eq "puss_i" or lcase(hcomid) eq "rvsb_i" or lcase(hcomid) eq "hnm_i">checkqtybsssb();<cfelse>checkqty();</cfif>" readonly>
					<input type="button" value="Grade" onclick="AssignGrade();">
    ...
    <tr>
        ...
        <th><cfif lcase(hcomid) eq "visionlaw_i">Fee<cfelseif lcase(hcomid) eq "marquis_i" or lcase(HcomID) eq "hempel_i">Cap<cfelseif tran eq 'RQ'>#words[1132]#<cfelse>#words[1096]#</cfif></th>
        <td> 
			<input name='price' id="pri6" tabindex="5" type='text' <cfif lcase(hcomid) eq "meisei_i" >size='10' maxlength='15' <cfelse>size='10' maxlength='15'</cfif> value='<cfif (lcase(hcomid) eq "marquis_i" or lcase(HcomID) eq "hempel_i") and isdefined('form.batchcap')>#numberformat(val(form.batchcap),stDecl_UPrice)#<cfelse>#numberformat(price,stDecl_UPrice)#</cfif>'<cfif (lcase(hcomid) eq "gel_i" and husergrpid neq "Admin" and husergrpid neq "Super") or (wpformula eq "1" and trim(getGeneralInfo.priceformula) neq "")> readonly</cfif> <cfif getpin2.h2F00 neq 'T'>readonly</cfif> onkeyup="<cfif url.type1 eq "Add" and checkpromotion.recordcount neq 0 and checkpromotion.type eq "buy" and lcase(hcomid) neq "mika_i">buypromo();</cfif><cfif lcase(hcomid) eq "topsteelhol_i" or lcase(hcomid) eq "topsteel_i">checkqtytopsteel();<cfelseif lcase(hcomid) eq "bsssb_i" or lcase(hcomid) eq "puss_i" or lcase(hcomid) eq "rvsb_i" or lcase(hcomid) eq "hnm_i">checkqtybsssb();<cfelse>checkqty();</cfif><cfif lcase(hcomid) eq "asiasoft_i">getProfit();</cfif>" onblur="checkqty();">
       ...
       <th><cfif lcase(hcomid) eq "visionlaw_i">Fee<cfelseif lcase(hcomid) eq "marquis_i" or lcase(HcomID) eq "hempel_i">Cap<cfelse>#words[1096]#</cfif></th>
        <td> 
			<input name='price' id="pri6" tabindex="5" type='text' <cfif lcase(hcomid) eq "meisei_i" >size='10' maxlength='15' <cfelse>size='10' maxlength='15'</cfif> value='<cfif (lcase(hcomid) eq "marquis_i" or lcase(HcomID) eq "hempel_i") and isdefined('form.batchcap')>#numberformat(val(form.batchcap),stDecl_UPrice)#<cfelse>#numberformat(price,stDecl_UPrice)#</cfif>'<cfif (lcase(hcomid) eq "gel_i" and husergrpid neq "Admin" and husergrpid neq "Super") or (wpformula eq "1" and trim(getGeneralInfo.priceformula) neq "")> readonly</cfif> <cfif getpin2.h2F00 neq 'T'>readonly</cfif> onkeyup="<cfif url.type1 eq "Add" and checkpromotion.recordcount neq 0 and checkpromotion.type eq "buy" and lcase(hcomid) neq "mika_i">buypromo();</cfif><cfif lcase(hcomid) eq "topsteelhol_i" or lcase(hcomid) eq "topsteel_i">checkqtytopsteel();<cfelseif lcase(hcomid) eq "bsssb_i" or lcase(hcomid) eq "puss_i" or lcase(hcomid) eq "rvsb_i" or lcase(hcomid) eq "hnm_i">checkqtybsssb();<cfelse>checkqty();</cfif>" onblur="checkqty();">
			<cfif hcomid eq "fincom_i">
				<cfinclude template="../fincom/select_special_item_price_to_transaction.cfm">
			</cfif>
			<cfif wpformula eq "1" and trim(getGeneralInfo.priceformula) neq "">
    ...
    <cfquery name="checkpromotion2" datasource="#dts#">
        ...
        <cfif getpin2.h1360 eq 'T' and (tran eq 'PO' or tran eq 'RC' or tran eq 'PR' or tran eq 'RQ')>
        <th <cfif lcase(hcomid) eq "visionlaw_i">style="visibility:hidden"</cfif>><cfif lcase(hcomid) eq "marquis_i" or lcase(HcomID) eq "hempel_i">Qty<cfelse>#words[1097]#</cfif></th>
        ...
        <cfelse>
            ...
            <cfelseif getpin2.h1361 eq 'T' and (tran eq 'INV' or tran eq 'DO' or tran eq 'CS' or tran eq 'CN' or tran eq 'DN' or tran eq 'QUO' or tran eq 'SO' or tran eq 'SAM')>
             <th><cfif lcase(hcomid) eq "marquis_i" or lcase(HcomID) eq "hempel_i">Qty<cfelse>#words[1097]#</cfif></th>
        <td>
        <cfif getGeneralInfo.itemdiscmethod eq 'byprice'>
        <input name="amt" id="amt" tabindex="6" type='text' size='10' maxlength='10' value='#numberformat(amt,".__")#'<cfif lcase(hcomid) eq "gel_i" and husergrpid neq "Admin" and husergrpid neq "Super">readonly</cfif> onkeyup="getDiscount2();<cfif getGeneralInfo.editamount eq '1'>document.form1.price.value=0;</cfif>">
        <cfelse>
    ...
    <tr>
		...
		<cfif lcase(hcomid) eq "mhca_i">Marketer<cfelseif lcase(hcomid) eq "nikbra_i">Dept Code<cfelse>#words[482]#</cfif></cfif></th>
		<input type="hidden" name="oldlocation" value="#xlocation#">
        <td 
        
		<cfif getdisplaysetup2.body_location neq "Y">style="visibility:hidden"</cfif>>
			<cfif multilocation eq "Y" and is_service neq 1>
			...
			</cfif>		<br /><a onClick="ColdFusion.Window.show('createlocationAjax');" onMouseOver="this.style.cursor='hand';" >#words[485]#</a></td>
        ...
        <th <cfif lcase(hcomid) eq "visionlaw_i" or getdisplaysetup2.body_discountbody neq "Y">style="visibility:hidden"</cfif>>#words[592]# (%)</th>
    ...
    <tr>
        ...
        <cfelseif lcase(hcomid) eq "SDC_i" or lcase(hcomid) eq "sdab_i" or lcase(hcomid) eq "demoindo_i">
        ...
        <!---Added by Nieo 20180118 to cater Perm billing Finance requirement--->
            <cfelseif left(lcase(hcomid),8) eq "manpower">
                <div style="width: 11em">
                    <input type='hidden' class="JOFilter"  name='requestdate' id="req3" value='#convertquote(brem1)#' size='20'>
                </div>
        <!---Added by Nieo 20180118 to cater Perm billing Finance requirement--->
		<cfelse>
        <cfif trim(getGeneralInfo.bodyremark1list) eq ''>
            ...
            <cfloop list="#getGeneralInfo.bodyremark1list#" index="i">
            <option value="#i#" <cfif brem1 eq i>selected</cfif>>#i#</option>
            </cfloop>
            </select>
            </cfif>
        ...
        <cfif trim(getGeneralInfo.bodyremark2list) eq ''>
            <input type='text' <cfif lcase(hcomid) eq "asiasoft_i">onkeyup="getProfit();"</cfif> name='crequestdate' id="creq4"  maxlength='40' value='<cfif (lcase(hcomid) eq "marquis_i" or lcase(HcomID) eq "hempel_i") and isdefined('form.importpermit')>#form.importpermit#<cfelseif lcase(hcomid) eq 'fixics_i'>#getitembal.qty2#<cfelse>#convertquote(brem2)#</cfif>' size='20'>
            <cfelse>
            <select name="crequestdate" id="creq4">
            <option value="">Please choose</option>
            <cfloop list="#getGeneralInfo.bodyremark2list#" index="i">
            <option value="#i#" <cfif brem2 eq i>selected</cfif>>#i#</option>
            </cfloop>
            </select>
            </cfif>
    ...
    <tr>
        ...
        <cfelse> 
          ...
          <cfif trim(getGeneralInfo.bodyremark3list) eq ''>
            <input type='text' name='brem3' id="br36" maxlength='40' value='#convertquote(brem3)#' size='30'<cfif lcase(hcomid) eq "accord_i">onblur='JavaScript:changebrem4(this.value);'</cfif> >
            <cfelse>
            <select name="brem3" id="br36">
            <option value="">Please choose</option>
            <cfloop list="#getGeneralInfo.bodyremark3list#" index="i">
            <option value="#i#" <cfif brem3 eq i>selected</cfif>>#i#</option>
            </cfloop>
            </select>
            </cfif>
        ...
        <td <cfif getdisplaysetup2.body_remark4 neq "Y">style="visibility:hidden"</cfif>><cfif lcase(hcomid) eq "aimpest_i"><textarea name='brem4' id="br46" size='20' rows="3">#convertquote(brem4)#</textarea>
        <cfelse>
        
        <cfif trim(getGeneralInfo.bodyremark4list) eq ''>
            <input type='text' name='brem4' id="br46" maxlength='40' value='#convertquote(brem4)#' size='20' <cfif lcase(hcomid) eq "litelab_i" and husergrpid neq 'admin'>readonly</cfif>>
            <cfelse>
            <select name="brem4" id="br46">
            <option value="">Please choose</option>
            <cfloop list="#getGeneralInfo.bodyremark4list#" index="i">
            <option value="#i#" <cfif brem4 eq i>selected</cfif>>#i#</option>
            </cfloop>
            </select>
            </cfif>
    ...
    <tr>
		<cfif lcase(hcomid) eq "avent_i" >
			<td colspan="2">
				&nbsp;<input type='checkbox' name='nodisplay' id="nodisplay" value='Y'  <cfif nodisplay eq "Y">checked</cfif>>&nbsp;&nbsp;#words[2172]#</td>
		...
		<cfelse>
        	...
        	<th <cfif getdisplaysetup2.body_nodisplay neq "Y">style="visibility:hidden"</cfif>>#words[2172]#</th>
            ...
            <th <cfif getdisplaysetup2.body_initial neq "Y">style="visibility:hidden"</cfif>>#words[2171]#</th>
            ...
            <th <cfif getdisplaysetup2.body_as_voucher neq "Y">style="visibility:hidden"</cfif>>#words[2170]#</th>
    	...
    	<th <cfif lcase(hcomid) eq "visionlaw_i" or getdisplaysetup2.body_discountbody neq "Y">style="visibility:hidden"</cfif>>#words[2169]#</th>
        ...
        <cfelseif getpin2.h1361 eq 'T' and (tran eq 'INV' or tran eq 'DO' or tran eq 'CS' or tran eq 'CN' or tran eq 'DN' or tran eq 'QUO' or tran eq 'SO' or tran eq 'SAM')>
        <th <cfif lcase(hcomid) eq "visionlaw_i" or getdisplaysetup2.body_discountbody neq "Y">style="visibility:hidden"</cfif>><cfif lcase(hcomid) eq "nil_i"><cfelse>#words[2169]#</cfif></th>

		<cfif lcase(hcomid) eq "nil_i">
        <td><input type="hidden" name="discamt" id="discamt_id" tabindex="11" size="10" onKeyUp="getDiscountControl();" value="#numberformat(discamt,stDecl_Discount)#" ></td>
        <cfelse>
			<cfif dts EQ "mika_i" AND (tran EQ "SO" OR tran EQ "CS")>
            	<td><input type="text" name="discamt" id="discamt_id" tabindex="11" size="10" onKeyUp="calcDiscLimit();" value="#numberformat(discamt,stDecl_Discount)#" ></td>
            <cfelse>
            	<td <cfif lcase(hcomid) eq "visionlaw_i" or getdisplaysetup2.body_discountbody neq "Y">style="visibility:hidden"</cfif>><input type="text" name="discamt" id="discamt_id" tabindex="11" size="10" onKeyUp="getDiscountControl();" value="#numberformat(discamt,stDecl_Discount)#" ></td>
        	</cfif>
        </cfif>
    ...
    <cfquery name="getasaikisample" datasource="#dts#">
    select refno from iserial where type="SAM" and (void="" or void is null) and (generated="" or generated is null) and fperiod <> "99" and itemno=<cfqueryparam cfsqltype="cf_sql_varchar" value="#itemno#"> group by type,refno
    </cfquery>
    <tr>
	        <th>PACKING LIST NO</th>
	        <td>
            <select name="brem5" id="brem5" onchange="ajaxFunction(document.getElementById('asaikipackinglist'),'asaikipackinglist.cfm?itemno=#URLENCODEDFORMAT(itemno)#&refno='+escape(this.value))">
            <option value="">Select a packing list</option>
            <cfloop query="getasaikisample">
            <option value="#getasaikisample.refno#" <cfif brem5 eq getasaikisample.refno>selected</cfif>>#getasaikisample.refno#</option>
            </cfloop>
            </select>
							</td>
			<th>PACKING LIST ITEM</th>
	        <td>
            	<div id="asaikipackinglist">
				<select name="brem6" id="brem6">
                <option value="">Choose an Item</option>
                 <cfif brem5 neq "">
                <cfquery name="getasaikisampleitem" datasource="#dts#">
select trancode,itemno,price_bil from ictran where type="SAM" and (void="" or void is null) and (toinv="" or toinv is null) and fperiod <> "99" and itemno = <cfqueryparam cfsqltype="cf_sql_varchar" value="#URLDECODE(url.itemno)#"> and refno = <cfqueryparam cfsqltype="cf_sql_varchar" value="#brem5#"> order by trancode
</cfquery>
                <cfloop query="getasaikisampleitem">
                <option value="#getasaikisampleitem.trancode#" <cfif brem6 eq getasaikisampleitem.trancode>selected</cfif>>#getasaikisampleitem.trancode# - #getasaikisampleitem.itemno# @ #numberformat(getasaikisampleitem.price_bil,',.__')#</option>
                </cfloop>
                </cfif>
                </select>
                </div>
                <input type='hidden' name='brem7' id="brem7" maxlength='40' value="#convertquote(brem7)#" size='20'>
                <input type='hidden' name='brem8' id="brem8" maxlength='40' value="#convertquote(brem7)#" size='20'>
                <input type='hidden' name='brem9' id="brem9" maxlength='40' value="#convertquote(brem7)#" size='20'>
                </td>
	    </tr>
    ...
    <th <cfif getdisplaysetup2.body_cost_code neq "Y">style="visibility:hidden"</cfif>><cfif lcase(hcomid) eq "excelhw_i" or lcase(hcomid) eq "syprecision_i">Unit Cost<cfelse>Cost Code</cfif></th>
    ...
    <tr><th>#words[191]#</th><td><input type='text' <cfif lcase(hcomid) eq "exceljb_i">style="background-color:##FF9;"</cfif> name='displayprice1' id="displayprice1" value="#numberformat(getallprice.price,',_.____')#" readonly></td></tr>
    <tr><th>#words[192]#</th><td><input type='text' <cfif lcase(hcomid) neq "exceljb_i">style="background-color:##FF9;"</cfif> name='displayprice2' id="displayprice2" value="#numberformat(getallprice.price2,',_.____')#" readonly></td></tr>
	<tr><th>#words[195]#</th><td><input type='text' name='displayprice3' id="displayprice3" value="#numberformat(getallprice.price3,',_.____')#" readonly></td></tr>
	<tr><th>#words[196]#</th><td><input type='text' name='displayprice4' id="displayprice4" value="#numberformat(getallprice.price4,',_.____')#" readonly></td></tr>

    </cfif>
    <cfif lcase(HcomID) eq "netsource_i">
    <cfquery name="getnetsourceprice" datasource="#dts#">
    select price_min,fprice_min,fcurrcode,fcurrcode2,fcurrcode3,fcurrcode4,fcurrcode5,fcurrcode6,fcurrcode7,fcurrcode8,fcurrcode9,fprice2_min,fprice3_min,fprice4_min,fprice5_min,fprice6_min,fprice7_min,fprice8_min,fprice9_min from icitem where itemno='#convertquote(itemno)#'
    </cfquery>
    <cfif getartran.currcode eq getnetsourceprice.fcurrcode>
    <cfset sellingmin = getnetsourceprice.fprice_min>
    <cfelseif getartran.currcode eq getnetsourceprice.fcurrcode2>
    <cfset sellingmin = getnetsourceprice.fprice2_min>
    <cfelseif getartran.currcode eq getnetsourceprice.fcurrcode3>
    <cfset sellingmin = getnetsourceprice.fprice3_min>
    <cfelseif getartran.currcode eq getnetsourceprice.fcurrcode4>
    <cfset sellingmin = getnetsourceprice.fprice4_min>
    <cfelseif getartran.currcode eq getnetsourceprice.fcurrcode5>
    <cfset sellingmin = getnetsourceprice.fprice5_min>
    <cfelseif getartran.currcode eq getnetsourceprice.fcurrcode6>
    <cfset sellingmin = getnetsourceprice.fprice6_min>
    <cfelseif getartran.currcode eq getnetsourceprice.fcurrcode7>
    <cfset sellingmin = getnetsourceprice.fprice7_min>
    <cfelseif getartran.currcode eq getnetsourceprice.fcurrcode8>
    <cfset sellingmin = getnetsourceprice.fprice8_min>
    <cfelseif getartran.currcode eq getnetsourceprice.fcurrcode9>
    <cfset sellingmin = getnetsourceprice.fprice9_min>
    <cfelse>
    <cfset sellingmin = getnetsourceprice.price_min>
    </cfif>
    
    <tr><th>#words[203]#</th><td><input type='text' style="background-color:##FF9;" name='displayprice4' id="displayprice4" value="#numberformat(sellingmin,',_.____')#" readonly></td></tr>
    </cfif>
    
    <cfif tran eq 'PO'>
		<tr>
	        <th <cfif getdisplaysetup2.body_requiredate neq "Y">style="visibility:hidden"</cfif>>#words[2174]#</th>
			...
			<th <cfif getdisplaysetup2.body_replydate neq "Y">style="visibility:hidden"</cfif>>#words[2175]#</th>
    ...
    <cfif tran eq 'SO'>
        <tr>
	        <th <cfif getdisplaysetup2.body_deliverydate neq "Y">style="visibility:hidden"</cfif>>#words[2176]#</th>
    ...
    <cfif wpitemtax eq "Y">
	    <tr>
		    <td colspan="4"></td>
		    <th <cfif lcase(hcomid) eq "visionlaw_i">style="visibility:hidden"</cfif>>#words[53]#</th>
		    <td <cfif lcase(hcomid) eq "visionlaw_i">style="visibility:hidden"</cfif>><input type="checkbox" name="taxincl" id="taxincl" value="T" onclick="getTax1();" <cfif isdefined('getitem.taxincl')><cfif getitem.taxincl eq "T"> checked="checked"</cfif></cfif> <cfif isdefined('additemtaxincluded')><cfif additemtaxincluded eq "T"> checked="checked"</cfif></cfif>  /></td>    
	    </tr>
    <cfelse>
    	<input type="hidden" name="taxincl" id="taxincl" value="#getartran.taxincl#"  />
    </cfif>
	<!--- ADD ON 10-12-2009 --->
	<tr>
		<th <cfif getdisplaysetup2.body_project neq "Y">style="visibility:hidden"</cfif>><!--- Project / Job --->#words[506]#<cfif getmodule.job eq "1">/  #words[475]#</cfif></th>
 ...
 <cfif getmodule.job eq "1">
        ...
        <cfif wpitemtax eq "Y">
            <th <cfif lcase(hcomid) eq "visionlaw_i">style="visibility:hidden"</cfif>>#words[1099]# (%)</th>
            <td <cfif lcase(hcomid) eq "visionlaw_i">style="visibility:hidden"</cfif>>
				...
				<select name="selecttax" id="selecttax" tabindex="13" onChange="JavaScript:document.getElementById('tax16').value=this.options[this.selectedIndex].id;getTax1();">
		...
		<cfelse>
        	<cfif lcase(hcomid) eq 'asiasoft_i'><!--- 20/6/2012 --->
                ...
                <td>
                <div id="profitfield">
               <input type="text" name="profit" id="profit" readonly size="10"/>
                </div>
                </td>
			</cfif>     
	...
	<tr>
        <th <cfif getdisplaysetup2.body_title neq "Y">style="visibility:hidden"</cfif>>#words[676]#</th>
        ...
        <cfif wpitemtax eq "Y">
            ...
            <td <cfif lcase(hcomid) eq "visionlaw_i">style="visibility:hidden"</cfif>>
            	<!---<input name="taxpec1" id="tax16" tabindex="14"  type="text" value="#taxpec1#" size="10" maxlength="10" onkeyup="getTax();" onBlur="settaxcode();"  readonly>--->
                <select name="taxpec1" id="tax16" tabindex="14" onChange="getTax();">
                    <option id="0" value="0" <cfif taxpec1 eq "0">selected</cfif>>0</option>
                    <option id="6" value="6" <cfif taxpec1 eq "6">selected</cfif>>6</option>
                    <option id="8" value="8" <cfif taxpec1 eq "8">selected</cfif>>8</option>
                </select>
                ...
                <input name="taxamt_bil" id="taxamt_bil" tabindex="24" type="text" value="#numberformat(taxamt_bil,stDecl_Discount)#" size="10" maxlength="10" readonly>            </td>
		<cfelse>
        	<cfif lcase(hcomid) eq 'asiasoft_i'><!--- 20/6/2012 --->
				<th>Margin</th>
                <td>
                <div id="marginfield">
                <input type="text" name="margin" id="margin" readonly size="10"/></td>
                </div>
			</cfif>
    ...
    <cfif getgenerallayer.bcurr EQ 'IDR'>
    	<cfif Hlinkams eq 'Y'>
        	<cfset dts2 = replace(LCASE(dts),'_i','_a','all')>
        <cfelse>
        	<cfset dts2 = dts>
        </cfif>
    	<cfquery name="getIndoPPH" datasource="#dts2#">
            SELECT code,rate1*100 AS rate 
        ...
        <th>PPH</th>
        <td>
            <select name="selectPPH" id="selectPPH" onChange="JavaScript:getIndoTax();">
                <option value="">Select PPH</option>
                    <cfloop query="getIndoPPH">
                        <option value="#getIndoPPH.code#" title="#getIndoPPH.rate#">#getIndoPPH.code#</option>
                    </cfloop>
            </select> 
            <input name="pph_amt_bil" size="10" id="pph_amt_bil" type="text" value="0.00" readonly> 	
        </td>       
        </tr>
    </cfif>
	<tr>
        ...
        <td colspan="3" <cfif getdisplaysetup2.body_titledesp neq "Y">style="visibility:hidden"</cfif>>
			<textarea name='title_desp' id="title_desp" cols='#getGeneralInfo.commentlimit#' rows='5' wrap="hard" style="font-family:'Courier New', Courier, monospace">#convertquote(titledesp)#</textarea>
        </td>
		<cfif wpitemtax eq "Y">
			<th style="vertical-align:top;" <cfif lcase(hcomid) eq "visionlaw_i">style="visibility:hidden"</cfif>>#words[2074]#</th>
			<td <cfif lcase(hcomid) eq "visionlaw_i">style="visibility:hidden"</cfif>>
	       		<cfset totalvalue = 0>
			  	...
			  	<input type="text" name="itemtotalamt" id="itemtotalamt" size="10" value="#numberformat(totalvalue,'.__')#" readonly />			</td>
		<cfelse>
        	<td></td>
	        <td> 
			  <input type="hidden" name="itemtotalamt" id="itemtotalamt" size="10" value="" readonly />	        
            </td>
		</cfif>
    </tr>   
	<cfif lcase(hcomid) eq "topsteel_i" or lcase(HcomID) eq "topsteelhol_i">
    ...
    <tr>
        ...
        <td>&nbsp;</td>
        <td>&nbsp;</td>
        <td><input type="button" name="cancel" id="cancel" value="#words[96]#" onclick="javascript:history.go(-1)" /></td>
        <td nowrap align="right"> <!--- <input type='button' name='back' value='back' onClick='javascript:history.back()'> --->
		      <input type="button" tabindex="16" name="beforeSubmit" id="beforeSubmit" value='<cfif mode eq 'Edit'>  Save  <cfelse>  #mode#  </cfif>' onclick="if(document.getElementById('req3').value == '' && ('#left(nexttranno,1)#' == '7' || '#len(nexttranno)#' == '5')){
                                                                                                alert('Please select a Placement No.')
                                                                                                }else{
                                                                                                 document.getElementById('Submit').click();
                                                                                                }">
            <div style="display: none"><cfif mode eq 'Edit'><input type='submit' tabindex="16" name='Submit' id='Submit' value='  Save  '><cfelse> <input type='submit' tabindex="16" name='Submit' id='Submit' value='  #mode#  '> </cfif></div>       </td>
    </tr>
...
<td>
	...
	<cfif url.type1 eq 'Add' and isservice neq 1>
    	<cfif getrecommendprice.recordcount gt 0>
        	<tr>
            	<th colspan="6">#words[2178]# (#refno3#)</th>
          	</tr>
          	<tr>
            	<td colspan="2"><strong>#words[2178]#</strong> - #numberformat(getrecommendprice.price,",.____")#<br><strong>Note</strong>: #getrecommendprice.ci_note#</td>
            	<td colspan="2"><strong>#words[74]#</strong> - #decimalformat(getrecommendprice.dispec)#%<br> <strong>#words[76]#</strong> - #decimalformat(getrecommendprice.dispec2)#%<br> <strong>#words[78]#</strong> - #decimalformat(getrecommendprice.dispec3)#%</td>
            	<td colspan="2"><strong>#words[1096]#</strong> - #numberformat(getrecommendprice.netprice,",.____")#</td>
          	</tr>
        </cfif>
	</cfif>
<cfif lcase(hcomid) neq "mastercare_i" and lcase(hcomid) neq "gorgeous_i" and lcase(hcomid) neq "visionlaw_i">
	<cfif getitembal.recordcount neq 0>
    	<tr>
          	<th colspan='6'>#words[2179]#</th>
        </tr>
        <tr>
          	<td><strong>#words[702]#</strong></td>
            <td><strong>#words[1087]#</strong></td>
          	<td><strong>#words[1096]#</strong></td>
          	<td><strong>#words[1121]#</strong></td>
          	<td><strong>#words[592]# %</strong></td>
          	<td>&nbsp;</td>
          	<td>&nbsp;</td>
        </tr>
        <tr>
...
<cfwindow name="discvalidatepass" center="true" closable="true" draggable="false" modal="true" refreshonshow="true" source="/default/transaction/disclimitvalidatepass.cfm" height="150" width="350"></cfwindow>
