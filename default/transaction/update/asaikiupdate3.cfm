     ...
     <cfif url.t1 eq "SO">
		...
		<cfloop from="1" to="#cnt#" index="i" step="#stepcount#">
			...
			<!--- Start of Update body --->
                ...
                <cfset xParam5 = listgetat(mylist,i+4,";")>
                
                <cfquery name="insertintotempiclink" datasource="#dts#">
                insert into iclinktemp (uuid,refno,itemno,trancode,serialno,samno) values ('#uuid#','#xParam1#',<cfqueryparam cfsqltype="cf_sql_varchar" value="#xParam2#">,<cfqueryparam cfsqltype="cf_sql_varchar" value="#xParam3#">,<cfqueryparam cfsqltype="cf_sql_varchar" value="#xParam4#">,<cfqueryparam cfsqltype="cf_sql_varchar" value="#xParam5#">)
                </cfquery>
        		...
        		<cfquery datasource="#dts#" name="getictrantemp">
					Select count(serialno) as fulfill,refno,trancode,itemno,substring(serialno,1,length(serialno)-(length(substring_index(serialno,"-",-1))+1)) as batchcode from iclinktemp where uuid = '#uuid#' group by refno,substring(serialno,1,length(serialno)-(length(substring_index(serialno,"-",-1))+1)),samno
				</cfquery>
        		<cfloop query="getictrantemp">
                
				<cfquery datasource="#dts#" name="getbody">
					Select * from ictran where refno = '#getictrantemp.refno#' and itemno =<cfqueryparam cfsqltype="cf_sql_varchar" value="#getictrantemp.itemno#"> and trancode = '#getictrantemp.trancode#' and shipped < qty and type = 'SO'
				</cfquery>
                ...
                <cfset totaltaxcharge = totaltaxcharge + newtaxamt_bil>
                    ...
                    <cfquery datasource="#dts#" name="getictempserial">
                        Select * from iclinktemp where refno='#getbody.refno#' and trancode='#getbody.trancode#' and itemno='#getbody.itemno#' and uuid='#uuid#' and serialno like "#getictrantemp.batchcode#%"
                    </cfquery>
                    
                    <cfloop query="getictempserial">
                    <cfquery datasource="#dts#" name="getupdateserial">
                        Select * from iserial where type='SAM' and refno='#getbody.brem5#' and serialno='#getictempserial.serialno#' <!---and trancode='#getbody.brem6#'--->
                    </cfquery>
                    
                    <cfquery name="getsambatchcode" datasource="#dts#">
                    select * from ictran where refno = '#getbody.brem5#' and itemno =<cfqueryparam cfsqltype="cf_sql_varchar" value="#getbody.itemno#"> and trancode = '#getupdateserial.trancode#' and type = 'SAM'
                    </cfquery>
                    <cfinvoke component="cfc.date" method="getFormatedDate" inputDate="#getsambatchcode.expdate#" returnvariable="getsambatchcode.expdate"/>
                    
                    <cfquery datasource="#dts#" name="updateserial">
                        update iserial set generated='#nexttranno#' where type='SAM' and refno='#getbody.brem5#' and serialno='#getictempserial.serialno#' <!---and trancode='#getbody.brem6#'--->
                    </cfquery>
                    ...
                    <cfquery datasource="#dts#" name="insertserial">
                        insert into iserial (type,refno,trancode,custno,fperiod,wos_date,itemno,serialno,agenno,location,currrate,sign,price,ctgno) 
                            ...
                            '#getupdateserial.currrate#','#getupdateserial.sign#','#getupdateserial.price#','#getupdateserial.ctgno#')
                    </cfquery>
					...
					<cfquery datasource="#dts#" name="insertictran">
						...
						`BATCHCODE`,`MILCERT`,`EXPDATE`,`POINT`,`INV_DISC`,`INV_TAX`,`SUPP`,`EDI_COU1`,`WRITEOFF`,`TOSHIP`,`SHIPPED`,`NAME`,`DEL_BY`,
                    ...
                    <cfqueryparam cfsqltype="cf_sql_varchar" value="#getbody.location#">,
						...
						'0.00', '0', '0', '#getbody.m_charge1#', '#getbody.m_charge2#', '#getbody.adtcost1#', '#getbody.adtcost2#', '0', '0', '#getsambatchcode.batchcode#', '#getsambatchcode.milcert#',<cfif getsambatchcode.expdate eq "" or getsambatchcode.expdate eq "0000-00-00">'0000-00-00'<cfelse>'#getsambatchcode.expdate#'</cfif>, '0', '0', '0',
					...
					<cfif getsambatchcode.batchcode neq "">
						<cfif getsambatchcode.location neq "">
							<cfquery name="updateobbatch" datasource="#dts#">
								update lobthob set bth_qut=(bth_qut+'#val(getictrantemp.fulfill)#')
								where location='#getsambatchcode.location#' and itemno =<cfqueryparam cfsqltype="cf_sql_varchar" value="#getbody.itemno#"> and batchcode = '#getsambatchcode.batchcode#'
							</cfquery>
							<cfquery name="updateobbatch" datasource="#dts#">
								update obbatch set bth_qut=(bth_qut+'#val(getictrantemp.fulfill)#')
								where itemno =<cfqueryparam cfsqltype="cf_sql_varchar" value="#getsambatchcode.itemno#"> and batchcode = '#getsambatchcode.batchcode#'
							</cfquery>
						<cfelse>
							<cfquery name="updateobbatch" datasource="#dts#">
								update obbatch set bth_qut=(bth_qut+'#val(getictrantemp.fulfill)#')
					...
					<cfquery name="inserticlink" datasource="#dts#">
						...
						'#ndatenow#','SO','#getictrantemp.refno#','#getbody.trancode#',
				...
				<cfif val(getictrantemp.fulfill) gte (val(getbody.qty)-val(getbody.shipped)-val(getbody.writeoff))>
					<cfquery datasource="#dts#" name="updateartran">
						...
						and toinv = '' and type = 'SO' and trancode = '#getictrantemp.trancode#'
					</cfquery>
			...
			<cfquery name="getictran" datasource="#dts#">
				select refno from ictran where refno = '#getictrantemp.refno#' and shipped < qty and type = 'SO'
			</cfquery>
			...
			<cfif getictran.recordcount eq 0>
				<cfquery name="updateartan" datasource="#dts#">
					...
					where refno = '#getictrantemp.refno#' and type = 'SO'
				</cfquery>
			</cfif>
...
<!---header--->
		...
		<cfquery datasource="#dts#" name="insertartran">
			...
			tax3,tax,taxp1,taxp2,taxp3,grand,note,term,van,pono,sono,quono,rem0,rem1,rem2,rem3,rem4,rem5,rem6,rem7,rem8,rem9,rem10,rem11,
