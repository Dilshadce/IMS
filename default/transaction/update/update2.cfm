...
<cfelseif url.t2 eq "DO">
	...
	<cfif url.t1 eq "SO" or url.t1 eq "QUO" or url.t1 eq "SAM">
        ...
        select * from ictran where brem5<>"" and type='#t1#' and (<cfloop from="1" to="#cnt#" index="i"><cfif i neq 1> or </cfif>refno='#listgetat(mylist,i,";")#'</cfloop>)  and (shipped+writeoff) < qty order by trancode				
        </cfquery>
...
document.updatepage.checkbox[i].checked=true;"> Outstanding Qty : #getupdate.qty_bil#</div></th>
                    </tr>
                    <tr>
                    <td colspan="3">&nbsp;</td>
                    </tr>
