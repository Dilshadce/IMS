...
<!--- 		if(nextflow != "")
	...
	if(answer)
	{
	var onholdurl = '/default/transaction/expressSO/onholdajax.cfm?uuid='+document.getElementById("uuid").value;
	...
	if(answer)
	{
	var onholdurl = '/default/transaction/expressSO/onholdajax.cfm?uuid='+document.getElementById("uuid").value+'&remark='+document.getElementById("rem9").value;
	ajaxFunction(document.getElementById('onholdajax'),onholdurl);
...
<cfoutput>
	var itemdespurl = '/default/transaction/expressSO/itemdespprocess.cfm?uuid=#URLEncodedFormat(uuid)#&trancode='+escape(itemdesptrancode)+'&itemdesp='+escape(itemdesp)+'&itemdespa='+escape(itemdespa)+'&itemcomment='+escape(itemcomment);
	...
	var ajaxurl2 = '/default/transaction/expressSO/addmultiproductsAjax.cfm?tran='+escape(tran)+'&tranno='+refno+'&uuid=#URLEncodedFormat(uuid)#&trancode='+escape(trancode)+'&custno='+escape(custno)+itemlisting;
	...
	{
    var urlloaditemdetail = '/default/transaction/expressSO/addItemAjax.cfm?itemno='+escape(encodeURI(detailitemno))+'&reftype='+document.getElementById('tran').value+'&custno='+document.getElementById('custno').value;
	...
	{
	  var urlloaditembal = '/default/transaction/expressSO/balonhand.cfm?itemno='+encodeURI(itemnobal)+'&location='+escape(document.getElementById('locationfr').value);
	...
	{
	<cfoutput>
    var urlload = '/default/transaction/expressSO/recalculateAjax.cfm?uuid=#URLEncodedFormat(uuid)#';
	</cfoutput>
	...
	{
	var ajaxurl = '/default/transaction/expressSO/recalculateAjax.cfm?uuid=#URLEncodedFormat(uuid)#';
	new Ajax.Request(ajaxurl,
      {
        method:'get',
        onSuccess: function(getdetailback){
	...
	{
	var customerurl = '/default/transaction/expressSO/memberdetailajax.cfm?detail=1&member='+document.getElementById("custno").value;
...
<cfinput type="hidden" name="tran" id="tran" value="SO">
...
<tr>
  ...
  <cfselect style="display:none" name="expunit" id="expunit"  onKeyUp="nextIndex(event,'expunit','expprice');" onChange="ajaxFunction(document.getElementById('ajaxfieldgetunitprice'),'/default/transaction/expressSO/getunitpriceAjax.cfm?itemno='+document.getElementById('expressservicelist').value+'&unit='+document.getElementById('expunit').value);setunitprice();"></cfselect>
...
<cfwindow center="true" width="700" height="500" name="searchitem" refreshOnShow="true" closable="true" modal="false" title="Search Item" initshow="false"
        source="/default/transaction/expressSO/searchitem2.cfm?reftype={tran}" />
<cfelse>
<cfwindow center="true" width="700" height="500" name="searchitem" refreshOnShow="true" closable="true" modal="false" title="Search Item" initshow="false"
        source="/default/transaction/expressSO/searchitem.cfm?reftype={tran}" />
</cfif>
...
<cfwindow center="true" width="600" height="400" name="changedaddr" refreshOnShow="true" closable="true" modal="false" title="Search Address" initshow="false"
        source="/default/transaction/expressSO/searchaddress.cfm" /> 
