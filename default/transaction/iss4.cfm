...
select lconsignin,lconsignout,lISS,lOAI,lOAR,lbatch,ddllocation from gsetup
</cfquery> 

<cfquery datasource="#dts#" name="getGeneralInfo">
	select a.*,(concat('.',repeat('_',decl_uprice))) as decl_uprice,(concat('.',repeat('_',decl_totalamt))) as decl_totalamt,decl_totalamt as totalamtdecl  
...
<script language="javascript" type="text/javascript">
	...
	function selectBatch(tran){
		...
		if(tran=='TR'){
			...
			var opt = 'Width=800px, Height=620px, scrollbars=yes, status=no';
...
<cfif url.type1 eq "Delete">
	<cfif tran eq 'TR'>
		...
		<cfif checkcustom.customcompany eq "Y">
			<cfquery datasource='#dts#' name="getitem">
				select * from ictran where itemno= <cfqueryparam cfsqltype="cf_sql_varchar" value="#url.itemno#"> and refno = '#nexttranno#' 
				and type = 'TROU' and itemcount = '#itemcount#'
			</cfquery>
		
			<cfquery datasource='#dts#' name="getbatchcode2">
				select batchcode,brem5,brem7 from ictran where itemno=<cfqueryparam cfsqltype="cf_sql_varchar" value="#url.itemno#"> and refno = '#nexttranno#' 
				and type = 'TRIN' and itemcount = '#itemcount#'
			</cfquery>
		<cfelse>
			<cfquery datasource='#dts#' name="getitem">
				select * from ictran where itemno= <cfqueryparam cfsqltype="cf_sql_varchar" value="#url.itemno#"> and refno = '#nexttranno#' and (type = 'TRIN' or type = 'TROU') and itemcount = '#itemcount#'
			</cfquery>
		</cfif>
	<cfelse>
		<cfquery datasource='#dts#' name="getitem">
			select * from ictran where itemno= <cfqueryparam cfsqltype="cf_sql_varchar" value="#url.itemno#"> and refno = '#nexttranno#' and type = '#tran#' and itemcount = '#itemcount#'
		</cfquery>
	</cfif>
		
	<cfquery datasource='#dts#' name="getpricehis">
		select price,wos_date,dispec1 from ictran where itemno= <cfqueryparam cfsqltype="cf_sql_varchar" value="#url.itemno#"> order by wos_date desc limit 3
	</cfquery>	
	
	<cfquery name="getitembal" datasource="#dts#">
		select qtybf,graded from icitem where itemno = <cfqueryparam cfsqltype="cf_sql_varchar" value="#url.itemno#">
	</cfquery>
...
<cfif url.type1 eq "Add">
	...
	<cfquery name="getitembal" datasource="#dts#">
		select qtybf,price,price2,price3,price4,price5,price6,graded,ucost,comment,packing,qty2 from icitem where itemno = '#itemno#'
	</cfquery>
	...
	<cfelse>
		...
		<cfelse>
            ...
            <cfelse>
				<cfset xlocation = gettranname.ddllocation>
            </cfif>
			</cfif>
		</cfif>
	</cfif>
	...
	<cfelse>
        ...
        <cfset price=getitembal.price>
        <cfelseif getGeneralInfo.df_trprice eq 'price2'>
        <cfset price=getitembal.price2>
        <cfelseif getGeneralInfo.df_trprice eq 'price3'>
        <cfset price=getitembal.price3>
        <cfelseif getGeneralInfo.df_trprice eq 'price4'>
        <cfset price=getitembal.price4>
        <cfelseif getGeneralInfo.df_trprice eq 'price5'>
        <cfset price=getitembal.price5>
        <cfelseif getGeneralInfo.df_trprice eq 'price6'>
        <cfset price=getitembal.price6>
        <cfelse>
        <cfset price=getitembal.ucost>
        </cfif>
        </cfif>  
    ...
    <cfif LCASE(HcomID) eq "alifestyles_i"> 
		<cfif tran eq "TR">
            <cfquery name="getRemark2_artran" datasource="#dts#">
                SELECT rem2
                FROM artran 
            ...
            <cfquery name="getCustno_iclocation" datasource="#dts#">
                SELECT custno
                FROM iclocation
                WHERE location = '#getRemark2_artran.rem2#';
            </cfquery>
            
            <cfquery name="getBusiness_arcust" datasource="#dts#">   
                SELECT business
                FROM #target_arcust#
                WHERE custno = '#getCustno_iclocation.custno#';
            </cfquery>
       
            <cfquery name="getPriceLvl_business" datasource="#dts#">   
                SELECT pricelvl
                FROM business
                WHERE business = '#getBusiness_arcust.business#';
            </cfquery>
            
            <cfquery name="getPrice_icitem" datasource="#dts#">   
                SELECT price,price2,price3,price4
                FROM icitem
                WHERE itemno = <cfqueryparam cfsqltype="cf_sql_varchar" value="#itemno#">;
            </cfquery>
        
			<cfif getPriceLvl_business.priceLvl EQ '1'>
                <cfset price=getPrice_icitem.price>
            <cfelseif getPriceLvl_business.priceLvl EQ '2'>
                <cfset price=getPrice_icitem.price2>
            <cfelseif getPriceLvl_business.priceLvl EQ '3'>
                <cfset price=getPrice_icitem.price3>
            <cfelse>
                <cfset price=getPrice_icitem.price4>    
            </cfif>
        </cfif>
    </cfif>
	...
	<cfset brem1="">
	<cfset brem2="">
    <cfif lcase(hcomid) eq "excelhw_i">	<!--- Add On 090908 --->
	<cfset Brem3 = getitembal.packing>
	<cfset Brem4 = getitembal.qty2>
    <cfelse>
	<cfset brem3="">
	<cfset brem4="">
    </cfif>
...
<cfif listfirst(tran) eq "TR" or listfirst(tran) eq "TROU" or listfirst(tran) eq "TRIN">
		<cfquery name="getItemBal" datasource="#dts#">
			...
			from ( SELECT itemno,locqfield,location FROM locqdbf
            WHERE 
            itemno = <cfqueryparam cfsqltype="cf_sql_varchar" value="#itemno#">
            and location = <cfqueryparam cfsqltype="cf_sql_varchar" value="#listfirst(trfrom)#">
            )
             as a 
			...
			(
				...
				and fperiod < '' 
				and fperiod<>'99'
				<cfif lcase(hcomid) neq "amgworld_i">
			...
			where a.location<>''
			and a.location=<cfqueryparam cfsqltype="cf_sql_varchar" value="#listfirst(trfrom)#">
			order by a.location
...
<cfoutput>
	...
	<cfform name="form1" method="post" action="issprocess.cfm?nDateCreate=#nDateCreate#" onsubmit="JavaScript:return data_validation();">
        ...
        <cfset formname = "issuepage"&newuuid>
		<cfset session[#formName#]="issuepage">
  ...
  <table align="center" class="data">
    		...
    		<tr> 
                ...
                <cfif (listfirst(tran) eq "TR" or listfirst(tran) eq "TROU" or listfirst(tran) eq "TRIN") and lcase(hcomid) eq "meisei_i">
                <cfquery name="getItemBal" datasource="#dts#">
                select 
			a.itemno,
			aa.desp,
			...
			from (SELECT itemno,locqfield,location FROM locqdbf 
             WHERE 
            itemno = <cfqueryparam cfsqltype="cf_sql_varchar" value="#checkitemno#">
            and location = <cfqueryparam cfsqltype="cf_sql_varchar" value="#listfirst(trfrom)#">
            ) as a 
			...
			(
				...
				from icitem 
				where itemno<>'' 
				and itemno = <cfqueryparam cfsqltype="cf_sql_varchar" value="#checkitemno#">
				order by itemno
			) as aa on a.itemno=aa.itemno 
			...
			from (SELECT itemno,locqfield,location FROM locqdbf 
             WHERE 
            itemno = <cfqueryparam cfsqltype="cf_sql_varchar" value="#checkitemno#">
            and location = <cfqueryparam cfsqltype="cf_sql_varchar" value="#listfirst(trto)#">
            ) as a 
			...
			where a.location<>''
			and a.location=<cfqueryparam cfsqltype="cf_sql_varchar" value="#listfirst(trto)#">
			order by a.location
                ...
                <!---- ---->
      			...
      			<th width="129" <cfif lcase(hcomid) eq "meisei_i">style="visibility:hidden"</cfif>>Balance on Hand</th>
      			<td width="60" <cfif lcase(hcomid) eq "meisei_i">style="visibility:hidden"</cfif>><input name="balance" id="balance" type="text" size="6" maxlength="10" value="#balonhand#" readonly>
                <div id="balanceAjax"></div></td>
    		...
    		<tr> 
      			...
      			<td colspan="3"nowrap>
					<cfif mode eq "Delete" or mode eq "Edit">
						<input name="desp" type="text" value='#convertquote(desp)#' size="40" maxlength="60">
					<cfelse>
						<input name="desp" type="text" value='#convertquote(getproductdetails.desp)#' size="40" maxlength="60">
					</cfif>
				</td>
			</tr>
   			<tr>
    		...
    		<cfquery datasource='#dts#' name="getcomment">
   			...
   			<tr>
						...
						<cfif form.enterbatch neq "">
							<cfquery name="checkbatchqty" datasource="#dts#">
								<cfif location neq "">
									...
									where location='#location#' and batchcode='#form.enterbatch#' and itemno='#itemno#' and ((bth_qob+bth_qin)-bth_qut) >0 order by itemno
                                --->
                                select 
                                ...
                                <!---
									...
									where batchcode='#form.enterbatch#' and itemno='#itemno#' and ((bth_qob+bth_qin)-bth_qut) >0 order by itemno--->
                                select 
                                a.batchcode,
				...
				<cfelseif isdefined("url.type1") and type1 eq 'Add'>
					...
					<cfelse>
						<cfinput name="qty" type="text" id="qty" size="6" maxlength="10" value="#qty#" required="yes" message="Please key in the quantity." validate="float" onKeyUp="document.getElementById('amt').value=parseFloat(document.getElementById('price').value*document.getElementById('qty').value).toFixed(#getGeneralInfo.totalamtdecl#)">
					</cfif>
    		...
    		<tr> 
      			<th>Price</th>
				<td><input name="price" id="price" type="text" size="15" maxlength="10" 
				<cfif lcase(Hcomid) eq "meisei_i">value="#numberformat(price,'.____')#"
				<cfelse>value="#numberformat(price,getgeneralinfo.decl_uprice)#"
				</cfif> onkeyUp='document.getElementById("amt").value=parseFloat(document.getElementById("price").value*document.getElementById("qty").value).toFixed(#getgeneralinfo.totalamtdecl#)'>
                </td>
    		<tr> 
      			<th>Amount</th>
     			<td><input name="amt" id="amt" type="text" size="15" maxlength="10" value="#numberformat(amt,getgeneralinfo.decl_totalamt)#"></td>
    		</tr>
    		</tr>
			<tr>
				<cfif tran neq "TR">
    		...
    		<tr> 
                ...
                <cfelse>
                <input name="brem2" type="text" value="#brem2#" size="40" maxlength="40">
                </cfif>
            ...
            <tr>
            <th nowrap>GL A/C</th>
            <td><input name='gltradac' id="glt6"  type='text' value='#gltradac#' size='10' maxlength='8'><cfif Hlinkams eq "Y"><a onClick="ColdFusion.Window.show('findglacc');"><img src="/images/down.png" /></a></cfif></td>
            </tr>
            </cfif>
			<tr> 
	    	...
	    	<tr> 
	     		...
	     		<td>&nbsp;</td>
		  		...
		  		<input type="submit" name="Submit" id="Submit" value="#mode#" > 
	      		</td>
			</tr>
		</table>
	</cfform>
...
<cfoutput>
<script type="text/javascript">
document.getElementById("amt").value=parseFloat(document.getElementById("price").value*document.getElementById("qty").value).toFixed(#getgeneralinfo.totalamtdecl#);
</script>
</cfoutput>
</cfif>
</body>
