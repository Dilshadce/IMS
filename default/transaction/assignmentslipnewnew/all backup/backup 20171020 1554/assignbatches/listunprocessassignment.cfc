...
<cffunction name="listAccount" access="remote" returntype="struct">
        ...
        <cfloop from="0" to="#form.iSortingCols-1#" index="i" step="1">
            <cfif Evaluate('form.bSortable_'&Evaluate('form.iSortCol_'&i)) EQ "true">
                <cfset sOrder=sOrder&Evaluate('form.mDataProp_'&Evaluate('form.iSortCol_'&i))>
                    <cfif Evaluate('form.sSortDir_'&i) NEQ "asc">
	...
	<cfquery name="getFilteredDataSet" datasource="#dts#">
		SELECT SQL_CALC_FOUND_ROWS refno,assignmentslipdate, placementno, custno, empno, empname, batches, locked
	...
	<cfquery name="getTotalDataSetLength" datasource="#dts#">
		...
		<cfquery name="getpolist" datasource="#dts#">
			SELECT jono FROM manpowerpolink where pono=<cfqueryparam cfsqltype="cf_sql_varchar" value="#trim(refno)#">
		</cfquery>
		...
		<cfset jolist=valuelist(getpolist.jono)>
            ...
            <cfset data["assignmentslipdate"]=dateformat(assignmentslipdate,'dd/mm/yyyy')>
            <cfset data["placementno"]=placementno>
            <cfset data["custno"]=custno>
            <cfset data["empname"]=empname>
			<cfset data["batches"]=batches>
			<cfset data["locked"]=locked>
