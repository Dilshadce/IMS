...
function selectmemberlist(custno){	
  ...
  for(var i=0;i<output.length;i++) {
	...
	function updaterow(rowno)
	{
		...
		var updateurl = 'updaterow.cfm?uuid='+escape(uuid)+'&coltype='+escape(coltypedata)+'&qty='+escape(qtylistdata)+'&trancode='+rowno+'&brem4='+escape(brem4data)+'&job='+escape(joblistdata)+'&promotiontype='+escape(promotiontypedata);
...
<th>Attention</th>
<td>
<input type="text" name="b_attn" id="b_attn" value="" maxlength="35" size="40"><cfif (lcase(hcomid) neq "bnbm_i" and lcase(hcomid) neq "bnbp_i")><input type="button" name="Sattn1" value="Ajax Search" onClick="javascript:ColdFusion.Window.show('findattention');"></cfif></td>
<th <cfif getmodule.auto eq 1>style="display:none" </cfif>>Attention</th>
<td <cfif getmodule.auto eq 1>style="display:none" </cfif>><input type="text" name="d_attn" id="d_attn" value="" maxlength="35" size="40"><cfif (lcase(hcomid) neq "bnbm_i" and lcase(hcomid) neq "bnbp_i")><input type="button" name="Sattn2" value="Ajax Search" onClick="javascript:ColdFusion.Window.show('findattention2');"></cfif></td>
</tr>
...
<tr>
<th style="vertical-align:top" <cfif getmodule.job neq "1">style="visibility:hidden"</cfif>>Job</th>
...
<tr>
<th>Choose a product <input type="checkbox" name="autoadditem" id="autoadditem" value="1"  <cfif getmodule.auto eq '1'>checked style="visibility:hidden"</cfif>></th>
<td colspan="1">
