...
<cfif url.t2 eq "INV">
	...
	<cfif url.t1 eq "DO">
	  	...
	  	<cfquery datasource="#dts#" name="getupdate">
			Select * from artran where custno = '#trim(url.custno)#' and type = '#t1#' and toinv = ''  and (void = '' or void is null) order by refno
	  	</cfquery>
...
document.updatepage.checkbox[i].checked=true;">
	...
	<cfelseif url.t1 eq "SO">
        ...
        <cfquery datasource="#dts#" name="getupdate">
			Select * from artran where custno = '#trim(url.custno)#' and type = '#t1#' and toinv = ''  and (void = '' or void is null) <cfif lcase(hcomid) eq "ltm_i"> and (rem45='' or rem45 is null or rem45='A')</cfif> order by refno
	  	</cfquery>
...
document.updatepage.checkbox[i].checked=true;">
        ...
        <cfquery datasource="#dts#" name="getupdate">
			Select * from artran where rem5 = '#url.rem5#' and type = '#t1#' and toinv = ''  and (void = '' or void is null) order by refno
	  	</cfquery>
...
document.updatepage.checkbox[i].checked=true;">
        ...
        <cfelse>
		<cfquery datasource="#dts#" name="getupdate">
			Select * from ictran where custno = '#trim(url.custno)#' and type = '#t1#' and (shipped+writeoff) < qty and (void = '' or void is null) group by refno order by refno
		</cfquery>
...
document.updatepage.checkbox[i].checked=true;">
	...
	<cfelseif url.t1 eq "QUO">
		<cfquery datasource="#dts#" name="getupdate">
			Select * from ictran where custno = '#trim(url.custno)#' and type = '#t1#' and shipped < qty and (void = '' or void is null) group by refno order by refno
		</cfquery>
...
<cfelseif url.t2 eq "DO">
	...
	<cfif url.t1 eq "SO">
	  	...
	  	<cfquery datasource="#dts#" name="getupdate">
			...
			if(i.itemno='',"YHFTOKCF",i.itemno) as itemno,a.userid from ictran i
			left join artran a on (i.type=a.type and i.refno=a.refno and i.custno=a.custno)
			where i.custno = '#url.custno#' and i.type='#t1#' and (shipped+writeoff) < qty  and (i.void = '' or i.void is null)
			group by refno 
			order by refno
	  	</cfquery>
...
<cfelseif url.t2 eq "RC">
	...
	<cfif url.t1 eq "PO">
        ...
        <cfquery datasource="#dts#" name="getupdate">
			Select a.* from ictran as a left join (select printstatus,refno from artran where type='#t1#')as b on a.refno=b.refno where a.custno = '#trim(url.custno)#' and a.type = '#t1#' and (a.shipped+a.writeoff) < a.qty and  b.printstatus = 'a3' and (a.void = '' or a.void is null)  group by a.refno order by a.refno
	  	</cfquery>
        <cfelse>
	  	<cfquery datasource="#dts#" name="getupdate">
			Select * from ictran where custno = '#url.custno#' and type = '#t1#' and (shipped+writeoff) < qty and (void = '' or void is null)  group by refno order by refno
	  	</cfquery>
		</cfif>
	  	<cfform action="update2.cfm?t1=#URLEncodedFormat(t1)#&t2=#URLEncodedFormat(t2)#&custno=#URLEncodedFormat(url.custno)#&invset=#URLEncodedFormat(invset)#" method="post" name="updatepage" onsubmit="return validate();">
        <cfset session.formName="updatepage">
...
document.updatepage.checkbox[i].checked=true;">
    ...
    <cfif url.t1 eq "SO">
	  	<cfquery datasource="#dts#" name="getupdate">
			...
			and a.exported = '' and b.exported = '' <cfif getgsetup.updatetopo neq 'Y'>and a.order_cl = '' and b.toinv = ''</cfif><cfif hcomid eq "asiasoft_i" and url.t1 eq "SO"> and printstatus = "a3"</cfif> and (b.void = '' or b.void is null) group by a.refno order by a.refno
	  	</cfquery>
...
document.updatepage.checkbox[i].checked=true;">
    ...
    <cfif url.t1 eq "RQ">
	  	...
	  	<cfquery datasource="#dts#" name="getupdate">
			Select * from ictran where <!---custno = '#url.custno#' and---> type = '#t1#' and toinv='' and (void = '' or void is null)  group by refno order by refno
	  	</cfquery>
		<!---</cfif>--->
	  	<cfform action="update2.cfm?t1=#URLEncodedFormat(t1)#&t2=#URLEncodedFormat(t2)#&invset=#URLEncodedFormat(invset)#" method="post" name="updatepage" onsubmit="return validate();">
        <cfset session.formName="updatepage">
...
<cfelseif url.t2 eq "SO">
	<cfif url.t1 eq "QUO" and lcase(HcomID) neq "steel_i">
        ...
        union all
			Select a.* from artran as a left join (select id,accountno from #replace(dts,'_i','_c','all')#.lead)as b on a.permitno=b.id where (b.accountno!='' ) and a.custno = '#url.custno#' and a.type = '#t1#' and a.order_cl = '' and (a.void = '' or a.void is null) and a.permitno!='' order by refno
	  	</cfquery>
        <cfelse>
	  	<cfquery datasource="#dts#" name="getupdate">
			Select * from artran where custno = '#trim(url.custno)#' and type = '#t1#' and order_cl = '' and (void = '' or void is null) order by refno
	  	</cfquery>
        </cfif>
        <cfif getgsetup.quoChooseItem eq 1>
			<cfset updateurl = "update2.cfm">
...
document.updatepage.checkbox[i].checked=true;">
        ...
        <cfelseif url.t1 eq "SAM">
	  	<!--- Get information on the outstanding unbill DO--->
	  	<cfquery datasource="#dts#" name="getupdate">
			Select * from artran where custno = '#url.custno#' and type = '#t1#' and order_cl = '' and (void = '' or void is null) order by refno
	  	</cfquery>
        <cfif getgsetup.quoChooseItem eq 1>
			<cfset updateurl = "update2.cfm">
        <cfelse>
...
<cfelseif url.t2 eq "CS">
	<cfif url.t1 eq "QUO">
	  	<!--- Get information on the outstanding unbill DO--->
	  	<cfquery datasource="#dts#" name="getupdate">
			Select * from artran where custno = '#url.custno#' and type = '#t1#' and generated = '' and toinv = '' and order_cl='' and (void = '' or void is null) order by refno
	  	</cfquery>
        <cfif getgsetup.quoChooseItem eq 1>
			<cfset updateurl = "update2.cfm">
        <cfelse>
...
document.updatepage.checkbox[i].checked=true;">
        ...
        <cfelseif url.t1 eq "SO">
	  	<!--- Get information on the outstanding unbill DO--->
	  	<cfquery datasource="#dts#" name="getupdate">
			Select * from artran where custno = '#trim(url.custno)#' and type = '#t1#' <!---and generated = ''---> and toinv = '' and order_cl='' and (void = '' or void is null) order by refno
	  	</cfquery>
        <cfif getgsetup.quoChooseItem eq 1>
			<cfset updateurl = "update2.cfm">
        <cfelse>
