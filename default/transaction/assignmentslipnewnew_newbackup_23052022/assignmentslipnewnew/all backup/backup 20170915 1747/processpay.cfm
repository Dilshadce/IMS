	        ...
	        <cfif epfno neq "" and epfcat neq "X" and fix_cpf eq 0 and epf_selected neq 0>
                ...
                <cfif epf_fyee neq '' and epf_fyee neq '0'>
                    <cfif findnocase('%',epf_fyee)>
						<cfset epf_yee = replace(replace(replace(replace(epf_yee,'0.08','#val(epf_fyee)/100#'),'0.11','#val(epf_fyee)/100#'),'0.04','#val(epf_fyee)/100#'),'0.055','#val(epf_fyee)/100#')>
                    <cfelse>
                ...
                <cfif epf_fyer neq '' and epf_fyer neq '0'>
                    <cfif findnocase('%',epf_fyer)>
						<cfset epf_yer = replace(replace(replace(replace(epf_yer,'0.12','#val(epf_fyer)/100#'),'0.13','#val(epf_fyer)/100#'),'0.06','#val(epf_fyer)/100#'),'0.065','#val(epf_fyer)/100#')>
                    <cfelse>
                ...
                <cfloop query="getcurrentassign">
					<cfif right(trim(getcurrentassign.emppaymenttype),1) lt right(weekpay,1)>
						...
						<cfif left(dts,12) eq 'manpowertest'>
							<cfoutput>
							<br>
							totalsocsowage:#totalsocsowage#<br>
							</cfoutput>
						</cfif>
					</cfif>
                </cfloop>
...
<!---            <cfif select_empdata.itaxno neq "" and select_empdata.itaxcat neq "X">--->
	  ...
	  <cfquery name="awqry" datasource="#db#">
			...
			<cfquery name="gettp3" datasource="#db#">
				SELECT c1_amount,c3_amount,c5_amount FROM tp3 WHERE empno ="#empno#"
			</cfquery>
...
<!--- </cfif> --->
            ...
            <cfset netpay = #val(grosspay)# - #val(EPFW)# - #val(tded)# - #val(socso_yee)#+ val(url.dedpay)>
            ...
            <cfloop index="a" from="1" to="6">
                <cfquery name="checkpayweek" datasource="#db#">
                    SELECT grosspay FROM payweek#a#
                    WHERE empno = <cfqueryparam cfsqltype="cf_sql_varchar" value="#empno#">
                    AND payyes="Y"
                    AND paydate=<cfif url.paydate eq 'paytra1'>"paytran"<cfelse>"paytra1"</cfif>
                </cfquery>

                <cfif checkpayweek.grosspay neq '' and isNumeric(checkpayweek.grosspay)>
                    <cfset grosscheck += val(checkpayweek.grosspay)>
                </cfif>                    
            </cfloop>
            
            <cfif grosscheck eq 0>
                <cfquery name="clearpaydate" datasource="#db#">
                    UPDATE <cfif url.paydate eq 'paytra1'>paytran<cfelse>paytra1</cfif> SET DW = 0.00 , 
                    BASICPAY = 0.00, 
                    NPLPAY = 0.00, 
                    itaxpcb = 0.00, 
                    ded115 = 0.00, 
