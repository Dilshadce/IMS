...
{
	var caladminurl = 'calculateadminfee.cfm?empno='+document.getElementById("empno").value+'&placement='+document.getElementById('placementno').value+'&emppaymenttype='+document.getElementById('emppaymenttype').value;
...
function getpanel(placeno)
{
	...
	var formurl = 'assignajax.cfm?<cfif isdefined('url.pno')>auto=#url.id#&</cfif><cfif isdefined('url.placementno')>auto2=#url.auto2#&</cfif><cfif url.type eq "edit" or url.type eq "delete">typenew=#url.type#&</cfif>type=Create&pno='+placeno+'&startdate='+escape(datestart)+'&enddate='+escape(datecomplete);
	<!--- ajaxFunction(document.getElementById('assignmentfield'),'assignajax.cfm?type=#url.type#&pno='+placeno+'&startdate='+escape(datestart)+'&enddate='+escape(datecomplete)); --->
	 new Ajax.Request(formurl,
      {
        method:'get',
...
function calcustottotal()
{
	...
	var custot5=document.getElementById('custot1').value * 1;
	var custot6=document.getElementById('custot2').value * 1;
	var custot7=document.getElementById('custot3').value * 1;
	var custot8=document.getElementById('custot4').value * 1;
...
<cfform name="AssignmentslipForm" id="AssignmentslipForm" action="/default/transaction/assignmentslipnewnew/Assignmentsliptableprocess.cfm" method="post" onsubmit="return checkmonth();" >
    ...
    <cfoutput> 
      <tr> 
        ...
        <td> <cfif mode eq "Delete" or mode eq "Edit">
            <cfinput type="text" size="20" name="refno" id="refno" value="#url.refno#" required="yes" message="Invoice No is Required" readonly><input type="hidden" size="20" name="refnotrue" id="refnotrue" value="#url.refno#" readonly>
            <cfelse>
 ...
 <cfelse>
      ...
      <th>Invoice Type</th>
      <td>
      <input type="text" name="assignmenttype" id="assignmenttype" value="#assignmenttype#" readonly>
     <!---  <select name="assignmenttype" id="assignmenttype" onChange="generaterefno()">
      ...
      <option value="paytra1" <cfif paydate eq "paytra1">Selected</cfif>>Normal Pay Out</option>
      <option value="paytran" <cfif paydate eq "paytran">Selected</cfif>>Exception Pay Out</option>
      </select>
      </td>
...
<cfwindow name="submitformwait" title="Processing!!! Please Wait!!!" modal="true" closable="false" width="350" height="260" center="true" ><p align="center">Processing, Please Wait!</p>
<p align="center"><img src="/images/loading.gif" name="Cash Sales" width="30" height="30"></p>
<br />
</cfwindow>
