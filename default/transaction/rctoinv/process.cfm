<cfif isdefined('url.refno') and isdefined('url.custno')>

<cfset form.ff_type = "RC">
<cfset form.ff_refnofrom = URLDECODE(url.refno)>
<cfquery name="checkupdate" datasource="#dts#">
SELECT rem34 FROM artran where type = "RC" and refno = "#URLDECODE(url.refno)#"
</cfquery>
<cfif checkupdate.rem34 neq "">
<cfoutput>
<script type="text/javascript">
alert('Receive Has Been Updated Before Already!');
window.location.href='list.cfm';
</script>
</cfoutput>
</cfif>
<cfset form.counter = 1>
<cfset form.ft_actualrefno = "">
<cfset form.ft = "INV">
<cfset form.ft_invtype = "1">
<cfquery datasource="#dts#" name="getRefnoset">
	select lastUsedNo as tranno, refnoused as arun from refnoset
	where type = 'INV'
	and counter = 1
</cfquery>
    <cfinvoke component="cfc.refno" method="processNum" oldNum="#getRefnoset.tranno#" returnvariable="newnextrefno" />
<cfset form.ft_refnofrom = newnextrefno>
<cfset deli = "`">
	...
	<cffunction name="updateIctran" output="false">
		...
		<cfquery name="insertictran" datasource="#dts#">
			...
			mc6_bil,mc7_bil,note_a,TITLE_ID,title_desp)
			...
			'#resultSet.agenno#','#resultSet.location#','#resultSet.qty_bil#','#val(resultSet.brem4)#',
			...
			'#resultSet.mc6_bil#','#resultSet.mc7_bil#','#resultSet.note_a#','#resultSet.TITLE_ID#','#resultSet.title_desp#')
		</cfquery>	
	</cffunction>
	...
	<cfif getartran.recordcount gt 0>
		<!--- Empty Cust/Suppl --->
			<cfoutput>
                ...
                SELECT name FROM #target_arcust# WHERE custno = "#URLDECODE(url.custno)#"
                </cfquery>
				<cfinvoke method="updateArtran" custno="#URLDECODE(url.custno)#" custname="#getcustname.name#" newRefno="#nexttranno#" newDate="#wosDate#" fromType="#form.ff_type#" newType="#form.ft#" resultSet="#getartran#"/>
                ...
                SELECT * FROM ictran 
                WHERE refno = <cfqueryparam cfsqltype="cf_sql_varchar" value="#nexttranno#">
                and type = "INV"
                </cfquery>
                
                <cfloop query="updatesub">
                <cfif val(updatesub.currrate) eq 0>
                </cfif>
                <cfset newqty = updatesub.qty_bil * updatesub.factor1 / updatesub.factor2 >
                <cfset newprice = updatesub.price_bil * updatesub.currrate>
                <cfset newamt1_bil = updatesub.qty_bil * updatesub.price_bil>
                
                <cfset newamt1 = newamt1_bil * updatesub.currrate>
                <cfset namt1_bil = newamt1_bil>
                ...
                <cfset newdisamt_bil = newamt1_bil - namt1_bil>
                <cfif newdisamt_bil eq 0>
                <cfset newdisamt_bil = updatesub.disamt_bil>
				</cfif>
				<cfset newamt_bil = namt1_bil > 
				<cfif updatesub.taxincl neq "T">
				<cfset newtaxamt_bil = newamt_bil * updatesub.taxpec1/100 >
                <cfelse>
                <cfset newtaxamt_bil = newamt_bil * (updatesub.taxpec1/(100 + (updatesub.taxpec1 * 1))) >
				</cfif>
                <cfset newamt = newamt_bil * updatesub.currrate>
                <cfset newdisamt = newdisamt_bil * updatesub.currrate>
				<cfset newtaxamt = newtaxamt_bil * updatesub.currrate >
				
               	<cfquery name="updatedetail" datasource="#dts#">
                UPDATE ictran
                SET
                qty = <cfqueryparam cfsqltype="cf_sql_varchar" value="#qty#">,
                amt1_bil = <cfqueryparam cfsqltype="cf_sql_varchar" value="#newamt1_bil#">,
                amt_bil = <cfqueryparam cfsqltype="cf_sql_varchar" value="#newamt_bil#">,
                price = <cfqueryparam cfsqltype="cf_sql_varchar" value="#newprice#">,
                amt1 = <cfqueryparam cfsqltype="cf_sql_varchar" value="#newamt1#">,
                amt = <cfqueryparam cfsqltype="cf_sql_varchar" value="#newamt#">,
                disamt_bil = <cfqueryparam cfsqltype="cf_sql_varchar" value="#newdisamt_bil#">,
                disamt = <cfqueryparam cfsqltype="cf_sql_varchar" value="#newdisamt#">,
                taxamt_bil = <cfqueryparam cfsqltype="cf_sql_varchar" value="#newtaxamt_bil#">,
                taxamt = <cfqueryparam cfsqltype="cf_sql_varchar" value="#newtaxamt#">
                WHERE refno = <cfqueryparam cfsqltype="cf_sql_varchar" value="#nexttranno#">
                and type = <cfqueryparam cfsqltype="cf_sql_varchar" value="INV">
                and itemno =<cfqueryparam cfsqltype="cf_sql_varchar" value="#trim(updatesub.itemno)#">
                and trancode = <cfqueryparam cfsqltype="cf_sql_varchar" value="#updatesub.trancode#"> 
                </cfquery>
                </cfloop>
                ...
                SELECT sum(amt_bil) as sumamt,sum(taxamt_bil) as taxamt FROM ictran
                WHERE refno = <cfqueryparam cfsqltype="cf_sql_varchar" value="#nexttranno#">
                and type = <cfqueryparam cfsqltype="cf_sql_varchar" value="INV">
                </cfquery>
                
                <cfquery name="getTax" datasource="#dts#">
                SELECT currrate,disp1,disp2,disp3,taxp1,taxincl FROM artran
                WHERE refno = <cfqueryparam cfsqltype="cf_sql_varchar" value="#nexttranno#">
                and type = <cfqueryparam cfsqltype="cf_sql_varchar" value="INV">
                </cfquery>
                ...
                tax_bil = <cfqueryparam cfsqltype="cf_sql_varchar" value="#tax1_bil#">,
...
<cfoutput>
<cfquery name="updatereceice" datasource="#dts#">
UPDATE artran SET rem34 = "Y" where type = "RC" and refno = "#URLDECODE(url.refno)#"
</cfquery>
<script type="text/javascript">
