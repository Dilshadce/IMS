...
<cfquery name="update_bonus_qry" datasource="#paydts#">
	UPDATE bonus
	SET basicpay = "#numberformat(bonuspay,'.__')#"
...
<!--- 		        <cfif epf1hd eq "Y">
			...
			<cfloop index="a" from="1" to="6">
				...
				<cfif 'payweek#a#' neq url.emppaymenttype>

				<cfquery name="gettotalsocsoweek" datasource="#db#">
                    SELECT socsoww,socsocc FROM payweek#a#
                    WHERE empno=<cfqueryparam cfsqltype="cf_sql_varchar" value="#empno#">
				</cfquery>
...
<cfset otherded=0>
            ...
            <cfset DW +=selectdw.DW>
            </cfif>
            <cfif selectdw.BASICPAY neq ''>
			<cfset BASICPAY +=selectdw.BASICPAY>
            </cfif>
            <cfif selectdw.NPLPAY neq ''>
            <cfset NPLPAY+=selectdw.NPLPAY>
            </cfif>
            <cfif selectdw.OT1 neq ''>
            <cfset OT1+=selectdw.OT1>
            </cfif>
            <cfif selectdw.OT2 neq ''>
            <cfset OT2+=selectdw.OT2>
            </cfif>
            <cfif selectdw.OT3 neq ''>
            <cfset OT3+=selectdw.OT3>
            </cfif>
            <cfif selectdw.OT4 neq ''>
            <cfset OT4+=selectdw.OT4>
            </cfif>
			<cfif selectdw.OT5 neq ''>
            <cfset OT5+=selectdw.OT5>
            </cfif>
            <cfif selectdw.OT6 neq ''>
            <cfset OT6+=selectdw.OT6>
            </cfif>
            <cfif selectdw.OTPay neq ''>
            <cfset OTPay+=selectdw.OTPay>
            </cfif>
            <cfif selectdw.TAW neq ''>
            <cfset TAW+=selectdw.TAW>
            </cfif>
            <cfif selectdw.grosspay neq ''>
            <cfset grosspay+=selectdw.grosspay>
            </cfif>
            <cfif selectdw.EPFWW neq ''>
            <cfset EPFEE+=selectdw.EPFWW>
            </cfif>
            <cfif selectdw.EPFCC neq ''>
            <cfset EPFER+=selectdw.EPFCC>
            </cfif>
            <cfif selectdw.epf_pay neq ''>
            <cfset epf_pay+=selectdw.epf_pay>
            </cfif>
            <cfif selectdw.TDED neq ''>
            <cfset TDED +=selectdw.TDED>
            </cfif>
            <cfif selectdw.TDEDU neq ''>
            <cfset TDEDU+=selectdw.TDEDU>
            </cfif>
            <cfif selectdw.NETPAY neq ''>
            <cfset NETPAY+=NETPAY>
            </cfif>
            <cfif selectdw.epf_pay_a neq ''>
            <cfset epf_pay_a+=selectdw.epf_pay_a>
            </cfif> 
            <cfif selectdw.cpf_amt neq ''>
            <cfset cpf_amt+=selectdw.cpf_amt>
            </cfif>
            <cfif isdefined('additionalwages')>
				<cfif selectdw.additionalwages neq ''>
					<cfset additionalwages+=selectdw.additionalwages>
				</cfif>
            </cfif>
            <cfif isdefined('hourrate')>
				<cfif selectdw.total_late_h neq ''>
					<cfset total_late_h+=selectdw.total_late_h>
				</cfif>
				<cfif selectdw.total_earlyD_h neq ''>
					<cfset total_earlyD_h+=selectdw.total_earlyD_h>
				</cfif>
				<cfif selectdw.total_noP_h neq ''>
					<cfset total_noP_h+=selectdw.total_noP_h>
				</cfif>
				<cfif selectdw.total_work_h neq ''>
					<cfset total_work_h+=selectdw.total_work_h>
				</cfif>
				<cfif selectdw.hourrate neq ''>
				<cfset hourrate+=selectdw.hourrate>
				</cfif>
            </cfif>
            <cfif selectdw.nsded neq ''>
            <cfset nsded +=selectdw.nsded>
            </cfif>
            <cfif selectdw.otherded neq ''>
            <cfset otherded+=selectdw.otherded>
            </cfif>
            
            </cfloop>

			<cfquery name="updatedw" datasource="#db#">
		        UPDATE #url.paydate# SET DW = #val(dW)# , 
				BASICPAY = #basicpay#, 
				NPLPAY = #totalNPL#, 
				OT1 = #OT1#, 
				OT2 = #OT2# , OT3 = #OT3# , OT4 = #OT4# , OT5 = #OT5# , OT6 = #OT6# , 
				OTPay = #OTtotal# , TAW = #taw# , grosspay=#grosspay#, EPFWW=#EPFEE#, 
				EPFCC=#EPFER#, 
				epf_pay = #epf_pay#, TDED=#tded# , 
				TDEDU = #tded#, NETPAY = #netpay#, 
				epf_pay_a = #epf_pay_a# , cpf_amt="#payin_2nd#"
                <cfif isdefined('additionalwages')>
                ,additionalwages="#val(additionalwages)#"
				</cfif>
                <cfif isdefined('hourrate')>
