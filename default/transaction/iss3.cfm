...
<cfif isdefined("form.type")>
	<cfif form.type eq "Delete">
		...
		<cfif tran eq "TR">
			...
			<cfelse>
				...
				<cfquery datasource="#dts#" name="deleteserial">
					Delete from iserial where refno = "#form.currefno#" and type='TR'
				</cfquery>
			</cfif>
		...
		<cfoutput>
		<form name="done" action="iss.cfm?consignment=#consignment#" method="post">
...
<cfif isdefined("form.type") and form.type eq "Edit">
	...
	<cfquery datasource="#dts#" name="updateartran">
		Update artran set wos_date = #ndatecreate#, fperiod = '#numberformat(readperiod,"00")#', agenno = '#form.agenno#',refno2 = '#form.refno2#',
	...
	<cfif tran eq "TR">
		...
		<cfquery name="updatetrfrom" datasource="#dts#">
            ...
            ,trdatetime = "#dateformatnew(form.invoicedate,'YYYY-MM-DD')# 00:00:02"
            </cfif>
			where refno = '#form.currefno#' and type = 'TROU'
		</cfquery>
...
select lastUsedNo as result, refnoused as arun from refnoset
	...
	<cfif getGeneralInfo.arun eq "1">
        ...
        <cfinvoke component="cfc.refno" method="processNum" oldNum="#getGeneralInfo.tranno#" returnvariable="nextno" />
	...
	<cfquery datasource="#dts#" name="insertartran">
		Insert into artran (type,refno,refno2,custno,fperiod,wos_date,agenno,source,job,
		rem0,rem1,rem2,rem3,rem4,rem5,rem6,rem7,rem8,rem9,rem10,rem11,name,exported,
		trdatetime,userid<cfif tran eq "TR">,term</cfif>,created_by,created_on,updated_by,updated_on,consignment<cfif gettranname.addonremark eq 'Y'>,rem30,rem31,rem32,rem33,rem34,rem35,rem36,rem37,rem38,rem39,rem40,rem41,rem42,rem43,rem44,rem45,rem46,rem47,rem48,rem49</cfif><cfif lcase(hcomid) eq "centraltruss_i"  or lcase(hcomid) eq "demoindo_i" or (lcase(hcomid) eq "amgworld_i" and tran eq "TR")>,rem14</cfif>)
			
		values('#tran#','#nexttranno#','#form.refno2#','#form.custno#','#numberformat(readperiod,"00")#',#nDateCreate#,'#form.agenno#',
		...
		'#form.name#','',#nDateNow#, '#HUserID#'<cfif tran eq "TR">,'#form.term#'</cfif>,'#HUserID#',#now()#,'#HUserID#',#now()#,'#form.consignment#'<cfif gettranname.addonremark eq 'Y'>
                ...
                </cfif><cfif lcase(hcomid) eq "centraltruss_i"  or lcase(hcomid) eq "demoindo_i" or (lcase(hcomid) eq "amgworld_i"  and tran eq "TR")>,'Order Pending'</cfif>)
	</cfquery>
</cfif>
...
<cfquery name="getitem" datasource="#dts#">
	select itemno,<cfif lcase(hcomid) eq "vsolutionspteltd_i">concat(aitemno,' ------',desp) as desp<cfelse>desp</cfif>,nonstkitem from icitem where (nonstkitem<>'T' or nonstkitem is null)
    <cfif Hitemgroup neq ''>
...
<cfoutput>
    ...
    <cfform name="form" action="iss4.cfm?type1=Add&ndatecreate=#ndatecreate#" method="post" onsubmit="return validate()">
		...
		&nbsp;&nbsp;|&nbsp;&nbsp;<a style="cursor:pointer;" onClick="window.open('import_tranexcelTR.cfm?refno=#URLENCODEDFORMAT(nexttranno)#&type=#tran#&consignment=#consignment#','UploadDocument','width=600, height=500, status=yes, menubar=no, location=no, scrollbars=yes')"><b>Import Item</b>
                    ...
                    <cfif (lcase(hcomid) eq "aipl_i" or lcase(hcomid) eq "belco_i" or lcase(hcomid) eq "gramas_i" or lcase(hcomid) eq "kjpe_i") and tran eq 'TR'>
...
<cfset eAdd="/default/transaction/issproducts/addproducts.cfm?tran="&tran&"&custno="&custno&"&tranno="&nexttranno >
        ...
        source="/default/transaction/searchitem/searchitem.cfm?currcode=" />
