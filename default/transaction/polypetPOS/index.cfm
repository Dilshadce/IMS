...
SELECT filterall,ECAOTA,ECAMTOTA,negstk,PCBLTC,ddlbilltype,expressdisc,displaycostcode,ldriver,autonextdate,ddllocation ,rem5,rem6,rem11,taxincluded,df_cs_cust,dfpos,comboard,comboardport FROM gsetup
</cfquery>
...
<script language="JScript">
	...
	ws.Run("cmd.exe /c mode com3:9600,n,8,1",'0');
	
	setTimeout('welcomemsg();',500)
	
	 function welcomemsg()  { 
	 var ws = new ActiveXObject("WScript.Shell"); 
	 ws.Run("cmd.exe /c echo Welcome to PolyPet                     >com3",'0');
	 }
	 function reboot()  { 
          var ws = new ActiveXObject("WScript.Shell"); 
          ws.Run("cmd.exe /c "+document.getElementById('displayitem').value,'0'); 

 
    } 
	function reboot2(price)  { 
          var ws = new ActiveXObject("WScript.Shell"); 
          ws.Run("cmd.exe /c "+price,'0'); 

 
    } 
	function reboot3()  { 
          var ws = new ActiveXObject("WScript.Shell"); 
          ws.Run("cmd.exe /c "+document.getElementById('displayitem3').value,'0'); 
	...
	function reboot5()  { 
          var ws = new ActiveXObject("WScript.Shell"); 
          ws.Run("cmd.exe /c "+document.getElementById('displayitem5').value,'0'); 

 
    } 
	function reboot8()  { 
          var ws = new ActiveXObject("WScript.Shell"); 
          ws.Run("cmd.exe /c "+document.getElementById('displayitem8').value,'0'); 
	...
	function displaydisc(){
		alert(1);
	ajaxFunction(document.getElementById('displaysetajax'),'/default/transaction/polypetpos/displaysetajax.cfm?value='+document.getElementById('grand').value+'&discount='+document.getElementById('disamt_bil').value);
	alert(1);
	setTimeout('reboot8();',500);	
	alert(1);
	}
...
function selectmemberlist(driverno){	
  ...
  for(var i=0;i<output.length;i++) {
	...
	function updaterow(rowno)
	{
		...
		var varitemnolist = 'itemnolist'+rowno;
		...
		var varitemno = document.getElementById(varitemnolist).value;
		var brem4data = document.getElementById(brem4).value;
		var vartrancode = 'trancodelist'+rowno;
		var newrowno = document.getElementById(vartrancode).value;
		var updateurl = 'updaterow.cfm?uuid='+escape(uuid)+'&coltype='+escape(coltypedata)+'&qty='+escape(qtylistdata)+'&trancode='+newrowno+'&brem4='+escape(brem4data)+'&itemno='+escape(varitemno);
	...
	function deleterow(rowno)
	{
		...
		var vartrancode = 'trancodelist'+rowno;
		var newrowno = document.getElementById(vartrancode).value;
		var updateurl = 'deleterow.cfm?uuid='+escape(uuid)+'&trancode='+newrowno+'&itemno='+escape(varitemno);
		new Ajax.Request(updateurl,
      {
        method:'get',
        onSuccess: function(getdetailback){
...
<!--- 		if(nextflow != "")
	...
	if(answer)
	{
	var onholdurl = '/default/transaction/polypetpos/onholdajax.cfm?uuid='+document.getElementById("uuid").value;
	...
	if(answer)
	{
	var onholdurl = '/default/transaction/polypetpos/onholdajax.cfm?uuid='+document.getElementById("uuid").value+'&remark='+document.getElementById("rem9").value;
	ajaxFunction(document.getElementById('onholdajax'),onholdurl);
	...
	{
    var urlloaditemdetail = '/default/transaction/polypetpos/addItemAjax.cfm?itemno='+escape(encodeURI(detailitemno))+'&reftype='+document.getElementById('tran').value;
	...
	{
	  var urlloaditembal = '/default/transaction/polypetpos/balonhand.cfm?itemno='+encodeURI(itemnobal)+'&location='+escape(document.getElementById('locationfr').value);
	...
	{
	<cfoutput>
    var urlload = '/default/transaction/polypetpos/recalculateAjax.cfm?uuid=#URLEncodedFormat(uuid)#';
	</cfoutput>
	...
	{
	var ajaxurl = '/default/transaction/polypetpos/recalculateAjax.cfm?uuid=#URLEncodedFormat(uuid)#';
	new Ajax.Request(ajaxurl,
      {
        method:'get',
        onSuccess: function(getdetailback){
	...
	{
	var memberurl = '/default/transaction/polypetpos/memberdetailajax.cfm?detail=1&member='+document.getElementById("driver").value;
	ajaxFunction(document.getElementById('getmemberajax'),memberurl);
	}
	else
	{
	var memberurl = '/default/transaction/polypetpos/memberdetailajax.cfm?detail=0&member='+document.getElementById("driver").value;
	ajaxFunction(document.getElementById('getmemberajax'),memberurl);
	}
	}
</script>
...
<cfinvoke component="cfc.comboard" method="display" firstline="Welcome To Polypet" secondlineleft="" secondlineright="" comchannel="#getgsetup.comboardport#" returnvariable="test"/></div>
...
<div id="displaysetajax" name="displaysetajax" style="display:none"> </div>
...
<input type="hidden" name="trancodelist#getictrantemp.trancode##REReplace(getictrantemp.itemno,'[^a-zA-Z0-9]','','ALL')#" id="trancodelist#getictrantemp.trancode##REReplace(getictrantemp.itemno,'[^a-zA-Z0-9]','','ALL')#" value="#getictrantemp.trancode#">
<input type="hidden" name="itemnolist#getictrantemp.trancode##REReplace(getictrantemp.itemno,'[^a-zA-Z0-9]','','ALL')#" id="itemnolist#getictrantemp.trancode##REReplace(getictrantemp.itemno,'[^a-zA-Z0-9]','','ALL')#" value="#getictrantemp.itemno#">
<input type="text" name="coltypelist#getictrantemp.trancode##REReplace(getictrantemp.itemno,'[^a-zA-Z0-9]','','ALL')#" id="coltypelist#getictrantemp.trancode##REReplace(getictrantemp.itemno,'[^a-zA-Z0-9]','','ALL')#" value="#getictrantemp.brem1#" onClick="if(this.value=='Collection'){this.value = 'Delivery';updaterow('#getictrantemp.trancode##REReplace(getictrantemp.itemno,'[^a-zA-Z0-9]','','ALL')#');} else {this.value = 'Collection';updaterow('#getictrantemp.trancode##REReplace(getictrantemp.itemno,'[^a-zA-Z0-9]','','ALL')#');}"  readonly="readonly">
</td>
<td nowrap align="right"><input type="text" name="qtylist#getictrantemp.trancode##REReplace(getictrantemp.itemno,'[^a-zA-Z0-9]','','ALL')#" id="qtylist#getictrantemp.trancode##REReplace(getictrantemp.itemno,'[^a-zA-Z0-9]','','ALL')#" value="#val(getictrantemp.qty_bil)#" size="5" onBlur="updaterow('#getictrantemp.trancode##REReplace(getictrantemp.itemno,'[^a-zA-Z0-9]','','ALL')#');" onKeyup="if(this.value != '#val(getictrantemp.qty_bil)#'){document.getElementById('updatebtn#getictrantemp.trancode##REReplace(getictrantemp.itemno,'[^a-zA-Z0-9]','','ALL')#').style.display='block';}else{document.getElementById('updatebtn#getictrantemp.trancode##REReplace(getictrantemp.itemno,'[^a-zA-Z0-9]','','ALL')#').style.display='none';}"></td>
<td nowrap align="right"><a style="cursor:pointer" onClick="document.getElementById('hidtrancode').value='#getictrantemp.trancode#';document.getElementById('hiditemno').value='#URLENCODEDFORMAT(getictrantemp.itemno)#';ColdFusion.Window.show('changeprice');getfocus5();">#numberformat(val(getictrantemp.price_bil),',.___')#</a></td>
<td nowrap align="right"><input type="text" name="brem4#getictrantemp.trancode##REReplace(getictrantemp.itemno,'[^a-zA-Z0-9]','','ALL')#" id="brem4#getictrantemp.trancode##REReplace(getictrantemp.itemno,'[^a-zA-Z0-9]','','ALL')#" value="#getictrantemp.brem4#" size="5" onBlur="updaterow('#getictrantemp.trancode##REReplace(getictrantemp.itemno,'[^a-zA-Z0-9]','','ALL')#');" onKeyup="if(this.value != '#getictrantemp.brem4#'){document.getElementById('updatebtn#getictrantemp.trancode##REReplace(getictrantemp.itemno,'[^a-zA-Z0-9]','','ALL')#').style.display='block';}else{document.getElementById('updatebtn#getictrantemp.trancode##REReplace(getictrantemp.itemno,'[^a-zA-Z0-9]','','ALL')#').style.display='none';}"></td>
<td nowrap align="right">#numberformat(val(getictrantemp.amt_bil),',.___')#</td>
<td nowrap><input type="button" name="deletebtn#getictrantemp.trancode##REReplace(getictrantemp.itemno,'[^a-zA-Z0-9]','','ALL')#" id="deletebtn#getictrantemp.trancode##REReplace(getictrantemp.itemno,'[^a-zA-Z0-9]','','ALL')#" onClick="if(confirm('Are You Sure You Want To Delete?')){deleterow('#getictrantemp.trancode##REReplace(getictrantemp.itemno,'[^a-zA-Z0-9]','','ALL')#')}" value="DELETE"/>&nbsp;<img id="updatebtn#getictrantemp.trancode#" name="updatebtn#getictrantemp.trancode##REReplace(getictrantemp.itemno,'[^a-zA-Z0-9]','','ALL')#" src="/images/tick.gif" width="15" height="15" style="cursor:pointer; display:none;"><!--- &nbsp;&nbsp;<input type="button" name="Updatebtn#getictrantemp.trancode#" id="updatebtn#getictrantemp.trancode#" onClick="updaterow('#getictrantemp.trancode#');" value="UPDATE" style="display:none"/> ---></td>
</tr>
</cfloop>
...
<tr>
<th nowarp style="font-size:16px">Choose a product</th>
<td colspan="1">
...
<th style="font-size:16px">Total Qty</th>
<td><div id="getqtytotal">
<cfquery name="getsumictrantemp" datasource="#dts#">
SELECT sum(qty_bil)as sumqty,sum(amt_bil) as sumamt FROM ictrantemp WHERE uuid = <cfqueryparam cfsqltype="cf_sql_varchar" value="#uuid#"> order by trancode desc
</cfquery>
...
<tr>
  ...
  <cfselect style="display:none" name="expunit" id="expunit"  onKeyUp="nextIndex(event,'expunit','expressservicelist');" onChange="ajaxFunction(document.getElementById('ajaxfieldgetunitprice'),'/default/transaction/polypetpos/getunitpriceAjax.cfm?itemno='+document.getElementById('expressservicelist').value+'&unit='+document.getElementById('expunit').value);setunitprice();"></cfselect>
...
<tr>
  <th>Price</th>
  <td colspan="2"><input type="<cfif getpin2.h1360 eq 'T'>text<cfelse>hidden</cfif>" name="expprice" id="expprice" size="15" maxlength="15" value="0.00" onKeyUp="calamtadvance();nextIndex(event,'expprice','expressservicelist')"  ></td>
</tr>
<tr>
<th>Discount</th>
<td colspan='2'>
  <cfif getgsetup.expressdisc eq "1">
  <input type="<cfif getpin2.h1360 eq 'T'>text<cfelse>hidden</cfif>" name="expunitdis1" id="expunitdis1" size="5" value="0" onKeyUp="caldisamt();calamtadvance();nextIndex(event,'expunitdis1','expressservicelist')"  >
%&nbsp;&nbsp;
<input type="<cfif getpin2.h1360 eq 'T' >text<cfelse>hidden</cfif>" name="expunitdis2" id="expunitdis2" size="5" value="0" onKeyUp="caldisamt();calamtadvance();nextIndex(event,'expunitdis2','expressservicelist')" />
%&nbsp;&nbsp;
<input type="<cfif getpin2.h1360 eq 'T'>text<cfelse>hidden</cfif>" name="expunitdis3" id="expunitdis3" size="5" value="0" onKeyUp="caldisamt();calamtadvance();nextIndex(event,'expunitdis3','expressservicelist')" />%
<cfelse>
...
<input type="<cfif getpin2.h1360 eq 'T'>text<cfelse>hidden</cfif>" name="expqtycount" id="expqtycount" size="3" value="1" onKeyUp="caldisamt();calamtadvance();nextIndex(event,'expqtycount','expressservicelist')" >
&nbsp;&nbsp;
<input type="<cfif getpin2.h1360 eq 'T'>text<cfelse>hidden</cfif>" name="expunitdis" id="expunitdis" size="5" value="0.00" onKeyUp="caldisamt();calamtadvance();nextIndex(event,'expdis','expressservicelist')" />
&nbsp;&nbsp;
</cfif>
<input type="<cfif getpin2.h1360 eq 'T'>text<cfelse>hidden</cfif>" name="expdis" id="expdis" size="10" maxlength="10" value="0.00" onKeyUp="calamtadvance();nextIndex(event,'expdis','expressservicelist')" onBlur="calamtadvance();"></td>
  </tr>
...
<th>Discount</th>
<td>
 <cfinput type="#inputtype#" size="3" name="dispec1" id="dispec1" value="0" validate="float" validateat="onblur" message="Please key in numeric value" onKeyUp="calcdisc();caltax();calcfoot();displaydisc();" />%<input type="hidden" name="disbil1" id="disbil1" />&nbsp;&nbsp;
  <cfinput type="#inputtype#" size="3" name="dispec2" id="dispec2" value="0" validate="float" validateat="onblur" message="Please key in numeric value" onKeyUp="calcdisc();caltax();calcfoot();displaydisc();"/>%<input type="hidden" name="disbil2" id="disbil2" />&nbsp;&nbsp;
  <cfinput type="#inputtype#" size="3" name="dispec3" id="dispec3" value="0" validate="float" validateat="onblur" message="Please key in numeric value" onKeyUp="calcdisc();caltax();calcfoot();displaydisc();"/>%<input type="hidden" name="disbil3" id="disbil3" />&nbsp;&nbsp;&nbsp;
  <cfinput type="#inputtype#" size="5" name="disamt_bil" id="disamt_bil" value="0.00" validate="float" validateat="onblur" message="Please key in numeric value" onKeyUp="caltax();calcfoot();displaydisc();" />
</td> 
</tr>
...
<cfwindow center="true" width="700" height="500" name="searchitem" refreshOnShow="true" closable="true" modal="false" title="Search Item" initshow="false"
        source="/default/transaction/polypetpos/searchitem2.cfm?reftype={tran}" />
<cfelse>
<cfwindow center="true" width="700" height="500" name="searchitem" refreshOnShow="true" closable="true" modal="false" title="Search Item" initshow="false"
        source="/default/transaction/polypetpos/searchitem.cfm?reftype={tran}" />
</cfif>
...
<!--- <cfwindow center="true" width="600" height="400" name="changedaddr" refreshOnShow="true" closable="true" modal="false" title="Search Address" initshow="false"
        source="/default/transaction/polypetpos/searchaddress.cfm" /> --->
...
<cfif isdefined('url.uuid')>
<cfinvoke component="cfc.comboard" method="display" firstline="" secondlineleft="Total :" secondlineright="#numberformat(getsumictrantemp.sumamt,',_.__')#" comchannel="#getgsetup.comboardport#" returnvariable="onholdamount"/></div>
<cfoutput>
<input type=hidden name='onholdamount' id='onholdamount' value='#onholdamount#'>
</cfoutput>

<script language="JScript">
	setTimeout('reonhold();',250);
	
	function reonhold(){
        
	 var ws = new ActiveXObject("WScript.Shell"); 
          ws.Run("cmd.exe /c "+document.getElementById('onholdamount').value,'0'); 
	}
...
<cfwindow center="true" width="250" height="150" name="changeprice" refreshOnShow="true" closable="true" modal="true" title="Edit Price" initshow="false" source="changeprice.cfm?uuid=#uuid#&trancode={hidtrancode}&itemno={hiditemno}" />  
