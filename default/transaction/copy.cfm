...
<cfset words_id_list = "1169,958,1176,1177,1170,22,877,1171,1173,1172,1174,681,1175,702,703,352">
...
<cfset bill = "RC,Receive,PR,Purchase Return,DO,Delivery Order,INV,Invoice,CN,Credit Note,CS,Cash Sales,DN,Debit Note,"
            & "ISS,Issue,TR,Transfer,PO,Purchase Order,SO,Sales Order,QUO,Quotation,SAM,Sample,RQ,Purchase Requisition">
				
	<!---cfquery name="getGSetup" datasource="#dts#">
	SELECT invno,invno_2,invno_3,invno_4,invno_5,invno_6,invoneset FROM gsetup
	</cfquery--->
<cfquery name="getgeneral" datasource="#dts#">
	SELECT lastaccyear,filterall 
    FROM gsetup
</cfquery>
<cfquery name="getGSetup" datasource="#dts#">
...
<html>
<head>
	<title><cfoutput>#words[1169]#</cfoutput></title>
...
<script language="javascript">
	...
	function getLastNum(type){
		getArunStatus(type,'1');
	}
	...
	function getRefnoRefresh(){
		getRefno("INV",document.copy1.ff_invtype.value,oneInv);
	}
	...
	<form name="copy1" action="copyprocess.cfm" method="post" onSubmit="return validation();">
        ...
        <table width="70%" border="0" cellspacing="0" cellpadding="2" class="data" align="center">
			<cfif isdefined("url.errorMessage")>
                <tr>
                    <td colspan="4">
                    <h3>#url.errorMessage#</h3>#words[958]# <a href="copy.cfm">#words[1176]#</a> #words[1177]#
                    </td>
                </tr>
            ...
            <tr>
                ...
                <fieldset style="border:1px ridge ##ff00ff; padding: 0.5em;">
                    <legend style="color: ##ff0000;">#UCase(words[1170])#</legend>
                    <select id="ff" name="ff" size="#listlen(bill)/2#" onChange="refresh_from()">
                        <cfloop from="1" to="#listlen(bill)#" index="i" step="+2">
                        	<option value="#listgetat(bill,i)#">#listgetat(bill,i+1)#</option>
                        </cfloop>
                    </select>
                </fieldset>
                </td>
                ...
                <fieldset style="border:1px ridge ##ff00ff; padding: 0.5em;">
                    <legend style="color: ##ff0000;">#UCase(words[22])#</legend>
            ...
            <tr>
                <td colspan="4"><b>#UCase(words[1170])#</b></td>
            </tr>
            <tr>
                <td width="20%">#words[877]#</td>
                <td colspan="3"><input style="background-color:##FFFF99" id="ff_type" type="text" name="ff_type" size="3" readonly>&nbsp;
                <div id="invf">
                    <select id="ff_invtype" name="ff_invtype" onChange="getRefnoRefresh()">
                        <cfloop from="1" to="#listlen(invtype)#" index="i" step="+2">
                        	<option value="#listgetat(invtype,i)#">#listgetat(invtype,i+1)#</option>
                        </cfloop>
                    </select>
                </div>
                </td>
            ...
            <tr>
                ...
                <td colspan="3">
                    <select id="ff_refnofrom" name="ff_refnofrom" onChange="usrCtrl()">
                        <option value="--">#words[1173]#</option>
                    </select>
                </td>
            </tr>
            <tr>
            ...
            <tr>
            	<td colspan="4"><b>#UCase(words[22])#</b></td>
            </tr>
            <tr>
                <td>#words[877]#</td>
                <td colspan="3">
                    <input style="background-color:##FFFF99" id="ft_type" type="text" name="ft_type" size="3" readonly>
                    &nbsp;
                    <div id="invt">
            ...
            <tr>
                <td>#words[1172]#</td>
                <td colspan="3">
                    <input style="background-color:##FFFF99" type="text" name="ft_refnoto" value="zzzzzzzz" size="9" readonly>
                </td>
            </tr>
            <tr><td colspan="4"><hr></td></tr>
            <tr>
                <td><label id="L1" >#words[1174]#</label></td>
                <td colspan="3">
            ...
            <tr>
                <td><label id="L2" >#words[1175]#</label></td>
                <td colspan="3">
