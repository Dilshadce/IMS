		 ...
		 new Ajax.Request(ajaxurl,
      {
		...
		document.getElementById('ajaxfield').innerHTML = trim(getdetailback.responseText);
            if(document.getElementById('empty').value == 'empty'){
                ...
                window.open("/default/transaction/assignmentslipnewnew/assignbatches/s_batchtable.cfm", "_self");
            }
        },
        onFailure: function(){ 
		alert('Error Found!'); },		
...
<cfoutput>
    ...
    <table align="center" class="data" width="1000px">
      ...
      <cfquery name="getdetail" datasource="#dts#">
     	SELECT empno,paydate,payrollperiod as amonth,refno FROM assignmentslip WHERE batches = "#exactResult.batches#"
      </cfquery>
...
<cfset nnetpay = numberformat(val(getpayroll.netpay),'.__') + numberformat(val(getassignment.totalamt),'.__') - numberformat(val(getassignment.totalded),'.__')>
    ...
    <table align="center" class="data" width="1000px">
      ...
      <cfoutput query="similarResult"> 
           <cfquery name="getdetail" datasource="#dts#">
     	SELECT empno,paydate,payrollperiod as amonth,refno FROM assignmentslip WHERE batches = "#similarResult.batches#"
      </cfquery>
...
<cfset nnetpay = numberformat(val(getpayroll.netpay),'.__') + numberformat(val(getassignment.totalamt),'.__') - numberformat(val(getassignment.totalded),'.__')>
  ...
  <table align="center" class="data" width="800px">
    ...
    <cfoutput query="type" maxrows="20"> 
     ...
     <cfquery name="getdetail" datasource="#dts#">
     	SELECT empno,paydate,payrollperiod as amonth,refno FROM assignmentslip WHERE batches = "#type.batches#"
      </cfquery>
