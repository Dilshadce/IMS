...
<cfif left(Huserid,6) eq 'mptest'>
<!--- Update Bonus (added by Nieo, 20170109)--->
<cfloop index="a" from="1" to="10">
  <cfif (evaluate('url.allowance#a#') eq 9 and evaluate('url.awee#a#') neq 0.00) or (evaluate('url.allowance#a#') eq 98 and evaluate('url.awee#a#') neq 0.00)>
    <cfquery name="update_bonus_qry" datasource="#paydts#">
	UPDATE bonus
	SET basicpay = "#evaluate('url.awee#a#')#"
	WHERE empno = <cfqueryparam cfsqltype="cf_sql_varchar" value="#empno#">
	</cfquery>
	<cfquery name="update_bonus_qry" datasource="#paydts#">
    UPDATE pay_tm
    SET bonus = "#evaluate('url.awee#a#')#"
    WHERE empno = <cfqueryparam cfsqltype="cf_sql_varchar" value="#empno#">
    </cfquery>
  </cfif>
</cfloop>
...
<!--- 		        <cfif epf1hd eq "Y">
			...
			<cfset socsowwtotalb = 0.00>
			<cfset socsocctotalb = 0.00>
			...
			<cfloop index="a" from="1" to="6">
				...
				<cfset socsowwtotalb += val(gettotalsocsoweek.socsoww)>
				<cfset socsocctotalb += val(gettotalsocsoweek.socsocc)>
			...
			<cfif val(socsowwtotal)+val(socso_yee) lte 19.75 >

			<cfquery name="update_socso_qry" datasource="#db#">
                UPDATE #weekpay# 
                SET  socsoww = (19.75 - #socsowwtotalb#)
                WHERE empno=<cfqueryparam cfsqltype="cf_sql_varchar" value="#empno#">
            </cfquery>

			</cfif>

			<cfif val(socsocctotal)+val(socso_yer) lte 69.05>

			<cfquery name="update_socso_qry" datasource="#db#">
                UPDATE #weekpay# 
                SET socsocc = (69.05 - #socsocctotalb#)  
                WHERE empno=<cfqueryparam cfsqltype="cf_sql_varchar" value="#empno#">
            </cfquery>
...
<!---            <cfif select_empdata.itaxno neq "" and select_empdata.itaxcat neq "X">--->
            ...
            <cfif findnocase('T',select_data.fixoesp) gt 0>
                ...
                <cfif left(Huserid,6) eq 'mptest'>
					...
					<cfloop index="a" from="1" to="6">
					<cfif weekpay neq 'payweek#a#'>
						<cfquery name="gettotalpcbgross" datasource="#db#">
						SELECT itaxpcb FROM payweek#a#
						WHERE empno = "#empno#"
						</cfquery>
          ...
          WHERE b.aw_cou < 18 and aw_tax = 0 
          ORDER BY shelf
      </cfquery>
                
       <cfloop index="a" from="1" to="10">
       <cfif find(evaluate('url.allowance#a#'),awqry.aw_cou)>
       		<cfset pcbgrosspay = evaluate("#pcbgrosspay# - val(url.awee#a#)")>
			<cfif left(huserid,6) eq 'mptest'>
				<cfset weekpcbgross = evaluate("#weekpcbgross# - val(url.awee#a#)")>
			</cfif>
            
            <!---<cfoutput>
            <br>#a#. pcbgrosspay after aw ded: #pcbgrosspay#<br>
           </cfoutput>--->
       </cfif>
       </cfloop>
...
<cfset otherded=0>
            ...
            <cfset EPFWW+=selectdw.EPFWW>
            </cfif>
            <cfif selectdw.EPFCC neq ''>
            <cfset EPFCC+=selectdw.EPFCC>
            </cfif>
