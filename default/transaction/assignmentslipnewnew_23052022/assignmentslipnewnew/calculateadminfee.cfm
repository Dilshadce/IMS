        ...
        <cfset "B#replace(getpm.itemid,'B-','')#" = 0>
              <cfloop from="1" to="6" index="bb">
                <cfif evaluate('url.billitem#bb#') eq replace(getpm.itemid,'B-','') and val(evaluate('url.billitemamt#bb#')) neq 0>
                    ...
                    <!---Special for HRDF--->
                    <cfif replace(getpm.itemid,'B-','') eq '136' and evaluate('getpm.billable') eq "Y" and evaluate('getpm.billableamt') neq "">
                        <cfset "B#replace(getpm.itemid,'B-','')#" = numberformat(evaluate(replace(getpm.billableamt,'=','')),'.__')>
                            <cfoutput>
                                <input type="hidden" name="totalhrdf" id="totalhrdf" value="#evaluate('B#replace(getpm.itemid,'B-','')#')#" >
                            </cfoutput>
                    </cfif>
                    <!---Special for HRDF--->
