...
<cfquery datasource="#dts#" name="gettranname">
	Select lRC,lPR,lDO,lINV,lCS,lCN,lDN,lPO,lQUO,lSO,lSAM,lRQ,simpleinvtype,ldescription,bodydo,bodypo,bodyso
...
<cfquery name="getdealer_menu" datasource="#dts#">
	select tran_edit_term from dealer_menu limit 1
</cfquery>
...
<cfquery name="getagent" datasource="#dts#">
  select agent from #target_icagent# where 0=0 and (discontinueagent='' or discontinueagent is null) <cfif getpin2.h1B40 eq 'T'>and (agent = '#huserid#' or agentid = '#huserid#' or agentlist like "%#ucase(huserid)#%")</cfif> order by agent
</cfquery>
...
<cfif form.type eq "Create">
	...
	<cfif day(currentdate) lte 7>
			<cfset intperiod = intperiod-1>
    </cfif>
    ...
    <cfquery name="getperiod" datasource="payroll_main">
    select * 
    from gsetup 
    where comp_id='#replacenocase(dts,'_i','')#'
    </cfquery>

    <cfif getperiod.recordcount neq 0>
        <cfset readperiod = numberformat(getperiod.mmonth,"00")>
    </cfif>
    ...
    <cfif readperiod eq '99' and (getgsetup.allowedityearend eq "N" or getgsetup.allowedityearend eq "")>
...
<cfoutput>
	<cfif form.type eq "Delete">
	  ...
	  <cfif getitem.currcode neq "">
        ...
        <cfset b_gstno = form.b_gstno>
        <cfset d_gstno = form.d_gstno>
        ...
        <cfset returnbillno = getitem.returnbillno>
        <cfset returnreason = getitem.returnreason>
        <cfset returndate = getitem.returndate>

	  <cfif tran eq 'PO' or tran eq 'SO' or tran eq 'DO' or tran eq 'INV' or tran eq 'SAM' or tran eq 'QUO' or tran eq 'CS' or tran eq 'DN' or tran eq 'CN'>
...
<cfset session[#formName#]="transpage">
	...
	<cfif form.type eq "Delete" or form.type eq "Edit">
       ...
       <!---  chonghow --->
        <cfif isdefined("form.recoverCoC") and form.type eq "Delete">
	...
	<table align="center" class="data" border="0" cellpadding="1" cellspacing="0">
    	...
    	<tr>
      		...
      		<td <cfif getdisplaysetup2.refno2 neq "Y">style="visibility:hidden"</cfif>><input type="text" name="refno2" value="#refno2#" <cfif lcase(hcomid) eq 'elsiedan_i'> maxlength="35"<cfelse>maxlength="24"</cfif>>
            <cfif isdefined('form.invset2')>
            <input type="hidden" name="invset2" value="#form.invset2#">
			</cfif>            </td>
    	</tr>
    	<tr>
      		<th <cfif getdisplaysetup2.customerno neq "Y">style="visibility:hidden"</cfif>>#ptype# #words[58]#</th>
    	...
    	<tr>
      		<td>&nbsp;</td>
      		<td colspan="2" <cfif getdisplaysetup2.customerno neq "Y">style="visibility:hidden"</cfif>><input name="name" type="text" size="40" maxlength="40" value="#convertquote(form.name)#" readonly></td>
			<th <cfif getdisplaysetup2.enduser neq "Y">style="visibility:hidden"</cfif>>#words[1358]#</th>
      		<td <cfif getdisplaysetup2.enduser neq "Y">style="visibility:hidden"</cfif>>
                ...
                <cfif getpin2.h1C10 eq 'T'><br /><a href="driver.cfm?type=Create" target="_blank">#words[2152]#</a></cfif>
				</cfif>
				<!---</cfif>--->
							</td>
    	</tr>
    	<tr>
      		...
      		<td <cfif getdisplaysetup2.agent neq "Y">style="visibility:hidden"</cfif>>
                ...
                <cfelse>
                <select name="agenno" id="agenno" <cfif lcase(HcomID) eq "hyray_i">onChange="ajaxFunction(document.getElementById('keeptrackbug'),'/default/transaction/keeptrackbug.cfm?agenno='+this.value+'&type=update&fieldtype=agent&custno=#listfirst(form.custno)#&tran=#tran#&refno=#nexttranno#');"</cfif>
    ...
    onChange="ajaxFunction(document.getElementById('projectjobajaxfield'),'/default/transaction/transactionprojectjobajax.cfm?agenno='+this.value);"
    	...
    	<tr>
            ...
            <cfelse>
            <select name="terms" id="terms" <cfif lcase(hcomid) eq "tmt_i" or lcase(HcomID) eq "taff_i">onChange="ChgDueDate()" <cfelseif lcase(hcomid) eq "techdirect_i">onChange="ChgDueDate2()"</cfif>
			<cfif getdealer_menu.tran_edit_term neq 'Y'>
            	onfocus="this.defaultIndex=this.selectedIndex;" 
                onchange="this.selectedIndex=this.defaultIndex;"
            </cfif>> 
			<!---<cfif lcase(hcomid) eq "bnbm_i" or lcase(HcomID) eq "bnbp_i">onChange="updatetermdetail()"</cfif>--->
                    ...
                    <cfset xterm = "immediate">
          			...
          			<cfloop query="getterm">
            			<option title="#getterm.days#" value="#term#"<cfif xterm eq term>Selected</cfif>>#term#<cfif lcase(hcomid) eq 'fdipx_i'>- #getterm.desp#</cfif></option>
          			</cfloop>
    	...
    	<tr>
            ...
            <!---for weiken--->
                ...
                <select name="Source" id="Source" <cfif lcase(HcomID) eq "ascend_i">onChange="document.getElementById('desp').value=document.getElementById('source').options[selectedIndex].title"</cfif>>
					...
					<a onMouseOver="JavaScript:this.style.cursor='hand';" ><img src="/images/find.jpg" width="20" height="14.5" onClick="javascript:ColdFusion.Window.show('findjob');" /></a>
					<cfif lcase(HcomID) eq "ascend_i" or getmodule.job neq "1" or getpin2.h1ZA0 neq "T"> </div></cfif>				
                    </div>
                    </td>
 ...
 <cfif lcase(HcomID) eq 'taftc_i'>
      ...
      <td></td><td></td><td></td>
        ...
        <cfif (tran eq "CN" or tran eq "DN" or tran eq "PR")>
         <tr><th><cfif tran eq "PR">Purchase Receive No<cfelse>Invoice No</cfif></th>
            <td><cfinput type="text" name="returnbillno" id="returnbillno" value="#refno2#" required="yes" message="Please Key In Related Bill No"></td>
            <td></td>
            <th>Reason</th>
         ...
         <tr><th><cfif tran eq "PR">Purchase Receive<cfelse>Invoice</cfif> Date</th>
             <cfquery name="getinvoicedate" datasource="#dts#">
                SELECT wos_date FROM artran WHERE refno=<cfqueryparam cfsqltype="cf_sql_varchar" value="#refno2#">
             </cfquery>
            <td><cfinput type="text" name="returndate" id="returndate" value="#dateformat(getinvoicedate.wos_date,'dd/mm/yyyy')#" required="yes" message="Please Key In Date"><img src="../../images/cal.gif" width=17 height=15 border=0 onClick="showCalendarControl(returndate);">(DD/MM/YYYY)</td>
         </tr>
        <cfelse>
        ...
        <td <cfif quodisplay neq "Y">style="display:none"</cfif>><input type="text" name="quono" value="#quono#" size="40" maxlength="35"></td>
        <td></td>
        </tr>
        <tr>
    ...
    Transfer To
        ...
        <option value="#j#">#replace(j,"_i","","all")#</option>
        </cfif>
        </cfloop>
        </select>
        ...
        <input type="text" name="permitno" value="#permitno#" size="40" maxlength="100" <cfif tran eq 'QUO' and getgsetup.quotationlead eq 'Y' and isdefined("form.leadno")>readonly</cfif>>
      	...
      	<tr>
        	<th height="20" <cfif podisplay neq "Y">style="visibility:hidden"</cfif>><cfif lcase(hcomid) eq "talerfood_i">Customer PO<cfelse>#words[795]#</cfif></th>
        	<td colspan="2" <cfif podisplay neq "Y">style="visibility:hidden"</cfif>><input type="text" name="pono" value="#pono#" size="40" maxlength="35"></td>
...
select * from deliveryTiming where id = '#general.DeliveryTiming#'
        ...
        <cfelse>  
      	...
      	<tr>
        	...
        	<td colspan="2" <cfif dodisplay neq "Y">style="visibility:hidden"</cfif>><input type="text" name="dono" value="#listfirst(dono)#" size="40" maxlength="35"></td>
...
<td>
       ...
       <cfelseif lcase(hcomid) eq "vsolutionspteltd_i" or lcase(hcomid) eq "vsyspteltd_i">
        ...
        <option value="" <cfif remark6 eq ''>selected</cfif>></option>
		 ...
		 <cfelseif lcase(hcomid) eq "ascend_i">
			...
			<td <cfif getmodule.auto eq "1" and (tran eq 'RC' or tran eq 'PO' or tran eq 'PR')>style="visibility:hidden"</cfif>><input type="text" name="remark6" value="#remark6#" size="40" maxlength="80"  onblur="validatetime('remark6')">
  ...
  <cfelseif lcase(hcomid) eq "dgalleria_i">
      <td>  <cfinput type="text" name="remark6" value="#remark6#" size="40" maxlength="10" validate="eurodate" message="Kindly key in date format">
        <img src="../../images/cal.gif" width=17 height=15 border=0 onClick="showCalendarControl(remark6);">(DD/MM/YYYY)</td>
                <cfelseif lcase(hcomid) eq "baronad_i" or lcase(hcomid) eq "baronconnect_i" or lcase(hcomid) eq "success_i">
		<td>  <cfinput type="text" name="remark6" value="#remark6#" size="40" maxlength="200" ></td>
		<cfelse>
			<td <cfif getmodule.auto eq "1" and (tran eq 'RC' or tran eq 'PO' or tran eq 'PR')>style="visibility:hidden"<cfelse><cfif rem6display neq "Y">style="visibility:hidden"</cfif></cfif>>
      	...
      	<tr>
			...
			<cfelse>
        		...
        		<td colspan="2" <cfif billtoadd_code neq "Y">style="visibility:hidden"</cfif>><input type="text" name="remark0" value="#form.BCode#" size="40" readonly></td>
			</cfif>
       ...
       <cfelseif lcase(hcomid) eq "vsolutionspteltd_i" or lcase(hcomid) eq "vsyspteltd_i">
        ...
        <cfelseif getmodule.auto eq "1" and lcase(hcomid) neq "imperial1_i">
      	...
      	<tr>
				...
				<td colspan="2" <cfif deladd_code neq "Y">style="visibility:hidden"</cfif>><input type="text" name="remark1" value="#form.DCode#" size="40" readonly></td>
            ...
            <cfelseif lcase(hcomid) eq "net_i" or lcase(hcomid) eq "gaf_i" or lcase(hcomid) eq "evco3_i">
        ...
        SELECT arrem1 from #target_arcust# where custno = '#custno#'
         ...
         <img src="../../images/cal.gif" width=17 height=15 border=0 onClick="showCalendarControl(remark9);">(DD/MM/YYYY)
      	...
      	<tr>
        		...
        		<td colspan="2" <cfif delattn neq "Y">style="visibility:hidden"</cfif>><input type="text" name="remark3" size="40" maxlength="35" <cfif lcase(HcomID) eq "hl_i" and tran eq "PO"> value=""<cfelse>value="#remark3#" readonly</cfif>></td>
	...
	<cfelseif lcase(hcomid) eq "aepl_i">
      	...
      	<tr>
			...
			<cfelseif lcase(hcomid) eq "eocean_i" or lcase(hcomid) eq "ideal_i" or lcase(hcomid) eq "idealb_i" or lcase(hcomid) eq "probulk_i" or lcase(hcomid) eq "ielm_i" or lcase(hcomid) eq "leatat_i" or lcase(hcomid) eq "dnet_i" or lcase(hcomid) eq "avoncleaning_i" or lcase(hcomid) eq "spcivil_i" or lcase(hcomid) eq "marico_i" or lcase(hcomid) eq "knights_i" or lcase(hcomid) eq "manhattan_i" or lcase(hcomid) eq "elmanhattan_i" or lcase(hcomid) eq "alsale_i" or lcase(hcomid) eq "letrain_i" or lcase(hcomid) eq "elitez_i" or lcase(hcomid) eq "reaktion_i" or lcase(hcomid) eq "amtaire_i" or lcase(hcomid) eq "taftc_i" or lcase(hcomid) eq "bofi_i" or lcase(hcomid) eq "avonservices_i" or lcase(hcomid) eq "widos_i" or lcase(hcomid) eq "avoncars_i" or lcase(hcomid) eq "viva_i" or lcase(hcomid) eq "kjcpl_i" or lcase(hcomid) eq "mlpl_i" or lcase(hcomid) eq "cleansing_i" or lcase(hcomid) eq "manhattan09_i" or lcase(hcomid) eq "marquis_i" or lcase(hcomid) eq "hempel_i" or lcase(hcomid) eq "bspl_i" or lcase(hcomid) eq "ccwpl_i" or lcase(hcomid) eq "anab_i" or lcase(hcomid) eq "zoenissi_i" or lcase(hcomid) eq "grace_i" or lcase(hcomid) eq "vsolutionspteltd_i" or lcase(hcomid) eq "risb_i" or lcase(hcomid) eq "gbi_i" or lcase(hcomid) eq "sjpst_i" or lcase(hcomid) eq "uniq_i" or lcase(hcomid) eq "sumiden_i" or lcase(hcomid) eq "almh_i" or lcase(hcomid) eq "leadbuilders_i" or getmodule.auto eq "1" or lcase(hcomid) eq "ciss_i" or lcase(hcomid) eq "vsyspteltd_i" or lcase(hcomid) eq "ascend_i" or lcase(hcomid) eq "epoint_i" or lcase(hcomid) eq "atc2005_i" or lcase(hcomid) eq "thats_i" or lcase(hcomid) eq "tranz_i" or lcase(hcomid) eq "chequer_i" or lcase(hcomid) eq "megaplast_i" or lcase(hcomid) eq "d72ipl_i" or lcase(hcomid) eq "mika_i" or lcase(hcomid) eq "thongsheng_i" or lcase(hcomid) eq "baronad_i">
        		...
        		<td rowspan="2" <cfif getmodule.auto eq "1" and (tran eq 'RC' or tran eq 'PO' or tran eq 'PR')>style="visibility:hidden"<cfelse><cfif rem11display neq "Y">style="visibility:hidden"</cfif></cfif>>
...
<cfinput type="text" name="remark11" id="remark11" value="#remark11#" bind="cfc:accordtran2.getremark11('#dts#',{remark5})"></cfoutput></td>
			...
			<cfelseif lcase(hcomid) eq "mcjim_i" or lcase(hcomid) eq "ideakonzepte_i" or lcase(hcomid) eq "bestform_i" or lcase(hcomid) eq "ovas_i"  or lcase(hcomid) eq "asiasoft_i" or lcase(hcomid) eq "bnbm_i" or lcase(hcomid) eq "bnbp_i" or lcase(hcomid) eq "success_i">
				<th>#getgsetup.rem11#</th>
	        	<td rowspan="2">
					<textarea name="remark11" id="remark11" cols='100' rows='3' onKeyDown="limitText(this.form.remark11,450);" onKeyUp="limitText(this.form.remark11,450);"></textarea>				</td>
        ...
        <th>#getGsetup.rem11#</th>
			...
			<cfelseif lcase(hcomid) neq "avt_i">
				...
				<input type="text" name="remark11" value="#remark11#" size="40" maxlength="300">				</td>
			<cfelse>
				<td colspan="2"></td>
			</cfif>
      	</tr>
      	<tr>
        		...
        		<td colspan="2" <cfif deltel neq "Y">style="visibility:hidden"</cfif>><input type="text" name="remark12" size="40" maxlength="35" <cfif lcase(HcomID) eq "hl_i" and tran eq "PO"> value=""<cfelse>value="#remark12#" readonly</cfif>></td>
				...
				<td colspan="5">
					<table align="center">
                        ...
                        <tr>
							...
							<td colspan="4"><input type="text" name="comm4" value="" maxlength="80" size="80"><input type="hidden" name="d_phone2" value="" maxlength="80" size="80">
                            <input type="hidden" name="d_email" value="" maxlength="80" size="80"></td>
						</tr>
                        <tr>
...
<cfif not isdefined("form.keepDeleted") and form.type eq "Delete">
<tr>
<td colspan="5" align="center"><h1 style="color:##F00">DELETED BILLS CANNOT BE RECOVERED</h1></td>
</tr>
</cfif>
			...
			<tr>
                ...
                <cfelse>
                    ...
                    <input type="hidden" name="d_country" value="#d_country#">
                    <input type="hidden" name="b_gstno" value="#b_gstno#">
                    <input type="hidden" name="d_gstno" value="#d_gstno#">
                   </cfif>
                    <cfif getGsetup.collectaddress eq 'Y'>
...
<cfoutput>
	...
	<cfif lcase(hcomid) eq "techdirect_i">
	function ChgDueDate2(){
  		if(document.invoicesheet.terms.value != ''){
			<cfoutput>
				<cfset Days = getDays.days>
				<cfset xDate = "#dateformat(NOW(), "yyyy-mm-dd")#">
				var dat = new Date("#xDate#")
			</cfoutput>
			alert(document.invoicesheet.terms.options[document.invoicesheet.terms.selectedIndex].title*1);
			var to = new Date();
