...
<cfif getgeneral.agentlistuserid eq "Y">and ucase(agent) in (SELECT agent FROM #target_icagent# WHERE agentlist like "%#ucase(huserid)#%")
    ...
    <option value="name">Debtor No - Name</option>
    <option value="name2">Debtor No - Name - Name 2</option>
    <option value="name3">Debtor No - Name -  Area - Agent</option>
    <option value="name4" <cfif lcase(HcomID) eq "bnbm_i" or lcase(HcomID) eq "bnbp_i">selected</cfif>>Debtor No - Name - Address</option>
    </select>
