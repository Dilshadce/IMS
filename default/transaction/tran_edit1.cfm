...
<cfif isdefined("form.invset") or isdefined("invset") and invset neq 0>
	...
	<cfquery datasource="#dts#" name="getGeneralInfo">
		Select #invset# as tranno, #tranarun# as arun,invoneset,filterall,suppcustdropdown,attnddl,transactiondate,collectaddress,quotationlead,agentlistuserid,locarap,tranuserid
        from GSetup
	</cfquery>
<cfelse>
	<cfquery datasource="#dts#" name="getGeneralInfo">
		Select #trancode# as tranno, #tranarun# as arun,invoneset,filterall,suppcustdropdown,attnddl,transactiondate,collectaddress,quotationlead,agentlistuserid,locarap,tranuserid
        from GSetup
	</cfquery>
	<cfset invset=0>
</cfif>
...
<script language="JavaScript">
	...
	function CopyProfileAddr()
	{
  		<cfoutput>
			...
			document.invoicesheet.country.value = "#getcust.country#";
			document.invoicesheet.postalcode.value = "#getcust.postalcode#";
  		</cfoutput>
  		return true;
	}
	...
	function CopyBillAddr()
	{
    	<cfoutput>
			...
			document.invoicesheet.d_country.value = "#getcust.d_country#";
			document.invoicesheet.d_postalCode.value = "#getcust.d_postalcode#";
  		</cfoutput>
  		return true;
	}
...
<cfoutput>
	<cfif isdefined("form.submit") and submit eq "Save & continue">	
        ...
        <cfif #tran# eq "rc" or #tran# eq "pr" or #tran# eq "po" or #tran# eq "rq">
      		<cfquery name="getupdateterm" datasource="#dts#">
        		Select term from #target_apvend# where custno = "#form.custno#"
      		</cfquery>
    	<cfelse>
      		<cfquery name="getupdateterm" datasource="#dts#">
        		Select term from #target_arcust# where custno = "#form.custno#"
      		</cfquery>
    	</cfif>
        </cfif>
		...
		<cfquery name="update_artran" datasource="#dts#">
			Update artran 
            set wos_date="#ndatecreate#"<!---, fperiod = "#cperiod#"---> ,custno='#form.custno#',
			...
			frem9 = '#form.frem9#', <cfif lcase(husergrpid) neq "super"><cfif getGeneralInfo.tranuserid eq "Y">userid='#Huserid#',</cfif></cfif> name='#form.name#',phonea='#form.b_phone2#',e_mail='#form.b_email#',b_gstno='#form.b_gstno#',d_gstno='#form.d_gstno#',
			...
			<!---, rem14 = '#form.d_add5#'--->, postalcode = '#form.postalcode#', d_postalcode = '#form.d_postalcode#', country = '#form.country#', d_country = '#form.d_country#'
			, frem7 = '#form.d_name#'
			, frem8 = '#form.d_name2#'
...
<form name="invoicesheet" action="" method="post" onSubmit="return checkbackdate()">
    <input type="hidden" name="type" id="type" value="Edit">
    <input type="hidden" name="tran" id="tran" value="#listfirst(tran)#">
    <input type="hidden" name="refno" id="refno" value="#listfirst(refno)#">
	...
	<input name="nexttranno" type="hidden" id="nexttranno" value="#nexttranno#">
    <cfif lcase(hcomid) eq "amgworld_i">
	...
	<tr>
		<th width="15%" colspan="3">#tranname# #words[58]#<div id="getcustomerdetailajax"></div></th>
		<td width="35%"><h3>#nexttranno#</h3></td>
		<th width="15%" colspan="3">#words[1088]#</th>
		<td width="35%"><h2>#words[2155]# #tranname#</h2></td>
	</tr>
	<tr>
        ...
        <input type="text" name="invoicedate" id="invoicedate" value="#dateformat(nDateCreate,"dd/mm/yyyy")#" validate="eurodate" size="10" maxlength="10">
			...
			(DD/MM/YYYY)
        <cfelse>
        <input type="text" name="invoicedate" id="invoicedate" value="#dateformat(nDateCreate,"dd/mm/yyyy")#" validate="eurodate" size="10" maxlength="10" readonly>
        </cfif>
		<cfelse>
        <input type="text" name="invoicedate" id="invoicedate" value="#dateformat(nDateCreate,"dd/mm/yyyy")#" validate="eurodate" size="10" maxlength="10">
			<img src="../../images/cal.gif" width=17 height=15 border=0 onClick="showCalendarControl(invoicedate);"> 
    ...
    <th colspan="3"><cfif lcase(hcomid) eq "supervalu_i">Bill Date<cfelse>Transaction Date</cfif></th>
		<td><input type="text" name="transactiondate" id="transactiondate" value="#dateformat(nDateCreate2,"dd/mm/yyyy")#" validate="eurodate" size="10" maxlength="10">
			...
			<cfif getGeneralInfo.suppcustdropdown eq "1">
				...
				<select name="custno" id="custno" onChange="updateDetails(this.value);">
          			...
          			<cfloop query="getcustsupp">
            			<option value="#trim(xcustno)#" <cfif trim(custno) eq trim(xcustno)>selected</cfif>>
                ...
                <a onMouseOver="JavaScript:this.style.cursor='hand';" ><img src="/images/find.jpg" width="20" height="14.5" onClick="javascript:ColdFusion.Window.show('findCustomer');" /><br>
				...
				<cfif tran eq "rc" or tran eq "pr" or tran eq "po" or tran eq "rq">
                <a href="supplier.cfm?type=Create" target="_blank">Create New #ptype1#</a>
                <cfelse>
                <a href="customer.cfm?type=Create" target="_blank">Create New #ptype1#</a>
                </cfif>
                </cfif>
                <cfif tran eq 'RC'>
						&nbsp;<input type="checkbox" name="internalrc" id="internalrc" onClick="assignCust('dropdown');"> #words[886]#
					</cfif>
				<br>
			<cfelse>
                ...
                <cfif tran eq 'RC'>
						&nbsp;<input type="checkbox" name="internalrc" id="internalrc" onClick="assignCust('');"> #words[886]#
					</cfif>
                <br>
			</cfif>
             <div id="issnoajax"></div>
			<input type="hidden" name="ptype" id="ptype" value="#ptype#">	
			<input name="name" id="name" type="text" size="45" maxlength="40" value="#convertquote(name)#" <cfif getdealer_menu.tran_edit_name neq 'Y'>readonly</cfif>><br>
			<input name="name2" id="name2" type="text" size="45" maxlength="40" value="#convertquote(name2)#" <cfif getdealer_menu.tran_edit_name neq 'Y'>readonly</cfif>>
			<!--- ADD ON 30-03-2009 --->
	...
	<tr>
		...
		<tr>
            ...
            <input type="button" name="BillToAddr" value="#words[958]#" onClick="javascript:CopyProfileAddr2()"><br/><br/>
            <input type="button" name="BillToAddr" value="#words[958]#" onClick="javascript:CopyBillAddr()"></td>
			<td rowspan="2" align="left" rowrap <cfif getdisplaysetup2.b_add_daddress neq 'Y'>style="visibility:hidden"</cfif>>#words[897]#<br/><br />#words[1170]# #ptype1# #words[2156]#</td>
        ...
        <tr>
			...
			<td><input type="text" id="b_gstno" name="b_gstno" value="#b_gstno#" size="45" maxlength="80"></td>
            
            <th colspan="3">#words[2157]# :</th>
			<td><input type="text" id="d_gstno" name="d_gstno" value="#d_gstno#" size="45" maxlength="80"></td>
		...
		<cfif getGeneralInfo.collectaddress eq 'Y'>
                ...
                <tr>
                    ...
                    <td><input type="text" name="CCode" id="CCode" value="#T_CCode#" size="15"></td>
                </tr>
                <tr>
                    <th colspan="3">Name</th>
                    <td><input type="text" name="c_name" id="c_name" value="#c_name#" size="45" maxlength="45"></td>
                </tr>
                <tr>
                    <td colspan="3">&nbsp;</td>
	...
	<tr>
		...
		<input name="p2" type="button" value="#words[1111]#" onClick="redirect('P1')">
		</td>
	</tr>
  	</table>
