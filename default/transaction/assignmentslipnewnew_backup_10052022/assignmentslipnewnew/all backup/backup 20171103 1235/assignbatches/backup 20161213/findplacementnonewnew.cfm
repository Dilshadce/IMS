...
<cfset nametype = url.type>
	...
	<cfquery name="getitemno" datasource="#dts#">
   		select placementno as xplacementno, custname as xcustname, empname as xempname,empno,custno,startdate,completedate from assignmentslip where batches = "" limit 15
	</cfquery>
    ...
    <td><a style="cursor:pointer" onClick="<cfif url.type neq "placementto">document.getElementById('placementfrom').value='#getitemno.xplacementno#';ColdFusion.Window.hide('findplacementnonew'); </cfif>document.getElementById('placementto').value='#getitemno.xplacementno#';ColdFusion.Window.hide('findplacementnonew');selectlist('#getitemno.xplacementno#','#nametype#');<cfif url.type eq "placementfrom">selectlist('#getitemno.xplacementno#','placementto');</cfif>ColdFusion.Window.hide('findplacementnonew');"><u>SELECT</u></a></td>
    </tr>
    </cfloop>    
    </table>
    </div>
