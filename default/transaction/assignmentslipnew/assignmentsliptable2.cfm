...
function getpanel(placeno)
{
	var datestart = document.getElementById('startdate').value;
	var datecomplete = document.getElementById('completedate').value;
	var formurl = 'assignajax.cfm?<cfif url.type eq "edit" or url.type eq "delete">typenew=#url.type#&</cfif>type=Create&pno='+placeno+'&startdate='+escape(datestart)+'&enddate='+escape(datecomplete);
...
{
	var assignmenttype1 = document.getElementById('assignmenttype').value;
	var newrefnourl = 'newrefno.cfm?<cfif url.type eq "edit" or url.type eq "delete">refno=#url.refno#&</cfif>assignmenttype='+assignmenttype1;
...
function caltax2()
	...
	for(var i =1;i<=10;i++)
	{
		...
		{
		lvltaxtotal = parseFloat(lvltaxtotal) + parseFloat(((taxper/100)*parseFloat(document.getElementById('lvltotaler'+i).value)+0.00001).toFixed(2));
		}
	}
	...
	for(var i =1;i<=3;i++)
	{
		if(document.getElementById('fixawer'+i).value !='' && parseFloat(document.getElementById('fixawer'+i).value) !=0)
		{
		lvltaxtotal = parseFloat(lvltaxtotal) + parseFloat(((taxper/100)*parseFloat(document.getElementById('fixawer'+i).value)+0.00001).toFixed(2));
		}
	}
	...
	for(var i =1;i<=3;i++)
	{
		if(document.getElementById('awer'+i).value !='' && parseFloat(document.getElementById('awer'+i).value) !=0)
		{
		lvltaxtotal = parseFloat(lvltaxtotal) + parseFloat(((taxper/100)*parseFloat(document.getElementById('awer'+i).value)+0.00001).toFixed(2));
		}
	}
	...
	{
		lvltaxtotal = parseFloat(lvltaxtotal) + parseFloat(((taxper/100)*parseFloat(document.getElementById('adminfee').value)+0.00001).toFixed(2));
	}
	...
	for(var i =1;i<=3;i++)
	{
		...
		{
		lvltaxtotal = parseFloat(lvltaxtotal) + parseFloat(((taxper/100)*parseFloat(document.getElementById('billitemamt'+i).value)+0.00001).toFixed(2));
		}
	}
	...
	for(var i =1;i<=3;i++)
	{
		...
		{
		lvltaxtotal = parseFloat(lvltaxtotal) - parseFloat(((taxper/100)*parseFloat(document.getElementById('dedcust'+i).value)+0.00001).toFixed(2));
		}
	}
	
	if(parseFloat(document.getElementById('secondrate').value) != 0 && parseFloat(document.getElementById('firstrate').value) != 0 )
	{
	taxval1 = parseFloat(parseFloat(((taxper/100)*parseFloat(document.getElementById('firstrate').value)+0.00001).toFixed(2)) + parseFloat(((taxper/100)*parseFloat(document.getElementById('secondrate').value)+0.00001).toFixed(2))).toFixed(2);
	}
	else
	{
...
function calselftotal()
{
	...
	document.getElementById('selftotal').value=(selfsalary+selfphnlsalaryamt+selfottotal-selfcpf+selfsdf+selfallowance+selfpbawsamt+selfdeductionamt+selfpayback).toFixed(2);
}
...
function calcusttotal()
{
	...
	document.getElementById('custtotal').value=(custsalary+custottotal+custphnlsalaryamt+custallowance+custcpf+custsdf+custpbawsamt+custdeductionamt+custpayback+taxamtamt).toFixed(2);
	document.getElementById('custtotalgross').value=(custsalary+custottotal+custphnlsalaryamt+custallowance+custcpf+custsdf+custpbawsamt+custdeductionamt+custpayback).toFixed(2);
 ...
 function calculatepay()
{
	...
	for(var i =1;i<=3;i++)
	{
		if(document.getElementById('fixawcode'+i).value == "15")
		{
			nocpfawee = parseFloat(nocpfawee) + parseFloat(document.getElementById('fixawee'+i).value);
			nocpfawer = parseFloat(nocpfawer) + parseFloat(document.getElementById('fixawer'+i).value);
		}
		
		if(document.getElementById('allowance'+i).value == "15")
		{
			nocpfawee = parseFloat(nocpfawee) + parseFloat(document.getElementById('awee'+i).value);
			nocpfawer = parseFloat(nocpfawer) + parseFloat(document.getElementById('awer'+i).value);
		}
...
function updateprocess()
{
	...
	var updateprocessurl = "/default/transaction/assignmentslipnew/updateprocess.cfm?empno="+escape(document.getElementById('empno').value)
...
var adminfeeamt = parseFloat(document.getElementById('adminfee').value).toFixed(2);
...
document.getElementById('custdeduction').value = (parseFloat(adminfeeamt) + parseFloat(billitem1amt) + parseFloat(billitem2amt) + parseFloat(billitem3amt) + parseFloat(addchargecustamt) + parseFloat(addchargecust2amt) + parseFloat(addchargecust3amt) - parseFloat(dedcust1amt) - parseFloat(dedcust2amt) - parseFloat(dedcust3amt) - parseFloat(nscustdedamt) - parseFloat(rebateamt )).toFixed(2);
caltax2();
calcusttotal();
}
...
select refno from artran where type='INV' and refno=<cfqueryparam cfsqltype="cf_sql_varchar" value="#getlastusedno.tranno#">
  ...
  <h1>#title#</h1>
	...
	<h4>
		<a href="/default/transaction/assignmentslipnew/Assignmentsliptable2.cfm?type=Create">Create Assignment Slip</a> 
		|| <a href="/default/transaction/assignmentslipnew/Assignmentsliptable.cfm?">List all Assignment Slip</a> 
		|| <a href="/default/transaction/assignmentslipnew/s_Assignmentsliptable.cfm?type=Assignmentslip">Search For Assignment Slip</a> 
		|| <a href="/default/transaction/assignmentslipnew/assignbatches/batchassignment.cfm">Assign Batch Control</a>
        || <a href="/default/transaction/assignmentslipnew/assignbatches/s_batchtable.cfm">List Batch Control</a>
<!---         ||  <a href="../assignmentslipnew/printcheque.cfm">Print Claim Cheque</a>
        || <a href="../assignmentslipnew/printclaim.cfm">Print Claim Voucher</a>
        || <a href="../assignmentslipnew/generatecheqno.cfm">Record Claim Cheque No</a>
        || <a href="../assignmentslipnew/definecheqno.cfm">Predefined Cheque No</a> --->
        	</h4>
</cfoutput> 

<cfform name="AssignmentslipForm" id="AssignmentslipForm" action="../assignmentslipnew/Assignmentsliptableprocess.cfm" method="post" onsubmit="return checkmonth();" >
  <cfoutput> 
    ...
    <cfoutput> 
      <tr> 
        ...
        <td> <cfif mode eq "Delete" or mode eq "Edit">
            <input type="text" size="20" name="refno" id="refno" value="#url.refno#" readonly><input type="hidden" size="20" name="refnotrue" id="refnotrue" value="#url.refno#" readonly>
            <cfelse>
      ...
      <td><cfinput type="text" name="empno" id="empno" value="#empno#" bind="cfc:assignmentslip.getempno({placementno},'#dts#')"></td>
      
      </tr>
      <tr>
      <th>Customer Name</th>
      <td><cfinput type="text" name="custname" id="custname" size='40' value="#custname#" bind="cfc:assignmentslip.getcustname({custno},'#dts#')"   readonly>
      </td>
      <th>Employee Name</th>
      <td>
      ...
      <tr>
       <th>Job Start Date</th><td><cfinput type="text" size="15" name="startdate" id="startdate" value="#startdate#" maxlength="10" validate="eurodate" required="yes" message="Job Start Date Required / Invalid"><img src="/images/cal.gif" width=17 height=15 border=0 onClick="showCalendarControl(document.getElementById('startdate'));">DD/MM/YYYY</td>
      <th>Job Completed Date</th><td><cfinput type="text" size="15" name="completedate" id="completedate" value="#completedate#" maxlength="10" validate="eurodate" required="yes" message="Job Start Completed Required / Invalid"><img src="/images/cal.gif" width=17 height=15 border=0 onClick="showCalendarControl(document.getElementById('completedate'));">DD/MM/YYYY</td>
      </tr>
      <tr>
      <td>&nbsp;</td>
      </tr>
