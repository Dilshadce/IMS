...
select refno from artran where type='INV' and right(refno,5)=<cfqueryparam cfsqltype="cf_sql_varchar" value="#right(refno,5)#"> and length(refno) = "12"
</cfquery>
<cfif checkexistrefno.recordcount eq 0>
<cfquery name="checkexistrefno" datasource="#dts#">
SELECT refno FROM assignmentslip WHERE right(refno,5)=<cfqueryparam cfsqltype="cf_sql_varchar" value="#right(refno,5)#"> and length(refno) = "12"
</cfquery>
</cfif>
        ...
        <cfif getGeneralInfo.arun eq "1" >
        	<cfset prefix = left(refno,1)>
			<cfset refnocheck = 0>
            <cfset refno1 = right(checkexistrefno.refno,5)>
                ...
                right(refno,5)=<cfqueryparam cfsqltype="cf_sql_varchar" value="#refno#"> and length(refno) = "12" and type = 'INV'
                </cfquery>
                <cfif checkexistrefno.recordcount eq 0>
                <cfquery name="checkexistrefno" datasource="#dts#">
                SELECT refno FROM assignmentslip WHERE right(refno,5)=<cfqueryparam cfsqltype="cf_sql_varchar" value="#refno#"> and length(refno) = "12"
                </cfquery>
                </cfif>
            ...
            <cfset form.refno = refno>
                ...
                SELECT max(substring(refno,4,4)) as monthlyrefno FROM assignmentslip WHERE left(refno,1) = "#prefix#" and month(assignmentslipdate) = "#dateformat(ndate,'m')#" and year(assignmentslipdate) = "#dateformat(ndate,'yyyy')#"
            </cfquery>
            
            <cfif getmaxrefno.monthlyrefno eq "">
                <cfset monthlyrefno = "0001">
            <cfelse>
                <cfset monthlyrefno = getmaxrefno.monthlyrefno + 1>
            </cfif>
            
            <cfset form.refno = prefix&numberformat(dateformat(ndate,'m'),'00')&numberformat(monthlyrefno,'0000')&numberformat(refno,'00000')>
...
SELECT refno FROM assignmentslip WHERE right(refno,5)=<cfqueryparam cfsqltype="cf_sql_varchar" value="#right(refno,5)#"> and length(refno) = "12"
        ...
        <cfabort />
            ...
            <cfset form.refno = prefix&numberformat(dateformat(ndate,'m'),'00')&numberformat(monthlyrefno,'0000')&numberformat(right(form.refno,5),'00000')>
...
<!---             '#val(form.selfexceptionday)#', --->
	...
	,
        ...
        <cfqueryparam cfsqltype="cf_sql_varchar" value="#form.custtotalgross#">,
        <cfqueryparam cfsqltype="cf_sql_varchar" value="0">,
        <cfqueryparam cfsqltype="cf_sql_varchar" value="0">,
        <cfqueryparam cfsqltype="cf_sql_varchar" value="0">,
        <cfqueryparam cfsqltype="cf_sql_varchar" value="0">,
	...
	"Being services rendered from #dateformat(startdate,'DD/MM/YYYY')# to #dateformat(completedate,'DD/MM/YYYY')# by Mr #form.empname# - #getposition.position#"
     ...
     <cfinclude template="ictranbody.cfm">
...
<cfelse>
  	...
  	<cfif checkitemExist.recordcount neq 0 >
		<cfif form.mode eq "Delete">
            ...
            <cfquery datasource='#dts#' name="deleteartran">
	    		UPDATE artran SET void = "Y",rem30 = <cfqueryparam cfsqltype="cf_sql_varchar" value="#form.reason#"> where refno='#form.refnotrue#' and type='INV'
	  		</cfquery>
            
            <cfquery datasource='#dts#' name="deleteictran">
	    		UPDATE ictran SET void = "Y" where refno='#form.refnotrue#' and type='INV'
	  		</cfquery>
        ...
        <cfif form.refno neq form.refnotrue>
        <cfset prefix = left(form.refno,1)>
	...
	<cfset refno = prefix&numberformat(company_details.mmonth,'00')&numberformat(monthlyrefnonew,'0000')&right(form.refno,5)>
	...
	"Being services rendered from #dateformat(startdate,'DD/MM/YYYY')# to #dateformat(completedate,'DD/MM/YYYY')# by Mr #form.empname# - #getposition.position#"
            ...
            "Being services rendered from #dateformat(startdate,'DD/MM/YYYY')# to #dateformat(completedate,'DD/MM/YYYY')# by Mr #form.empname# - #getposition.position#"
                ...
                <cfqueryparam cfsqltype="cf_sql_varchar" value="SGD">
                 ...
                 delete from ictran where refno = '#form.refnotrue#' and type='INV'
                 </cfquery>
...
<form name="done" id="done" action="../assignmentslipnew/Assignmentsliptable2.cfm?type=Create" method="post">
  <input name="status" value="#status#" type="hidden">
</form>
<cfelse>
<form name="done" id="done" action="../assignmentslipnew/s_assignmentsliptable.cfm?type=assignmentslip&amp;process=done" method="post">
  <input name="status" value="#status#" type="hidden">
</form>
</cfif>
</cfoutput>
