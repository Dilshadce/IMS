...
<td colspan="3"><select name='expressservicelist' id="expressservicelist" onChange="document.getElementById('desp').value = this.options[this.selectedIndex].id;ajaxFunction(window.document.getElementById('itemDetail'),'/default/transaction/Products/addItemAjax.cfm?itemno='+escape(encodeURIComponent(this.value)+'&custno=#url.custno#&tran=#url.tran#');setTimeout('calamt();',500);" onBlur="calamt();">
		...
		</select>&nbsp;&nbsp;<br><input type="text" name="searchitemnew" id="searchitemnew" size="10" onKeyUp="searchSel('expressservicelist','searchitemnew')" onBlur="ajaxFunction(window.document.getElementById('itemDetail'),'/default/transaction/Products/addItemAjax.cfm?itemno='+document.getElementById('expressservicelist').value+'&custno=#url.custno#&tran=#url.tran#');setTimeout('setdespprice();',500);setTimeout('calamt();',500);" >&nbsp;<input type="button" id="searchitembtn" onClick="ColdFusion.Window.show('multiaddsearchitem');" value="Search" align="right" />&nbsp;
		<input type="button" align="right" value="CLOSE" onClick="releaseDirtyFlag();submitinvoice();"></td>
        <td rowspan="4"><div id="itemDetail"></div></td>
</tr>
<tr>
<th>Description</th>
<td colspan="3"><input type="text" name="desp" id="desp" size="60" ><input type="hidden" name="expcomment" id="expcomment" value=""></td>
</tr>
<tr>
<th>Quantity</th>
