...
<cfquery name="getitem" datasource="#dts#">
    SELECT unit,despa,wos_group,category,comment,taxcode,desp
    FROM icitem where itemno = <cfqueryparam value="#url.servicecode#" cfsqltype="cf_sql_varchar">;
</cfquery>
...
<cfif getitem.taxCode NEQ ''>
	<cfset taxCodeSales = getitem.taxCode>
<cfelse>
	<cfset taxCodeSales = getGsetup.df_salestax>
</cfif>        
<cfset taxCodePurchase = getGsetup.df_purchasetax>

<cfquery name='getTax' datasource='#dts#'>
  	SELECT rate1
    FROM #target_taxtable#
    WHERE code = '#taxCodeSales#'
</cfquery>

<cfset taxPercentage = getTax.rate1>
...
<cfquery name="insertictran" datasource="#dts#">
	...
	(
        ...
        <cfif getGsetup.bcurr eq "HKD" or URLDecode(url.desp) contains "?">'#getitem.desp#'<cfelse>'#URLDecode(url.desp)#'</cfif>, 
