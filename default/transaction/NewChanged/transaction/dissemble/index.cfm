...
<html>
<head>
<title>Item Assemble and Dissemble</title>
...
<script src="/SpryAssets/SpryCollapsiblePanel.js" type="text/javascript"></script>
	...
	document.getElementById('issbtn_add').value = "Add";
	...
	document.getElementById('rcbtn_add').value = "Add";
...
<cfquery datasource="#dts#" name="getlocation">
	...
	select max(refno) as refno from assmtran
	</cfquery>
...
<cfquery name="getitem" datasource="#dts#">
	select a.itemno,b.desp from billmat as a left join (select desp,itemno from icitem) as b on a.itemno=b.itemno group by a.itemno
</cfquery>
...
<cfoutput> 
	<table width="80%" border="0" cellspacing="0" cellpadding="0" align="center" class="data">
    	...
    	<tr> 
        	<th height="21">Next Assembly No<cfinput type="hidden" name="uuid" id="uuid" value="#uuid#"></th>
        	<td colspan="3">
            	  <h3>#nexttranno#</h3>
    	...
    	<tr> 
      		<th>Assembly Date</th>
      		<td><cfinput type="text" name="wos_date" id="wos_date" size="10" value="#dateformat(now(),"dd/mm/yyyy")#" validate="eurodate" maxlength="10" onKeyUp="nextIndex(event,'wos_date','headerdesp');">(DD/MM/YYYY) </td>
    	</tr>
    	<tr> 
    		<th>Description</th>
      		<td colspan="3"><cfinput name="headerdesp" type="text" size="50" maxlength="40" value="Receive" onKeyUp="nextIndex(event,'headerdesp','headerdespa');"></td>
      		<td>
        	...
        	union
        	SELECT trancode FROM issuetemp where uuid = <cfqueryparam cfsqltype="cf_sql_varchar" value="#uuid#"> 
        	...
        	union
        	SELECT trancode FROM receivetemp where uuid = <cfqueryparam cfsqltype="cf_sql_varchar" value="#uuid#"> 
        	ORDER BY trancode desc
        	</cfquery>
        ...
        <tr><td colspan="100%"><hr><div id="ajaxFieldPro" name="ajaxFieldPro"></div></td></tr>
        <tr> 
      		<th>Bom Item No.</th>
      		<td colspan="3">
        ...
        <tr> 
            ...
            <cfloop query="getlocation"><option value="#getlocation.location#">#getlocation.location# - #getlocation.desp#</option></cfloop>
      		</select>
            </td>
      		<td>&nbsp;</td>
    	</tr>
        ...
        <tr> 
           ...
           <cfinput name="btn_add" id="btn_add" type="button" value="Dissemble" onClick="addItemAdvancebom();">
        ...
        <tr><td colspan="2" align="center"><strong>Issue</strong></td><td></td><td colspan="2" align="center"><strong>Receive</strong>
        ...
        <tr>
        <th>Itemno</th><td><cfinput type="text" name="itemno" id="itemno" size="26" onBlur="this.value = this.value.split('___', 1);getitemdetail(this.value);" autosuggest="cfc:itemno.findItem({cfautosuggestvalue},'#dts#')" autosuggestminlength="1" autosuggestBindDelay="1" onKeyUp="nextIndex(event,'itemno','desp');">&nbsp;<cfinput type="button" name="searchitembtn" id="searchitembtn" onClick="document.getElementById('searchitemtype').value='itemno';ColdFusion.Window.show('searchitem');" value="Search" align="right" /><div id="itemDetail"></div></td>
        <td>&nbsp;</td>
        <th>Itemno</th><td><cfinput type="text" name="itemno2" id="itemno2" size="26" onBlur="this.value = this.value.split('___', 1);getitemdetail2(this.value);" autosuggest="cfc:itemno.findItem({cfautosuggestvalue},'#dts#')" autosuggestminlength="1" autosuggestBindDelay="1" onKeyUp="nextIndex(event,'itemno2','desp2');">&nbsp;<cfinput type="button" name="searchitembtn" id="searchitembtn" onClick="document.getElementById('searchitemtype').value='itemno2';ColdFusion.Window.show('searchitem2');" value="Search" align="right" /><cfinput type="hidden" name="searchitemtype" id="searchitemtype" value=""></td>
        </tr>
        <tr>
        <th>Description</th><td><cfinput type="text" name="desp" id="desp" size="40" onKeyUp="nextIndex(event,'desp','despa');" ></td>
        <td>&nbsp;</td>
        <th>Description</th><td><cfinput type="text" name="desp2" id="desp2" size="40" onKeyUp="nextIndex(event,'desp2','despa2');" ></td>
        </tr>
        <tr>
        <th></th><td><cfinput type="text" name="despa" id="despa" size="40" onKeyUp="nextIndex(event,'despa','location');" ></td>
        <td>&nbsp;</td>
        <th></th><td><cfinput type="text" name="despa2" id="despa2" size="40" onKeyUp="nextIndex(event,'despa2','location2');" ></td>
        </tr>
        
        <tr>
        <th>Location</th><td><select name="location"  id="location">
			<option value="">Choose an Location</option>
            <cfloop query="getlocation"><option value="#getlocation.location#">#getlocation.location# - #getlocation.desp#</option></cfloop>
      		</select></td>
        <td>&nbsp;</td>
        <th>Location</th><td><select name="location2"  id="location2">
			<option value="">Choose an Location</option>
            <cfloop query="getlocation"><option value="#getlocation.location#">#getlocation.location# - #getlocation.desp#</option></cfloop>
      		</select></td>
        </tr>
        ...
        <tr>
        <th>Qty</th><td><cfinput type="text" name="qty" id="qty" size="10" maxlength="10" value="1" onKeyUp="calamtadvance();nextIndex(event,'qty','price');" onBlur="calamtadvance();"></td>
        <td>&nbsp;</td>
        <th>Qty</th><td><cfinput type="text" name="qty2" id="qty2" size="10" maxlength="10" value="1" onKeyUp="calamtadvance2();nextIndex(event,'qty2','price2');" ></td>
        </tr>
        
        <tr>
        <th>Price</th><td><cfinput type="text" name="price" id="price" size="15" maxlength="15" value="0.00" onKeyUp="calamtadvance();nextIndex(event,'price','amount');" onBlur="calamtadvance();"></td>
        <td>&nbsp;</td>
        <th>Price</th><td><cfinput type="text" name="price2" id="price2" size="15" maxlength="15" value="0.00" onKeyUp="calamtadvance2();nextIndex(event,'price2','amount2');" ></td>
        </tr>
        <tr>
        <th>Amount</th><td><cfinput type="text" name="amount" id="amount" size="10" value="0.00" readonly  onKeyUp="nextIndex(event,'amount','btn_add');" >&nbsp;&nbsp;&nbsp;&nbsp;<cfinput name="issbtn_add" id="issbtn_add" type="button" value="Add" onClick="addnewitem();"></td>
        <td>&nbsp;</td>
        <th>Amount</th><td><cfinput type="text" name="amount2" id="amount2" size="10" value="0.00" readonly  onKeyUp="nextIndex(event,'amount2','btn_add2');" >&nbsp;&nbsp;&nbsp;&nbsp;<cfinput name="rcbtn_add"  id="rcbtn_add" type="button" value="Add" onClick="addnewitem2();"></td>
        </tr>
        <tr>
        <td colspan="2">
        ...
        <td nowrap>#getissuetemp.qty#<input type="button" name="issbatchbtn" id="issbatchbtn" value="B" onClick="document.getElementById('issbatchtrancode').value='#getissuetemp.trancode#';ColdFusion.Window.show('issbatch');" ></td>
        
        <td nowrap>#getissuetemp.price#</td>
        <td nowrap>#getissuetemp.amt#</td>
        <td><cfinput type="button" name="deletebtn#getissuetemp.trancode#" id="deletebtn#getissuetemp.trancode#" onClick="if(confirm('Are You Sure You Want To Delete?')){deleterow('#getissuetemp.trancode#')}" value="DELETE"/></td>
        </tr>
        </cfloop>
        ...
        <td><cfinput type="button" name="deletebtn#getreceivetemp.trancode#" id="deletebtn#getreceivetemp.trancode#" onClick="if(confirm('Are You Sure You Want To Delete?')){deleterow2('#getreceivetemp.trancode#')}" value="DELETE"/></td>
        </tr>
        </cfloop>
        ...
        <tr>
        <td colspan="100%" align="center"><cfinput type="button" name="subbtn" id="subbtn" value="Accept" onClick="validformfield();"></td>
        </tr>
  	</table>
    </cfoutput>
</cfform>
