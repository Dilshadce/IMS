...
<head>
    ...
    select source FROM #target_project# where porj='P'
    </cfquery>
    <cfquery name="getjob" datasource="#dts#">
    select source FROM #target_project# where porj='J'
    </cfquery>
    ...
    <table width="100%">
    <tr>
    <th colspan="12">Step 1 :Choose a recurring bill</th>
    </tr>
    <tr>
    <th>Bill Type</th>
    <td>:</td>
 ...
 <cfselect name="groupid" id="groupid" query="getgroup" display="desp" value="groupid" />
    ...
    SELECT "ALL" as groupid, "ALL" as desp
    union all
     SELECT "" as groupid, "No Group" as desp
     union all
    SELECT groupid,desp FROM recurrgroup
    </cfquery>
 
                    Show Group: <cfselect name="groupid1" id="groupid1" query="getgroup1" display="desp" value="groupid" />
						Filer By: <cfselect id="filtercolumn" name="filtercolumn" bind="cfc:addbill.getBillColumns('#dts#')"
						...
						<cfinput type="button" name="filterbutton" value="Go" id="filterbutton"
							onclick="ColdFusion.Grid.refresh('usersgrid',false)">					</td>
				</tr>
				<tr>
					<td id="gridtd" style="padding-top:10px;">
                    <cfset listyn = "Y,N">
						<div style="min-heigh:200px;">
							<cfgrid name="usersgrid" pagesize="10" format="html" width="100%" height="255"
								bind="cfc:addbill.getBill({cfgridpage},{cfgridpagesize},{cfgridsortcolumn},{cfgridsortdirection},{filtercolumn},{filter},'#dts#',{groupid1})"
                                onchange="cfc:addbill.editBill({cfgridaction},
                                            {cfgridrow},
                                            {cfgridchanged},'#dts#','#HUserID#','#target_apvend#','#target_arcust#')" selectmode="edit" textcolor="##000000" delete="yes" deletebutton="Delete">
								<cfgridcolumn name="wos_date" header=" Last Date" select="no" width="150">
                                <cfgridcolumn name="type" header="TYPE" width="50" select="no" >
								<cfgridcolumn name="refno" header="Bill No" width="70" select="no">
								<cfgridcolumn name="custno" header="Cust No " width="80">
                                <cfgridcolumn name="name" header="Name" width="200">
                                <cfgridcolumn name="desp" header="Description" width="200">
                                <cfgridcolumn name="void" header="Generate" select="yes"  values="Y,N" >
                                <cfgridcolumn name="counter" header="Bill Set" select="yes">
                                <cfgridcolumn name="eInvoice_Submited" header="Recurr Group" select="no"><cfgridcolumn name="nextdate" header="Next Date" select="no" type="string_nocase" width="150">
                                <cfgridcolumn name="source" header="Project Code" values="#projectlist#" select="yes">
                                <cfgridcolumn name="job" header="Job Code" values="#joblist#" select="yes">
							</cfgrid>
						</div>					</td>
				</tr>
			</table>    </td>
    ...
    <tr>
      <td colspan="12" align="left">
        ...
        <table border="1" width="90%" align="center">
        <tr><td>Bill Type:<input type="text" name="rowtype" id="rowtype" readonly size="5" />&nbsp;&nbsp; Bill No:<input type="text" name="rowbill" id="rowbill" readonly />&nbsp;&nbsp; Custno No:<input type="text" name="rowcustno" id="rowcustno" value="" /></td></tr>
        <tr>
					<td id="gridtd" style="padding-top:10px;">
						<div style="min-heigh:200px;">
							<cfgrid name="detailgrid" pagesize="7" format="html" width="100%" height="200"
								bind="cfc:additem.getItem({cfgridpage},{cfgridpagesize},{cfgridsortcolumn},{cfgridsortdirection},{rowbill},{rowtype},'#dts#',{rowcustno})"
                                onchange="cfc:additem.editItem({cfgridaction},
                                            {cfgridrow},
                                            {cfgridchanged},'#dts#','#HUserID#',{rowbill},{rowtype},{rowcustno})" selectmode="edit" textcolor="##000000" delete="yes" deletebutton="Delete">
								<cfgridcolumn name="trancode" header="ID" select="no" width="50" display="no">
                                <cfgridcolumn name="itemno" header="Item No" width="150" select="no" type="string_nocase" >
								<cfgridcolumn name="desp" header="Description" width="300">
								<cfgridcolumn name="despa" header="Description 2" width="200">
                                <cfgridcolumn name="qty_bil" header="Qty" width="70">
                                <cfgridcolumn name="price_bil" header="Price" width="100">
                                <cfgridcolumn name="amt_bil" header="Amount" width="100" select="no">
							</cfgrid>
						</div>					</td>
				</tr>
			</table>      </td>
    ...
    <tr>
                <th>Date :</th>
                <td></td>
                <td><cfinput type="text" name="date" id="date" value="#dateformat(now(),'DD/MM/YYYY')#" /><img src="/images/cal.gif" width=17 height=15 border=0 onClick="showCalendarControl(date);">(dd/mm/yyyy)</td>
                <td></td>
                <th>Generate Group</th>
    ...
    SELECT groupid,desp FROM recurrgroup
                ...
                <input type="Submit" name="submitbtn" id="submitbtn" value="Generate" /></td>
                <td>&nbsp;</td>
                <td></td>
      </tr>
    <tr>
    ...
    SELECT counter,lastUsedNo FROM getrefnoset WHERE type = <cfqueryparam cfsqltype="cf_sql_varchar" value="INV" >
    </cfquery>
    <td>
    <cfselect query="getinv" name="invrefno" id="invrefno" value="counter" display="lastUsedNo" selected="1" />    </td>
    <th>Quotation</th>
    <cfquery name="getquo" dbtype="query">
    SELECT counter,lastUsedNo FROM getrefnoset WHERE type = <cfqueryparam cfsqltype="cf_sql_varchar" value="QUO" >
    </cfquery>
    <td><cfselect query="getquo" name="quorefno" id="quorefno" value="counter" display="lastUsedNo" selected="1" /></td>
    <th>Sales Order</th>
    <cfquery name="getso" dbtype="query">
    SELECT counter,lastUsedNo FROM getrefnoset WHERE type = <cfqueryparam cfsqltype="cf_sql_varchar" value="SO" >
    </cfquery>
    <td><cfselect query="getso" name="sorefno" id="sorefno" value="counter" display="lastUsedNo" selected="1" /></td>
    <th>Cash Sales</th>
    <cfquery name="getcs" dbtype="query">
    SELECT counter,lastUsedNo FROM getrefnoset WHERE type = <cfqueryparam cfsqltype="cf_sql_varchar" value="CS" >
    </cfquery>
    <td><cfselect query="getcs" name="csrefno" id="csrefno" value="counter" display="lastUsedNo" selected="1" /></td>
    <th>Debit Note</th>
    <cfquery name="getdn" dbtype="query">
    SELECT counter,lastUsedNo FROM getrefnoset WHERE type = <cfqueryparam cfsqltype="cf_sql_varchar" value="DN" >
    </cfquery>
    <td><cfselect query="getdn" name="dnrefno" id="dnrefno" value="counter" display="lastUsedNo" selected="1" /></td>
    </tr>
        <tr>
    <th>Purchase Order</th>
    <cfquery name="getpo" dbtype="query">
    SELECT counter,lastUsedNo FROM getrefnoset WHERE type = <cfqueryparam cfsqltype="cf_sql_varchar" value="PO" >
    </cfquery>
    <td>
    <cfselect query="getpo" name="porefno" id="porefno" value="counter" display="lastUsedNo" selected="1" />    </td>
    <th>Delivery Order</th>
    <cfquery name="getdo" dbtype="query">
    SELECT counter,lastUsedNo FROM getrefnoset WHERE type = <cfqueryparam cfsqltype="cf_sql_varchar" value="DO" >
    </cfquery>
    <td><cfselect query="getdo" name="dorefno" id="dorefno" value="counter" display="lastUsedNo" selected="1" /></td>
    <th>Purchase Return</th>
    <cfquery name="getpr" dbtype="query">
    SELECT counter,lastUsedNo FROM getrefnoset WHERE type = <cfqueryparam cfsqltype="cf_sql_varchar" value="PR" >
    </cfquery>
    <td><cfselect query="getpr" name="prrefno" id="prrefno" value="counter" display="lastUsedNo" selected="1" /></td>
    <th>Purchase Receive</th>
    <cfquery name="getrc" dbtype="query">
    SELECT counter,lastUsedNo FROM getrefnoset WHERE type = <cfqueryparam cfsqltype="cf_sql_varchar" value="RC" >
    </cfquery>
    <td><cfselect query="getrc" name="rcrefno" id="rcrefno" value="counter" display="lastUsedNo" selected="1" /></td>
    <th>Credit Note</th>
    <cfquery name="getcn" dbtype="query">
    SELECT counter,lastUsedNo FROM getrefnoset WHERE type = <cfqueryparam cfsqltype="cf_sql_varchar" value="CN" >
    </cfquery>
    <td><cfselect query="getcn" name="cnrefno" id="cnrefno" value="counter" display="lastUsedNo" selected="1" /></td>
    </tr>
    </table>    </td>
    </tr>
    </table>
