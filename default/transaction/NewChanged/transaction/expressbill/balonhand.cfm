...
<!---
<cfquery name="getlasacc" datasource="#dts#">
SELECT LasTACCYEAR FROM GSETUP
</cfquery>

<cfquery name="getdoqty" datasource="#dts#">
select frdate,frrefno from iclink where frtype = "DO" and frdate <= "#dateformat(getlasacc.LasTACCYEAR,'YYYY-MM-DD')#" and wos_date > "#dateformat(getlasacc.LastACCYEAR,'YYYY-MM-DD')#" and itemno=<cfqueryparam cfsqltype="cf_sql_varchar" value="#itemno#"> group by frrefno
</cfquery>

<cfset doupqty = 0>
<cfif getdoqty.recordcount neq 0>

<cfquery name="getlastyearupdate" datasource= "#dts#">
SELECT sum(qty) as qty FROM ictran WHERE type = "inv"
and (void = '' or void is null)
and fperiod <> '99'
and itemno=<cfqueryparam cfsqltype="cf_sql_varchar" value="#itemno#">
and dono in (<cfqueryparam cfsqltype="cf_sql_varchar" value="#valuelist(getdoqty.frrefno)#" list="yes" separator=",">)
</cfquery>

<cfif getlastyearupdate.recordcount neq 0>
<cfset doupqty = val(getlastyearupdate.qty)>
</cfif>
