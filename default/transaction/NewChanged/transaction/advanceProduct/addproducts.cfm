...
<br><input type="text" name="searchitemnew" id="searchitemnew" size="10" onKeyUp="searchSel('expressservicelist','searchitemnew')" onBlur="ajaxFunction(window.document.getElementById('itemDetail'),'/default/transaction/advanceProduct/addItemAjax.cfm?itemno='+document.getElementById('expressservicelist').value+'&custno=#url.custno#&tran=#url.tran#');setTimeout('updateVal();',500);setTimeout('calamtadvance();',500);" >&nbsp;<input type="button" id="searchitembtn" onClick="ColdFusion.Window.show('multiaddsearchitem');" value="Search" align="right" />&nbsp;
<cfelse>
