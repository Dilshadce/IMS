<cfcomponent>
	<cffunction name="getlist" access="remote" returntype="query">
    	...
    	<cfelse>
      		<cfquery name="getcust" datasource="#dts#">
                ...
                union all
        		Select concat(custno,'--',name) as custname,custno from #target_arcust# <cfif dts eq 'ovas_i' and tran eq 'CS'>where custno in ('300C/999','300M/999') </cfif> order by custno
      		</cfquery>
    	</cfif>
    ...
    <cffunction name="getagent" access="remote" returntype="query">
        <cfargument name="dts" type="string">
            ...
            union all
            SELECT concat(agent,' - ',desp) as agentdesp, agent FROM #target_icagent# where discontinueagent !='Y'
            </cfquery>
