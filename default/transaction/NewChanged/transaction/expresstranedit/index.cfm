...
function selectmemberlist(custno){	
  ...
  for(var i=0;i<output.length;i++) {
	...
	function updaterow(rowno)
	{
		...
		var updateurl = 'updaterow.cfm?tran='+escape(tran)+'&refno='+escape(refno)+'&coltype='+escape(coltypedata)+'&qty='+escape(qtylistdata)+'&trancode='+rowno+'&brem4='+escape(brem4data)+'&brem1='+escape(brem1data)+'&job='+escape(joblistdata);
...
<!--- 		if(nextflow != "")
	...
	{
		var ajaxurl = '/default/transaction/expresstranedit/getunitpriceAjax.cfm?itemno='+document.getElementById('expressservicelist').value+'&unit='+document.getElementById('expunit').value;
...
<td><cfinput type="text" name="refno2" id="refno2" value="#refno2#" maxlength="20"/></td>
</tr>
...
<th>Attention</th>
<td>
<input type="text" name="b_attn" id="b_attn" value="#getcustremark.rem2#" maxlength="35" size="40"><cfif (lcase(hcomid) neq "bnbm_i" and lcase(hcomid) neq "bnbp_i")><br><input type="button" name="Sattn1" value="Ajax Search" onClick="javascript:ColdFusion.Window.show('findattention');"></cfif></td>
<th <cfif getmodule.auto eq 1>style="display:none" </cfif>>Attention</th>
<td <cfif getmodule.auto eq 1>style="display:none" </cfif>><input type="text" name="d_attn" id="d_attn" value="#getcustremark.rem3#" maxlength="35" size="40"><cfif (lcase(hcomid) neq "bnbm_i" and lcase(hcomid) neq "bnbp_i")><br><input type="button" name="Sattn2" value="Ajax Search" onClick="javascript:ColdFusion.Window.show('findattention2');"></cfif></td>
</tr>
...
<th <cfif (lcase(hcomid) eq "bnbm_i" or lcase(hcomid) eq "bnbp_i")>style="visibility:hidden"</cfif>>PO No</th><td <cfif (lcase(hcomid) eq "bnbm_i" or lcase(hcomid) eq "bnbp_i")>style="visibility:hidden"</cfif>><input type="text" name="pono" id="pono" value="#pono#"></td>
<cfelse>
<th style="visibility:hidden">PO No</th><td style="visibility:hidden"><input type="text" name="pono" id="pono" value="#pono#"></td>
</cfif>
...
<cfwindow center="true" width="250" height="150" name="changeprice" refreshOnShow="true" closable="true" modal="true" title="Edit Price" initshow="false" source="changeprice.cfm?tran=#tran#&refno=#refno#&trancode={hidtrancode}" />  
<cfif dts eq "tcds_i" and url.tran eq "RC">
