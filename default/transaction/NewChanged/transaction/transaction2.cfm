...
<!--- ADD ON 10-12-2009 --->
	...
	<cfquery name="getProject" datasource="#dts#">
	  select * FROM #target_project# where porj = "P" and completedproject<>'Y'
      order by source
	</cfquery>
...
<cfif lcase(HcomID) eq "avent_i" or lcase(HcomID) eq "chemline_i" or lcase(hcomid) eq "techpak_i" or lcase(hcomid) eq "techpakbill_i">
...
<cfoutput>
	<cfif form.type eq "Delete">
		...
		lcase(HcomID) eq "ideal_i" or lcase(HcomID) eq "idealb_i" or lcase(HcomID) eq "probulk_i" or lcase(hcomid) eq "techpak_i" or lcase(hcomid) eq "techpakbill_i">
...
<h4>
    ...
    <cfif getmodule.simpletran eq 1>
         <cfif alsimple eq 1>
         <cfif tran eq 'INV'>
         <cfif gettranname.simpleinvtype eq '2'>
         ...
         || <a href="/newbody.cfm" target="mainFrame" onClick="window.open('/default/transaction/expressSO/index.cfm?first=true','','fullscreen=yes')">
				Simple New SO
			</a>
...
<cfset session[#formName#]="transpage">
	...
	<table align="center" class="data" border="0" cellpadding="1" cellspacing="0">
    	...
    	<tr>
      		...
      		<th height="20" <cfif getdisplaysetup2.refno2 neq "Y">style="visibility:hidden"</cfif>><cfif lcase(hcomid) eq 'ascend_i' and tran eq 'INV'>PO No<cfelse>#getGsetup.refno2#</cfif></th>
    	...
    	<tr>
      		<td>&nbsp;</td>
      		<td colspan="2" <cfif getdisplaysetup2.customerno neq "Y">style="visibility:hidden"</cfif>><cfif lcase(hcomid) eq "bnbm_i" or lcase(HcomID) eq "bnbp_i"><input name="name" type="text" size="40" maxlength="40" value="#convertquote(form.name)#" readonly><cfelse><input name="name" type="text" size="40" maxlength="40" value="#convertquote(getcustomere.name)#" readonly></cfif></td>
...
onChange="ajaxFunction(document.getElementById('projectjobajaxfield'),'/default/transaction/transactionprojectjobajax.cfm?agenno='+this.value);"
    	...
    	<tr>
			...
			<th <cfif getdisplaysetup2.terms neq "Y">style="visibility:hidden"</cfif>>#getGsetup.lterm#</th>
      		<td <cfif getdisplaysetup2.terms neq "Y">style="visibility:hidden"</cfif>><select name="terms" id="terms" <cfif lcase(hcomid) eq "tmt_i" or lcase(HcomID) eq "taff_i">onChange="ChgDueDate()"</cfif> <cfif getdealer_menu.tran_edit_term neq 'Y'>onfocus="this.defaultIndex=this.selectedIndex;" onchange="this.selectedIndex=this.defaultIndex;"</cfif> <!---<cfif lcase(hcomid) eq "bnbm_i" or lcase(HcomID) eq "bnbp_i">onChange="updatetermdetail()"</cfif>--->>
          			<option value="">Choose a #getGsetup.lterm#</option>
    	...
    	<tr>
            ...
            <!---for weiken--->
	      		<th <cfif getdisplaysetup2.project neq "Y">style="visibility:hidden"</cfif>><!--- Project / Job --->#getGsetup.lPROJECT# <cfif lcase(hcomid) neq "ascend_i"><cfif getmodule.job eq "1">/ #getGsetup.lJOB#</cfif></cfif></th>
        ...
        <a onClick="ColdFusion.Window.show('createJobAjax');" onMouseOver="this.style.cursor='hand';" >Create New #getgsetup.ljob#</a></cfif></cfif></td>
        </tr>
        <cfelse>
        <tr>
        <cfif getGsetup.transactiondate eq 'Y'>
 ...
 <cfif lcase(HcomID) eq 'taftc_i'>
      ...
      <td></td><td></td><td></td>
        ...
        <tr>
        <th <cfif quodisplay neq "Y">style="display:none"</cfif>>QUO</th>
        <td <cfif quodisplay neq "Y">style="display:none"</cfif>><input type="text" name="quono" value="#quono#" size="40" maxlength="35"></td>
        <td></td>
        </tr>
        <tr>
        <th <cfif sodisplay neq "Y">style="visibility:hidden"</cfif>>#gettranname.bodyso#</th>
        <td <cfif sodisplay neq "Y">style="visibility:hidden"</cfif>><input type="text" name="sono" value="#sono#" size="40" maxlength="35"></td>
        <td></td>
        <th <cfif permitnodisplay neq "Y">style="visibility:hidden"</cfif>><cfif lcase(hcomid) eq "fdipx_i">Invoice No.<cfelseif lcase(hcomid) eq "mastercare_i" or lcase(hcomid) eq "gorgeous_i">Factory<cfelseif lcase(hcomid) eq "sdc_i" or lcase(hcomid) eq "sdab_i">Validity<cfelseif (lcase(hcomid) eq "weikeninv_i" or lcase(hcomid) eq "weikenint_i" or lcase(hcomid) eq "weikenid_i" or lcase(hcomid) eq "futurehome_i" or lcase(hcomid) eq "weikenbuilder_i" or lcase(hcomid) eq "weikendecor_i")>Status<cfelseif lcase(hcomid) eq "apnt_i">Inv No<cfelseif lcase(hcomid) eq "atc2005_i">Tag<cfelse>Permit No</cfif></th>
        ...
        <input type="text" name="permitno" value="#permitno#" size="40" maxlength="100" <cfif tran eq 'QUO' and getgsetup.quotationlead eq 'Y' and isdefined("form.leadno")>readonly</cfif>>
      	...
      	<tr>
        	<th height="20" <cfif podisplay neq "Y">style="visibility:hidden"</cfif>><cfif lcase(hcomid) eq "talerfood_i">Customer PO<cfelse>#gettranname.bodypo#</cfif></th>
...
select * from deliveryTiming where id = '#general.DeliveryTiming#'
        ...
        <cfelse>  
      	...
      	<tr>
        	<th height="20" <cfif dodisplay neq "Y">style="visibility:hidden"</cfif>>#gettranname.bodydo#</th>
      ...
      <td>  <cfinput type="text" name="remark6" value="#remark6#" size="40" maxlength="10" validate="eurodate" message="Kindly key in date format">
        <img src="../../images/cal.gif" width=17 height=15 border=0 onClick="showCalendarControl(remark6);">(DD/MM/YYYY)</td>
      	...
      	<tr>
			...
			<cfelse>
				<th <cfif billtoadd_code neq "Y">style="visibility:hidden"</cfif>>#getgsetup.rem0#</th>
       ...
       <cfelseif lcase(hcomid) eq "vsolutionspteltd_i" or lcase(hcomid) eq "vsyspteltd_i">
        ...
        <cfelseif getmodule.auto eq "1" and lcase(hcomid) neq "imperial1_i">
      	...
      	<tr>
				...
				<th <cfif deladd_code neq "Y">style="visibility:hidden"</cfif>>#getgsetup.rem1#</th>
...
<cfelseif lcase(hcomid) eq "taftc_i">
        ...
        <cfelseif getmodule.auto eq "1" and lcase(hcomid) neq "imperial1_i">
      	...
      	<tr>
				...
				<th <cfif billattn neq "Y">style="visibility:hidden"</cfif>>#getgsetup.rem2#</th>
...
<cfelseif lcase(hcomid) eq "taftc_i">
        ...
        SELECT arrem1 from #target_arcust# where custno = '#custno#'
         ...
         <img src="../../images/cal.gif" width=17 height=15 border=0 onClick="showCalendarControl(remark9);">(DD/MM/YYYY)
      	...
      	<tr>
				<th <cfif delattn neq "Y">style="visibility:hidden"</cfif>>#getgsetup.rem3#</th>
        	...
        	<th <cfif getmodule.auto eq "1" and (tran eq 'RC' or tran eq 'PO' or tran eq 'PR')>style="visibility:hidden"<cfelse><cfif rem10display neq "Y">style="visibility:hidden"</cfif></cfif>><cfif (lcase(HcomID) eq "net_i" or lcase(hcomid) eq "netm_i") and tran eq "INV">Start On<cfelse>#getgsetup.rem10#</cfif></th>
			...
			<cfelseif lcase(hcomid) eq "ideal_i" or lcase(hcomid) eq "idealb_i" or lcase(hcomid) eq "decor_i" >
                ...
                <cfelseif lcase(hcomid) eq "bestform_i" or lcase(hcomid) eq "alsale_i" or lcase(hcomid) eq "gbi_i" or getmodule.auto eq "1" or lcase(hcomid) eq "litelab_i">
...
<cfinput type="text" name="remark10" id="remark10" value="#remark10#" bind="cfc:accordtran2.getremark10('#dts#',{remark5})"></cfoutput>
	...
	<cfelseif lcase(hcomid) eq "aepl_i">
        ...
        <input type="button" name="updateshipvia" id="updateshipvia" value="Update" onClick="ajaxFunction(document.getElementById('updateshipviaajax'),'/default/transaction/tran2asaikiajax.cfm?custno='+escape(document.getElementById('custno').value));updateshipvia2();"><div id="updateshipviaajax"></div>
      	...
      	<tr>
				<th <cfif billtel neq "Y">style="visibility:hidden"</cfif>>#getgsetup.rem4#</th>
...
<cfinput type="text" name="remark11" id="remark11" value="#remark11#" bind="cfc:accordtran2.getremark11('#dts#',{remark5})"></cfoutput></td>
			...
			<cfelseif lcase(hcomid) eq "mcjim_i" or lcase(hcomid) eq "ideakonzepte_i" or lcase(hcomid) eq "bestform_i" or lcase(hcomid) eq "ovas_i"  or lcase(hcomid) eq "asiasoft_i" or lcase(hcomid) eq "bnbm_i" or lcase(hcomid) eq "bnbp_i">
				<th>#getgsetup.rem11#</th>
	        	<td rowspan="2">
        ...
        <th>#getGsetup.rem11#</th>
      	...
      	<tr>
				<th <cfif deltel neq "Y">style="visibility:hidden"</cfif>><cfif lcase(hcomid) eq "glenn_i" or lcase(hcomid) eq "glenndemo_i">Heading<cfelse>#getgsetup.rem12#</cfif></th>
