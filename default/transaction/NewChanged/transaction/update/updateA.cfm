...
<cfif url.t2 eq "INV">
	...
	<cfif url.t1 eq "DO">
	  	...
	  	<cfquery datasource="#dts#" name="getupdate">
			Select * from artran where custno = '#url.custno#' and type = '#t1#' and toinv = ''  and (void = '' or void is null) order by refno
	  	</cfquery>
...
document.updatepage.checkbox[i].checked=true;">
	...
	<cfelseif url.t1 eq "SO">
        ...
        <cfquery datasource="#dts#" name="getupdate">
			Select * from artran where custno = '#url.custno#' and type = '#t1#' and toinv = ''  and (void = '' or void is null) <cfif lcase(hcomid) eq "ltm_i"> and (rem45='' or rem45 is null or rem45='A')</cfif> order by refno
	  	</cfquery>
...
document.updatepage.checkbox[i].checked=true;">
	...
	<cfelseif url.t1 eq "QUO">
		<cfquery datasource="#dts#" name="getupdate">
			Select * from ictran where custno = '#url.custno#' and type = '#t1#' and shipped < qty and (void = '' or void is null) group by refno order by refno
		</cfquery>
...
<cfelseif url.t2 eq "RC">
	...
	<cfif url.t1 eq "PO">
        ...
        <cfquery datasource="#dts#" name="getupdate">
			Select a.* from ictran as a left join (select printstatus,refno from artran where type='#t1#')as b on a.refno=b.refno where a.custno = '#url.custno#' and a.type = '#t1#' and (a.shipped+a.writeoff) < a.qty and  b.printstatus = 'a3' and (a.void = '' or a.void is null)  group by a.refno order by a.refno
	  	</cfquery>
        <cfelse>
	  	<cfquery datasource="#dts#" name="getupdate">
			Select * from ictran where custno = '#url.custno#' and type = '#t1#' and (shipped+writeoff) < qty and (void = '' or void is null)  group by refno order by refno
