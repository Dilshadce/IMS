...
<cffunction name="RemoveRecord_In" output="true" returntype="string">
	...
	<cfquery name="getLocation" datasource="#dts#">
		select serialno, location from iserial where 
        type=<cfqueryparam cfsqltype="cf_sql_varchar" value="#URLDECODE(url.tran)#"> 
        and refno=<cfqueryparam cfsqltype="cf_sql_varchar" value="#URLDECODE(url.nexttranno)#"> 
        and itemno=<cfqueryparam cfsqltype="cf_sql_varchar" value="#URLDECODE(url.itemno)#"> 
        and trancode=<cfqueryparam cfsqltype="cf_sql_varchar" value="#URLDECODE(url.itemcount)#">
	</cfquery>
	<cfif getLocation.recordcount gt 0>
		<cfif getLocation.location[1] neq URLDECODE(url.location)>
			<cfquery name="getOldRecord" datasource="#dts#">
				select type,refno,serialno from iserial where itemno='#URLDECODE(url.itemno)#' and location='#getLocation.location[1]#' and sign=-1 
				and serialno in ('#valuelist(getLocation.serialno,"','")#')
			</cfquery>
			<cfif getOldRecord.recordcount gt 0>
				<cfquery name="deleteRecord" datasource="#dts#">
					delete from iserial where itemno=<cfqueryparam cfsqltype="cf_sql_varchar" value="#URLDECODE(url.itemno)#">  and location='#getLocation.location[1]#' and sign=-1 and 
			...
			<cftry>
				<cfquery name="updateRecord" datasource="#dts#">
					update iserial set location='#URLDECODE(url.location)#' 
					where 
                    type=<cfqueryparam cfsqltype="cf_sql_varchar" value="#URLDECODE(url.tran)#">  
                    and refno=<cfqueryparam cfsqltype="cf_sql_varchar" value="#URLDECODE(url.nexttranno)#"> 
                    and itemno=<cfqueryparam cfsqltype="cf_sql_varchar" value="#URLDECODE(url.itemno)#"> and 
					trancode=<cfqueryparam cfsqltype="cf_sql_varchar" value="#URLDECODE(url.itemcount)#"> and location='#getLocation.location#'
				</cfquery>
...
<cffunction name="RemoveRecord_Out" output="false">
	<cfquery name="getLocation" datasource="#dts#">
		select location from iserial where 
        type=<cfqueryparam cfsqltype="cf_sql_varchar" value="#URLDECODE(url.tran)#">  and refno=<cfqueryparam cfsqltype="cf_sql_varchar" value="#URLDECODE(url.nexttranno)#">  and 
		itemno=<cfqueryparam cfsqltype="cf_sql_varchar" value="#URLDECODE(url.itemno)#"> and trancode=<cfqueryparam cfsqltype="cf_sql_varchar" value="#URLDECODE(url.itemcount)#"> limit 1
	</cfquery>
	<cfif getLocation.recordcount gt 0>
		<cfif getLocation.location neq URLDECODE(url.location)>
			<cfquery name="deleteRecord" datasource="#dts#">
				delete from iserial where type=<cfqueryparam cfsqltype="cf_sql_varchar" value="#URLDECODE(url.tran)#"> and refno=<cfqueryparam cfsqltype="cf_sql_varchar" value="#URLDECODE(url.nexttranno)#"> and 
				itemno=<cfqueryparam cfsqltype="cf_sql_varchar" value="#URLDECODE(url.itemno)#"> and trancode=<cfqueryparam cfsqltype="cf_sql_varchar" value="#URLDECODE(url.itemcount)#">
			</cfquery>
		</cfif>
	</cfif>
</cffunction>
<cfinclude template = "/CFC/convert_single_double_quote_script.cfm">
<cfif url.type neq "Delete">
    ...
    select * from gsetup
		...
		<script language="javascript" type="text/javascript">
			...
			<cfoutput>
			var type='#URLDECODE(url.tran)#';
			 var refno='#URLDECODE(url.nexttranno)#';
			
			var trancode='#URLDECODE(url.itemcount)#';
			var qty='#URLDECODE(url.qty)#';
			var newqty = '#URLDECODE(url.qty)#';
			var custno='#URLDECODE(url.custno)#';
			var period='#URLDECODE(url.readperiod)#';
			var date='#URLDECODE(url.nDateCreate)#';
			var agenno='#URLDECODE(url.agenno)#';
			var batchcode='#URLDECODE(url.batchcode)#';
			var location1='#URLDECODE(url.location)#';
			var currrate='#URLDECODE(url.currrate)#';
			var price='#URLDECODE(url.price)#';
			var sign='#sign#'; 
			</cfoutput>
			...
			function getResult(){
				...
				ajaxFunction(document.getElementById('serialtablelist'),'serialtableajax.cfm?itemno='+escape(itemno)+'&refno='+escape(refno)+'&type='+escape(type)+'&trancode='+escape(trancode)+'&custno='+escape(custno)+'&period='+escape(period)+'&qty='+escape(qty)+'&agenno='+escape(agenno)+'&location='+escape(location1)+'&currrate='+escape(currrate)+'&sign='+escape(sign)+'&price='+escape(price));
				var ajaxurl = 'addedserialajax.cfm?itemno='+escape(itemno)+'&refno='+escape(refno)+'&type='+escape(type)+'&trancode='+escape(trancode)+'&custno='+escape(custno)+'&period='+escape(period)+'&qty='+escape(qty)+'&agenno='+escape(agenno)+'&location='+escape(location1)+'&currrate='+escape(currrate)+'&sign='+escape(sign)+'&price='+escape(price)+'&batchcode='+escape(batchcode);
				...
				new Ajax.Request(ajaxurl,
					{
					  ...
					  <cfif lcase(hcomid) eq "dental_i" or lcase(hcomid) eq "dental10_i" or lcase(hcomid) eq "mfss_i" or lcase(hcomid) eq "hcss_i" or lcase(hcomid) eq "iaf_i" or lcase(hcomid) eq "scpl_i">
						<cfif sign eq '1'>
						...
						document.serialForm.text_serialno.focus();
						<cfelse>
						document.serialForm.select_serialno.focus();
						</cfif>
						</cfif>
					  }
					})
			...
			function addNewSerialno(){
				...
				<cfif (lcase(hcomid) eq "dental_i" or lcase(hcomid) eq "dental10_i" or lcase(hcomid) eq "mfss_i" or lcase(hcomid) eq "hcss_i" or lcase(hcomid) eq "iaf_i" or lcase(hcomid) eq "scpl_i") and sign neq "-1">
				...
				<cfif (lcase(hcomid) eq "dental_i" or lcase(hcomid) eq "dental10_i" or lcase(hcomid) eq "mfss_i" or lcase(hcomid) eq "hcss_i" or lcase(hcomid) eq "iaf_i" or lcase(hcomid) eq "scpl_i") and sign eq "-1">
			...
			function addNewSerialno2(){
				...
				{alert("Please make sure you have select/key-in Serial No.");}
					...
					var ajaxurl = 'serialnoaddnew2ajax.cfm?itemno='+escape(itemno)+'&date='+escape(date)+'&refno='+escape(refno)+'&type='+escape(type)+'&trancode='+escape(trancode)+'&custno='+escape(custno)+'&period='+escape(period)+'&qty='+escape(qty)+'&agenno='+escape(agenno)+'&location='+escape(location1)+'&currrate='+escape(currrate)+'&sign='+escape(sign)+'&price='+escape(price)+'&serialnofr='+escape(serialnofr.value)+'&serialnoto='+escape(serialnoto.value)+'&prefix='+escape(prefix)+'&endfix='+escape(endfix)+'&seriallen='+escape(seriallen);
			...
			function addNewSerialno3(){
				...
				var ajaxurl = 'serialnoaddnew3ajax.cfm?itemno='+escape(itemno)+'&date='+escape(date)+'&refno='+escape(refno)+'&type='+escape(type)+'&trancode='+escape(trancode)+'&custno='+escape(custno)+'&period='+escape(period)+'&qty='+escape(qty)+'&agenno='+escape(agenno)+'&location='+escape(location1)+'&currrate='+escape(currrate)+'&sign='+escape(sign)+'&price='+escape(price)+'&serialnofr='+escape(serialnofr.value)+'&serialnoto='+escape(serialnoto.value);
<!---
					new Ajax.Request(ajaxurl,
					{
					  method:'get',
			...
			function deleteSerialno(serialno){
				var itemno= document.serialForm.itemno.value;
				var ajaxurl = 'serialnoaddnewajax.cfm?proces=Delete&itemno='+escape(itemno)+'&serialno='+escape(serialno)+'&date='+escape(date)+'&refno='+escape(refno)+'&type='+escape(type)+'&trancode='+escape(trancode)+'&custno='+escape(custno)+'&period='+escape(period)+'&qty='+escape(qty)+'&agenno='+escape(agenno)+'&location='+escape(location1)+'&currrate='+escape(currrate)+'&sign='+escape(sign)+'&price='+escape(price);
			...
			function changectg(serialno,ctgno){
				var itemno= document.serialForm.itemno.value;
				var ajaxurl = 'changectg.cfm?itemno='+escape(itemno)+'&serialno='+escape(serialno)+'&date='+escape(date)+'&refno='+escape(refno)+'&type='+escape(type)+'&trancode='+escape(trancode)+'&custno='+escape(custno)+'&period='+escape(period)+'&qty='+escape(qty)+'&agenno='+escape(agenno)+'&location='+escape(location)+'&currrate='+escape(currrate)+'&sign='+escape(sign)+'&price='+escape(price)+'&ctgno='+escape(ctgno);
    ...
    <cfquery name="getremainingserialno" datasource="#dts#">
        select * from iserial where type='#url.tran#' and refno='#url.nexttranno#' and trancode='#url.itemcount#' and itemno='#itemno#'
        </cfquery>
        ...
        and (void is null or void='') and serialno like '#URLDECODE(url.batchcode)#%'
        group by serialno
		order by serialno) as a
        where a.sign=1
        </cfquery>
		...
		from iserial 
		where itemno='#url.itemno#'
        and location='#url.location#' and type<>'SAM'
        and (void is null or void='')
        group by serialno
		order by serialno) as a
        where a.sign=1
	...
	<body >
		...
		<form name="serialForm" id="serialForm">
			<cfoutput><input type="hidden" name="itemno" id="itemno" value="#URLDECODE(url.itemno)#"></cfoutput>
		<div id="ajaxFieldPro"></div>
        <cfif getgsetup.serialnorun eq 'Y' and url.tran eq "INV" or url.tran eq "DO" or url.tran eq "CN" or url.tran eq "DN" or url.tran eq "SAM">
        <cfoutput>
        <table class="data">
            <tr>
            ...
            <tr>
              ...
              <td><input type="button" name="add_text3" id="add_text3" value="Range Add" onClick="addNewSerialno3()"></td>
            </tr>
          </table>
        </cfoutput>
		</cfif>
        
		<cfif getgsetup.serialnorun eq 'Y' and url.tran eq 'RC'>
          <table class="data">
            <tr>
            ...
            <tr>
              <th>Running Number From :</th>
              <td><input type="text" name="runningnumfr" id="runningnumfr" value="" size="10" /></td>
            </tr>
            <tr>
              <th>Running Number To :</th>
              <td><input type="text" name="runningnumto" id="runningnumto" value="" size="10" /></td>
            </tr>
            <tr>
              <th>End Fix :</th>
              <td><input type="text" name="endfix" id="endfix" size="5" /></td>
              <td><input type="button" name="add_text2" id="add_text2" value="Add" onClick="addNewSerialno2()"></td>
            </tr>
          </table>
        </cfif>
		...
		<cfoutput>
		<tr>
			<td>#URLDECODE(url.nexttranno)#</td>
			<td>#URLDECODE(url.custno)#</td>
			<td>#URLDECODE(url.itemno)#</td>
			<td align="center">#URLDECODE(url.qty)#</td>
			<td align="center"><label id="balQty"></label></td>
			<cfif sign eq 1><td><select name="added_serialno" id="added_serialno"></select></td></cfif>
		</tr>
		</cfoutput>
		</table>
        </div>
		...
		<tr>
			...
			<th><cfoutput>#getgsetup.lserial#</cfoutput></th>
...
<cfif getremainingserialno.recordcount lt qty>
		...
		<tr>
			<td>New</td>
			<td align="right"><input type="text" name="text_serialno" maxlength="<cfif lcase(hcomid) neq "dental_i" or lcase(hcomid) neq "dental10_i" or lcase(hcomid) neq "mfss_i" or lcase(hcomid) neq "hcss_i" or lcase(hcomid) neq "iaf_i" or lcase(hcomid) eq "scpl_i">100<cfelse>100</cfif>" size="30" onKeyPress="return handleEnter(event)"></td>
...
<cfelse>
	<cfquery name="getUnwantedRecord" datasource="#dts#">
		...
		where type='URLDECODE(url.tran)' and itemno='#URLDECODE(url.itemno)#' and refno='#URLDECODE(url.nexttranno)#' and trancode='#URLDECODE(url.itemcount)#'
	</cfquery>
	<cfif getUnwantedRecord.recordcount gt 0>
		<cfquery name="deleteRecord" datasource="#dts#">
			delete from iserial where itemno='#URLDECODE(url.itemno)#' and 
