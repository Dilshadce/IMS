	...
	select ddlitem from gsetup
		...
		<cfelse>
            ...
            <cfquery name='getitemlist' datasource='#dts#'>
                    ...
                    union all
                    select concat(itemno," - ",desp,despa) as itemdesp, itemno,despa,desp from icitem 
...
<a style="cursor:pointer;" onClick="window.open('uploadfile.cfm?refno=#URLENCODEDFORMAT(nexttranno)#&type=#tran#','UploadDocument','width=600, height=500, status=yes, menubar=no, location=no, scrollbars=yes')"><b>Upload Document</b></a>&nbsp;&nbsp;|&nbsp;&nbsp;<a style="cursor:pointer;" onClick="window.open('import_tranexcel.cfm?refno=#URLENCODEDFORMAT(nexttranno)#&type=#tran#','UploadDocument','width=600, height=500, status=yes, menubar=no, location=no, scrollbars=yes')"><b>Import Item</b></a>&nbsp;&nbsp;|&nbsp;&nbsp;<a style="cursor:pointer;" onClick="window.open('itembody.cfm?refno=#URLENCODEDFORMAT(nexttranno)#&type=#tran#','UploadDocument','width=600, height=500, status=yes, menubar=no, location=no, scrollbars=yes')"><b>Export Item</b></a></cfif>
        <cfif (lcase(hcomid) eq "aipl_i" or lcase(hcomid) eq "gramas_i" or lcase(hcomid) eq "kjpe_i" or lcase(hcomid) eq "demo_i"  or lcase(hcomid) eq "supporttest_i") and tran eq 'INV'>
        ...
        <cfelseif (lcase(hcomid) eq "aipl_i" or lcase(hcomid) eq "gramas_i" or lcase(hcomid) eq "kjpe_i" or lcase(hcomid) eq "demo_i"  or lcase(hcomid) eq "supporttest_i") and (tran eq 'RC' or tran eq 'PR')>
        ...
        <cfelseif (lcase(hcomid) eq "aipl_i" or lcase(hcomid) eq "gramas_i" or lcase(hcomid) eq "kjpe_i" or lcase(hcomid) eq "demo_i"  or lcase(hcomid) eq "supporttest_i") and (tran eq 'CN' or tran eq 'DN')>
        ...
        <cfelseif (lcase(hcomid) eq "aipl_i" or lcase(hcomid) eq "gramas_i" or lcase(hcomid) eq "kjpe_i" or lcase(hcomid) eq "demo_i"  or lcase(hcomid) eq "supporttest_i") and tran eq 'SAM'>
        <!---
...
<cfif isdefined ('url.complete') or type eq 'Edit'>
	...
	<form name='invoicesheet' action='transaction3b.cfm' method='post' onSubmit="releaseDirtyFlag();return permit();">
		...
		<tr>
			...
			<td <cfif getdisplaysetup2.f_acc_code neq "Y">style="visibility:hidden"</cfif>><input type="hidden" name="special_account_code"  value="#getartran.special_account_code#" maxlength="8" size="10" onClick="select();"></td>--->
        ...
        <tr <cfif lcase(HcomID) eq "weikeninv_i" or lcase(HcomID) eq "weikenint_i" or lcase(HcomID) eq "weikenbuilder_i" or lcase(HcomID) eq "futurehome_i" or lcase(HcomID) eq "weikenid_i" or hcomid eq 'weikendecor_i' or lcase(HcomID) eq "weikentrial_i"><cfelse>style="display:none"</cfif>>
        <td>&nbsp;</td>
        <td>&nbsp;</td>
        <td>&nbsp;</td>
        <td>&nbsp;</td>
		...
		<tr>
			...
			<td <cfif (lcase(HcomID) eq "weikeninv_i" or lcase(HcomID) eq "weikenint_i" or lcase(HcomID) eq "weikenbuilder_i" or lcase(HcomID) eq "futurehome_i" or lcase(HcomID) eq "weikenid_i" or hcomid eq 'weikendecor_i' or lcase(HcomID) eq "weikentrial_i") and (tran eq 'RC' or tran eq 'PO' or tran eq 'PR')><cfelse>style="visibility:hidden"</cfif>>
		...
		<tr>
			...
			<cfif lcase(hcomid) eq "mhsl_i" and tran eq "RC">
            	<td nowrap><input name="taxincl" id="taxincl" type="checkbox" value="T" #iif(getartran.taxincl eq "T",DE("checked"),DE(""))# onClick="javascript:check_taxincls();"> Tax Already Included
                </td>
			<!--- ADD ON 17-08-2009 --->
		...
		<tr>
			...
			<td <cfif getdisplaysetup2.f_pay_cheque neq "Y">style="visibility:hidden"</cfif>><cfif tran eq "RQ">Sales Tax %<cfelse>Cheque No</cfif></td>
			...
			<td><cfif tran eq "RQ"><input name="RQpTax" type="text" size="5" maxlength="5" value="#xrqtaxp1#"><cfelse>&nbsp;</cfif></td>
			<td>&nbsp;</td>
		</tr>
		<cfif (lcase(hcomid) eq "ecraft_i" or lcase(hcomid) eq "ovas_i") and (tran eq 'PO' or tran eq "RC" or tran eq "PR")>
			<tr>
...
where type in('DO','CN','Transfer')
        ...
        <tr>
			<td>&nbsp;</td>
			<td>&nbsp;</td>
			<td <cfif getdisplaysetup2.f_pay_dc neq "Y">style="visibility:hidden"</cfif>><cfif lcase(hcomid) eq "bama_i" or lcase(hcomid) eq "dgalleria_i">NETS<cfelseif getmodule.auto eq '1'>NETS<cfelse>Debit Card</cfif></td>
    ...
    <!---end get footer currate & code--->
		...
		<tr>
		  ...
		  <td <cfif getdisplaysetup2.f_pay_deposit neq "Y">style="visibility:hidden"</cfif>><input name="deposit" id="deposit" type="#hidestatus#" value="#numberformat(getartran.deposit,'0.00')#" size="10" maxlength="15" onKeyUp="getDepositCount();" <cfif lcase(hcomid) eq "gel_i" and husergrpid neq "Admin" and husergrpid neq "Super">readonly</cfif>></td>
			<cfif getgsetup.footerexchange eq 'Y'>
            <td>Currency Rate</td>
...
<tr>
	...
	<th><cfoutput>#getdefault.ldescription#</cfoutput></th>
    </cfif>
    <cfoutput>
 ...
 <td><div align='right'><font face='Arial, Helvetica, sans-serif'>#getqtybalance.balance#</font></div></td>
		...
		<td><div align='center'><font face='Arial, Helvetica, sans-serif'>
			...
			<cfelse>
                ...
                <a onClick="releaseDirtyFlag();" href='transaction4.cfm?tran=#tran#&hmode=#hmode#&type1=Edit&itemno=#URLEncodedFormat(itemno)#&service=#linecode#&itemcount=#itemcount#&ndatecreate=#ndatecreate#&nexttranno=#nexttranno#&refno3=#listfirst(refno3)#&currrate=#xcurrrate#&agenno=#agenno#
				&enterbatch=#URLENCODEDFORMAT(batchcode)#&oldqty=#qty#&grade=#grade#&newtrancode=#newtrancode#&multilocation=#multilocation#&editinfoonly=1&noeditloc=1'><img height='18px' width='18px' src='../../images/edit.ICO' alt='Edit' border='0'>Edit</a>&nbsp;
            </cfif>
            <cfelse>
            ...
            <cfelse>
            	<a onClick="releaseDirtyFlag();" href='transaction4.cfm?tran=#tran#&hmode=#hmode#&type1=Edit&itemno=#URLEncodedFormat(itemno)#&service=#linecode#&itemcount=#itemcount#&ndatecreate=#ndatecreate#&nexttranno=#nexttranno#&refno3=#listfirst(refno3)#&currrate=#xcurrrate#&agenno=#agenno#
				&enterbatch=#URLENCODEDFORMAT(batchcode)#&oldqty=#qty#&grade=#grade#&newtrancode=#newtrancode#&multilocation=#multilocation#&editinfoonly=1'><img height='18px' width='18px' src='../../images/edit.ICO' alt='Edit' border='0'>Edit</a>&nbsp;
            </cfif>
            </cfif>
			...
			<cfif tran eq "INV" or tran eq "CS">
				...
				&enterbatch=#URLENCODEDFORMAT(batchcode)#&oldqty=#qty#&newtrancode=#newtrancode#&multilocation=#multilocation#'><img height='18px' width='18px' src='../../images/delete.ICO' alt='Delete' border='0'>Delete</a>
                <cfelse>
...
<script type="text/javascript">
	...
	function adjustgrand()
	{
		...
		document.invoicesheet.roundadj.value=(((document.invoicesheet.viewGrand.value* 2).toFixed(1))/2).toFixed(2)-document.invoicesheet.viewGrand.value;
		...
		document.invoicesheet.roundadj.value=(grandafterround/2).toFixed(2)-document.invoicesheet.viewGrand.value;
...
document.invoicesheetpost.submit();
        ...
        source="/default/transaction/products/searchitem.cfm" />
