...
<cfset sort = url.sort >
		...
		<cfif sort eq "wos_date" >

			Date From 
			<input type="text" name="datefrom" id="datefrom" size="10" maxlength="10" readonly>
            <img src="/images/cal.gif" width=17 height=15 border=0 onClick="showCalendarControl(datefrom);">
			Date To 
			<input type="text" name="dateto" id="dateto" size="10" maxlength="10" readonly>
            <img src="/images/cal.gif" width=17 height=15 border=0 onClick="showCalendarControl(dateto);">
            
            <input type="button" name="searchbtn" id="searchbtn" value="Go"
            onclick="ajaxFunction(document.getElementById('ajaxField2'),'postingdepositajax.cfm?sort='+document.getElementById('sort').value+'&datefrom='+document.getElementById('datefrom').value+'&dateto='+document.getElementById('dateto').value);"
            >
		
		<cfelseif sort eq "fperiod" >
			Period 
            ...
            <input type="button" name="searchbtn" id="searchbtn" value="Go" onclick="ajaxFunction(document.getElementById('ajaxField2'),'postingdepositajax.cfm?sort='+document.getElementById('sort').value+'&fperiod='+document.getElementById('fperiod').value);">
		<cfelseif sort eq "sono" >

			Reference From 
			<input type="text" name="sonofrom" id="sonofrom" size="20" maxlength="100" >
			Reference To 
			<input type="text" name="sonoto" id="sonoto" size="20" maxlength="100">
            
            <input type="button" name="searchbtn" id="searchbtn" value="Go"
            onclick="ajaxFunction(document.getElementById('ajaxField2'),'postingdepositajax.cfm?sort='+document.getElementById('sort').value+'&sonofrom='+document.getElementById('sonofrom').value+'&sonoto='+document.getElementById('sonoto').value);"
            >
         <cfelseif sort eq "depositno" >

			Deposit No From 
			<input type="text" name="depositfrom" id="depositfrom" size="10" maxlength="100" >

			Deposit No To 
			<input type="text" name="depositto" id="depositto" size="10" maxlength="100" >

            
            <input type="button" name="searchbtn" id="searchbtn" value="Go"
            onclick="ajaxFunction(document.getElementById('ajaxField2'),'postingdepositajax.cfm?sort='+document.getElementById('sort').value+'&depositfrom='+document.getElementById('depositfrom').value+'&depositto='+document.getElementById('depositto').value);"
            >
</cfif>
