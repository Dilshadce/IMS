...
select agentlistuserid from gsetup
	...
	<cfquery name="getcustsupp" datasource="#dts#">
   		select custno as xcustno,name,name2,add1,add2,add3,add4,agent,area from #url.dbtype# WHERE custno like <cfqueryparam cfsqltype="cf_sql_varchar" value="%#url.custno#%"> and name like <cfqueryparam cfsqltype="cf_sql_varchar" value="%#url.custname#%"> and name like <cfqueryparam cfsqltype="cf_sql_varchar" value="#url.leftcustname#%">
...
<cfif getgeneral.agentlistuserid eq "Y">and ucase(agent) in (SELECT agent FROM #target_icagent# WHERE agentlist like "%#ucase(huserid)#%")
    ...
    <td><a onMouseOver="JavaScript:this.style.cursor='hand';" onClick="selectlistRQ('#getcustsupp.xcustno#');ColdFusion.Window.hide('findRQCustomer');" >SELECT</a></td>
    <cfelseif url.displaytype eq 'name2'>
    <td>#getcustsupp.name#</td>
