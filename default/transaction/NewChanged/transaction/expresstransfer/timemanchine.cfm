...
SELECT uuid,trdatetime,type,driver,rem9 from ictrantemp where type='TROU' <!---where onhold='Y'---> group by uuid order by trdatetime desc limit 20 
</cfquery>
<cfoutput>
<table>
<tr>
...
<cfoutput>
	...
	opener.window.location.href="index.cfm?type=#url.tran#&consignment=#url.consignment#&uuid="+newuuid;
	window.close();
	}
	}
</cfoutput>
