<cfif url.type1 eq "Edit">
	...
	<cfif url.multilocation eq "Y" and url.service neq "SV">
		<cfquery datasource="#dts#" name="getitem">
            ...
            requiredate,replydate,deliverydate,invlinklist,invcnitem,
			shelf,dono,gst_item,dispec1,dispec2,dispec3,taxpec1,gltradac,factor1,factor2,
			linecode,comment,sv_part,sercost,grade,nodisplay,totalupdisplay,deductableitem,note1,title_id,title_desp<cfif hcomid eq "hodaka_i">,sono,soitem</cfif><cfif lcase(hcomid) eq "topsteel_i" or lcase(hcomid) eq "topsteelhol_i">,title_despa</cfif>,
			...
			group by itemno,desp,despa,custno,price_bil,packing,unit_bil,brem1,brem2,brem3,brem4,<cfif lcase(hcomid) eq "avent_i" or lcase(hcomid) eq "mcjim_i" or lcase(hcomid) eq "redhorn_i">brem5,brem6,<cfif lcase(hcomid) eq "avent_i" or lcase(hcomid) eq "mcjim_i" or lcase(hcomid) eq "redhorn_i">brem7,brem8,brem9,</cfif> </cfif>
			shelf,dono,gst_item,dispec1,dispec2,dispec3,taxpec1,gltradac,factor1,factor2,linecode,
			comment,sv_part,sercost,grade,nodisplay,totalupdisplay,deductableitem,note1,title_id,title_desp,supp,qty1,qty2,qty3,qty4,qty5,qty6,qty7,note_a,source,job
		</cfquery>
	...
	<cfquery name="getitembal" datasource="#dts#">
		...
		minimum,
		graded,
		remark1,wpformula,wqformula,itemtype
		from icitem 
		where itemno='#url.itemno#';
	</cfquery>
