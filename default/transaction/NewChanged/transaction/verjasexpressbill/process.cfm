...
<cfif type eq "TR">
 <cfquery name="getaddress" datasource="#dts#">
    SELECT addr1,addr2,addr3,addr4 FROM iclocation
    WHERE location = <cfqueryparam cfsqltype="cf_sql_varchar" value="#form.locationto#">
    </cfquery>
    <cfif getaddress.recordcount neq 0>
    <cfif comm0 eq "">
    <cfset comm0 = getaddress.addr1>
    <cfset comm1 = getaddress.addr2>
    <cfset comm2 = getaddress.addr3>
    <cfset comm3 = getaddress.addr4>
    </cfif>
	</cfif>
</cfif>
...
INSERT INTO artran
(type,refno,refno2,trancode,custno,fperiod,wos_date,desp,despa,agenno,source,job,currrate,gross_bil,disc1_bil,disc2_bil,disc3_bil,disc_bil,net_bil,tax1_bil,tax_bil,grand_bil,debit_bil,credit_bil,invgross,disp1,disp2,disp3,discount1,discount2,discount3,discount,net,tax1,tax,taxp1,grand,debitamt,creditamt,frem0,frem1,frem2,frem3,frem4,frem5,frem6,frem7,frem8,rem0,rem1,rem2,rem3,rem4,rem5,rem6,rem7,rem8,rem9,rem12,rem13,rem14,comm0,comm1,comm2,comm3,comm4,taxincl,note,created_by,created_on,rem10,PONO,DONO,userid,name,trdatetime,term)
values
(
...
<cfquery datasource="#dts#" name="getGeneralInfo">
    ...
    <cfform name="form1" id="form1" action="/default/transaction/verjasdone.cfm" method="post">
