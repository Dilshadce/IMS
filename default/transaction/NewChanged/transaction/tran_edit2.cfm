...
<cfform name="#formname#" id="#formid#" action="transaction3.cfm" method="post" onsubmit="return checkcurrency();">
	...
	<tr>
		<th <cfif getdisplaysetup2.customerno neq "Y">style="visibility:hidden"</cfif>>#iif(tran eq 'RC' or tran eq 'PR' or tran eq 'PO',DE('Supplier'),DE('Customer'))# No</th>
 ...
 <cfif lcase(hcomid) neq "ascend_i">
	...
	<cfelse>
     ...
     select wsq FROM #target_project# where source="#getartran.source#"
      ...
      <tr>
      <th>Invoice No</th><td>#getartran.toinv#</td>
      </tr>
      </cfif>
    ...
    <td></td>
    <th <cfif permitnodisplay neq "Y">style="visibility:hidden"</cfif>><cfif lcase(hcomid) eq "fdipx_i">Invoice No.<cfelseif lcase(hcomid) eq "mastercare_i" or lcase(hcomid) eq "gorgeous_i">Factory<cfelseif lcase(hcomid) eq "sdc_i" or lcase(hcomid) eq "sdab_i">Validity<cfelseif lcase(hcomid) eq "apnt_i">Inv No<cfelseif lcase(hcomid) eq "atc2005_i">Tag<cfelseif (lcase(hcomid) eq "weikeninv_i" or lcase(hcomid) eq "weikenint_i" or lcase(hcomid) eq "weikenid_i" or lcase(hcomid) eq "futurehome_i" or lcase(hcomid) eq "weikenbuilder_i" or lcase(hcomid) eq "weikendecor_i")>Status<cfelse>Permit No</cfif></th>
	...
	<tr>
		<th height="20" <cfif podisplay neq "Y">style="visibility:hidden"</cfif>><cfif lcase(hcomid) eq "viva_i" or lcase(hcomid) eq "kjcpl_i" or lcase(hcomid) eq "mlpl_i">#gettranname.bodypo#<cfelseif lcase(hcomid) eq "talerfood_i">Customer PO<cfelse>#gettranname.bodypo#</cfif></th>
...
<cfinput type="text" name="remark11" id="remark11" value="#remark11#" bind="cfc:accordtran2.getremark11('#dts#',{remark5})"></cfoutput></td>
		...
		<cfelseif lcase(HcomID) neq "avt_i">
			<th <cfif rem11display neq "Y">style="visibility:hidden"</cfif>>#getGsetup.rem11#</th>
			<td <cfif rem11display neq "Y">style="visibility:hidden"</cfif>><input type="text" name="remark11" value="#remark11#" size="40" maxlength="35"></td>
 ...
 <cfif getgsetup.multiagent eq 'Y'>
	...
	<tr>
        ...
        <cfelse>
            ...
            <cfif lcase(hcomid) eq "mastercare_i" or lcase(hcomid)eq "gorgeous_i">
            <input name="main" type="button" value="Update Customer" onClick="location.href='tran_edit1.cfm?tran=#tran#&ttype=Edit&refno=#refno#&custno=#URLEncodedFormat(custno)#&first=0'">
            <cfelseif tran eq "RC" or tran eq "PO" or tran eq "PR" or tran eq "RQ"> 
			<input name="main" type="button" value="Change Supplier" onClick="location.href='tran_edit1.cfm?tran=#tran#&ttype=Edit&refno=#refno#&custno=#URLEncodedFormat(custno)#&first=0'">
            <cfelse>
            <input name="main" type="button" value="Change Customer" onClick="location.href='tran_edit1.cfm?tran=#tran#&ttype=Edit&refno=#refno#&custno=#URLEncodedFormat(custno)#&first=0'">
            </cfif>
			<!--- ADD ON 10-12-2009 --->
