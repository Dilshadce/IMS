...
<cfquery datasource="#dts#" name="getlocation">
	...
	from iclocation 
    where 0=0
    <cfif (lcase(hcomid) eq "kjcpl_i" or lcase(hcomid) eq "mlpl_i" or lcase(hcomid) eq "viva_i" or lcase(hcomid) eq "kjctrial_i" or lcase(hcomid) eq "tcds_i") and (HUserGrpID neq 'Cust Svr' and HUserGrpID neq 'Van Sales')>
    <cfelse>
...
<!--- 		if(nextflow != "")
	...
	if(answer)
	{
	var onholdurl = '/default/transaction/expresstransfer/onholdajax.cfm?uuid='+document.getElementById("uuid").value;
	...
	window.open('timemanchine.cfm?consignment=#consignment#&tran=#type#&uuid='+escape(document.getElementById("uuid").value), '',opt);
	});
	</cfoutput>
	...
	{
	<cfoutput>
	window.location.href="index.cfm?type=#tran#&consignment=#consignment#";
	</cfoutput>
	}
	}
	...
	if(answer)
	{
	var onholdurl = '/default/transaction/expresstransfer/onholdajax.cfm?uuid='+document.getElementById("uuid").value+'&remark='+document.getElementById("rem9").value;
	ajaxFunction(document.getElementById('onholdajax'),onholdurl);
	...
	window.location.href="index.cfm?type=#tran#&consignment=#consignment#&uuid="+newuuid;
	}
	}
	var t1;
	var t2;
...
<cfoutput>
	var itemdespurl = '/default/transaction/expresstransfer/itemdespprocess.cfm?uuid=#URLEncodedFormat(uuid)#&trancode='+escape(itemdesptrancode)+'&itemdesp='+escape(itemdesp)+'&itemdespa='+escape(itemdespa)+'&itemcomment='+escape(itemcomment);
	...
	var consignment = trim(document.getElementById('consignment').value);
	

	var ajaxurl = '/default/transaction/expresstransfer/addproductsAjax.cfm?servicecode='+escape(expressservice)+'&desp='+escape(desp)+'&expressamt='+escape(expressamt)+'&expqty='+escape(expqty)+'&expprice='+escape(expprice)+'&unit='+escape(expunit)+'&dispec1='+escape(expunitdis1)+'&dispec2='+escape(expunitdis2)+'&dispec3='+escape(expunitdis3)+'&dis='+escape(expdis)+'&tran='+escape(tran)+'&tranno='+refno+'&uuid=#URLEncodedFormat(uuid)#&driver='+escape(driver)+'&isservi='+escape(isservi)+'&trancode='+escape(trancode)+'&glacc='+escape(glacc)+'&locationfr='+escape(locationfr)+'&locationto='+escape(locationto)+'&custno='+escape(custno)+'&rem9='+escape(rem9)+'&consignment='+escape(consignment);
	...
	var ajaxurl2 = '/default/transaction/expresstransfer/addmultiproductsAjax.cfm?tran='+escape(tran)+'&tranno='+refno+'&uuid=#URLEncodedFormat(uuid)#&trancode='+escape(trancode)+'&custno='+escape(custno)+'&locationfr='+escape(locationfr)+'&locationto='+escape(locationto)+'&consignment='+escape(consignment)+'&itemlisting='+escape(itemlisting);
	...
	{
    var urlloaditemdetail = '/default/transaction/expresstransfer/addItemAjax.cfm?itemno='+escape(encodeURI(detailitemno))+'&reftype='+document.getElementById('tran').value+'&custno='+document.getElementById('custno').value;
	...
	{
	  var urlloaditembal = '/default/transaction/expresstransfer/balonhand.cfm?itemno='+encodeURI(itemnobal)+'&location='+escape(document.getElementById('locationfr').value);
	...
	{
	<cfoutput>
    var urlload = '/default/transaction/expresstransfer/recalculateAjax.cfm?uuid=#URLEncodedFormat(uuid)#';
	</cfoutput>
	...
	{
	var ajaxurl = '/default/transaction/expresstransfer/recalculateAjax.cfm?uuid=#URLEncodedFormat(uuid)#';
	new Ajax.Request(ajaxurl,
      {
        method:'get',
        onSuccess: function(getdetailback){
	...
	{
		<cfoutput>
	var customerurl = '/default/transaction/expresstransfer/memberdetailajax.cfm?detail=1&type=#url.type#&member='+document.getElementById("custno").value;
...
<input type="hidden" name="consignment" id="consignment" value="#url.consignment#">
</cfoutput>

<cfoutput>
<table width="100%">
<tr>
<th width="20%">Date<cfif huserid eq 'ultralung'> #uuid#</cfif></th>
<cfset datenow=now()>
...
<cfinput type="text" name="refno" id="refno" required="yes" onKeyUp="nextIndex(event,'refno','wos_date');" bind="cfc:refnobill.getrefno({tran},'#dts#',{refnoset})" bindonload="yes" value="#nexttranno#" readonly>
<cfselect name="refnoset" id="refnoset" bind="cfc:refnobill.getrefnoset({tran},'#dts#','#consignment#')" bindonload="yes" display="lastno" value="counter" />
&nbsp;&nbsp;&nbsp;&nbsp;<b>#url.type#</b>
...
<th><div id="ajaxFieldPro" name="ajaxFieldPro" style="display:none"> </div><div id="ajaxFieldPro2" name="ajaxFieldPro2" style="display:none"> </div>Currency</th>
<td><!---<cfinput type="text" name="currcode" id="currcode" size="10"  /><input type="hidden" name="currcodehid" id="currcodehid" value="">&nbsp;<cfinput type="text" name="currrate" id="currrate" bind="cfc:custsupp.getcurrrate('#dts#',{currcode},'#target_currency#')" bindonload="yes" size="5" />--->
<cfinput type="hidden" name="currcode" id="currcode" size="10"  /><input type="hidden" name="currcodehid" id="currcodehid" value="">&nbsp;<cfinput type="hidden" name="currrate" id="currrate" value='1' size="5" />
</td>
</tr>
<tr>
...
<th width="8%" <cfif getdisplay.simple_packing neq 'Y'>style="display:none"</cfif>>Packing</th>
...
<td nowrap <cfif getdisplay.simple_location neq 'Y'>style="visibility:hidden"</cfif>>
...
<td nowrap align="right" <cfif getdisplay.simple_packing neq 'Y'>style="display:none"</cfif>>
...
<td nowrap align="right" <cfif getdisplay.simple_price neq 'Y'>style="visibility:hidden"</cfif>><a style="cursor:pointer" onClick="document.getElementById('hidtrancode').value='#getictrantemp.trancode#';ColdFusion.Window.show('changeprice');getfocus5();">#numberformat(val(getictrantemp.price_bil),',.__')#</a></td>
...
<cfquery name="getsumictrantemp" datasource="#dts#">
SELECT sum(qty_bil)as sumqty FROM ictrantemp WHERE uuid = <cfqueryparam cfsqltype="cf_sql_varchar" value="#uuid#"> and type='TROU' order by trancode desc
</cfquery>
...
<tr>
  ...
  <cfselect style="display:none" name="expunit" id="expunit"  onKeyUp="nextIndex(event,'expunit','expprice');" onChange="ajaxFunction(document.getElementById('ajaxfieldgetunitprice'),'/default/transaction/expresstransfer/getunitpriceAjax.cfm?itemno='+document.getElementById('expressservicelist').value+'&unit='+document.getElementById('expunit').value);setunitprice();"></cfselect>
...
<cfselect name="taxcode" id="taxcode" query="taxrate" value="code" display="code" onChange="setTimeout('caltax()',500);" bindonload="yes"/>
  ...
  <input type="checkbox" name="taxincl" id="taxincl" value="T" onClick="caltax()" />&nbsp;
  <cfquery name="getTaxCode" datasource="#dts#">
  SELECT "" as code, "" as rate1
  union all
  SELECT code,rate1 FROM #target_taxtable#
...
<input type="hidden" name="taxcode" id="taxcode" value="" display="code"/>
<cfinput type="hidden" name="taxper" id="taxper" value="0" size="8"/>&nbsp;&nbsp;&nbsp;
<cfinput type="hidden" name="taxamt" id="taxamt" value="0.00" size="10" />
</td>
</tr>
...
<cfwindow center="true" width="900" height="700" name="searchitem" refreshOnShow="true" closable="true" modal="false" title="Search Item" initshow="false"
        source="/default/transaction/expresstransfer/searchitem2.cfm?reftype={tran}" />
<cfelse>
<cfwindow center="true" width="1000" height="700" name="searchitem" refreshOnShow="true" closable="true" modal="false" title="Search Item" initshow="false"
        source="/default/transaction/expresstransfer/searchitem.cfm?reftype={tran}" />
</cfif>
...
<cfwindow center="true" width="600" height="400" name="changedaddr" refreshOnShow="true" closable="true" modal="false" title="Search Address" initshow="false"
        source="/default/transaction/expresstransfer/searchaddress.cfm" /> 
