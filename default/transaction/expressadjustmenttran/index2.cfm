...
<link href="/SpryAssets/SpryCollapsiblePanel.css" rel="stylesheet" type="text/css" />
	...
	<cfset uuid = url.uuid>
	<cfoutput>
	<cfquery name='getadjtrantemp' datasource='#dts#'>
	select date,period from locadjtran_temp where uuid='#url.uuid#'
	</cfquery>
	</cfoutput>
	</cfif>
	...
	if (thisid == 'expressservicelist')
	{
		if(e.keyCode==13){
		document.getElementById('qtyactual').focus();
	}
	}
	...
	if(answer)
	{
	var onholdurl = 'onholdajax.cfm?uuid='+document.getElementById("uuid").value;
	ajaxFunction(document.getElementById('onholdajax'),onholdurl);
	releaseDirtyFlag();
	window.location.href="index2.cfm?type=Create";
	...
	if (itemno == "" || document.getElementById('btn_add').value == "Item No Existed")
	{
...
<cfquery datasource='#dts#' name="getadjtrantemp">
	Select * from locadjtran_temp where uuid="#url.uuid#"
</cfquery>

<cfset xdate=dateformat(getadjtrantemp.date,'DD/MM/YYYY')>
<cfset nowdate = dateformat(getadjtrantemp.date,"dd/mm/yyyy")>
<cfset perioddate = getadjtrantemp.date>
...
<cfinput type="text" name="expressservicelist" id="expressservicelist" size="26" onBlur="updateitemdetail(escape(encodeURI(this.value)));" onKeyUp="nextIndex(event,'expressservicelist','expqty');" />
<cfelse>
<cfinput type="text" name="expressservicelist" id="expressservicelist" size="26" onBlur="this.value = this.value.split('___', 1);updateitemdetail(escape(encodeURI(this.value)));" autosuggest="cfc:itemno.findItem({cfautosuggestvalue},'#dts#')" autosuggestminlength="1" />
</cfif>

<input type="button" onClick="ColdFusion.Window.show('searchitem');" value="Search" align="right" />
...
<tr <cfif type neq 'Create'>style="display:none"</cfif>>
 <th><div id="itemDetail"></div>Qty On Hand</th>
  ...
  <th>Actual Qty</th>
  <td><input type="text"  name="qtyactual" id="qtyactual" maxlength="10" value="" onKeyUp="document.getElementById('qtydiff').value=document.getElementById('qtyonhand').value-document.getElementById('qtyactual').value;nextIndex(event,'qtyactual','btn_add');"></td>
...
<cfif type eq 'Create'>
<cfgrid name="itemlist" pagesize="7" format="html" width="1000" height="400"
bind="cfc:itemlist.getictran({cfgridpage},{cfgridpagesize},{cfgridsortcolumn},{cfgridsortdirection},'#dts#','#refno#','#uuid#','#type#',{pagesize})"
...
<tr>
  <td colspan="100%" align="center"><input type="button" style="font: medium bolder" name="preview" id="preview" value="Preview" onClick="window.open('l_itemlist.cfm?refno=#refno#')">
  <div <cfif type eq 'Edit' >style="display:none"</cfif>><cfinput type="button" style="font: medium bolder" name="Save" id="Save" value="Save" onClick="ctrl1();"/>&nbsp;&nbsp;&nbsp;&nbsp;<cfinput type="button" style="font: medium bolder" name="Load" id="Load" value="Load" onClick="ctrl7();"/></div>
  <br>
  <br>
  <cfinput type="button" style="font: large bolder" name="Submit" id="Submit" value="#url.type#" onClick="releaseDirtyFlag();form1.submit();"/> &nbsp;&nbsp;&nbsp;<cfinput type="button" style="font: large bolder" name="Close" id="Close" value="Close" onClick="window.close();"/></div></td>
...
<cfwindow center="true" width="1200" height="600" name="searchitem" refreshOnShow="true" closable="true" modal="false" title="Search Item" initshow="false"
