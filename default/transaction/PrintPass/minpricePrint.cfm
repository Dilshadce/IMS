...
select printstatus,created_by,grand,grand_bil,currcode,custno,name,created_on from artran where type='#url.type#' and refno='#url.refno#'
</cfquery>
...
#url.type# No : #url.refno#
Total Amount:#getbillstatus.currcode# #getbillstatus.grand_bil#

To Approve Click click on link below.

http://crm.netiquette.com.sg/minpricebillapprove.cfm?comid=#URLENCODEDFORMAT(dts)#&billtype=#URLENCODEDFORMAT(url.type)#&refno=#URLENCODEDFORMAT(url.refno)#&code=#hash(unlockcode)#

In case Above Link Does Not work Click Below Link

http://crm.mynetiquette.com/minpricebillapprove.cfm?comid=#URLENCODEDFORMAT(dts)#&billtype=#URLENCODEDFORMAT(url.type)#&refno=#URLENCODEDFORMAT(url.refno)#&code=#hash(unlockcode)#
...
<cfelse> 
<div align="center">
<cfform name="form" action="/default/transaction/printpass/minpricePrintProcess.cfm?type=#url.type#&refno=#url.refno#" method="post">
<cfset task = "">
<cfset task = "Printing">
...
<input type="button" name="submit_btn" value="Close" onclick="javascript:window.location='/default/transaction/transaction.cfm?tran=#url.type#';"  />
 </cfform>
</div>
</cfif>
</cfoutput>