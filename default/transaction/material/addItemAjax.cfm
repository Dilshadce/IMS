...
SELECT desp,unit,unit2,unit3,unit4,unit5,unit6,price,ucost,price2,price3,price4 from icitem where itemno = <cfqueryparam cfsqltype="cf_sql_varchar" value="#trim(trim(urldecode(url.itemno)))#">
<cfif Hitemgroup neq ''>
...
<cfif getItemDetails.recordcount neq 1 and trim(urldecode(url.itemno)) neq "">
...
select * from icl3p<cfif url.tran eq 'PO' or url.tran eq 'RC' or url.tran eq 'PR'><cfelse>2</cfif> where itemno='#trim(urldecode(url.itemno))#' and custno='#url.custno#'
</cfquery>
