...
<cfif isdefined('url.currency')>
    <cfif url.currency eq 'Y'>
        <cfquery datasource='#dts#' name="unlocktransaction">
            update artran
            set posted='T'
            where refno='#refno#' and type = "#tran#"
        </cfquery>
    </cfif>
</cfif>
...
<cfif getGsetup.alloweditposted neq 'Y'>
	<cfif getartran.posted eq "P">
        ...
        <!---unlock posted invoice for currency, added by Nieo 20171019 0945--->
        <cfoutput>
        <br>
        <h4>To <span style="color: red">update</span> currency rate/ PO NO/ Invoice Remarks, click <a href="tran_edit2.cfm?tran=#tran#&ttype=Edit&refno=#URLEncodedFormat(refno)#&custno=#URLEncodedFormat(custno)#&first=0&currency=Y">here</a>.</h4>        
        <h4>To <span style="color: red">return</span> to previous page, click <a href="transaction.cfm?tran=#tran#" target="_self" >here</a>.</h4>
        </cfoutput>
        <!---unlock posted invoice for currency, added by Nieo 20171019 0945--->
        <cfabort>
    </cfif>
...
<cfform name="#formname#" id="#formid#" action="transaction3.cfm" method="post" onsubmit="return checkcurrency();">
  	...
  	<cfoutput>
    <cfif isdefined('url.currency')>
        <input type="hidden" name="currency" id="currency" value="#url.currency#">
    </cfif>
 ...
 <cfif getgsetup.multiagent eq 'Y'>
	...
	<tr>
        ...
        <cfelse>
            ...
            <cfelseif isdefined('url.currency') neq true>
            <input name="main" type="button" value="#words[1825]#" onClick="location.href='tran_edit1.cfm?tran=#tran#&ttype=Edit&refno=#refno#&custno=#URLEncodedFormat(custno)#&first=0'">
            </cfif>
			<!--- ADD ON 10-12-2009 --->
