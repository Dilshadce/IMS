<cfif isdefined('form.asnrefno')>
    ...
    WHERE refno in (<cfqueryparam cfsqltype="cf_sql_varchar" value="#form.asnrefno#" list="yes" separator=",">)
    GROUP BY branch
    </cfquery>
        
    <cfif checkentity.recordcount gt 1>
        <script>
        alert('Mix Entity are selected. It is only allowed to batch assignmentslip with same Entity.');
        window.close();
        </script>
        <cfabort>
    </cfif>
        
    <!---Added by Nieo 20180523, to control batching for same Clients requested by Nabila--->
    <cfquery name="checkcustname" datasource="#dts#">
    SELECT 
        case when length(SUBSTRING_INDEX(custname, ' ', 1)) =1 or SUBSTRING_INDEX(custname, ' ', 1) ="the" 
        then SUBSTRING_INDEX(custname, ' ', 2) 
        else SUBSTRING_INDEX(custname, ' ', 1)
        end as custname 
    FROM assignmentslip
    WHERE refno in (<cfqueryparam cfsqltype="cf_sql_varchar" value="#form.asnrefno#" list="yes" separator=",">)
    GROUP BY 
    ...
    <cfif checkcustname.recordcount gt 1>
        <script>
        alert('Mix Clients are selected. It is only allowed to batch assignmentslip with same Client.');
        window.close();
        </script>
        <cfabort>
    </cfif>  
...
SELECT paydate,empno,assignmentslipdate,placementno FROM assignmentslip WHERE refno in (<cfqueryparam cfsqltype="cf_sql_varchar" value="#form.asnrefno#" list="yes" separator=",">)
</cfquery>
...
and placementno = <cfqueryparam cfsqltype="cf_sql_varchar" value="#checkassignnew.placementno#">
and day(assignmentslipdate) = "#dateformat(checkassignnew.assignmentslipdate,'d')#"
