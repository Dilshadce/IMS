...
<cfquery name="getitemnoictrantemp" datasource="#dts#">
SELECT itemno FROM ictrantemp 
WHERE 
...
<cfquery name="checkpromotion" datasource="#dts#">
    SELECT * FROM promoitem as a right join promotion as b on a.promoid = b.promoid WHERE a.itemno = <cfqueryparam cfsqltype="cf_sql_varchar" value="#getitemnoictrantemp.itemno#"> and b.periodfrom <="#dateformat(now(),'yyyy-mm-dd')#" and b.periodto >= "#dateformat(now(),'yyyy-mm-dd')#" and (b.customer='') and (b.location like <cfqueryparam cfsqltype="cf_sql_varchar" value="%#huserloc#%"> or b.location='' or b.location is null)  order by rangefrom
</cfquery>
