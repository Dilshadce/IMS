...
<cfquery name="getItemDetails" datasource="#dts#">
SELECT itemno,desp,despa,unit,unit2,unit3,unit4,unit5,unit6,price,price2,price3,price4,ucost,costformula,"0" as isservi from icitem where aitemno = <cfqueryparam cfsqltype="cf_sql_varchar" value="#trim(url.itemno)#"> or barcode = <cfqueryparam cfsqltype="cf_sql_varchar" value="#trim(url.itemno)#">
</cfquery>
...
<cfif checkenable.itempriceprior eq "1">
    <cfquery name="getcustomerprice" datasource="#dts#">
        select * 
        from icl3p<cfif url.reftype eq 'PO' or url.reftype eq 'RC' or url.reftype eq 'PR'><cfelse>2</cfif> 
...
<cfif lcase(hcomid) eq "nil_i" or lcase(hcomid) eq "mika_i" or lcase(hcomid) eq "posdemo_i">
<cfif trim(url.member) neq "">
<cfset getItemDetails.price = getItemDetails.price2>
</cfif>
</cfif>
