...
<cfif text neq "">
<cfif lcase(HcomID) eq "kjcpl_i" or lcase(HcomID) eq "mlpl_i" or lcase(HcomID) eq "viva_i" or lcase(HcomID) eq "maven_i" or lcase(HcomID) eq "uniq_i" or lcase(HcomID) eq "unqhk_i">
...
<cfquery name="getitem" datasource="#dts#">
		...
		<cfif searchtype neq "" and text neq "">
            ...
            <cfif supp contains "4000">
            	and (upper(supp) = '#ucase(supp)#' or upper(supp) = '')
            </cfif>
		</cfif>
        <cfif Hitemgroup neq ''>
            and wos_group='#Hitemgroup#'
