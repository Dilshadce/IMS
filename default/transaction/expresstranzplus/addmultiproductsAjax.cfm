...
SELECT itemno FROM expresspickitem WHERE uuid = <cfqueryparam cfsqltype="cf_sql_varchar" value="#URLDECODE(url.itemlisting)#"> ORDER BY CREATED_ON
</cfquery>
...
<cfquery name="getitemdetail" datasource="#dts#">
    select '' as wos_group,'' as category,'' as despa,desp,'' as barcode,'' as comment from icservi
    where servi = <cfqueryparam cfsqltype="cf_sql_varchar" value="#itemno#">
    union all
    select 
    wos_group,category,despa,desp,barcode,comment
    from icitem
