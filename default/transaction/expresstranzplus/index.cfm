...
<link href="/SpryAssets/SpryCollapsiblePanel.css" rel="stylesheet" type="text/css" />
	...
	function UpdateTotalQty(){
			...
			var totqty = 0;
			for(i=1;i<=(document.getElementById('multilocationtotallocation').value*1);i++){
				if(document.getElementById("multilocation_qty_"+i).value == ''){
					document.getElementById("multilocation_qty_"+i).value = 0;
				}
				
				totqty = totqty + parseFloat(document.getElementById("multilocation_qty_"+i).value);
				
			}
			document.getElementById('multilocation_totqty').value = totqty;
		}
	...
	var ajaxurl2 = '/default/transaction/expresstranzplus/addpackageprocessAjax.cfm?tran='+escape(tran)+'&tranno='+refno+'&uuid='+document.getElementById('uuid').value+'&trancode='+escape(trancode)+'&custno='+escape(custno)+'&packagecode='+escape(packagecode)+'&location='+escape(location);
...
function selectmemberlist(custno){	
  ...
  for(var i=0;i<output.length;i++) {
	...
	function updaterow(rowno)
	{
		...
		var unitlistdata = document.getElementById(uom).value;
		
		var brem4data = document.getElementById(brem4).value;
		var brem1data = document.getElementById(brem1).value;
		
		<cfif url.type eq 'QUO'>
		var updateurl = 'updaterow.cfm?uuid='+escape(uuid)+'&trancode='+rowno+'&qty='+escape(qtylistdata)+'&brem4='+escape(brem4data)+'&brem1='+escape(brem1data)+'&job='+escape(joblistdata)+'&unit='+escape(unitlistdata)+'&promotiontype='+escape(promotiontypedata);
		<cfelse>
		var updateurl = 'updaterowgroup.cfm?uuid='+escape(uuid)+'&trancode='+rowno+'&qty='+escape(qtylistdata)+'&brem4='+escape(brem4data)+'&brem1='+escape(brem1data)+'&job='+escape(joblistdata)+'&unit='+escape(unitlistdata)+'&promotiontype='+escape(promotiontypedata);
		</cfif>
...
<!--- 		if(nextflow != "")
	...
	if(answer)
	{
	var onholdurl = '/default/transaction/expresstranzplus/onholdajax.cfm?uuid='+document.getElementById("uuid").value;
	...
	if(answer)
	{
	var onholdurl = '/default/transaction/expresstranzplus/onholdajax.cfm?uuid='+document.getElementById("uuid").value+'&remark='+document.getElementById("rem9").value;
	ajaxFunction(document.getElementById('onholdajax'),onholdurl);
	...
	{
		var ajaxurl = '/default/transaction/expresstranzplus/getunitpriceAjax.cfm?itemno='+document.getElementById('expressservicelist').value+'&unit='+document.getElementById('expunit').value;
...
<cfoutput>
	var itemdespurl = '/default/transaction/expresstranzplus/itemdespprocess.cfm?uuid=#URLEncodedFormat(uuid)#&trancode='+escape(itemdesptrancode)+'&itemdesp='+escape(itemdesp)+'&itemdespa='+escape(itemdespa)+'&itemcomment='+escape(itemcomment)+'&glaccno='+escape(glaccno);
...
<!--- 	for (k=1;k<=200;k=k+1)
	...
	var ajaxurl2 = '/default/transaction/expresstranzplus/addmultiproductsAjax.cfm?tran='+escape(tran)+'&tranno='+refno+'&uuid=#URLEncodedFormat(uuid)#&trancode='+escape(trancode)+'&custno='+escape(custno)+'&location='+escape(location)+'&itemlisting='+escape(itemlisting);
	...
	{
    var urlloaditemdetail = '/default/transaction/expresstranzplus/addItemAjax.cfm?itemno='+escape(encodeURI(detailitemno))+'&reftype='+document.getElementById('tran').value+'&custno='+document.getElementById('custno').value;
	...
	{
	  var urlloaditembal = '/default/transaction/expresstranzplus/balonhand.cfm?itemno='+encodeURI(itemnobal)+'&location='+escape(document.getElementById('locationfr').value);
	...
	{
	<cfoutput>
    var urlload = '/default/transaction/expresstranzplus/recalculateAjax.cfm?uuid=#URLEncodedFormat(uuid)#';
	</cfoutput>
	...
	{
	var ajaxurl = '/default/transaction/expresstranzplus/recalculateAjax.cfm?uuid=#URLEncodedFormat(uuid)#';
	new Ajax.Request(ajaxurl,
      {
        method:'get',
        onSuccess: function(getdetailback){
	...
	{
		<cfoutput>
	var customerurl = '/default/transaction/expresstranzplus/memberdetailajax.cfm?detail=1&type=#url.type#&member='+document.getElementById("custno").value;
...
updatecustomerdetail();
	...
	var outstandingurl = '/default/transaction/expresstranzplus/outstandinglimitajax.cfm?custno='+document.getElementById("custno").value;
...
<cfif (lcase(hcomid) neq "bnbm_i" and lcase(hcomid) neq "bnbp_i")>
<cfif url.type eq 'PR' or  url.type eq 'RC' or url.type eq 'PO'>
<input type="button" id="cnc" onClick="javascript:ColdFusion.Window.show('createSupplier');" value="New" align="right" />

<cfelse>
<input type="button" id="cnc" onClick="javascript:ColdFusion.Window.show('createCustomer');" value="New" align="right" />
...
</th><td <cfif getmodule.auto eq 1>style="display:none" </cfif>><input type="text" name="d_add3" id="d_add3" value="" maxlength="35" size="40"></td>
</tr>
<tr>
<th></th>
<td>
...
<cfif getdisplay.simple_qty eq 'Y'><td>
<cfif type eq 'QUO'>
...
&nbsp;<cfif (lcase(hcomid) neq "bnbm_i" and lcase(hcomid) neq "bnbp_i")><cfif getpin2.h1310 eq 'T'>
<input type="button" id="cni" onClick="window.open('/default/maintenance/icitem2.cfm?type=Create&express=1');" value="New" align="right" />
</cfif></cfif>
<br>
...
<div id="itembal" style="display:none"></div></td>
</tr>
...
--->
<td align="center" valign="middle" style="font:'Times New Roman', Times, serif; font-size:16px" bgcolor="##CCFF33" onMouseOver="this.style.cursor='hand'" style="display:none"><a onClick="ctrl7();" style="cursor:pointer">Time Machine<br><font size="1">ctrl+8</a></font></td>
</tr>
<tr>
<td colspan="3">
...
<cfwindow center="true" width="1300" height="500" name="searchitem" refreshOnShow="true" closable="true" modal="false" title="Search Item" initshow="false"
        source="/default/transaction/expresstranzplus/searchitem2.cfm?reftype={tran}&custno={custno}" />
<cfelse>
<cfwindow center="true" width="1300" height="500" name="searchitem" refreshOnShow="true" closable="true" modal="false" title="Search Item" initshow="false"
        source="/default/transaction/expresstranzplus/searchitem.cfm?reftype={tran}&custno={custno}" />
</cfif>
...
<cfwindow center="true" width="600" height="400" name="changedaddr" refreshOnShow="true" closable="true" modal="false" title="Search Address" initshow="false"
        source="/default/transaction/expresstranzplus/searchaddress.cfm" /> 
...
<cfwindow center="true" width="400" height="600" name="multilocationwindow" refreshOnShow="true" closable="true" modal="true" title="Edit Price" initshow="false" source="multilocation.cfm?uuid=#uuid#&trancode={hidtrancode}" />  
...
<cfwindow center="true" width="580" height="400" name="findattention2" refreshOnShow="true"
        ...
        source="/default/transaction/expresstranzplus/addpackage.cfm?reftype={tran}" />
...
<cfif tran eq 'QUO' and getgsetup.quotationlead eq 'Y'>
        ...
        source="/default/transaction/expresstranzplus/findlead.cfm" />
        </cfif>
 <cfif isdefined('url.uuid')>
 <script type="text/javascript">
 recalculateamt();
