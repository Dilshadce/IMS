<cfcomponent>
	<cffunction name="getlist" access="remote" returntype="query">
    	...
    	<cfelse>
            ...
            <cfelseif dts eq 'acht_i' and tran eq 'CS'>    
            	Select concat(custno,'--',name) as custname,custno from #target_arcust# where custno='#getgsetup.df_cs_cust#' 
                union all
        		Select concat(custno,'--',name) as custname,custno from #target_arcust# where custno<>'#getgsetup.df_cs_cust#' 
            <cfelse>    
    ...
    <cffunction name="getagent" access="remote" returntype="query">
		...
		<cfif Hlinkams eq 'Y'>
            ...
            SELECT "Choose an agent" as agentdesp, "" as agent
            union all
    ...
    <cffunction name="getterms" access="remote" returntype="query">
        ...
        <cfargument name="target_icterm" type="string">
            ...
            SELECT "Choose a terms" as termdesp, "" as term
            union all 
