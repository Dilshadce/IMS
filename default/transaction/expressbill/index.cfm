...
<cfset words_id_list = "1088,666,668,673,185,690,665,664,188,689,667,1070,1087,959,963,953,702,29,23,67,506,857,954,475,1286,1287,9,795,892,1288,793,440,1289,65,1290,51,300,11,127,1291,227,492,1096,592,1097,1116,1292,1293,202,482,1082,58,120,1098,1294,1295,1099,1296,1297,1298,1104,960">
<cfinclude template="/latest/words.cfm">
<cfajaximport tags="cfform">
<cfsetting showdebugoutput="no">
<cfquery name="getgsetup" datasource="#dts#">
    SELECT filterall,ECAOTA,ECAMTOTA,negstk,PCBLTC,ddlbilltype,expressdisc,displaycostcode,ldriver,autonextdate,ddllocation ,rem5,rem6,rem11,ngstcustdisabletax,df_cs_cust 
    FROM gsetup
</cfquery>
...
<head>
	...
	setTimeout("document.getElementById('expprice').value=document.getElementById('priceforunit').value;",500) --->
		...
		function addItemAdvance() {
			...
			var expqty = trim(document.getElementById('expqty').value);
			var expprice = trim(document.getElementById('expprice').value);
			var expcomment1 = convertToEntities(document.getElementById('expcomment').value);
		...
		function calculatefooter() {
			<cfif lcase(HcomID) eq "simplysiti_i">	
				if (document.getElementById('itemtransferlimit').value == 1) {
					...
					clearformadvance();
					document.getElementById('allowtransferlimit').value=0;
				}
			</cfif>
...
<body <cfif lcase(hcomid) eq 'acht_i'>onLoad="document.getElementById('tran').focus();updateDetails(this.value);"<cfelse> onLoad="document.getElementById('tran').focus()"</cfif>>
	<cfoutput>
    ...
    <cfform name="invoicesheet" id="invoicesheet" action="process.cfm" method="post">
        <div id="CollapsiblePanel1" class="CollapsiblePanel">
            ...
            <div class="CollapsiblePanelContent">
				...
				<cfif lcase(HcomID) eq "simplysiti_i">
                	<input type="hidden" name="allowtransferlimit" id="allowtransferlimit" value="0">
                </cfif>
                <table width="100%"  >
                    <tr>
                        ...
                        <cfselect name="refnoset" id="refnoset" bind="cfc:refnobill.getrefnoset({tran},'#dts#')" bindonload="yes" display="lastno" value="counter" /></td>
                        <td width="5%"></td>
                    ...
                    <tr>
                        ...
                        <cfif getGsetup.filterall eq "1">
							<cfif lcase(hcomid) eq 'acht_i'>
                                <cfselect name="custno" id="custno" bind="cfc:custsupp.getlist({tran},'#target_arcust#','#target_apvend#','#dts#',{zcustno})" bindonload="yes" required="yes" selected="#getgsetup.df_cs_cust#" display="custname" value="custno" onChange="updateDetails(this.value);" onKeyUp="nextIndex(event,'custno','wos_date');" message="Customer/Supplier is Required" />
                                ...
                                <input type="button" name="Scust1"  id="Scust1" value="#words[953]#" onClick="javascript:ColdFusion.Window.show('achtfindCustomer');getfocus();" >
                            <cfelse>
								...
								<cfif getdealermenu.custformat eq '2'>
                                    <input type="button" name="Scust1"  id="Scust1" value="#words[953]#" onClick="javascript:ColdFusion.Window.show('findCustomer');getfocus4();" >
                                <cfelse>
                                    <input type="button" name="Scust1"  id="Scust1" value="#words[953]#" onClick="javascript:ColdFusion.Window.show('findCustomer');getfocus();" >
                                </cfif>
                            </cfif>
                        <cfelse>
                        ...
                        <td>
						<cfif hcomid eq "hairo_i"  or hcomid eq "freshways_i">
                            <input type="text" name="wos_date" id="wos_date" value="#dateformat(datenow,'DD/MM/YYYY')#" onKeyUp="nextIndex(event,'wos_date','searchitembtn');" />
                        <cfelse>
                            <input type="text" name="wos_date" id="wos_date" value="#dateformat(datenow,'DD/MM/YYYY')#" onKeyUp="nextIndex(event,'wos_date','expressservicelist');" />
                        </cfif>
                        ...
                        <cfelseif hcomid eq "hyray_i">
                            <cfselect name="agent" id="agent" bind="cfc:custsupp.getagent('#dts#','#Hlinkams#')" bindonload="yes" display="agentdesp" value="agent" onChange="selectOptionByValue(document.getElementById('agent'),document.getElementById('agenthid').value);" /><input type="hidden" name="agenthid" id="agenthid" value="">
                        <cfelse>
                            <cfselect name="agent" id="agent" bind="cfc:custsupp.getagent('#dts#','#Hlinkams#')" bindonload="yes" display="agentdesp" value="agent" /><input type="hidden" name="agenthid" id="agenthid" value="">
                        </cfif>
                        </td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                        ...
                        <th <cfif getmodule.job neq "1">style="visibility:hidden"</cfif>>#words[475]#</th>
                        <td <cfif getmodule.job neq "1">style="visibility:hidden"</cfif>><cfselect name="job" id="job" bind="cfc:custsupp.getjob('#dts#','#Hlinkams#')" bindonload="yes" display="jobdesp" value="source" /></td>
                    </tr>
                    <tr>
                        <td>
                        ...
                        <td></td>
                        <td><cfinput type="button" name="changedaddr" id="changedaddr" onClick="ColdFusion.Window.show('changedaddr');" value="#words[1287]#"></td>
					...
					<cfif lcase(hcomid) eq "kjcpl_i" or lcase(hcomid) eq "kjctrial_i" or lcase(hcomid) eq "mlpl_i" or lcase(hcomid) eq "viva_i">
                        ...
                        <tr>
                            ...
                            <td></td>
                            <th>#getgsetup.rem6#</th><td><input type="text" name="rem6" id="rem6" value="" size="40" maxlength="35"></td>
                            <td></td>
            ...
            <div class="CollapsiblePanelContent">
                <table width="100%" border="0">
                    <tr>
                        ...
                        <cfelse>
                            <cfinput type="text" name="expressservicelist" id="expressservicelist" size="26" onBlur="this.value = this.value.split('___', 1);getitemdetail(this.value);" onKeyUp="nextIndex(event,'expressservicelist','desp2');" autosuggest="cfc:itemno.findItem({cfautosuggestvalue},'#dts#')" autosuggestminlength="1"/>
                        </cfif>
                    ...
                    <tr>
                        ...
                        <input <cfif hcomid eq "hairo_i" or hcomid eq "freshways_i">style="visibility:hidden"</cfif> type="<cfif getpin2.h1360 eq 'T'>text<cfelse>hidden</cfif>" name="expdis" id="expdis" size="10" maxlength="10" value="0.00" onKeyUp="calamtadvance();nextIndex(event,'expdis','btn_add')" onBlur="calamtadvance();">
                        </td>
                        <td>&nbsp;</td>
                        <th>#words[1097]#</th>
                        <td>
                        ...
                        <input name="btn_add" style="font: medium bolder" id="btn_add" type="button" value="#words[1116]#" onClick="this.disabled=true;addnewitem2();" disabled></td>
                        <td>&nbsp;</td>
                    ...
                    <tr>
                        ...
                        <div id="itemlocationajax">
                        	<select name="locationfr" id="locationfr" onChange="ajaxFunction(window.document.getElementById('itembal'),'/default/transaction/expressbill/balonhand.cfm?itemno='+encodeURI(document.getElementById('expressservicelist').value)+'&location='+escape(this.value));getitemdetail(this.value);">
                    ...
                    <tr onClick="setTimeout('recalculateamt();',750)">
                        ...
                        <cfgrid name="itemlist" pagesize="7" format="html" width="1000" height="1000"
                    ...
                    <tr>
                        ...
                        <cfif lcase(hcomid) eq "acht_i">
                            ...
                            <cfinput type="#inputtype#" name="taxper" id="taxper" value="7" size="8" onKeyUp="caltax()" bind="CFC:tax.getTax('#dts#','#target_taxtable#',{taxcode})" />&nbsp;&nbsp;&nbsp;
                        <cfelse>
                            <cfselect name="taxcode" id="taxcode" bind="CFC:tax.getTaxQry('#dts#','#target_taxtable#',{tran})" value="code" display="code" onChange="setTimeout('caltax()',500);" bindonload="yes"/>&nbsp;&nbsp;
                            <cfinput type="#inputtype#" name="taxper" id="taxper" readonly value="0" size="8" bind="CFC:tax.getTax('#dts#','#target_taxtable#',{taxcode})" onKeyUp="caltax()" bindonload="yes"  />&nbsp;&nbsp;&nbsp;
                        </cfif>
    ...
    <cfif lcase(HcomID) eq "simplysiti_i">
        <cfwindow center="true" width="300" height="300" name="transferlimitstock" refreshOnShow="true" closable="true" modal="true" 
            title="Transfer Limit" initshow="false"
    ...
    <cfquery name="emptytemp" datasource="#dts#">
        ...
        WHERE trdatetime < "#dateformat(dateadd('d','-2',now()),'YYYY-MM-DD')#" AND onhold <> "Y" AND uuid NOT IN (<cfqueryparam cfsqltype="cf_sql_varchar" value="#valuelist(getlast.uuid)#" list="yes" separator=",">)
    </cfquery>