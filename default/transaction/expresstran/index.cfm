...
<cfset defaultlocation=getgsetup.ddllocation>
<cfelse>
<cfset defaultlocation=huserloc>
</cfif>
...
function selectmemberlist(custno){	
  ...
  for(var i=0;i<output.length;i++) {
	...
	function updaterow(rowno)
	{
		...
		var updateurl = 'updaterow.cfm?uuid='+escape(uuid)+'&coltype='+escape(coltypedata)+'&qty='+escape(qtylistdata)+'&trancode='+rowno+'&brem4='+escape(brem4data)+'&brem1='+escape(brem1data)+'&brem2='+escape(brem2data)+'&job='+escape(joblistdata)+'&promotiontype='+escape(promotiontypedata);
		new Ajax.Request(updateurl,
      {
        method:'get',
        onSuccess: function(getdetailback){
...
<!--- 	for (k=1;k<=200;k=k+1)
	...
	{
	  var urlloaditembal = '/default/transaction/expresstran/balonhand.cfm?itemno='+encodeURI(itemnobal)+'&location='+escape(document.getElementById('coltype').value);
...
<cfinput type="text" name="refno" id="refno" bind="cfc:refnobill.getrefno({tran},'#dts#',{refnoset},'#huserloc#')" bindonload="yes" required="yes" onKeyUp="nextIndex('refno','custno');">
...
where (status<>'T' or status is null)
...
<td <cfif tran eq 'ISS'>style="display:none" </cfif>><cfinput type="text" name="currcode" id="currcode" size="10" readonly /><input type="hidden" name="currcodehid" id="currcodehid" value="">&nbsp;<cfinput type="text" name="currrate" id="currrate" bind="cfc:custsupp.getcurrrate('#dts#',{currcode},'#target_currency#')" bindonload="yes" size="5" /></td>
</tr>
...
<th>Attention</th>
<td>
<input type="text" name="b_attn" id="b_attn" value="" maxlength="35" size="20"><cfif (lcase(hcomid) neq "bnbm_i" and lcase(hcomid) neq "bnbp_i")><input type="button" name="Sattn1" value="Ajax Search" onClick="javascript:ColdFusion.Window.show('findattention');"></cfif></td>
<th <cfif getmodule.auto eq 1>style="display:none" </cfif>>Attention</th>
<td <cfif getmodule.auto eq 1>style="display:none" </cfif>><input type="text" name="d_attn" id="d_attn" value="" maxlength="35" size="20"><cfif (lcase(hcomid) neq "bnbm_i" and lcase(hcomid) neq "bnbp_i")><input type="button" name="Sattn2" value="Ajax Search" onClick="javascript:ColdFusion.Window.show('findattention2');"></cfif></td>
</tr>
...
<cfif lcase(hcomid) eq "elsiedan_i">
<tr><th>Remark</th><td><input type="text" name="rem30" id="rem30" value="" readonly size="50"></td></tr>
</cfif>
...
<td>
<cfif lcase(hcomid) eq 'elsiedan_i'>
<cfinput type="text" name="refno2" id="refno2" value="" maxlength="35"/>
<cfelse>
<cfinput type="text" name="refno2" id="refno2" value="" maxlength="20"/>
</cfif>
</td>
...
<cfif (tran eq 'RC' or tran eq 'PO' or tran eq 'DO' or tran eq 'INV' or tran eq 'SO')>
    <th <cfif (lcase(hcomid) eq "bnbm_i" or lcase(hcomid) eq "bnbp_i" or lcase(hcomid) eq "snnpl_i")>style="visibility:hidden"</cfif>>
        PO No
    </th>
    <td <cfif (lcase(hcomid) eq "bnbm_i" or lcase(hcomid) eq "bnbp_i" or lcase(hcomid) eq "snnpl_i")>style="visibility:hidden"</cfif>>
    	<input type="text" name="pono" id="pono" value="">
    </td>
<cfelse>
	<th style="visibility:hidden">PO No</th>
...
<cfselect name="agent" id="agent" bind="cfc:custsupp.getagent('#dts#','#Hlinkams#')" bindonload="yes" display="agentdesp" value="agent" onfocus="this.defaultIndex=this.selectedIndex;" onchange="this.selectedIndex=this.defaultIndex;"/>
<cfelse>
...
<cfif getdisplay.simple_brem2 eq 'Y'><th width="10%" >#getgsetup.brem2#</th></cfif>
...
<cfif lcase(hcomid) eq "lkabb_i" or lcase(hcomid) eq "lkabp_i" or lcase(hcomid) eq "lkab_i" <!---or lcase(hcomid) eq "lkatlb_i"--->
				...
				or lcase(hcomid) eq "fttk_i" or lcase(hcomid) eq "svcnc_i" or lcase(hcomid) eq "autoserv_i" or lcase(hcomid) eq "3ree_i" or lcase(hcomid) eq "pinssb_i"><cfelse>
 desc</cfif>
</cfquery>
...
<cfif getpin2.h2J01 eq "T"><a  style="cursor:pointer;color:##000" onClick="document.getElementById('hidtrancode').value='#getictrantemp.trancode#';ColdFusion.Window.show('changediscount');getfocus4();">#numberformat(val(getictrantemp.disamt_bil),'.__')#</a><cfelse>#numberformat(val(getictrantemp.disamt_bil),stDecl_UPrice)#</cfif>
...
<tr>
<th><cfif (lcase(hcomid) neq "bnbm_i" and lcase(hcomid) neq "bnbp_i" and lcase(hcomid) neq "3ree_i")><cfselect name="nextransac" id="nextransac" query="newtranqy" display="trancode" value="trancode" /><cfelse><cfselect name="nextransac" id="nextransac" query="newtranqy" display="trancode" value="trancode" /></cfif>Choose a product <input type="checkbox" name="autoadditem" id="autoadditem" value="1"  <cfif getmodule.auto eq '1'>checked style="visibility:hidden"</cfif>></th>
<td colspan="1">
<cfif lcase(hcomid) eq "eocean_i">
<cfinclude template="/selectitem/newselect.cfm">
<cfelse>
<cfinput type="text" name="expressservicelist" id="expressservicelist" size="26" onBlur="this.value = this.value.split('___', 1);bluradditem(this.value);" onKeyUp="nextIndex(event,'expressservicelist','expqty');" autosuggest="cfc:itemno.findItem({cfautosuggestvalue},'#dts#','#hitemgroup#')" autosuggestminlength="1"/>
</cfif>
...
<option value="">Choose a location</option>
<cfloop query="getlocation">
<option value="#getlocation.location#" <cfif defaultlocation eq getlocation.location>selected</cfif>>#getlocation.location# - #getlocation.desp#</option>
</cfloop>
</select>
<cfelse>
<select name="coltype" id="coltype" onKeyUp="nextIndex(event,'coltype','expressservicelist');">
...
<input name="Submit" style="font: medium bolder;background-color:##FFCC00; color:##FFFFFF" id="Submit" type="text" value="Accept"  onClick="document.getElementById('paytype').value='6';gopay('totalup6');" disabled>
<cfelse>
...
<cfwindow center="true" width="1100" height="500" name="searchitem" refreshOnShow="true" closable="true" modal="false" title="Search Item" initshow="false"
        source="/default/transaction/expresstran/tcdssearchitem.cfm?reftype={tran}" />
<cfelse>
<cfwindow center="true" width="1100" height="500" name="searchitem" refreshOnShow="true" closable="true" modal="false" title="Search Item" initshow="false"
        source="/default/transaction/expresstran/searchitem.cfm?reftype={tran}&currcode={currcode}" />
</cfif>
...
<cfwindow center="true" width="700" height="500" name="changediscount" refreshOnShow="true" closable="true" modal="true" title="Edit Discount" initshow="false" source="changediscount.cfm?uuid=#uuid#&trancode={hidtrancode}&custno={custno}" /> 
