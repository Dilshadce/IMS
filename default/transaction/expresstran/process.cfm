...
<cfset refnoget = 0>
<cfset failsafe = 0>
<cfloop condition="refnoget eq 0">

<cfquery name="updaterefno" datasource="#dts#">
UPDATE refnoset SET uuidtemp = <cfqueryparam cfsqltype="cf_sql_varchar" value="#uuid#"> WHERE 
uuidtemp = "" 
and type = '#type#'
and counter = '#refnoset#'
</cfquery>
...
SELECT uuidtemp FROM refnoset
WHERE type = '#type#'
and counter = '#refnoset#'
</cfquery>

<cfif getuuid.uuidtemp eq uuid>
<cfset refnoget = 1>
<cfelse>
<cfthread action="sleep" duration="1000"></cfthread>
<cfset failsafe = failsafe + 1>
</cfif>
...
,<cfqueryparam cfsqltype="cf_sql_varchar" value="#form.d_email#">
<cfif isdefined("form.rem30")>
,<cfqueryparam cfsqltype="cf_sql_varchar" value="#form.rem30#">
</cfif>
,'#val(roundadj)#'
)
</cfquery>
...
<cfif dts eq "tcds_i" and type eq "RC">

<cfloop query="getictranpromotion">
<cfif getictranpromotion.rem7 eq "Y">
<cfquery name="getoldprice" datasource="#dts#">
SELECT price FROM icitem WHERE itemno = <cfqueryparam cfsqltype="cf_sql_varchar" value="#getictranpromotion.itemno#">
</cfquery>

<cfquery name="thatcdat" datasource="#dts#">
INSERT INTO sellingpriceat (oldprice,newprice,changed_on, changed_by,itemno)
VALUES
(
<cfqueryparam cfsqltype="cf_sql_varchar" value="#val(getoldprice.price)#">,
<cfqueryparam cfsqltype="cf_sql_varchar" value="#val(getictranpromotion.rem8)#">,
now(),
"#getauthuser()#",
<cfqueryparam cfsqltype="cf_sql_varchar" value="#getictranpromotion.itemno#">
)
</cfquery>

<cfquery name="updateprice" datasource="#dts#">
UPDATE icitem SET price = "#numberformat(val(getictranpromotion.rem8),'.__')#", updated_on = now(), updated_by = "#getauthuser()#"
WHERE itemno = <cfqueryparam cfsqltype="cf_sql_varchar" value="#getictranpromotion.itemno#">
</cfquery>
</cfif>
</cfloop>
</cfif>
...
<cfif checkitemExist.recordcount GT 0>
			...
			<cfif val(getictranpromotion.rem11) neq 0> 
            <cfset itemfreeqty =  val(getictranpromotion.rem11)>
            <cfelse>
