...
<cfquery name="getaddress" datasource="#dts#">
	select code,name,add1,add2,add3,add4 from address order by code
</cfquery>
...
<cfoutput>
		...
		<form name="itemsales" action="addressproducttype2.cfm?trantype=#trantype#&alown=#alown#" method="post" target="_blank">
...
<input type="button" size="10" value="Ajax Search" onClick="document.getElementById('fromto').value='to';ColdFusion.Window.show('findCustomer');" />&nbsp;<input type="text" name="searchsuppto" onKeyUp="getSupp('custto','Customer');">
    ...
    <th>Address From</th>
      <td><select name="addressfrom">
				...
				<cfloop query="getaddress">
					<option value="#code#">#code# - #name# - #add1#,#add2#,#add3#,#add4#</option>
				</cfloop>
			</select>
    ...
    <tr> 
      ...
      <td><select name="addressto">
				<option value="">Choose a Address</option>
				<cfloop query="getaddress"><option value="#code#">#code# - #name# - #add1#,#add2#,#add3#,#add4#</option>
				</cfloop>
	  </select>
