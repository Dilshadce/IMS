...
function editRecord(serialno,location){
	...
	window.showModalDialog('dsp_editopeningserial.cfm?itemno='+escape(itemno)+'&serialno='+serialno+'&location='+location, '',opt);
...
function deleteRecord(serialno,location){
	if (confirm("Are you sure you want to delete?")) {
		var itemno = document.form1.itemno.value;
 		document.all.feedcontact1.dataurl="databind/act_deleteserialno.cfm?itemno=" + escape(itemno) + "&oriserialno=" + serialno + "&location=" + location + "&counter=";
