...
<cfif getrecordcount.recordcount neq 0>
	...
	<cfform action="splacement_newest.cfm" method="post" target="_self">
        ...
        <cfquery datasource='#dts#' name="gethmlist">
			select hrmgr from (
                select hrmgr from placement
                order by placementno desc
                limit 1,20
            ) a
            where hrmgr !=''
        ...
        <cfset hmusers_list = []>
        <cfloop query="gethmlist">
            <cfscript>
            ArrayAppend(hmusers_list,gethmlist.hrmgr,true);
            </cfscript>
        </cfloop>
            
        <cfquery datasource='#dts#' name="getjob">
            <cfif gethmlist.RecordCount gt 0>
            select p.*,hm.username as "hmusername" from placement p
            left join (
              select * from payroll_main.hmusers
                where entryid in (#arraytolist(hmusers_list)#)
            ) hm
            on p.hrmgr = hm.entryid
            <cfelse>
                select *, "" as hmusername from placement p
            </cfif>
            order by placementno desc
			limit 1,20;
		</cfquery>
		...
		<table align="center" class="data" width="1000px">
          ...
          <td>#getjob.hmusername#</td>
            ...
            <a href="Placementtable2.cfm?type=Delete&Placementno=#getjob.Placementno#" target="mainFrame"><img height="18px" width="18px" src="/images/delete.ICO" alt="Delete" border="0">Delete</a>&nbsp;<a target="mainFrame"  href="Placementtable2.cfm?type=Create&Placementno=#getjob.Placementno#"><img height="18px" width="18px" src="/images/edit.ICO" alt="Edit" border="0">Create View</a>&nbsp;  --->
              <a href="Placementtable2.cfm?type=Edit&Placementno=#getjob.Placementno#" target="mainFrame"><img height="18px" width="18px" src="/images/edit.ICO" alt="Edit" border="0">Update</a></div></td>
