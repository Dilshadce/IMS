...
<cfquery name="getcodepatern" datasource="#dts#">
	select debtorfr,debtorto,custSuppNo 
	from gsetup
</cfquery>
...
<cfoutput>
  	...
  	<cfif url.type eq "Create" and lcase(HcomID) neq "glenn_i" and lcase(HcomID) neq "glenndemo_i">
		<cfif lcase(HcomID) eq "mhsl_i" or lcase(HcomID) eq "mpt_i" or lcase(HcomID) eq "mhca_i">
			onSubmit="javascript:if(validate('#getcodepatern.debtorfr#','#getcodepatern.debtorto#','#getcodepatern.custSuppNo#')&&check_float()&&check_compulsoryfield()&&check_field()){return true;}else{return false;}"
		<cfelseif lcase(hcomid) eq "nikbra_i">
			onSubmit="javascript:if(check_float()&&check_field()){return true;}else{return false;}"
		<cfelse>
			onSubmit="javascript:if(validate('#getcodepatern.debtorfr#','#getcodepatern.debtorto#','#getcodepatern.custSuppNo#')&&check_float()&&check_field()){return true;}else{return false;}"
		</cfif>
	...
	<table align="center" class="data" width="660px">
		...
		<cfif lcase(HcomID) eq "tmt_i" or lcase(HcomID) eq "taff_i" or lcase(HcomID) eq "taftc_i">
	  		...
	  		<tr>
		    	<th>Forum Id :</th>
		    	<td><input type="text" size="40" name="DAddr1" value="#DAddr1#" maxlength="40"></td>
		  	</tr>
		  	<tr>
		      	<th>Product key :</th>
		      	<td><input type="password" size="40" name="DAddr2"  value="#DAddr2#" maxlength="35"></td>
		  	</tr>
		  	<tr>
		  	...
		  	<input type="hidden" name="dphone" value="#dphone#">
		  	<input type="hidden" name="dfax" value="#dfax#">
	...
	<tr>
		...
		<td>
			<select name="currcode" onChange="javascript:displayrate()">
	...
	<cfoutput>
		...
		<tr>
			...
			<td>
				...
				<cfelse>
					<input type="text" size="40" name="date" value="#date#" readonly>
				</cfif>
			</td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td><input type="text" size="40" name="dispec1" value="#dispec1#" validate="float" maxlength="5"></td>
		</tr>
		<tr>
