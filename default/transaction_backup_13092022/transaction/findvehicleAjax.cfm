	...
	<cfquery name="getcustsupp" datasource="#dts#">
   		select entryno as xentryno,model,make from vehicles WHERE  0=0<cfif lcase(hcomid) eq 'coolnlite_i' or lcase(hcomid) eq 'imperial1_i'>and custcode='#url.cust#'</cfif> and entryno like <cfqueryparam cfsqltype="cf_sql_varchar" value="%#url.custno#%"> and model like <cfqueryparam cfsqltype="cf_sql_varchar" value="%#url.custname#%"> and make like <cfqueryparam cfsqltype="cf_sql_varchar" value="%#url.leftcustname#%"> order by entryno limit 500
	</cfquery>
	<cfoutput>  
    <table width="480px">
    <tr>
