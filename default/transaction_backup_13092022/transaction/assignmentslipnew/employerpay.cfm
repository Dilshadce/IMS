...
<cfset additionalwages = 0>
				...
				<CFSET PAYIN = val(url.custbasicpay)+val(url.custphnlsalary) + val(url.custexception) + val(url.custotpay) + val(url.custallowance)+val(url.custpayback)+val(url.pber) + val(awser)>
