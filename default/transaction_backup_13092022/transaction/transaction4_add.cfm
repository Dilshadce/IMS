<cfif url.type1 eq "Add">
	<cfif url.multilocation eq "Y" and service eq "">
        ...
        <cfif geticlocation.recordcount EQ 0>
        	<script>
				alert("Are you using multi location? If yes, kindly create location first (at location profile)!")
				window.history.back();
			</script>
        	<cfabort>
        </cfif>
    ...
    <cfif getGeneralInfo.cost eq 'WEIGHT' and getGeneralInfo.periodficposting eq 'Y'>
	...
	INSERT INTO dolink SELECT frrefno FROM iclink WHERE frtype = "DO" 
		...
		and itemno = '#itemno#' group by frrefno
	</cfquery>
    ...
    <cfquery name="getweightcost" datasource="#dts#">
			select a.itemno,a.aitemno,a.unit,a.desp,a.despa,a.ucost,
			...
			(
				...
				and fperiod+0 <= '#getartran.fperiod#'
	    		and wos_date <= #getartran.wos_date#
				group by itemno
    ...
    <cfset it_cos = getweightcost.unitcost>
    <cfelse>
    <cfset it_cos = "">
    </cfif>
    ...
    <cfset totalupdisplay = "">		<!--- ADD ON 24-09-2012, FOR TOTALUP DISPAY ITEM IN BILL --->
    <cfset deductableitem = "">
    <cfset note1 = "">
	...
	<cfquery name="getitembal" datasource="#dts#">
		select desp,despa,comment,ucost,price,price2,price3,price4,price5,price6,barcode,qtybf,minimum,fcurrcode,fucost,fprice,category,packing,qty2,
		unit,graded,remark1,wpformula,wqformula,taxcode,pur_taxcode,itemtype
		<cfif lcase(hcomid) eq "ge_i" or lcase(hcomid) eq "gecn_i" or lcase(hcomid) eq "idi_i" or lcase(hcomid) eq "gwa_i" or lcase(hcomid) eq "gwachina_i" or lcase(hcomid) eq "saehan_i">
		,aitemno as productcode
    ...
    <cfif getitembal.itemtype eq 'SV'>
    <cfset isservice = 1>
    </cfif>
    
    <cfif (lcase(HcomID) eq "bnbm_i" or lcase(HcomID) eq "bnbp_i") and tran eq 'RQ'>
    ...
    <cfif getGeneralInfo.wpitemtax eq "1" > <!---For per item tax--->
    <!---default tax--->
    <cfif (tran eq 'RC' or tran eq 'PO' or tran eq 'PR' or tran eq 'RQ')>
    
    <cfset xnote_a = getGeneralInfo.df_purchasetax>
    <cfquery name="selected_taxnew" datasource="#dts#">
        SELECT * FROM #target_taxtable#
        WHERE
            code=<cfqueryparam cfsqltype="cf_sql_varchar" value="#getGeneralInfo.df_purchasetax#">
    </cfquery>
    ...
    <cfelse>
    <cfset xnote_a = getGeneralInfo.df_salestax>
    <cfquery name="selected_taxnew" datasource="#dts#">
        SELECT * FROM #target_taxtable#
        WHERE
            code=<cfqueryparam cfsqltype="cf_sql_varchar" value="#getGeneralInfo.df_salestax#">
    </cfquery>
    <cfif selected_taxnew.recordcount neq 0>
        <cfset selected_taxrate = selected_taxnew.rate1 * 100>
        <cfset taxpec1 = selected_taxrate>
    ...
    <cfquery name="getcusttaxcode" datasource="#dts#">
    	SELECT taxcode from #target_arcust# where custno = '#getartran.custno#'
    </cfquery>
    <cfif getcusttaxcode.taxcode neq "">
    <cfset xnote_a = getcusttaxcode.taxcode>
    <cfquery name="selected_taxnew" datasource="#dts#">
        SELECT * FROM #target_taxtable#
        WHERE
            code=<cfqueryparam cfsqltype="cf_sql_varchar" value="#getcusttaxcode.taxcode#">
    </cfquery>
    <cfif selected_taxnew.recordcount neq 0>
        <cfset selected_taxrate = selected_taxnew.rate1 * 100>
        <cfset taxpec1 = selected_taxrate>
    ...
    <!---item tax --->
    <cfif (tran eq 'RC' or tran eq 'PO' or tran eq 'PR' or tran eq 'RQ') and getitembal.pur_taxcode neq "">
    
    <cfset xnote_a = getitembal.pur_taxcode>
    <cfquery name="selected_taxnew" datasource="#dts#">
        SELECT * FROM #target_taxtable#
        WHERE
            code=<cfqueryparam cfsqltype="cf_sql_varchar" value="#getitembal.pur_taxcode#">
    </cfquery>
    <cfif selected_taxnew.recordcount neq 0>
        <cfset selected_taxrate = selected_taxnew.rate1 * 100>
        <cfset taxpec1 = selected_taxrate>
    ...
    <cfelseif getitembal.taxcode neq "">
    <!---sales tax--->
	...
	<cfif service neq "">
        ...
        <cfif getGeneralInfo.appDisSupCusitem eq "Y">
        <cfif tran eq 'rc' or tran eq 'pr' or tran eq 'po' or tran eq 'rq'>
        <cfquery name="getdisccust" datasource="#dts#">
        SELECT dispec1,dispec2,dispec3 from #target_apvend# where custno = '#getartran.custno#'
        </cfquery>
        <cfelse>
        <cfquery name="getdisccust" datasource="#dts#">
        SELECT dispec1,dispec2,dispec3 from #target_arcust# where custno = '#getartran.custno#'
        </cfquery>
        </cfif>
        <cfset dispec1 = val(getdisccust.dispec1)>
	  	<cfset dispec2 = val(getdisccust.dispec2)>
	  	<cfset dispec3 = val(getdisccust.dispec3)>
	...
	<cfelse>
        ...
        <cfelseif lcase(hcomid) eq "excelhw_i" or lcase(hcomid) eq "exceljb_i">	<!--- Add On 090908 --->
			<cfset Brem3 = getitembal.packing>
			<cfset Brem4 = getitembal.qty2>
        <cfelseif lcase(hcomid) eq "tranz_i">
        	<cfset Brem1 = getitembal.barcode>
	...
	<cfelse>
        ...
        <cfelseif lcase(hcomid) eq "haikhim_i">
        	<cfset xlocation = "#getartran.source#">
		<cfelse>
	...
	<cfset taxpec1 = 0>
	<cfset gltradac = ""--->
    <cfif getGeneralInfo.taxincluded eq "Y">
    <cfset additemtaxincluded="T">
    </cfif>
    ...
    <cfif (tran eq "INV" or tran eq "DO" or tran eq "QUO" or tran eq "SO" or tran eq "CS") and lcase(hcomid) neq "mika_i">
    ...
    SELECT * FROM promotion where promoid = 0
        ...
        <cfif (lcase(hcomid) eq "mika_i" or lcase(hcomid) eq "posdemo_i") and getartran.van neq "" and getitembal.price2 neq 0>
        <cfset price = getitembal.price2>
        </cfif>
		...
		<cfif url.type1 eq "Add">
        <cfif url.multilocation eq "Y" and service eq "">
		<cfif lcase(hcomid) eq "litelab_i">
        <cfquery name="getcatecommission" datasource="#dts#">
        select commission from drivercommission where driverno='#getartran.van#' and category='#getitembal.category#'
        </cfquery>
        <cfset brem4=val(getcatecommission.commission)>
        </cfif>
        </cfif>
        </cfif>
