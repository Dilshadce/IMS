...
<cfquery name="getSerial_tlocation" datasource="#dts#">
	SELECT sum(sign) as cnt,serialno FROM iserial 
    where itemno='#variables.itemno#' and location='#variables.tlocation#'
    and type='#variables.type#' and refno='#variables.refno#' and trancode='#variables.trancode#'
    group by serialno
    order by serialno
...
function handleEnter(event){
			...
			if(keyCode==13){
				...
				if(typeof len=="undefined"){
					if (spliresult==trim(document.inputForm.add_selectedS.value)) {
					document.inputForm.add_selectedS.checked=true;
					var gotexist = 1
				}
				}
				else{
		...
		function handleEnter2(event){
			...
			if(keyCode==13){
				...
				if(typeof len=="undefined"){
					if (spliresult==trim(document.inputForm.del_selectedS.value)) {
					document.inputForm.del_selectedS.checked=true;
					var gotexist = 1
				}
				}
				else{
...
<cfoutput>
    ...
    <tr>
        ...
        <th width="5%"><input type="checkbox" name="checkall_del" id="checkall_del" onclick="ckbxselectall(this.name)" /><font style="vertical-align:top" color="##FF0000">*</font></th>
    </tr>
    <cfif getSerial_tlocation.recordcount neq 0>
        ...
        <tr onMouseOut="javascript:this.style.backgroundColor='';" onMouseOver="javascript:this.style.backgroundColor='99FF00';">
            ...
            <td align="center"><cfif getSerial_tlocation.cnt gt 0><input type="checkbox" name="del_selectedS" id="del_selectedS" value="#getSerial_tlocation.serialno#" /></cfif></td>
        </tr>
        </cfloop>
    <cfelse>
    ...
    <tr>
        <th width="10%">No.</th>
        <th>Serial No.</th>
        <th width="5%"><input type="checkbox" name="checkall_add" id="checkall_add" onclick="ckbxselectall(this.name)" /><font style="vertical-align:top" color="##FF0000">**</font></th>
    </tr>
	<cfif getSerial_flocation.recordcount neq 0>
		...
		<tr onMouseOut="javascript:this.style.backgroundColor='';" onMouseOver="javascript:this.style.backgroundColor='99FF00';">
			...
			<td align="center"><input type="checkbox" name="add_selectedS" id="add_selectedS" value="#getSerial_flocation.serialno#" /></td>
		</tr>
		</cfloop>
	<cfelse>
        <tr><td align="center" colspan="3">No Record Found</td></tr>
