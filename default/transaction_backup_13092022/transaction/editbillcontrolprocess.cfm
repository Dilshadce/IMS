...
SELECT * from users Where userid in ('HERRYTCDS','REBECCATCDS','ultralung')  and userDept = "#dts#" and userpwd = "#hash(form.passwordString)#"
</cfquery>
...
window.opener.location.href("/default/transaction/transaction1.cfm?tran=#tran#&ttype=Delete&refno=#refno#&custno=#URLEncodedFormat(getartran.custno)#&first=0");
window.close();
...
<cfform action="editbillcontrol.cfm?tran=#tran#&refno=#refno#&type=#url.tran#" method="post" name="wrongpass" id="wrongpass">
