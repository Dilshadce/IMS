...
<cfif url.ttype eq "Create">
	...
	<cfif getGeneralInfo.arun eq "1">
		...
		<cfif cnt gt 1>
			...
			<!---cfset nexttranno = nextno & numberformat(nextno,zero)>--->
...
<cfform name="#formname#" id="#formid#" action="iss3.cfm" method="post" onSubmit="return test();">
	...
	<cfif url.ttype eq "Delete" or url.ttype eq "Edit">
		...
		<input type="hidden" name="nexttranno" id="nexttranno" value="#url.refno#">
	<cfelse>
		<input type="hidden" name="currefno" value="">
		<input type="hidden" name="nexttranno" id="nexttranno" value="">
	</cfif>
	<cfif lcase(hcomid) eq "amgworld_i">
  	...
  	<table align="center" class="data">
		<tr> 
   	 	...
   	 	<tr> 
      		...
      		<td>Reference Number 2</td>
      		<td><input type="text" name="refno2" id="refno2" value="#refno2#" maxlength="24"></td>
    	</tr>
    	<tr> 
      		<th>Authorised By</th>
		...
		<cfif tran eq 'TR'>
			<cfquery name="getloc" datasource="#dts#">
				SELECT location,desp,custno,(SELECT name FROM #target_arcust# WHERE custno=iclocation.custno) AS name 
                FROM iclocation
                WHERE 0 = 0
                <cfif lcase(HcomID) eq "simplysiti_i">
					<cfif lcase(husergrpid) neq 'admin'>
                	AND noactivelocation <>'Y'
                	</cfif>
                <cfelse>
                </cfif>
			...
			<tr><input type="hidden" name="oldtrfrom" value="#remark1#">
                ...
                <select name="trfrom" id="trfrom">
						...
						<cfloop query="getloc">
                        <cfif Huserloc neq 'all_loc' and lcase(HcomID) eq "amgworld_i" and url.ttype eq 'create'>
                        <cfif Huserloc eq getloc.location>
                        <option value="#getloc.location#" >#getloc.location# - #getloc.desp# - #getloc.custno#</option></option>
                        </cfif>
                        <cfelse>
