...
SELECT EDControl,refno2#tran# as refno2valid,ASACTP,ASDA,attnddl,transactiondate,df_cs_cust,quotationlead,agentlistuserid,locarap,grpinitem FROM gsetup 
</cfquery>
...
<cfquery datasource="#dts#" name="getGsetup">
	...
	po_oneset,so_oneset, quo_oneset, assm_oneset, tr_oneset, oai_oneset, oar_oneset, sam_oneset,rq_oneset,
	filterall,suppcustdropdown,debtorfr,debtorto,creditorfr,creditorto,keepDeletedBills,prefixbycustquo,prefixbycustso,prefixbycustinv,collectaddress,bcurr
	from GSetup
</cfquery>
...
<head>
		...
		function displayrate()
	{
		...
		function updateDetails(columnvalue){
		 ...
		 --->
		 	<cfoutput>
		 	ajaxFunction(document.getElementById('ajaxfieldbadstatus'),'/default/transaction/checkbadstatusajax.cfm?tran=#tran#&custno='+document.getElementById('custno').value);
...
<cfif tran eq "rc" or tran eq "pr" or tran eq "po" or tran eq "rq">
	...
	<cfset ptype1 = words[104]>
    <cfset ptype2 = "supplier">
<cfelse>
	<cfset ptype = target_arcust>
	<cfset ptype1 = words[5]>
    <cfset ptype2 = "customer">
</cfif>
...
<cfif getGsetup.suppcustdropdown eq "1">
	...
	<cfquery name="getcustsupp" datasource="#dts#">
   		select custno as xcustno,name,add1,add2,add3,add4 from #ptype# 
        WHERE 0=0 
        and (status <>"B" or status is null)
        <cfif tran eq "rc" or tran eq "pr" or tran eq "po" or tran eq "rq">
        <cfif getsetting.grpinitem eq "Y">
         <cfif Hitemgroup neq ''>
        and name='#Hitemgroup#'
        </cfif>
		</cfif>
...
<!---   
	...
	<form name="invoicesheet" id="invoicesheet" action="transaction2.cfm" method="post" onSubmit="return test()">
        ...
        <cfif isdefined("form.d_country")>
        <input type="hidden" name="d_country" value="#form.d_country#">
        </cfif>
	...
	<script type="text/javascript">
		...
		today = yyyy+mm+dd;
		var keydate = document.invoicesheet.invoicedate.value.substring(6,10)+document.invoicesheet.invoicedate.value.substring(3,5)+document.invoicesheet.invoicedate.value.substring(0,2);
		try{
		if(keydate < today && document.invoicesheet.allowbackdate.value != "Y")
		{
			ColdFusion.Window.show('backdatepass');
			return false;
		}
		}
		catch(err)
...
<body>
	...
	<cfif alcreate eq 1>
			...
			<cfelse>
                ...
                <cfelseif getGsetup.rq_oneset neq '1' and tran eq 'RQ'>
...
<form name="invoicesheet" id="invoicesheet" action="" method="post" onSubmit="return checkbackdate()">
    <input type="hidden" name="type" id="type" value="#listfirst(mode)#">
    <input type="hidden" name="tran" id="tran" value="#listfirst(tran)#">
	
    <cfif lcase(hcomid) eq "amgworld_i">
    <input type="hidden" name="allowbackdate" id="allowbackdate" value="N">
    </cfif>
	...
	<cfif T_DCode neq "">
    	<cfif T_DCode eq "Profile">
            ...
            <cfset d_country = getcust.d_country>
     	...
     	<cfelse>
			...
			<cfset DCode = getDeliveryAddr.code>
		  	...
		  	<cfset d_name2 = getDeliveryAddr.name2>
		  	...
		  	<cfset d_add5 = "">
            <cfset d_postalcode = getDeliveryAddr.postalcode>
            <cfset d_country = getDeliveryAddr.country>
            ...
            <cfset d_gstno = getDeliveryAddr.gstno>
	...
	<table align="center" class="data" cellspacing="0">
      	<tr>
			...
			<cfif ttype eq "Create">
            	<cfif tranarun_1 eq "1">
              	...
              	<input name="nexttranno" id="nexttranno" type="text" value="#url.nexttranno#" onvalidate="javascript:NextTransNo()" size="10" onKeyUp="this.value=trim(this.value)" <cfif lcase(HcomID) eq "leatat_i">maxlength="30"<cfelse><cfif tran eq 'SO' or tran eq 'PO' or tran eq 'QUO'>maxlength="24"<cfelse><cfif lcase(HcomID) eq "meisei_i" and tran eq "RC">maxlength = "10"<cfelse>maxlength="20"</cfif></cfif></cfif>>
            ...
            <cfelse>
       	  ...
       	  <th width="15%" colspan="3">#words[1088]#</th>
        	<td width="35%"><h2>
            	<cfif ttype eq "Create">
              		New
            	</cfif>
    	...
    	<tr>
            ...
            <img src="../../images/cal.gif" width=17 height=15 border=0 onClick="showCalendarControl(invoicedate);">
				(DD/MM/YYYY)
            <cfelse>
            <input type="text" name="invoicedate" id="invoicedate" value="<cfif isdefined("url.invoicedate")>#url.invoicedate#<cfelse>#dateformat(now(),"dd/mm/yyyy")#</cfif>" validate="eurodate" size="10" maxlength="10" readonly>
            </cfif>
                    ...
                    <cfloop query="getcustsupp">
                      <option value="#xcustno#" <cfif trim(custno) eq xcustno>selected</cfif>>
                        <cfif lcase(HcomID) eq "glenn_i" or lcase(HcomID) eq "glenndemo_i">
							#name# - #xcustno#
                        <cfelseif LCASE(HcomID) EQ "luxor_i">
                            #name#
...
(Search by Accno/Name) ---><br>
					...
					<cfif getpin2.h1210 eq 'T'><a onClick="javascript:ColdFusion.Window.show('create#ptype2#');" onMouseOver="this.style.cursor='hand';">#words[2152]# #ptype1#</a></cfif>
                    </cfif>
					...
					<br>
				<cfelse>
					<input name="custno" id="custno" type="text" value="#custno#"  size="20" maxlength="8" onBlur="ajaxFunction(document.getElementById('ajaxfieldbadstatus'),'/default/transaction/checkbadstatusajax.cfm?tran=#tran#&custno='+document.getElementById('custno').value);setTimeout('rpicheckoutstanding()',500);">
					...
					<input type="button" name="Scust1" value="Account Search" onClick="javascript:ColdFusion.Window.show('findCustomer');">&nbsp;&nbsp;<input type="submit" name="Scust" value="#words[11]#"><br>
                    ...
                    <cfif lcase(hcomid) neq "mastercare_i" and lcase(hcomid) neq "gorgeous_i">
							...
							<a onClick="javascript:ColdFusion.Window.show('create#ptype2#');" onMouseOver="this.style.cursor='hand';" >#words[2152]# #ptype1#</a>	
        ...
        <cfif tran eq 'RQ'>
        <tr><th colspan="3"></th><td></td><th colspan="3">#words[5]#</th><td>
		...
		<tr>
			<th colspan="3" <cfif getdisplaysetup2.b_add_baddresscode neq 'Y'>style="visibility:hidden"</cfif>>#words[889]#</th>
            ...
            <!---
            <cfif not isdefined("url.custno")>
            <cfif GetSetting.ASDA eq "N">
            <cfset T_BCode="">
            <cfelse>
            <cfset T_BCode="Profile">
            </cfif>
			...
			<cfif tran eq 'PO' or tran eq 'SO' or tran eq 'DO' or tran eq 'INV' or tran eq 'SAM' or tran eq 'CN' or tran eq 'DN' or tran eq 'PR' or tran eq 'RC' or tran eq 'CS' or tran eq 'QUO' or hcomid eq "kimlee_i" or lcase(HcomID) eq "bakersoven_i">
				<th colspan="3" <cfif getdisplaysetup2.b_add_daddresscode neq 'Y'>style="visibility:hidden"</cfif>>#words[1421]#</th>
		...
		<tr>
			<th colspan="3" <cfif getdisplaysetup2.b_add_bname neq 'Y'>style="visibility:hidden"</cfif>>#words[23]#</th>
			...
			<cfif tran eq 'PO' or tran eq 'SO' or tran eq 'RQ' or tran eq 'DO' or tran eq 'INV' or tran eq 'SAM' or tran eq 'CN' or tran eq 'DN' or tran eq 'PR' or tran eq 'RC' or tran eq 'CS' or tran eq 'QUO' or hcomid eq "kimlee_i" or lcase(HcomID) eq "bakersoven_i">
				<th colspan="3" <cfif getdisplaysetup2.b_add_dname neq 'Y'>style="visibility:hidden"</cfif>>#words[23]#</th>
		...
		<tr>
			<th colspan="3" <cfif getdisplaysetup2.b_add_baddress neq 'Y'>style="visibility:hidden"</cfif>>#words[957]# :</th>
			...
			<cfif tran eq 'PO' or tran eq 'SO' or tran eq 'RQ' or tran eq 'DO' or tran eq 'INV' or tran eq 'SAM' or tran eq 'CN' or tran eq 'DN' or tran eq 'PR' or tran eq 'RC' or tran eq 'CS' or tran eq 'QUO' or hcomid eq "kimlee_i" or lcase(HcomID) eq "bakersoven_i">
				<th colspan="3" <cfif getdisplaysetup2.b_add_daddress neq 'Y'>style="visibility:hidden"</cfif>>#words[954]# :</th>
                ...
                <cfelse>
                <input type="text" name="d_add1" id="d_add1" value="#d_add1#" size="45" maxlength="35">
                </cfif>
                </td>
			</cfif>
		</tr>
		<tr>
			<td rowspan="2" align="left" rowrap <cfif getdisplaysetup2.b_add_baddress neq 'Y'>style="visibility:hidden"</cfif>><input type="Button" name="ProfileAddr" value="#words[958]#" onClick="javascript:CopyProfileAddr()"></td>
		  <td rowspan="2" align="left" rowrap <cfif getdisplaysetup2.b_add_baddress neq 'Y'>style="visibility:hidden"</cfif>>#words[1170]# #ptype1# #words[2156]#</td>
		  <td rowspan="2" align="left" rowrap <cfif getdisplaysetup2.b_add_baddress neq 'Y'>style="visibility:hidden"</cfif>>&nbsp;&nbsp;&nbsp;</td>
           ...
           <cfif lcase(HcomID) eq "fdipx_i">
			...
			<cfif tran eq 'PO' or tran eq 'SO' or tran eq 'RQ' or tran eq 'DO' or tran eq 'INV' or tran eq 'SAM' or tran eq 'CN' or tran eq 'DN' or tran eq 'PR' or tran eq 'RC' or tran eq 'CS' or tran eq 'QUO' or hcomid eq "kimlee_i" or lcase(HcomID) eq "bakersoven_i">
				<td rowspan="2" align="left" rowrap <cfif getdisplaysetup2.b_add_daddress neq 'Y'>style="visibility:hidden"</cfif>><input type="button" name="BillToAddr" value="#words[958]#" onClick="javascript:CopyBillAddr()"><br/><br/><input type="button" name="BillToAddr2" value="#words[958]#" onClick="javascript:CopyProfileAddr2()" /></td>
				<td rowspan="2" align="left" rowrap <cfif getdisplaysetup2.b_add_daddress neq 'Y'>style="visibility:hidden"</cfif>>#words[897]#<br/><br />#words[1170]# <cfoutput>#ptype1#</cfoutput> #words[2156]#</td>
		...
		<tr>
			<td align="left" rowrap <cfif getdisplaysetup2.b_add_baddress neq 'Y'>style="visibility:hidden"</cfif>><input type="submit" name="ChgBillToAddr" value="#words[958]#"></td>
			<td align="left" rowrap <cfif getdisplaysetup2.b_add_baddress neq 'Y'>style="visibility:hidden"</cfif>>#words[890]# </td>
			...
			<cfif tran eq 'PO' or tran eq 'SO' or tran eq 'RQ' or tran eq 'DO' or tran eq 'INV' or tran eq 'SAM' or tran eq 'CN' or tran eq 'DN' or tran eq 'PR' or tran eq 'RC' or tran eq 'CS' or tran eq 'QUO' or hcomid eq "kimlee_i" or lcase(HcomID) eq "bakersoven_i">
				<td align="left" rowrap <cfif getdisplaysetup2.b_add_daddress neq 'Y'>style="visibility:hidden"</cfif>><br /><input type="submit" name="ChgDeliveryAddr" value="#words[958]#"></td>
				<td align="left" rowrap <cfif getdisplaysetup2.b_add_daddress neq 'Y'>style="visibility:hidden"</cfif>><br />#words[890]# </td>
        ...
        <tr>
        <th colspan="3">#words[38]#</th>
			<td <cfif getdisplaysetup2.b_add_baddress neq 'Y'>style="visibility:hidden"</cfif>>
            <input type="text" name="country" id="country" value="#country#" size="45" maxlength="20">
            </td>
            <th colspan="3">#words[38]#</th>
			<td <cfif getdisplaysetup2.b_add_daddress neq 'Y'>style="visibility:hidden"</cfif>>
            <input type="text" name="d_country" id="d_country" value="#d_country#" size="45" maxlength="20">
            </td>
		</tr>
        <tr>
        	<th colspan="3">#words[39]#</th>
			<td <cfif getdisplaysetup2.b_add_baddress neq 'Y'>style="visibility:hidden"</cfif>>
		...
		<tr>
			<th colspan="3" <cfif getdisplaysetup2.b_add_battn neq 'Y'>style="visibility:hidden"</cfif>><cfif lcase(HcomID) eq "ugateway_i" and tran eq "INV" >Order By :<cfelse>#words[892]# :</cfif></th>
            ...
            <input type="button" name="Sattn1" value="#words[953]#" onClick="javascript:ColdFusion.Window.show('findattention');">
            </td>
			<cfif tran eq 'PO' or tran eq 'SO' or tran eq 'RQ' or tran eq 'DO' or tran eq 'INV' or tran eq 'SAM' or tran eq 'CN' or tran eq 'DN' or tran eq 'PR' or tran eq 'RC' or tran eq 'CS' or tran eq 'QUO' or hcomid eq "kimlee_i" or lcase(HcomID) eq "bakersoven_i">
				<th colspan="3" <cfif getdisplaysetup2.b_add_dattn neq 'Y'>style="visibility:hidden"</cfif>>#words[892]# :</th>
                ...
                <input type="button" name="Sattn2" value="#words[953]#" onClick="javascript:ColdFusion.Window.show('findattention2');">
                </td></cfif>
			
		</tr>
		<tr>
			<th colspan="3" <cfif getdisplaysetup2.b_add_btel neq 'Y'>style="visibility:hidden"</cfif>><cfif lcase(HcomID) eq "ugateway_i" and tran eq "INV" >Contact :<cfelse>#words[40]# :</cfif></th>
			...
			<cfif tran eq 'PO' or tran eq 'SO' or tran eq 'RQ' or tran eq 'DO' or tran eq 'INV' or tran eq 'SAM' or tran eq 'CN' or tran eq 'DN' or tran eq 'PR' or tran eq 'RC' or tran eq 'CS' or tran eq 'QUO' or hcomid eq "kimlee_i" or lcase(HcomID) eq "bakersoven_i">
				<th colspan="3" <cfif getdisplaysetup2.b_add_dtel neq 'Y'>style="visibility:hidden"</cfif>><cfif lcase(hcomid) eq "glenn_i" or lcase(hcomid) eq "glenndemo_i">Heading<cfelseif lcase(hcomid) eq "winbells_i">Phone Number<cfelseif lcase(hcomid) eq "chemline_i">HP<cfelse>#words[40]#</cfif> :</th>
		...
		<tr>
			<th colspan="3" <cfif getdisplaysetup2.b_add_btel2 neq 'Y'>style="visibility:hidden"</cfif>>#words[42]# :</th>
			...
			<th colspan="3" <cfif getdisplaysetup2.b_add_dcontact neq 'Y'>style="visibility:hidden"</cfif>><cfif lcase(HcomID) eq "varz_i">REP<cfelse>#words[42]#</cfif> :</th>
		...
		<tr>
			<th colspan="3" <cfif getdisplaysetup2.b_add_bfax neq 'Y'>style="visibility:hidden"</cfif>>#words[300]# :</th>
			...
			<!--- <cfif tran eq 'PO' or tran eq 'SO' or tran eq 'DO' or tran eq 'INV'>
				...
				<cfif tran eq 'PO' or tran eq 'SO' or tran eq 'RQ' or tran eq 'DO' or tran eq 'INV' or tran eq 'SAM' or tran eq 'CN' or tran eq 'DN' or tran eq 'PR' or tran eq 'RC' or tran eq 'CS' or tran eq 'QUO' or hcomid eq "kimlee_i" or lcase(HcomID) eq "bakersoven_i">
				<th colspan="3" <cfif getdisplaysetup2.b_add_dfax neq 'Y'>style="visibility:hidden"</cfif>>#words[300]# :</th>
        ...
        <tr>
			<th colspan="3" <cfif getdisplaysetup2.b_add_bemail neq 'Y'>style="visibility:hidden"</cfif>>#words[45]# :</th>
        ...
        <tr>
			<th colspan="3" >#words[2157]# :</th>
			<td ><input type="text" name="b_gstno" id="b_gstno" value="#b_gstno#" size="45" maxlength="45"></td>
            <th colspan="3" >#words[2157]# :</th>
			<td ><input type="text" name="d_gstno" id="d_gstno" value="#d_gstno#" size="45" maxlength="45"></td>
    	...
    	<tr>
			...
			<cfif mode eq "Delete">	<!--- ADD ON 22-12-2009 --->
				<cfif getGsetup.keepDeletedBills eq "1">
					<h3>
                    <cfif isdefined("getitem.void")>
                    <cfif getitem.void eq "Y">
                    <input name="keepDeleted" id="keepDeleted" type="checkbox" value="1" style="visibility:hidden">
                    <cfelse>
            ...
            <cfelseif mode eq "Create" and getdisplaysetup2.project_compulsory eq "Y">
            <cfelseif mode eq "Create" and getGsetup.bcurr eq "MYR" and (tran eq "CN" or tran eq "DN")>
            <cfelse>
...
<script language="JavaScript">
	...
	function CopyProfileAddr()
	{
		...
		document.invoicesheet.b_gstno.value = "#getcust.gstno#";
  		</cfoutput>
  		return true;
	}
...
<cfif tran eq "rc" or tran eq "pr" or tran eq "po" or tran eq "rq">
<cfwindow center="yes" modal="true"  width="1100" height="600" name="create#ptype2#" refreshOnShow="true"
...
<cfwindow center="true" width="580" height="400" name="findCustomer" refreshOnShow="true"
        ...
        title="#words[902]#" initshow="false"
...
<cfwindow center="true" width="580" height="400" name="findsupp" refreshOnShow="true"
        ...
        <cfwindow center="true" width="350" height="300" name="backdatepass" refreshOnShow="true" closable="true" modal="true" title="Enter Password" initshow="false"
        source="/default/transaction/backdatepassword.cfm?tran={tran}&nexttranno={nexttranno}" />
</body>
</html>