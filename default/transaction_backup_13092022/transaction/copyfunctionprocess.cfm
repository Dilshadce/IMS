...
<cfif not isdefined("url.status")>
	...
	<cffunction name="updateQinQout" output="false">
		...
		<cftry>
			...
			<cfelseif arguments.type neq "SO" and arguments.type neq "PO" and arguments.type neq "RQ" and arguments.type neq "QUO">
	...
	<cffunction name="updateArtran" output="false">
        ...
        <cfargument name="crossover" default="">
		...
		<cfquery datasource="#dts#" name="insertartran">
			...
			m_charge6,m_charge7,special_account_code,created_by,created_on,updated_by,updated_on,termscondition,consignment,e_mail,d_email,rem30,rem31,rem32,rem33,rem34,rem35,rem36,rem37,rem38,rem39,rem40,rem41,rem42,rem43,rem44,rem45,rem46,rem47,rem48,rem49,country,d_country,postalcode,d_postalcode)
			...
			'#resultSet.trancode#',<cfif arguments.newType eq 'INV'>'SALES'<cfelseif arguments.newType eq 'RC'>'PURCHASES'<cfelseif arguments.newType eq 'PR'>'PURCHASES RETURN'<cfelseif arguments.newType eq 'DO'>'DELIVERY'<cfelseif arguments.newType eq 'CS'>'CASH SALES'<cfelseif arguments.newType eq 'CN'>'GOOD RETURNED'<cfelseif arguments.newType eq 'DN'>'DEBIT NOTE'<cfelse>'#resultSet.desp#'</cfif>,'#resultSet.despa#','#resultSet.area#','#resultSet.source#','#resultSet.job#','#resultSet.mc1_bil#',
			'#resultSet.mc2_bil#','#resultSet.m_charge1#','#resultSet.m_charge2#','#resultSet.cs_pm_cash#','#resultSet.cs_pm_cheq#','#resultSet.cs_pm_crcd#',
			'#resultSet.cs_pm_crc2#','#resultSet.cs_pm_dbcd#','#resultSet.cs_pm_vouc#',<cfif lcase(HcomID) eq "rpi270505_i">'0','#resultSet.grand_bil#'<cfelse>'#resultSet.deposit#','#resultSet.cs_pm_debt#'</cfif>,'#resultSet.cs_pm_wht#',
			...
			'#resultSet.m_charge6#','#resultSet.m_charge7#','#resultSet.special_account_code#','#HUserID#',#now()#,'#HUserID#',#now()#,'#resultSet.termscondition#',<cfif arguments.newType eq 'TR'><cfif crossover eq '1'><cfif resultSet.consignment eq 'out'>'return'<cfelse>'out'</cfif><cfelse>'#resultSet.consignment#'</cfif><cfelse>''</cfif>,'#resultSet.e_mail#','#resultSet.d_email#'
            ,'#resultSet.rem30#','#resultSet.rem31#','#resultSet.rem32#','#resultSet.rem33#','#resultSet.rem34#'
            ,'#resultSet.rem35#','#resultSet.rem36#','#resultSet.rem37#','#resultSet.rem38#','#resultSet.rem39#'
            ,'#resultSet.rem40#','#resultSet.rem41#','#resultSet.rem42#','#resultSet.rem43#','#resultSet.rem44#'
            ,'#resultSet.rem45#','#resultSet.rem46#','#resultSet.rem47#','#resultSet.rem48#','#resultSet.rem49#',
            <cfqueryparam cfsqltype="cf_sql_varchar" value="#resultSet.country#">,<cfqueryparam cfsqltype="cf_sql_varchar" value="#resultSet.d_country#">,
            <cfqueryparam cfsqltype="cf_sql_varchar" value="#resultSet.postalcode#">,<cfqueryparam cfsqltype="cf_sql_varchar" value="#resultSet.d_postalcode#">
            )
		</cfquery>
                
        <cfif left(arguments.newRefno,1) eq 7>
            <cfquery name="insertictran" datasource="#dts#">
                UPDATE artran
                SET rem40 = null
                WHERE refno = '#arguments.newRefno#'
            </cfquery>
        </cfif>
	</cffunction>
	...
	<cffunction name="updateIctran" output="false">
		...
		<cfquery name="insertictran" datasource="#dts#">
			...
			mc6_bil,mc7_bil,note_a,TITLE_ID,title_desp,consignment<cfif lcase(dts) eq "mastercare_i" or lcase(dts) eq "gorgeous_i">,ictranfilename</cfif>)
			...
			'#resultSet.mc6_bil#','#resultSet.mc7_bil#','#resultSet.note_a#','#resultSet.TITLE_ID#','#resultSet.title_desp#',<cfif form.ft eq 'TR'><cfif crossover eq "1"><cfif resultSet.consignment eq 'out'>'return'<cfelse>'out'</cfif><cfelse>'#resultSet.consignment#'</cfif><cfelse>''</cfif><cfif lcase(dts) eq "mastercare_i" or lcase(dts) eq "gorgeous_i">,'#resultSet.ictranfilename#'</cfif>)
		</cfquery>	
	</cffunction>
	...
	<cffunction name="updateAddress" output="false">
		...
		<cfquery name="update_artran" datasource="#dts#">
			Update artran, 
			<cfif arguments.type eq "RC" or arguments.type eq "PO" or arguments.type eq "RQ" or arguments.type eq "PR">#target_apvend#<cfelse>#target_arcust#</cfif> as getCust
			...
			<cfif arguments.type eq 'PO' or arguments.type eq 'RQ' or arguments.type eq 'SO' or arguments.type eq 'DO' or arguments.type eq 'INV'>
    ...
    <cffunction name="updateIgrade" output="false">
        ...
        <cfquery name="insertigrade" datasource="#dts#">
            ...
            <cfif form.ft neq "TR">
				<cfif form.ft eq "RC" or form.ft eq "PO" or form.ft eq "RQ" or form.ft eq "CN" or form.ft eq "OAI">'1'<cfelse>'-1'</cfif>
			<cfelse>
        ...
        <cfif form.ft neq "SO" and form.ft neq "PO" and form.ft neq "RQ" and form.ft neq "QUO" and form.ft neq "SAM">
			<cfif form.ft neq "TR">
	...
	<cfif getartran.recordcount gt 0>
		<!--- Empty Cust/Suppl --->
			<cfoutput query="getartran">
                ...
                <cfif isdefined('form.crossover')>
                <cfinvoke method="updateArtran" crossover="1" newRefno="#nexttranno#" newDate="#wosDate#" fromType="#form.ff_type#" newType="#form.ft#" resultSet="#getartran#"/>
                <cfelse>
				<cfinvoke method="updateArtran" crossover="" newRefno="#nexttranno#" newDate="#wosDate#" fromType="#form.ff_type#" newType="#form.ft#" resultSet="#getartran#"/>
                </cfif>
				...
				<cfif getartran.type eq "TR">
                    ...
                    <cfif isdefined('form.crossover')>
						<cfinvoke method="updateIctran"  crossover="1" newRefno="#nexttranno#" newDate="#wosDate#" resultSet="#getictran#"/>
                    <cfelse>
                    	<cfinvoke method="updateIctran" crossover="" newRefno="#nexttranno#" newDate="#wosDate#" resultSet="#getictran#"/>
                    </cfif>
						<cfinvoke method="updateQinQout" itemno="#itemno#" qty="#qty#" type="#form.ft#"/>
					</cfloop>
                    
                     <cfif getgsetup2.autolocbf eq "Y">
                    <cfinvoke component="cfc.countlocbal" method="countlocbal" dts="#dts#" refno="#nexttranno#" type="#form.ft#" returnvariable="done" />
                    </cfif>
