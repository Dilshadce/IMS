...
<cfquery name="getrecordcount" datasource="#dts#">
	...
	where type='#tran#' and <cfif searchType eq 'brem2'>refno in (select refno from ictran where brem2 like <cfqueryparam cfsqltype="cf_sql_varchar" value="%#searchStr#%"> and type='#tran#')<cfelseif searchType eq 'allphone'>
		...
		<cfelseif searchType eq "status"><cfif searchStr eq 'updated'>toinv<>''<cfelseif searchStr eq 'approved'>printstatus ='a3'<cfelse>(toinv='' or toinv is null)</cfif>
		<cfelseif searchType eq "leftname">name like <cfqueryparam cfsqltype="cf_sql_varchar" value="#searchstr#%"><cfelse>#searchtype# like <cfqueryparam cfsqltype="cf_sql_varchar" value="%#searchstr#%"></cfif>
	<cfif alown eq 1>
		<cfif getGeneralInfo.agentlistuserid eq "Y">and (ucase(artran.agenno) in (SELECT agent FROM #target_icagent# WHERE agentlist like "%#ucase(huserid)#%") or ucase(artran.userid)='#ucase(huserid)#')
			<cfelse>
            and (ucase(artran.userid)='#ucase(huserid)#' or ucase(artran.agenno)='#ucase(huserid)#' or (ucase(agenno) in (SELECT agent FROM #target_icagent# WHERE ucase(agentid) like "%#ucase(huserid)#%")))  
			</cfif>
...
<cfif getrecordcount.recordcount neq 0>
	...
	<cfform action="stransaction_similar.cfm?tran=#urlencodedformat(tran)#&searchtype=#urlencodedformat(searchtype)#&searchstr=#urlencodedformat(searchstr)#" method="post" target="_self">
		...
		<cfelseif searchType eq "status"><cfif searchStr eq 'updated'>toinv<>''<cfelseif searchStr eq 'approved'>printstatus ='a3'<cfelse>(toinv='' or toinv is null)</cfif>
		<cfelseif searchtype eq "leftname">name like <cfqueryparam cfsqltype="cf_sql_varchar" value="#searchstr#%"><cfelse>artran.#searchtype# like <cfqueryparam cfsqltype="cf_sql_varchar" value="%#searchstr#%"></cfif>
				<cfif alown eq 1>
		...
		<cfelseif searchtype eq "leftname">name like <cfqueryparam cfsqltype="cf_sql_varchar" value="#searchstr#%"><cfelse>artran.#searchtype# like <cfqueryparam cfsqltype="cf_sql_varchar" value="%#searchstr#%"></cfif>
            ...
            and (ucase(artran.userid)='#ucase(huserid)#' or ucase(artran.agenno)='#ucase(huserid)#' or (ucase(agenno) in (SELECT agent FROM #target_icagent# WHERE ucase(agentid) like "%#ucase(huserid)#%")))  
				...
				<!--- and fperiod <> '99' ---> 
				order by <cfif (lcase(hcomid) eq "iel_i" or lcase(hcomid) eq "ielm_i") and tran eq "INV">substring_index(artran.refno,'/',-1) desc,substring_index(substring_index(artran.refno,'/',1),'.',-1) desc,substring_index(artran.refno,'.',1) desc<cfelse>artran.wos_date desc ,artran.refno desc</cfif> 
				limit #start-1#,20;
			</cfquery>
		</cfif>

		<cfif start neq 1>
			|| <a target="_self" href="stransaction_similar.cfm?tran=#tran#&start=#prevTwenty#&searchtype=#urlencodedformat(searchtype)#&searchstr=#urlencodedformat(searchstr)#">#words[972]#</a> ||
		</cfif>
		
		<cfif page neq noOfPage>
			<a target="_self" href="stransaction_similar.cfm?tran=#tran#&start=#nextTwenty#&searchtype=#urlencodedformat(searchtype)#&searchstr=#urlencodedformat(searchstr)#">#words[900]#</a> ||
		</cfif>
    	...
    	<th><div align="center">#words[10]#</div></th>
			...
			<cfloop query="getjob">
				<tr onMouseOut="javascript:this.style.backgroundColor='';" onMouseOver="javascript:this.style.backgroundColor='99FF00';">
                    ...
                    <td nowrap>#getjob.van# - #getvandesp.name#</td>
                    </cfif>
                    <cfif getdisplaysetup.bill_SO eq 'Y'>
                    ...
                    <cfif getdisplaysetup.billbody_dadd eq 'Y'>
        <td nowrap>#getjob.frem7# #getjob.frem8#</td>
        </cfif>
                    <cfif getdisplaysetup.bill_b_attn eq 'Y'>
