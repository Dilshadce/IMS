...
<!---Get Flexible Fix Allwance--->
<cfquery name="getFixAllowanceConfig" datasource='#dts#'>
    SELECT group_concat(allowance_id) as allowance_id_list FROM exclude_fix_allowances 
</cfquery>

<cfset allowance_id_list = listToArray(getFixAllowanceConfig.allowance_id_list)>
<!---Get Flexible Fix Allwance--->
...
<!---add on common variable for date used in OT and leave, [20170913, Alvin]--->
    <cfif #IsDefined('url.bdflag')# AND '#url.bdflag#' EQ 'Yes'>
        <cfset realstartdate = "#LsDateFormat(url.bdstartdate, 'yyyy-mm-dd', 'en_AU')#">
        <cfset realenddate = "#LsDateFormat(url.bdenddate, 'yyyy-mm-dd', 'en_AU')#">
    <cfelse>
        <cfset realstartdate = "#LsDateFormat(url.startdate, 'yyyy-mm-dd', 'en_AU')#">
        <cfset realenddate = "#LsDateFormat(url.enddate, 'yyyy-mm-dd', 'en_AU')#">
    </cfif>
    
    <cfoutput>
    <input type="hidden" name="timesheetstart" id="timesheetstart" value="#realstartdate#">
    <input type="hidden" name="timesheetend" id="timesheetend" value="#realenddate#">
    </cfoutput>
<!---add on common variable--->
...
<!---Commented by Nieo 20170926 1136--->
...
<!---proration rate for 2 month if timesheet cycle is not 01-31, [20170219, Alvin]--->
	<cfif month(lsdateformat(realstartdate, 'yyyy-mm-dd', "en_AU")) neq month(lsdateformat(realenddate, 'yyyy-mm-dd', "en_AU"))>
        <cfset endStartDate = dateformat(createdate(val(year(#realstartdate#)),val(month(#realstartdate#)),val(daysinmonth(#realstartdate#))), 'yyyy-mm-dd')>
        <cfset beginEndDate = dateadd('d',1,endStartDate)>
        
        <cfquery name="getNplTaken1" datasource="#replace(dts,'_i','_p')#">
            SELECT ifnull(sum(case ampm
                       when 'am' then 0.5
                       when 'pm' then 0.5
             ...
             AND pdate BETWEEN "#lsdateformat(realstartdate, 'yyyy-mm-dd', 'en_AU')#" AND "#lsdateformat(endStartDate, 'yyyy-mm-dd', 'en_AU')#"
             AND stcol = 'NPL'
     	</cfquery>
        ...
        <cfquery name="getNplTaken2" datasource="#replace(dts,'_i','_p')#">
            SELECT ifnull(sum(case ampm
             ...
             FROM timesheet 
             WHERE placementno = <cfqueryparam cfsqltype="cf_sql_varchar" value="#url.pno#">
             AND pdate BETWEEN '#lsdateformat(beginEndDate, 'yyyy-mm-dd', "en_AU")#' AND '#lsdateformat(realenddate, 'yyyy-mm-dd', "en_AU")#'
             AND stcol = 'NPL'
     	</cfquery>
        
        <cfset lvltype1 = "NPL">
		<cfset lvldesp1 = "No Pay Leave for #lsdateformat(realstartdate, 'mmm', "en_AU")#">
        <cfset lvltype2 = "NPL2">
		<cfset lvldesp2 = "No Pay Leave for #lsdateformat(realenddate, 'mmm', "en_AU")#">
        
        <!---to set a common variable for noofdays and special days for backdated timesheet, [20171114, Alvin]--->
        <cfif #IsDefined('url.bdflag')# AND '#url.bdflag#' EQ 'Yes'>
            <cfset realdayswork = DateDiff('d', '#realstartdate#', '#realenddate#') + 1>
        <cfelse>
            <cfset realdayswork = #noofdays#>
        </cfif>
        <!---to set a common variable--->
        ...
        <cfif val(noofdays) neq 0> <!---if eq 0 means work for 0 days--->
			<cfset lvleerate1 = numberformat(employeepay/daysinmonth(realstartdate),'.__')*-1>
            <cfset lvlerrate1 = numberformat(employerpay/daysinmonth(realstartdate),'.__')*-1>
            <cfset lvleerate2 = numberformat(employeepay/daysinmonth(realenddate),'.__')*-1>
            <cfset lvlerrate2 = numberformat(employerpay/daysinmonth(realenddate),'.__')*-1>
        <cfelse>
			<cfset lvleerate1 = 0>
            <cfset lvlerrate1 = 0>
            <cfset lvleerate2 = 0>
        ...
        <!---current month npl for backdated process only, [20171114, Alvin]--->
        <cfif #IsDefined('url.bdflag')# AND '#url.bdflag#' EQ 'Yes'>
        
            <cfset oriStartdate = #LsDateFormat(startdate, 'yyyy-mm-dd', 'en_AU')#>
            <cfset oriEnddate = #LsDateFormat(enddate, 'yyyy-mm-dd', 'en_AU')#>
            <cfset realdayswork2 = DateDiff('d', '#oriStartdate#', '#oriEnddate#') + 1>
            
            <cfset endStartDate2 = dateformat(createdate(val(year(#oriStartdate#)),val(month(#oriStartdate#)),val(daysinmonth(#oriStartdate#))), 'yyyy-mm-dd')>
            <cfset beginEndDate2 = dateadd('d',1,endStartDate2)>
            
            <cfquery name="getNplTaken3" datasource="#replace(dts,'_i','_p')#">
                SELECT ifnull(sum(case ampm
                           when 'am' then 0.5
                           when 'pm' then 0.5
                           when 'full day' then 1
                 ...
                 FROM timesheet 
                 WHERE placementno = <cfqueryparam cfsqltype="cf_sql_varchar" value="#url.pno#">
                 AND pdate BETWEEN "#lsdateformat(oriStartdate, 'yyyy-mm-dd', 'en_AU')#" AND "#lsdateformat(endStartDate2, 'yyyy-mm-dd', 'en_AU')#"
                 AND stcol = 'NPL'
            </cfquery>
           
            <cfquery name="getNplTaken4" datasource="#replace(dts,'_i','_p')#">
                SELECT ifnull(sum(case ampm
                           when 'am' then 0.5
                           when 'pm' then 0.5
                           when 'full day' then 1
                 ...
                 FROM timesheet 
                 WHERE placementno = <cfqueryparam cfsqltype="cf_sql_varchar" value="#url.pno#">
                 AND pdate BETWEEN '#lsdateformat(beginEndDate2, 'yyyy-mm-dd', "en_AU")#' AND '#lsdateformat(oriEnddate, 'yyyy-mm-dd', "en_AU")#'
                 AND stcol = 'NPL'
            </cfquery>
            
            <cfset lvltype3 = "NPL3">
            <cfset lvldesp3 = "No Pay Leave for #lsdateformat(oriStartdate, 'mmm', "en_AU")#">
            <cfset lvltype4 = "NPL4">
            <cfset lvldesp4 = "No Pay Leave for #lsdateformat(oriEnddate, 'mmm', "en_AU")#">
            
            <cfset lvleedayhr3 = #getNplTaken3.npltaken#>
            <cfset lvlerdayhr3 = #getNplTaken3.npltaken#>
            <cfset lvleedayhr4 = #getNplTaken4.npltaken#>
            <cfset lvlerdayhr4 = #getNplTaken4.npltaken#>
            
            <cfif val(noofdays) neq 0> <!---if eq 0 means work for 0 days--->
                <cfset lvleerate3 = numberformat(employeepay/val(realdayswork2),'.__')*-1>
                <cfset lvlerrate3 = numberformat(employerpay/val(realdayswork2),'.__')*-1>
                <cfset lvleerate4 = numberformat(employeepay/val(realdayswork2),'.__')*-1>
                <cfset lvlerrate4 = numberformat(employerpay/val(realdayswork2),'.__')*-1>
            <cfelse>
                <cfset lvleerate3 = 0>
                <cfset lvlerrate3 = 0>
                <cfset lvleerate4 = 0>
    ...
    <cfelse> <!---cycle is 01-31--->
        ...
        <cfquery name="getnpltaken" datasource="#replace(dts,'_i','_p')#">
            SELECT ifnull(sum(case ampm
             ...
             FROM timesheet 
             WHERE placementno = <cfqueryparam cfsqltype="cf_sql_varchar" value="#url.pno#">
             AND pdate BETWEEN '#lsdateformat(realstartdate, 'yyyy-mm-dd', "en_AU")#' AND '#lsdateformat(realenddate, 'yyyy-mm-dd', "en_AU")#'
         	 ANd stcol = 'NPL'
        </cfquery>
        ...
        <cfif val(noofdays) neq 0> <!---if eq 0 means work for 0 days--->
            <cfset lvleerate1 = numberformat(employeepay/val(realdayswork),'.__')*-1>
            <cfset lvlerrate1 = numberformat(employerpay/val(realdayswork),'.__')*-1>
        <cfelse>
            <cfset lvleerate1 = 0>
            <cfset lvlerrate1 = 0>
        </cfif>
        
        <!---npl for current month, [20171114, Alvin]--->
        <cfif #IsDefined('url.bdflag')# AND '#url.bdflag#' EQ 'Yes'>
            ...
            <cfquery name="getnpltaken2" datasource="#replace(dts,'_i','_p')#">
                SELECT ifnull(sum(case ampm
                 ...
                 FROM timesheet 
                 WHERE placementno = <cfqueryparam cfsqltype="cf_sql_varchar" value="#url.pno#">
                 AND pdate BETWEEN '#lsdateformat(oriStartdate, 'yyyy-mm-dd', "en_AU")#' AND '#lsdateformat(oriEnddate, 'yyyy-mm-dd', "en_AU")#'
                 ANd stcol = 'NPL'
            </cfquery>
            
            <cfset lvltype2 = "NPL2">
            <cfset lvldesp2 = "No Pay Leave for #lsdateformat(oriStartdate, 'mmm', "en_AU")#">
            <cfset lvleedayhr2 = #getnpltaken2.npltaken#>
            <cfset lvlerdayhr2 = #getnpltaken2.npltaken#>
            <cfset lvleerate2 = numberformat(employeepay/val(realdayswork2),'.__')*-1>
            <cfset lvlerrate2 = numberformat(employerpay/val(realdayswork2),'.__')*-1>
                
        <cfelse>
            <!---Added by Nieo, NPL for previous month Loreal or Accenture--->
            <!---<cfif findnocase("l'oreal",getplacement.custname) or findnocase("accenture",getplacement.custname)>--->

                <cfset prevStartdate = #LsDateFormat(startdate, 'yyyy-mm-dd', 'en_AU')#>
                <cfset prevmonth = #dateAdd('m', -1, prevStartdate)#>

                <cfset lvltype2 = "NPL2">
                <cfset lvldesp2 = "No Pay Leave for #lsdateformat(prevmonth, 'mmm', "en_AU")#">
                <cfset lvleedayhr2 = 0>
                <cfset lvlerdayhr2 = 0>
                <cfset lvleerate2 = numberformat(employeepay/daysinmonth(prevmonth),'.__')*-1>
                <cfset lvlerrate2 = numberformat(employerpay/daysinmonth(prevmonth),'.__')*-1>

            <!---</cfif>--->
            <!---Added by Nieo, NPL for previous month Loreal or Accenture--->
        </cfif>
        <!---npl for current month--->                
        
    </cfif>
<!---proration for NPL--->
</cfif>
                
<cfif isdefined('url.newworkd')>
    <cfif lcase(url.newworkd) eq 'y'>
        <cfset nextmonth = month(datestartof)+1>
        <cfset yearto = year(datestartof)>
            
        <cfif nextmonth eq 13>
            <cfset nextmonth = 1>
            <cfset yearto = year(datestartof)+1>
        </cfif>
            
        <cfset noofdays = businessDaysBetween(createDate(year(datestartof),month(datestartof),1),dateAdd('d',-1,createDate(yearto,nextmonth,1)))>        
        <cfset selfsalarydayrecord = businessDaysBetween(createDate(year(datestartof),month(datestartof),day(datestartof)),createDate(year(dateendof),month(dateendof),day(dateendof)))>
        <cfset custsalarydayrecord = selfsalarydayrecord>
            
        <cfset selfsalaryrecord = numberformat(employeepay,'.__') * numberformat((selfsalarydayrecord / noofdays),'.__')>
        <cfset custsalaryrecord = numberformat(employerpay,'.__') * numberformat((custsalarydayrecord / noofdays),'.__')>
    </cfif>
</cfif>

<!--- For Samsung meal allowance, [20171019, Alvin]--->
<!---<cfif #IsDefined('url.bdflag')# AND '#url.bdflag#' EQ 'Yes'>
    
    <cfquery name="getMealOT" datasource="#replace(dts,'_i','_p')#">
        SELECT  CASE stcol 
                    WHEN "" THEN 'WEEKDAY'
                    WHEN "OD" THEN 'WEEKEND'
                    WHEN "RD" THEN 'WEEKEND'
                    WHEN "PH" THEN 'WEEKEND' END AS daytype,
                IFNULL(SUM(
                            CASE stcol 
                            WHEN 'OD' THEN (CASE WHEN workhours >= 4 AND workhours < 8 THEN 50 WHEN workhours >= 8 THEN 100 END)
                            WHEN 'RD' THEN (CASE WHEN workhours >= 4 AND workhours < 8 then 50 WHEN workhours >= 8 THEN 100 END)
                            WHEN 'PH' THEN (CASE WHEN workhours >= 4 AND workhours < 8 then 50 WHEN workhours >= 8 THEN 100 END)
                            WHEN '' THEN (CASE WHEN ot3 != 0 then 30 WHEN ot2 != 0 THEN 20 ELSE 0 END)
                            END)
                ,0) AS MEAL
        FROM timesheet
        WHERE placementno = <cfqueryparam cfsqltype="cf_sql_varchar" value="#url.pno#">
    ...
    <cfloop query="getMealOT">
        <cfif #getMealOT.daytype# NEQ "">
            <cfset "#getMealOT.daytype#meal" = "#getMealOT.meal#">   
        </cfif> 
    </cfloop>
    
</cfif>--->
<!---For Samsung meal allowance--->
...
<cfset totalfixaw = 0>
  ...
  <cfloop from="1" to="6" index="i">
      ...
      <cfif evaluate('getplacement.allowancebillable#i#') eq "Y">
        <cfquery name="getpbbillrate" datasource="#dts#">
            SELECT dballid,dbjoballpayrate,dbjoballbillrate,dbjoballqty FROM ftjoballow
            WHERE dbjobno =<cfqueryparam cfsqltype="cf_sql_varchar" value="#url.pno#">
            AND dballid=<cfqueryparam cfsqltype="cf_sql_varchar" value="#evaluate('fixawcode#i#')#">
            AND dbjoballpayrate=<cfqueryparam cfsqltype="cf_sql_varchar" value="#evaluate('getplacement.allowanceamt#i#')#">
        </cfquery>
        <cfset "fixawer#i#" = val(getpbbillrate.dbjoballbillrate)>
      	<cfif evaluate('getplacement.prorated#i#') eq "Y">
        	<cfif getplacement.clienttype eq "hr">
            <cfset "fixawer#i#" = numberformat(val(getpbbillrate.dbjoballbillrate) * val(selfsalaryhrsrecord) / val(counthour),'.__')>
            <cfelse>
            <cfset "fixawer#i#" = numberformat(val(getpbbillrate.dbjoballbillrate) * val(selfsalarydayrecord) / val(noofdays),'.__')>
			</cfif>
        </cfif>
	  </cfif>
        ...
        <cfif not ArrayContains(allowance_id_list, evaluate('getplacement.aw#i#'))>
            <cfset customtotalfixaw =customtotalfixaw + numberformat(evaluate("fixawee#i#"),'.__')>
        </cfif>
      </cfif>
      </cfloop>
...
<cfquery name="geticshelf" datasource="#dts#">
  ...
  <cfif getplacement.pm neq "">
      ...
      <cfloop query="geticshelf">
        ...
        <cfloop query="getpm">
            ...
            <cfelseif getpm.itemid eq "B-136">
                <cfif getpm.billable eq "Y">
                    <cfset BASIC = selfusualpay>
                    <cfset monthlybillableitemB136 = numberformat(evaluate(replace(getpm.billableamt,'=','')),'.__')>
                </cfif>
            </cfif>
        </cfloop>
		</cfif>
  ...
  <cfif isdefined('totalfixawamtee')>
        ...
        <cfset custnet= "0.00">
        <cfset taxcode="SR">        
        <cfif datediff('d',lsdateformat("01/06/2018",'YYYY-MM-DD', 'en_AU'),lsdateformat((url.enddate),'YYYY-MM-DD', 'en_AU')) lt 0 or datediff('d',lsdateformat("31/08/2018",'YYYY-MM-DD', 'en_AU'),lsdateformat((url.enddate),'YYYY-MM-DD', 'en_AU')) gt 0>
            <cfset taxper="6.00">
        <cfelse>
            <cfset taxper="#numberformat(getGsetup.gst,'.__')#">
        </cfif>
        <cfset taxamt="0.00">
...
SELECT * FROM placement WHERE placementno = <cfqueryparam cfsqltype="cf_sql_varchar" value="#getitem.placementno#">
  ...
  <cfloop from="5" to="8" index="c">
        ...
        <cfset selfeis= getitem.selfeis>
        <cfset custeis= getitem.custeis>
        ...
        <cfset lorealremarks = getitem.lorealremarks>
  ...
  <cfloop from="1" to="6" index="i">
        ...
        <cfset "fixawdesp#i#" = evaluate('getitem.fixawdesp#i#')>
            <cfset "fixaweerate#i#" = numberformat(evaluate('getitem.fixaweerate#i#'),'.__')>
            <cfset "fixawerrate#i#" = numberformat(evaluate('getitem.fixawerrate#i#'),'.__')>
            <cfset "fixaweeqty#i#" = numberformat(evaluate('getitem.fixaweeqty#i#'),'.__')>
            <cfset "fixawerqty#i#" = numberformat(evaluate('getitem.fixawerqty#i#'),'.__')>
        ...
        <cfset "fixaworiamt#i#" = numberformat(evaluate('getitem.fixaworiamt#i#'),'.__')>
            ...
            <!--- for HRDF --->
            <cfif not ArrayContains(allowance_id_list, evaluate('getitem.fixawcode#i#'))>
                <cfset customtotalfixaw =customtotalfixaw + numberformat(evaluate("fixawee#i#"),'.__')>
            </cfif>
            <!--- for HRDF --->
  ...
  <cfloop from="1" to="18" index="a">
        ...
        <cfset "allowancedesp#a#" = evaluate('getitem.allowancedesp#a#')>
            <cfset "aweerate#a#" = numberformat(evaluate('getitem.aweerate#a#'),'.__')>
            <cfset "awerrate#a#" = numberformat(evaluate('getitem.awerrate#a#'),'.__')>
            <cfset "aweeqty#a#" = numberformat(evaluate('getitem.aweeqty#a#'),'.__')>
            <cfset "awerqty#a#" = numberformat(evaluate('getitem.awerqty#a#'),'.__')>
...
<cfoutput>
    <input type="hidden" id="customtotalfixaw" name="customtotalfixaw" value="#customtotalfixaw#">
</cfoutput>
...
<table align="center" class="data" >
      ...
      <th>Working Day</th>
      <td><input type="text" size='10' name="workd" id="workd" value="#workd#" onKeyUp="selfnpl();custnpl(); " onBlur="selfnpl();custnpl(); "></td>
      <td></td>
      <td></td>
      <td></td>
      <td></td>
      ...
      <tr>
          <th colspan="6"><div align="right">Basic Rate</div></th>
                <td><input type="text" name="selfusualpay" id="selfusualpay" value="#numberformat(selfusualpay,'.__')#" onKeyUp="calculateselfhour();" onBlur="calculateselfhour();"  size='10'></td>
          <td style="background-color:#bgcolor#"><input type="text" name="custusualpay" id="custusualpay" value="#numberformat(custusualpay,'_.__')#" onKeyUp="calculatecusthour();" onBlur="calculatecusthour();"  size='10'></td>
      </tr>
...
Please verify to ensure amounts are correct. </font></td></cfif>
    ...
    AND placementno = '#pno#'
    <!--- and tmonth = '#url.auto#'--->
    AND pdate between "#lsdateformat(realstartdate, 'yyyy-mm-dd', 'en_AU')#"
    AND "#lsdateformat(realenddate,'yyyy-mm-dd', 'en_AU')#"
    ORDER by pdate    
    </cfquery>
   ...
   <cfset custothour8 =val(getot.ot8)>
    ...
    <cfif val(getalldays.workhours) neq 0>
		<cfif getalldays.ampm eq "am" AND getalldays.ampm eq "pm">
...
order by  id
	 ...
	 <cfelse>
      ...
      <th>NPL</th>
      <td></td>
      <td><input type="text" size='10' name="NPL" id="NPL" value="#NPL#" onKeyUp="selfnpl()" onBlur="selfnpl()"></td>
      <td  style="background-color:#bgcolor#"><input type="text" size='10' name="CUSTNPL" id="CUSTNPL" value="#CUSTNPL#" onKeyUp="custnpl()" onBlur="custnpl()"></td>
      </tr>
      <tr>
      <th>Days Worked</th>
      <td></td>
      <td><input type="text" size='10' name="selfsalaryday" id="selfsalaryday" value="#selfsalaryday#" onKeyUp="setselfallow();" onBlur="setselfallow();"></td>
      <td style="background-color:#bgcolor#">
      <input type="text" size='10' name="custsalaryday" id="custsalaryday" value="#custsalaryday#" onKeyUp="setcustallow();" onBlur="setcustallow();"> 
      </td>
      </tr>
      ...
      <th>Hours Worked</th>
      <td></td>
      <td><input type="text" size='10' name="selfsalaryhrs" id="selfsalaryhrs" value="#selfsalaryhrs#" onKeyUp="setselfallow();" onBlur="setselfallow();"></td>
      <td style="background-color:#bgcolor#"><input type="text" size='10' name="custsalaryhrs" id="custsalaryhrs" value="#custsalaryhrs#" onKeyUp="setcustallow();" onBlur="setcustallow();"></td>
      </tr>
      ...
      <td style="background-color:#bgcolor#" colspan="3"><input type="text" name="invdesp" id="invdesp" value="#invdesp#" size="64" /></td>
      <td><input type="text" size="10" name="firstrate" id="firstrate" value="#numberformat(firstrate,'.__')#" onkeyup="calextra();" onBlur="calextra();" /></td>
      <th><div align="right">Basic Pay</div></th>
             <td><input type="text" size='10' name="selfsalary" id="selfsalary" value="#numberformat(selfsalary,'_.__')#" onKeyUp="<cfif findnocase('venture',getplacement.custname) or findnocase('Straits Orthopaedics',getplacement.custname)>checkforOT();</cfif>calselftotal();" onBlur="<cfif findnocase('venture',getplacement.custname) or findnocase('Straits Orthopaedics',getplacement.custname)>checkforOT();refreshotrate();</cfif>calselftotal();"></td>
       <td style="background-color:#bgcolor#"><input type="text" size='10' name="custsalary" id="custsalary" value="#numberformat(custsalary,'_.__')#" onKeyUp="<cfif findnocase('venture',getplacement.custname) or findnocase('Straits Orthopaedics',getplacement.custname)>checkforOT();</cfif>calcusttotal();" onBlur="<cfif findnocase('venture',getplacement.custname) or findnocase('Straits Orthopaedics',getplacement.custname)>checkforOT();refreshotrate();</cfif>calcusttotal();"></td>
      </tr>
      <tr>
      <th style="background-color:#bgcolor#">Invoice Desp 2:</th>
      <td style="background-color:#bgcolor#" colspan="3"><input type="text" name="invdesp2" id="invdesp2" value="#invdesp2#" size="64" /></td>
      <td><input type="text" size="10" name="secondrate" id="secondrate" value="#numberformat(secondrate,'.__')#" onkeyup="calextra();" onBlur="calextra();"/></td>
      </tr>
      <tr>
      <th>Pay Slip Desp:</th>
      ...
      <td>
      <input type="text" name="lvleedayhr#i#" id="lvleedayhr#i#" value="#evaluate('lvleedayhr#i#')#" size="10" onkeyup="leaveself();document.getElementById('lvlerdayhr#i#').value=this.value;leavecust();" onBlur="leaveself();document.getElementById('lvlerdayhr#i#').value=this.value;leavecust();"/>
      </td>
      <td>
      <input type="text" name="lvleerate#i#" id="lvleerate#i#" value="#numberformat(evaluate('lvleerate#i#'),'_.__')#" size="10" onkeyup="leaveself();" onBlur="leaveself();"/>
      </td>
      <td style="background-color:#bgcolor#">
      <input type="text" name="lvlerdayhr#i#" id="lvlerdayhr#i#" value="#evaluate('lvlerdayhr#i#')#" size="10" onkeyup="leavecust();" onBlur="leavecust();"/>
      </td>
      <td style="background-color:#bgcolor#">
      <input type="text" name="lvlerrate#i#" id="lvlerrate#i#" value="#numberformat(evaluate('lvlerrate#i#'),'_.__')#" size="10" onkeyup="leavecust();" onBlur="leavecust();"/>
      </td>
      <td>
      ...
      <td>
      <input type="text" name="lvleedayhr#i#" id="lvleedayhr#i#" value="0.00" size="10" onkeyup="leaveself();" onBlur="leaveself();" readOnly />
      </td>
      <td>
      <input type="text" name="lvleerate#i#" id="lvleerate#i#" value="0.00" size="10" onkeyup="leaveself();" onBlur="leaveself();" readOnly />
      </td>
      <td style="background-color:#bgcolor#">
      <input type="text" name="lvlerdayhr#i#" id="lvlerdayhr#i#" value="0.00" size="10" onkeyup="leavecust();" onBlur="leavecust();" readOnly />
      </td>
      <td style="background-color:#bgcolor#">
      <input type="text" name="lvlerrate#i#" id="lvlerrate#i#" value="0.00" size="10" onkeyup="leavecust();" onBlur="leavecust();" readOnly />
      </td>
      <td>
      ...
      <td>
      <input type="hidden" name="lvltotaler#i#" id="lvltotaler#i#" value="0.00" />
      </td>
      </tr>
      <cfelse>
      ...
      <input type="hidden" name="lvlhr#i#" id="lvlhr#i#" value="" />
        ...
        <!---Added by Nieo 20180522, for Loreal Remarks--->
        <cfif findnocase("l'oreal",getplacement.custname)>
            <tr>
                <th>Remarks <br>For Loreal Report </th>
                <td>
                  <input type="text" name="lorealremarks" id="lorealremarks" value="<cfif isdefined('lorealremarks')>#lorealremarks#</cfif>" size="30"/>
                </td>
            </tr>
        <cfelse>
            <input type="hidden" name="lorealremarks" id="lorealremarks" value="<cfif isdefined('lorealremarks')>#lorealremarks#</cfif>" size="30"/>
        </cfif>
        <!---Added by Nieo 20180522, for Loreal Remarks--->
	 
      <!---Add samsung meal indicator, [20171019, Alvin]--->
      <!---<cfif #IsDefined('url.bdflag')# AND '#url.bdflag#' EQ 'Yes'>
          <tr>
              <th><div align="left"><!--- PH and Leaves ---></div></th>
              <th><div align="left">Total Meal Earned</div></th>
              <th><div align="left">Final Meal Earned</div></th>
              <!---<th style="background-color:#bgcolor#"><div align="left">Hour/Day</div></th>
              <th style="background-color:#bgcolor#"><div align="left">Rate</div></th>
              <th style="background-color:#bgcolor#"><!--- <div align="left">Hrs for D/M EE</div> ---></th>--->
          </tr>
          <cfloop query="getMealOT">
              <cfif #getMealOT.daytype# NEQ "">
                  <tr>
                      <th>
                          #getMealOT.daytype# (#lsdateformat(realstartdate, 'mmm', "en_AU")#)
                      </th>
                      <td>
                          <input type="text" name="#getMealOT.daytype#meal" id=" #getMealOT.daytype#meal" value="#getMealOT.meal#" size="30" />
                      </td>
                      <cfif #getMealOT.daytype# EQ 'WEEKDAY'>
                          <cfif #getMealOT.meal# GTE 300>
                              <td><input type="text" name="#getMealOT.daytype#final" id="#getMealOT.daytype#final" value="300" size="10"></td>
                          <cfelse>
                              <td><input type="text" name="#getMealOT.daytype#final" id="#getMealOT.daytype#final" value="#getMealOT.meal#" size="10"></td>
                          </cfif>
                      <cfelse>
                          <cfif #getMealOT.meal# GTE 500>
                              <td><input type="text" name="#getMealOT.daytype#final" id="#getMealOT.daytype#final" value="500" size="10"></td>
                          <cfelse>
                              <td><input type="text" name="#getMealOT.daytype#final" id="#getMealOT.daytype#final" value="#getMealOT.meal#" size="10"></td>
                          </cfif>
                      </cfif> 
      ...
      <!---Add samsung meal indicator--->
     
      <tr style="display:none">
       <th colspan="6"><div align="right">Paid PH and Leaves</div></th>
             <td><input type="text" size='10' name="selfphnlsalary" id="selfphnlsalary" value="#numberformat(selfphnlsalary,'_.__')#" onKeyUp="calselftotal();" onBlur="calselftotal();" readOnly></td>
       <td style="background-color:#bgcolor#"><input type="text" size='10' name="custphnlsalary" id="custphnlsalary" value="#numberformat(custphnlsalary,'_.__')#" onKeyUp="calcusttotal();" onBlur="calcusttotal();" readOnly></td>
      </tr>
      <tr>
      <td>&nbsp;</td>
      </tr>
      ...
      <td></td>
      <td>
      <input type="text" size='10' name="selfotrate1" id="selfotrate1" value="#selfotrate1#" onKeyUp="calselfot1();" onBlur="calselfot1();"> </td>
      <td><input type="text" size='10' name="selfothour1" id="selfothour1" value="#selfothour1#" onKeyUp="calselfot1();document.getElementById('custothour1').value=this.value;calcustot1();" onBlur="calselfot1();document.getElementById('custothour1').value=this.value;calcustot1();"> </td>
      
      <td style="background-color:#bgcolor#">
      <input type="text" size='10' name="custotrate1" id="custotrate1" value="#custotrate1#" onKeyUp="calcustot1();" onBlur="calcustot1();"> </td>
      <td style="background-color:#bgcolor#"><input type="text" size='10' name="custothour1" id="custothour1" value="#custothour1#" onKeyUp="calcustot1();" onBlur="calcustot1();" > </td>

       <td><input type="text" size='10' name="selfot1" id="selfot1" value="#numberformat(selfot1,'_.__')#" readOnly></td><td style="background-color:#bgcolor#"><input type="text" size='10' name="custot1" id="custot1" value="#numberformat(custot1,'_.__')#" readOnly></td>
      </tr>
      ...
      <th>OT 1.5</th>
       <td></td>
      <td>
      <input type="text" size='10' name="selfotrate2" id="selfotrate2" value="#selfotrate2#" onKeyUp="calselfot2();" onBlur="calselfot2();"> </td>
      <td><input type="text" size='10' name="selfothour2" id="selfothour2" value="#selfothour2#" onKeyUp="calselfot2();document.getElementById('custothour2').value=this.value;calcustot2();" onBlur="calselfot2();document.getElementById('custothour2').value=this.value;calcustot2();" > </td>
      
      <td style="background-color:#bgcolor#">
      <input type="text" size='10' name="custotrate2" id="custotrate2" value="#custotrate2#" onKeyUp="calcustot2();" onBlur="calcustot2();"> </td>
      <td style="background-color:#bgcolor#"><input type="text" size='10' name="custothour2" id="custothour2" value="#custothour2#" onKeyUp="calcustot2();" onBlur="calcustot2();" > </td>

       <td><input type="text" size='10' name="selfot2"  id="selfot2" value="#numberformat(selfot2,'_.__')#" readOnly></td><td style="background-color:#bgcolor#"><input type="text" size='10' name="custot2" id="custot2" value="#numberformat(custot2,'_.__')#" readOnly></td>
      </tr>
      ...
      <th>OT 2.0</th>
       <td></td>
      <td>
      <input type="text" size='10' name="selfotrate3" id="selfotrate3" value="#selfotrate3#" onKeyUp="calselfot3();" onBlur="calselfot3();"> </td>
      <td><input type="text" size='10' name="selfothour3" id="selfothour3" value="#selfothour3#" onKeyUp="calselfot3();document.getElementById('custothour3').value=this.value;calcustot3();" onBlur="calselfot3();document.getElementById('custothour3').value=this.value;calcustot3();" > </td>
      
      <td style="background-color:#bgcolor#">
      <input type="text" size='10' name="custotrate3" id="custotrate3" value="#custotrate3#" onKeyUp="calcustot3();" onBlur="calcustot3();"> </td>
      <td style="background-color:#bgcolor#"><input type="text" size='10' name="custothour3" id="custothour3" value="#custothour3#" onKeyUp="calcustot3();" onBlur="calcustot3();" > </td>

       <td><input type="text" size='10' name="selfot3" id="selfot3" value="#numberformat(selfot3,'_.__')#" readOnly></td><td style="background-color:#bgcolor#"><input type="text" size='10' name="custot3" id="custot3" value="#numberformat(custot3,'_.__')#" readOnly></td>
      </tr>
      ...
      <th>OT 3.0</th>
       <td></td>
      <td>
      <input type="text" size='10' name="selfotrate4" id="selfotrate4" value="#selfotrate4#" onKeyUp="calselfot4();" onBlur="calselfot4();"> </td>
      <td><input type="text" size='10' name="selfothour4" id="selfothour4" value="#selfothour4#" onKeyUp="calselfot4();document.getElementById('custothour4').value=this.value;calcustot4();" onBlur="calselfot4();document.getElementById('custothour4').value=this.value;calcustot4();" > </td>
    
      <td style="background-color:#bgcolor#">
      <input type="text" size='10' name="custotrate4" id="custotrate4" value="#custotrate4#" onKeyUp="calcustot4();" onBlur="calcustot4();"> </td>
      <td style="background-color:#bgcolor#"><input type="text" size='10' name="custothour4" id="custothour4" value="#custothour4#" onKeyUp="calcustot4();" onBlur="calcustot4();" > </td>

       <td><input type="text" size='10' name="selfot4" id="selfot4" value="#numberformat(selfot4,'_.__')#" readOnly></td><td style="background-color:#bgcolor#"><input type="text" size='10' name="custot4" id="custot4" value="#numberformat(custot4,'_.__')#" readOnly></td>
      </tr>
      <tr>
      <th>RD 1.0</th>
       <td></td>
      <td>
      <input type="text" size='10' name="selfotrate5" id="selfotrate5" value="#selfotrate5#" onKeyUp="calselfot(5);" onBlur="calselfot(5);"> </td>
      <td><input type="text" size='10' name="selfothour5" id="selfothour5" value="#selfothour5#" onKeyUp="calselfot(5);document.getElementById('custothour5').value=this.value;calcustot(5);" onBlur="calselfot(5);document.getElementById('custothour5').value=this.value;calcustot(5);" > </td>
    
      <td style="background-color:#bgcolor#">
      <input type="text" size='10' name="custotrate5" id="custotrate5" value="#custotrate5#" onKeyUp="calcustot(5);" onBlur="calcustot(5);"> </td>
      <td style="background-color:#bgcolor#"><input type="text" size='10' name="custothour5" id="custothour5" value="#custothour5#" onKeyUp="calcustot(5);" onBlur="calcustot(5);" > </td>

       <td><input type="text" size='10' name="selfot5" id="selfot5" value="#numberformat(selfot5,'_.__')#" readOnly></td><td style="background-color:#bgcolor#"><input type="text" size='10' name="custot5" id="custot5" value="#numberformat(custot5,'_.__')#" readOnly></td>
      </tr>
      <tr>
      <th>RD 2.0</th>
       <td></td>
      <td>
      <input type="text" size='10' name="selfotrate6" id="selfotrate6" value="#selfotrate6#" onKeyUp="calselfot(6);" onBlur="calselfot(6);"> </td>
      <td><input type="text" size='10' name="selfothour6" id="selfothour6" value="#selfothour6#" onKeyUp="calselfot(6);document.getElementById('custothour6').value=this.value;calcustot(6);" onBlur="calselfot(6);document.getElementById('custothour6').value=this.value;calcustot(6);" > </td>
    
      <td style="background-color:#bgcolor#">
      <input type="text" size='10' name="custotrate6" id="custotrate6" value="#custotrate6#" onKeyUp="calcustot(6);" onBlur="calcustot(6);"> </td>
      <td style="background-color:#bgcolor#"><input type="text" size='10' name="custothour6" id="custothour6" value="#custothour6#" onKeyUp="calcustot(6);" onBlur="calcustot(6);" > </td>

       <td><input type="text" size='10' name="selfot6" id="selfot6" value="#numberformat(selfot6,'_.__')#" readOnly></td><td style="background-color:#bgcolor#"><input type="text" size='10' name="custot6" id="custot6" value="#numberformat(custot6,'_.__')#" readOnly></td>
      </tr>
      ...
      <th>PH 1.0</th>
       <td></td>
      <td>
      <input type="text" size='10' name="selfotrate7" id="selfotrate7" value="#selfotrate7#" onKeyUp="calselfot(7);" onBlur="calselfot(7);"> </td>
      <td><input type="text" size='10' name="selfothour7" id="selfothour7" value="#selfothour7#" onKeyUp="calselfot(7);document.getElementById('custothour7').value=this.value;calcustot(7);" onBlur="calselfot(7);document.getElementById('custothour7').value=this.value;calcustot(7);" > </td>
    
      <td style="background-color:#bgcolor#">
      <input type="text" size='10' name="custotrate7" id="custotrate7" value="#custotrate7#" onKeyUp="calcustot(7);" onBlur="calcustot(7);"> </td>
      <td style="background-color:#bgcolor#"><input type="text" size='10' name="custothour7" id="custothour7" value="#custothour7#" onKeyUp="calcustot(7);" onBlur="calcustot(7);" > </td>

       <td><input type="text" size='10' name="selfot7" id="selfot7" value="#numberformat(selfot7,'_.__')#" readOnly></td><td style="background-color:#bgcolor#"><input type="text" size='10' name="custot7" id="custot7" value="#numberformat(custot7,'_.__')#" readOnly></td>
      </tr>
      <tr>
      <th>PH 2.0</th>
       <td></td>
      <td>
      <input type="text" size='10' name="selfotrate8" id="selfotrate8" value="#selfotrate8#" onKeyUp="calselfot(8);" onBlur="calselfot(8);"> </td>
      <td><input type="text" size='10' name="selfothour8" id="selfothour8" value="#selfothour8#" onKeyUp="calselfot(8);document.getElementById('custothour8').value=this.value;calcustot(8);" onBlur="calselfot(8);document.getElementById('custothour8').value=this.value;calcustot(8);" > </td>
    
      <td style="background-color:#bgcolor#">
      <input type="text" size='10' name="custotrate8" id="custotrate8" value="#custotrate8#" onKeyUp="calcustot(8);" onBlur="calcustot(8);"> </td>
      <td style="background-color:#bgcolor#"><input type="text" size='10' name="custothour8" id="custothour8" value="#custothour8#" onKeyUp="calcustot(8);" onBlur="calcustot(8);"> </td>

       <td><input type="text" size='10' name="selfot8" id="selfot8" value="#numberformat(selfot8,'_.__')#" readOnly></td><td style="background-color:#bgcolor#"><input type="text" size='10' name="custot8" id="custot8" value="#numberformat(custot8,'_.__')#" readOnly></td>
      </tr>
      ...
      <tr>
      <th><div align="left">Fixed Allowance Code</div></th>
      ...
      <tr>
      <td><input type="text" name="fixawcode#i#" id="fixawcode#i#" value="#evaluate('fixawcode#i#')#" readOnly/></td>
      <td><input type="text" name="fixawdesp#i#" id="fixawdesp#i#" size="30" value="#evaluate('fixawdesp#i#')#"  readOnly/></td>
        <td>
            <input type="number" name="fixaweerate#i#" id="fixaweerate#i#" value="<cfif isdefined('fixaweerate#i#')>#numberformat(evaluate('fixaweerate#i#'),'.__')#<cfelseif val(evaluate('fixawee#i#')) neq 0>#numberformat(evaluate('fixawee#i#'),'.__')#<cfelse>0.00</cfif>" onkeyup="if(this.readOnly == false){document.getElementById('fixawerrate#i#').value=this.value;calcvaraw(this.value,document.getElementById('fixaweeqty#i#').value,'fixawee#i#');calselfallow();calcvaraw(document.getElementById('fixawerrate#i#').value,document.getElementById('fixawerqty#i#').value,'fixawer#i#');calcustallow();};<cfif findnocase('venture',getplacement.custname) or findnocase('Straits Orthopaedics',getplacement.custname)>refreshotrate();</cfif>" onBlur="if(this.readOnly == false){document.getElementById('fixawerrate#i#').value=this.value;calcvaraw(this.value,document.getElementById('fixaweeqty#i#').value,'fixawee#i#');calselfallow();calcvaraw(document.getElementById('fixawerrate#i#').value,document.getElementById('fixawerqty#i#').value,'fixawer#i#');calcustallow();};<cfif findnocase('venture',getplacement.custname) or findnocase('Straits Orthopaedics',getplacement.custname)>refreshotrate();</cfif>" style="width: 100px"  />
        </td>
        <td>
            <input type="number" name="fixaweeqty#i#" id="fixaweeqty#i#" value="<cfif isdefined('fixaweeqty#i#')>#numberformat(evaluate('fixaweeqty#i#'),'.__')#<cfelseif evaluate('fixawee#i#') neq 0>1<cfelse>0</cfif>" onkeyup="if(this.readOnly == false){document.getElementById('fixawerqty#i#').value=this.value;calcvaraw(document.getElementById('fixaweerate#i#').value,this.value,'fixawee#i#');calselfallow();calcvaraw(document.getElementById('fixawerrate#i#').value,document.getElementById('fixawerqty#i#').value,'fixawer#i#');calcustallow();};<cfif findnocase('venture',getplacement.custname) or findnocase('Straits Orthopaedics',getplacement.custname)>checkforOT();</cfif><cfif findnocase('venture',getplacement.custname) or findnocase('Straits Orthopaedics',getplacement.custname)>refreshotrate();</cfif>" onBlur="if(this.readOnly == false){document.getElementById('fixawerqty#i#').value=this.value;calcvaraw(document.getElementById('fixaweerate#i#').value,this.value,'fixawee#i#');calselfallow();calcvaraw(document.getElementById('fixawerrate#i#').value,document.getElementById('fixawerqty#i#').value,'fixawer#i#');calcustallow();};<cfif findnocase('venture',getplacement.custname) or findnocase('Straits Orthopaedics',getplacement.custname)>checkforOT();</cfif><cfif findnocase('venture',getplacement.custname) or findnocase('Straits Orthopaedics',getplacement.custname)>refreshotrate();</cfif>" style="width: 100px"  />
        </td>
        <td style="background-color:#bgcolor#">
            <input type="number" name="fixawerrate#i#" id="fixawerrate#i#" value="<cfif isdefined('fixawerrate#i#')>#numberformat(evaluate('fixawerrate#i#'),'.__')#<cfelseif evaluate('fixawer#i#') neq 0>#numberformat(evaluate('fixawer#i#'),'.__')#<cfelse>0.00</cfif>" onkeyup="calcvaraw(this.value,document.getElementById('fixawerqty#i#').value,'fixawer#i#');calcustallow();<cfif findnocase('venture',getplacement.custname) or findnocase('Straits Orthopaedics',getplacement.custname)>refreshotrate();</cfif>" onBlur="calcvaraw(this.value,document.getElementById('fixawerqty#i#').value,'fixawer#i#');calcustallow();<cfif findnocase('venture',getplacement.custname) or findnocase('Straits Orthopaedics',getplacement.custname)>refreshotrate();</cfif>" style="width: 100px"  />
        </td>
        <td style="background-color:#bgcolor#">
            <input type="number" name="fixawerqty#i#" id="fixawerqty#i#" value="<cfif isdefined('fixawerqty#i#')>#numberformat(evaluate('fixawerqty#i#'),'.__')#<cfelseif evaluate('fixawer#i#') neq 0>1<cfelse>0</cfif>" onkeyup="calcvaraw(document.getElementById('fixawerrate#i#').value,this.value,'fixawer#i#');calcustallow();<cfif findnocase('venture',getplacement.custname) or findnocase('Straits Orthopaedics',getplacement.custname)>checkforOT();</cfif><cfif findnocase('venture',getplacement.custname) or findnocase('Straits Orthopaedics',getplacement.custname)>refreshotrate();</cfif>" onBlur="calcvaraw(document.getElementById('fixawerrate#i#').value,this.value,'fixawer#i#');calcustallow();<cfif findnocase('venture',getplacement.custname) or findnocase('Straits Orthopaedics',getplacement.custname)>checkforOT();</cfif><cfif findnocase('venture',getplacement.custname) or findnocase('Straits Orthopaedics',getplacement.custname)>refreshotrate();</cfif>" style="width: 100px"  />
        </td>
      <td><input type="text" name="fixawee#i#" id="fixawee#i#" value="#numberformat(evaluate('fixawee#i#'),'.__')#" onkeyup="calselfallow();" onblur="calselfallow();" style="width: 100px" readOnly/></td>
       <td style="background-color:#bgcolor#"><input type="text" name="fixawer#i#" id="fixawer#i#" value="#numberformat(evaluate('fixawer#i#'),'.__')#" onkeyup="calcustallow();" onblur="calcustallow();" style="width: 100px" readOnly/></td>
      ...
      <input type="hidden" name="fixawdesp#i#" id="fixawdesp#i#"  value=""/>
            <input type="hidden" name="fixaweerate#i#" id="fixaweerate#i#" value="0" />
            <input type="hidden" name="fixaweeqty#i#" id="fixaweeqty#i#" value="0"  />
            <input type="hidden" name="fixawerrate#i#" id="fixawerrate#i#" value="0"   />
            <input type="hidden" name="fixawerqty#i#" id="fixawerqty#i#" value="0"  />
      ...
      <th></th>
        ...
        <th><div align="left">Employee</div></th>
        <th ></th>
        <th ><div align="left">Employeer</div></th>
        <th ></th>
      </tr> 
      <tr>
      <th ><div align="left">Variable Allowance Code</div></th>
        <th><div align="left">Description</div></th>
        <th><div align="left">Rate</div></th>
        <!---Added by Nieo 20171206 1538--->
        <th ><div align="left">Qty</div></th>
        <th ><div align="left">Rate</div></th>
        <th ><div align="left">Qty</div></th>
        <!---Added by Nieo 20171206 1538--->
      ...
      <cfset "awerexp#getvarallowance.dballid##getvarallowance.currentrow#" = getvarallowance.dbjoballbillrate>
      <cfset "aweeexp#getvarallowance.dballid##getvarallowance.currentrow#" = getvarallowance.dbjoballpayrate>
      </cfloop>
      ...
      <cfset "allowancedesp#getvarallowance.currentrow#" = getdesp.desp>
       ...
       <!---<cfif isDefined('url.auto')>  									<!---only applicable to validate timesheet--->
            ...
            AND submit_date BETWEEN "#lsdateformat(realstartdate, 'yyyy-mm-dd', 'en_AU')#"
            AND "#lsdateformat(realenddate, 'yyyy-mm-dd', 'en_AU')#"
            AND status = 'Approved'
            </cfquery>
      ...
      <tr>
            ...
            <select style="width:200px" name="allowance#a#" id="allowance#a#" onChange="document.getElementById('allowancedesp#a#').value=this.options[this.selectedIndex].id;document.getElementById('allowancedesp#a#').readOnly = false;document.getElementById('aweerate#a#').readOnly = false;document.getElementById('awerrate#a#').readOnly = false;document.getElementById('aweeqty#a#').readOnly = false;document.getElementById('awerqty#a#').readOnly = false;checkitem(this.value,#a#);">
...
<!---             <cfif awnowdesp neq "">
            ...
            <td>
            <input type="text" name="allowancedesp#a#" id="allowancedesp#a#" size="30" value="#evaluate('allowancedesp#a#')#" onclick="if(document.getElementById('allowancedesp#a#').value == ''){alert('Please select an Allowance to edit this field')}" <cfif evaluate('allowancedesp#a#') eq ''>readOnly</cfif>>
            </td>
            <cfif evaluate('aw#a#') neq "">
                <cfset awid = evaluate('aw#a#')>
                    
                <!---Added by Nieo 20171213 1131 to add ability to show rate and qty in invoice--->
                <cfif isdefined('aweeexp#awid##a#')>
                    <!---EE: #numberformat(evaluate('aweeexp#awid##a#'),'.__')# / ER: #numberformat(evaluate('awerexp#awid##a#'),'.__')#<br>--->
                    <td>
                     <input type="number" name="aweerate#a#" id="aweerate#a#" style="width: 100px" value="<cfif val(evaluate('aweerate#a#')) neq 0>#numberformat(evaluate('aweerate#a#'),'.__')#<cfelse><cfif val(evaluate('aweeexp#awid##a#')) neq 0>#numberformat(evaluate('aweeexp#awid##a#'),'.__')#<cfelse>#numberformat(evaluate('awee#a#'),'.__')#</cfif></cfif>" onKeyUp="if(this.readOnly == false){<cfif findnocase('accenture',getplacement.custname)>if(document.getElementById('awerrate#a#').readOnly == false)</cfif>document.getElementById('awerrate#a#').value=this.value;calcvaraw(this.value,document.getElementById('aweeqty#a#').value,'awee#a#');calselfallow();calcvaraw(document.getElementById('awerrate#a#').value,document.getElementById('awerqty#a#').value,'awer#a#');calcustallow();};<cfif findnocase('venture',getplacement.custname) or findnocase('Straits Orthopaedics',getplacement.custname)>refreshotrate();</cfif>" onBlur="if(this.readOnly == false){<cfif findnocase('accenture',getplacement.custname)>if(document.getElementById('awerrate#a#').readOnly == false)</cfif>document.getElementById('awerrate#a#').value=this.value;calcvaraw(this.value,document.getElementById('aweeqty#a#').value,'awee#a#');calselfallow();calcvaraw(document.getElementById('awerrate#a#').value,document.getElementById('awerqty#a#').value,'awer#a#');calcustallow();};<cfif findnocase('venture',getplacement.custname) or findnocase('Straits Orthopaedics',getplacement.custname)>refreshotrate();</cfif>" onclick="if(document.getElementById('allowancedesp#a#').value == ''){alert('Please select an Allowance to edit this field')}" <cfif evaluate('allowancedesp#a#') eq ''>readOnly</cfif>>
                    </td>
                    <td>
                     <input type="number" name="aweeqty#a#" id="aweeqty#a#" style="width: 100px" value="<cfif evaluate('aweeqty#a#') neq 0>#numberformat(evaluate('aweeqty#a#'),'.__')#<cfelse><cfif evaluate('awee#a#') neq 0 and url.type neq 'create'>1<cfelse>0</cfif></cfif>" onKeyUp="if(this.readOnly == false){<cfif findnocase('accenture',getplacement.custname)>if(document.getElementById('awerqty#a#').readOnly == false)</cfif>document.getElementById('awerqty#a#').value=this.value;calcvaraw(document.getElementById('aweerate#a#').value,this.value,'awee#a#');calselfallow();calcvaraw(document.getElementById('awerrate#a#').value,document.getElementById('awerqty#a#').value,'awer#a#');calcustallow();};<cfif findnocase('venture',getplacement.custname) or findnocase('Straits Orthopaedics',getplacement.custname)>checkforOT();</cfif><cfif findnocase('venture',getplacement.custname) or findnocase('Straits Orthopaedics',getplacement.custname)>refreshotrate();</cfif>" onBlur="if(this.readOnly == false){<cfif findnocase('accenture',getplacement.custname)>if(document.getElementById('awerqty#a#').readOnly == false)</cfif>document.getElementById('awerqty#a#').value=this.value;calcvaraw(document.getElementById('aweerate#a#').value,this.value,'awee#a#');calselfallow();calcvaraw(document.getElementById('awerrate#a#').value,document.getElementById('awerqty#a#').value,'awer#a#');calcustallow();};<cfif findnocase('venture',getplacement.custname) or findnocase('Straits Orthopaedics',getplacement.custname)>checkforOT();</cfif><cfif findnocase('venture',getplacement.custname) or findnocase('Straits Orthopaedics',getplacement.custname)>refreshotrate();</cfif>" onclick="if(document.getElementById('allowancedesp#a#').value == ''){alert('Please select an Allowance to edit this field')}" <cfif evaluate('allowancedesp#a#') eq ''>readOnly</cfif>>
                    </td>
                <cfelse>
                    <td>
                     <input type="number" name="aweerate#a#" id="aweerate#a#" style="width: 100px" value="<cfif evaluate('aweerate#a#') neq 0>#numberformat(evaluate('aweerate#a#'),'.__')#<cfelse><cfif evaluate('awee#a#') neq 0>#numberformat(evaluate('awee#a#'),'.__')#<cfelse>0</cfif></cfif>" onKeyUp="if(this.readOnly == false){<cfif findnocase('accenture',getplacement.custname)>if(document.getElementById('awerrate#a#').readOnly == false)</cfif>document.getElementById('awerrate#a#').value=this.value;calcvaraw(this.value,document.getElementById('aweeqty#a#').value,'awee#a#');calselfallow();calcvaraw(document.getElementById('awerrate#a#').value,document.getElementById('awerqty#a#').value,'awer#a#');calcustallow();}<cfif findnocase('venture',getplacement.custname) or findnocase('Straits Orthopaedics',getplacement.custname)>refreshotrate();</cfif>" onBlur="if(this.readOnly == false){<cfif findnocase('accenture',getplacement.custname)>if(document.getElementById('awerrate#a#').readOnly == false)</cfif>document.getElementById('awerrate#a#').value=this.value;calcvaraw(this.value,document.getElementById('aweeqty#a#').value,'awee#a#');calselfallow();calcvaraw(document.getElementById('awerrate#a#').value,document.getElementById('awerqty#a#').value,'awer#a#');calcustallow();}<cfif findnocase('venture',getplacement.custname) or findnocase('Straits Orthopaedics',getplacement.custname)>refreshotrate();</cfif>" onclick="if(document.getElementById('allowancedesp#a#').value == ''){alert('Please select an Allowance to edit this field')}" <cfif evaluate('allowancedesp#a#') eq ''>readOnly</cfif>>
                    </td>
                    <td>
                     <input type="number" name="aweeqty#a#" id="aweeqty#a#" style="width: 100px" value="<cfif evaluate('aweeqty#a#') neq 0>#numberformat(evaluate('aweeqty#a#'),'.__')#<cfelse><cfif evaluate('awee#a#') neq 0 and url.type neq 'create'>1<cfelse>0</cfif></cfif>" onKeyUp="if(this.readOnly == false){<cfif findnocase('accenture',getplacement.custname)>if(document.getElementById('awerqty#a#').readOnly == false)</cfif>document.getElementById('awerqty#a#').value=this.value;calcvaraw(document.getElementById('aweerate#a#').value,this.value,'awee#a#');calselfallow();calcvaraw(document.getElementById('awerrate#a#').value,document.getElementById('awerqty#a#').value,'awer#a#');calcustallow();};<cfif findnocase('venture',getplacement.custname) or findnocase('Straits Orthopaedics',getplacement.custname)>checkforOT();</cfif><cfif findnocase('venture',getplacement.custname) or findnocase('Straits Orthopaedics',getplacement.custname)>refreshotrate();</cfif>" onBlur="if(this.readOnly == false){<cfif findnocase('accenture',getplacement.custname)>if(document.getElementById('awerqty#a#').readOnly == false)</cfif>document.getElementById('awerqty#a#').value=this.value;calcvaraw(document.getElementById('aweerate#a#').value,this.value,'awee#a#');calselfallow();calcvaraw(document.getElementById('awerrate#a#').value,document.getElementById('awerqty#a#').value,'awer#a#');calcustallow();};<cfif findnocase('venture',getplacement.custname) or findnocase('Straits Orthopaedics',getplacement.custname)>checkforOT();</cfif><cfif findnocase('venture',getplacement.custname) or findnocase('Straits Orthopaedics',getplacement.custname)>refreshotrate();</cfif>" onclick="if(document.getElementById('allowancedesp#a#').value == ''){alert('Please select an Allowance to edit this field')}" <cfif evaluate('allowancedesp#a#') eq ''>readOnly</cfif>>
                    </td>
            ...
            <cfif evaluate('aw#a#') neq "">
                ...
                <cfif isdefined('aweeexp#awid##a#')>
                     <td style="background-color:#bgcolor#">
                     <input type="number" name="awerrate#a#" id="awerrate#a#" style="width: 100px" value="<cfif val(evaluate('awerrate#a#')) neq 0>#numberformat(evaluate('awerrate#a#'),'.__')#<cfelse><cfif val(evaluate('awerexp#awid##a#')) neq 0>#numberformat(evaluate('awerexp#awid##a#'),'.__')#<cfelse>#numberformat(evaluate('awer#a#'),'.__')#</cfif></cfif>" onkeyup="calcvaraw(this.value,document.getElementById('awerqty#a#').value,'awer#a#');calcustallow();<cfif findnocase('venture',getplacement.custname) or findnocase('Straits Orthopaedics',getplacement.custname)>refreshotrate();</cfif>" onBlur="calcvaraw(this.value,document.getElementById('awerqty#a#').value,'awer#a#');calcustallow();<cfif findnocase('venture',getplacement.custname) or findnocase('Straits Orthopaedics',getplacement.custname)>refreshotrate();</cfif>" onclick="if(document.getElementById('allowancedesp#a#').value == ''){alert('Please select an Allowance to edit this field')}" <cfif evaluate('allowancedesp#a#') eq ''>readOnly</cfif>>
                    </td>
                    <td style="background-color:#bgcolor#">
                     <input type="number" name="awerqty#a#" id="awerqty#a#" style="width: 100px" value="<cfif evaluate('awerqty#a#') neq 0>#numberformat(evaluate('awerqty#a#'),'.__')#<cfelse><cfif evaluate('awer#a#') neq 0 and url.type neq 'create'>1<cfelse>0</cfif></cfif>" onkeyup="calcvaraw(document.getElementById('awerrate#a#').value,this.value,'awer#a#');calcustallow();<cfif findnocase('venture',getplacement.custname) or findnocase('Straits Orthopaedics',getplacement.custname)>checkforOT();</cfif><cfif findnocase('venture',getplacement.custname) or findnocase('Straits Orthopaedics',getplacement.custname)>refreshotrate();</cfif>" onBlur="calcvaraw(document.getElementById('awerrate#a#').value,this.value,'awer#a#');calcustallow();<cfif findnocase('venture',getplacement.custname) or findnocase('Straits Orthopaedics',getplacement.custname)>checkforOT();</cfif><cfif findnocase('venture',getplacement.custname) or findnocase('Straits Orthopaedics',getplacement.custname)>refreshotrate();</cfif>" onclick="if(document.getElementById('allowancedesp#a#').value == ''){alert('Please select an Allowance to edit this field')}" <cfif evaluate('allowancedesp#a#') eq ''>readOnly</cfif>>
                    </td>
                <cfelse>
                     <td style="background-color:#bgcolor#">
                     <input type="number" name="awerrate#a#" id="awerrate#a#" style="width: 100px" value="<cfif evaluate('awerrate#a#') neq 0>#numberformat(evaluate('awerrate#a#'),'.__')#<cfelse><cfif evaluate('awer#a#') neq 0>#numberformat(evaluate('awer#a#'),'.__')#<cfelse>0</cfif></cfif>" onkeyup="calcvaraw(this.value,document.getElementById('awerqty#a#').value,'awer#a#');calcustallow();<cfif findnocase('venture',getplacement.custname) or findnocase('Straits Orthopaedics',getplacement.custname)>refreshotrate();</cfif>" onBlur="calcvaraw(this.value,document.getElementById('awerqty#a#').value,'awer#a#');calcustallow();<cfif findnocase('venture',getplacement.custname) or findnocase('Straits Orthopaedics',getplacement.custname)>refreshotrate();</cfif>" onclick="if(document.getElementById('allowancedesp#a#').value == ''){alert('Please select an Allowance to edit this field')}" <cfif evaluate('allowancedesp#a#') eq ''>readOnly</cfif>>
                    </td>
                     <td style="background-color:#bgcolor#">
                     <input type="number" name="awerqty#a#" id="awerqty#a#" style="width: 100px" value="<cfif evaluate('awerqty#a#') neq 0>#numberformat(evaluate('awerqty#a#'),'.__')#<cfelse><cfif evaluate('awer#a#') neq 0 and url.type neq 'create'>1<cfelse>0</cfif></cfif>" onkeyup="calcvaraw(document.getElementById('awerrate#a#').value,this.value,'awer#a#');calcustallow();<cfif findnocase('venture',getplacement.custname) or findnocase('Straits Orthopaedics',getplacement.custname)>checkforOT();</cfif><cfif findnocase('venture',getplacement.custname) or findnocase('Straits Orthopaedics',getplacement.custname)>refreshotrate();</cfif>" onBlur="calcvaraw(document.getElementById('awerrate#a#').value,this.value,'awer#a#');calcustallow();<cfif findnocase('venture',getplacement.custname) or findnocase('Straits Orthopaedics',getplacement.custname)>checkforOT();</cfif><cfif findnocase('venture',getplacement.custname) or findnocase('Straits Orthopaedics',getplacement.custname)>refreshotrate();</cfif>" onclick="if(document.getElementById('allowancedesp#a#').value == ''){alert('Please select an Allowance to edit this field')}" <cfif evaluate('allowancedesp#a#') eq ''>readOnly</cfif>>
                    </td>
            ...
            <td>
             <input type="text" name="awee#a#" id="awee#a#" size="10" value="#numberformat(evaluate('awee#a#'),'.__')#" readOnly>
            </td>
             <td style="background-color:#bgcolor#">
             <input type="text" name="awer#a#" id="awer#a#" size="10" value="#numberformat(evaluate('awer#a#'),'.__')#" readOnly>
            </td>
      </tr>
      </cfloop>
      <tr>
      ...
      <td><input type="text" size='10' name="selfallowancerate4"  id="selfallowancerate4" value="#selfallowancerate4#" onKeyUp="document.getElementById('custallowancerate4').value=this.value;calselfallow();calcustallow();" onBlur="document.getElementById('custallowancerate4').value=this.value;calselfallow();calcustallow();">
       </td>
      <td style="background-color:#bgcolor#"><input type="text" size='10' name="custallowancerate4" value="#custallowancerate4#" id="custallowancerate4" onKeyUp="calcustallow();" onBlur="calcustallow();"></td>
      </tr>
      <tr style="display:none">
      ...
      <td><input type="text" size='10' name="selfallowancerate5"  id="selfallowancerate5" value="#selfallowancerate5#" onKeyUp="document.getElementById('custallowancerate5').value=this.value;calselfallow();calcustallow();"
                 onBlur="document.getElementById('custallowancerate5').value=this.value;calselfallow();calcustallow();">
       </td>
      <td style="background-color:#bgcolor#"><input type="text" size='10' name="custallowancerate5" value="#custallowancerate5#" id="custallowancerate5" onKeyUp="calcustallow();" onBlur="calcustallow();"></td>
      </tr>
      <tr style="display:none">
      ...
      <td><input type="text" size='10' name="selfallowancerate6"  id="selfallowancerate6" value="#selfallowancerate6#" onKeyUp="document.getElementById('custallowancerate6').value=this.value;calselfallow();calcustallow();" onBlur="document.getElementById('custallowancerate6').value=this.value;calselfallow();calcustallow();">
       </td>
      <td style="background-color:#bgcolor#"><input type="text" size='10' name="custallowancerate6" value="#custallowancerate6#" id="custallowancerate6" onKeyUp="calcustallow();" onBlur="calcustallow();"></td>
      </tr>
       <tr style="display:none">
      ...
      <td colspan="2">
      <input type="text" size='5' name="selfallowancerate1" id="selfallowancerate1" value="#selfallowancerate1#"  onKeyUp="calselfallow();" onBlur="calselfallow();"> Rate&nbsp;&nbsp;&nbsp;<input type="text" size='5' name="selfallowancehour1" id="selfallowancehour1" value="#selfallowancehour1#" onKeyUp="calselfallow();" onBlur="calselfallow();"> Hrs</td>
      
      <td>
      <input type="text" size='5' name="custallowancerate1" id="custallowancerate1" value="#custallowancerate1#" onKeyUp="calcustallow();" onBlur="calcustallow();"> </td>
      <td><input type="text" size='5' name="custallowancehour1" id="custallowancehour1" value="#custallowancehour1#" onKeyUp="calcustallow();" onBlur="calcustallow();"> Hrs</td>
      ...
      <td colspan="2">
      <input type="text" size='5' name="selfallowancerate2"  id="selfallowancerate2"value="#selfallowancerate2#" onKeyUp="calselfallow();" onBlur="calselfallow();"> Rate&nbsp;&nbsp;&nbsp;<input type="text" size='5' name="selfallowancehour2" id="selfallowancehour2" value="#selfallowancehour2#" onKeyUp="calselfallow();" onBlur="calselfallow();"> Hrs</td>
  
      <td>
      <input type="text" size='5' name="custallowancerate2"  id="custallowancerate2" value="#custallowancerate2#" onKeyUp="calcustallow();" onBlur="calcustallow();"> </td>
      <td><input type="text" size='5' name="custallowancehour2" id="custallowancehour2" value="#custallowancehour2#" onKeyUp="calcustallow();" onBlur="calcustallow();"> Hrs</td>
      ...
      <td colspan="2">
      <input type="text" size='5' name="selfallowancerate3" id="selfallowancerate3" value="#selfallowancerate3#" onKeyUp="calselfallow();" onBlur="calselfallow();"> Rate&nbsp;&nbsp;&nbsp;<input type="text" size='5' name="selfallowancehour3" id="selfallowancehour3" value="#selfallowancehour3#" onKeyUp="calselfallow();" onBlur="calselfallow();"> Hrs</td>
      
      <td>
      <input type="text" size='5' name="custallowancerate3"  id="custallowancerate3" value="#custallowancerate3#" onKeyUp="calcustallow();" onBlur="calcustallow();"> </td>
      <td><input type="text" size='5' name="custallowancehour3" id="custallowancehour3" value="#custallowancehour3#" onKeyUp="calcustallow();" onBlur="calcustallow();"> Hrs</td>
      ...
      <td><input type="text" size='10' name="selfallowance"  id="selfallowance" value="#numberformat(selfallowance,'_.__')#" readOnly></td>
      <td style="background-color:#bgcolor#"><input type="text" size='10' name="custallowance"  id="custallowance" value="#numberformat(custallowance,'_.__')#" readOnly></td>
      </tr>
      ...
      <td><input type="text" size='10' name="selfcpf" id="selfcpf" value="#numberformat(selfcpf,'_.__')#" readOnly></td>
      <td style="background-color:#bgcolor#"><input type="text" size='10' name="custcpf" id="custcpf" value="#numberformat(custcpf,'_.__')#" onKeyUp="calcusttotal();" onBlur="calcusttotal();"></td>
      </tr>
      <tr>
      ...
      <td><input type="Text" size='10' name="selfsdf" id="selfsdf" value="#numberformat(selfsdf,'_.__')#" readOnly></td>
      <td style="background-color:#bgcolor#"><input type="text" size='10' name="custsdf" id="custsdf" value="#numberformat(custsdf,'_.__')#" onKeyUp="calcusttotal();" onBlur="calcusttotal();"></td>
      </tr>
    <!---Added by Nieo 20180108 1548 to add EIS into MP4U--->
    <tr>
      <th colspan="6"><div align="right">EIS</div><div align="right"></div></th>
      <td><input type="Text" size='10' name="selfeis" id="selfeis" value="#numberformat(selfeis,'_.__')#" readOnly></td>
      <td style="background-color:#bgcolor#"><input type="text" size='10' name="custeis" id="custeis" value="#numberformat(custeis,'_.__')#" onKeyUp="calcusttotal();" onBlur="calcusttotal();"></td>
      </tr>
    <!---Added by Nieo 20180108 1548 to add EIS into MP4U--->
      <tr style="display:none">
      ...
      <td><input type="text" name="nsded" id="nsded" value="#numberformat(nsded,'_.__')#" onKeyUp="dedcal();" onBlur="dedcal();"  size='10' ></td>
       <td style="background-color:#bgcolor#"><input type="text" name="nscustded" id="nscustded" value="#numberformat(nscustded,'_.__')#" onKeyUp="dedcalcust();" onBlur="dedcalcust();"  size='10' ></td>
      </tr>
      <tr>
      ...
      <td></td>
      <td style="background-color:#bgcolor#"><input type="text" name="adminfee" id="adminfee" value="#numberformat(adminfee,'_.__')#" onKeyUp="dedcalcust();" onBlur="dedcalcust();"  size='10'  ></td>
      </tr>
      <tr>
      <td>&nbsp;</td>
      </tr>
      ...
      <td></td>
        <td style="background-color:#bgcolor#">
        <cfif evaluate('billitem#i#') eq 136 and isdefined('monthlybillableitemB136')>
            <input type="text" name="billitemamt#i#" id="billitemamt#i#" value="#monthlybillableitemB136#" size='10' onkeyup="dedcalcust()" onblur="dedcalcust()" />
        <cfelse>
            <input type="text" name="billitemamt#i#" id="billitemamt#i#" value="#numberformat(evaluate('billitemamt#i#'),'.__')#"    size='10' onkeyup="dedcalcust()" onblur="dedcalcust()" />
        </cfif>
      ...
      <td></td>
      <td style="background-color:#bgcolor#"> 
      <input type="text" name="billitemamt#i#" id="billitemamt#i#" value="0"  size='10' onkeyup="dedcalcust()" onblur="dedcalcust()"/>
      ...
      <td colspan="2"><input type="text" name="addchargedesp" id="addchargedesp" value="#addchargedesp#"  size="30"></td>
       ...
       <td><input type="text" name="addchargeself" id="addchargeself" value="#numberformat(addchargeself,'_.__')#" onKeyUp="dedcal();" onBlur="dedcal();"  size='10'  ><input type="checkbox" style="display:none" name="claimadd1" id="claimadd1" value="1" checked></td>
       <td style="background-color:#bgcolor#"><input type="text" name="addchargecust" id="addchargecust" value="#numberformat(addchargecust,'_.__')#" onKeyUp="dedcalcust();" onBlur="dedcalcust();" size='10'></td>
      </tr>
       <tr>
      ...
      <td colspan="2"><input type="text" name="addchargedesp2" id="addchargedesp2"  value="#addchargedesp2#"  size="30"></td>
       ...
       <td><input type="text" name="addchargeself2" id="addchargeself2" value="#numberformat(addchargeself2,'_.__')#" onKeyUp="dedcal();" onBlur="dedcal();"  size='10'  ><input type="checkbox" name="claimadd2" id="claimadd2" value="2" checked style="display:none"></td>
       <td style="background-color:#bgcolor#"><input type="text" name="addchargecust2" id="addchargecust2" value="#numberformat(addchargecust2,'_.__')#" onKeyUp="dedcalcust();" onBlur="dedcalcust();" size='10'></td>
      </tr>
      ...
      <td colspan="2" ><input type="text" name="addchargedesp3" id="addchargedesp3" value="#addchargedesp3#" size="30"></td>
       ...
       <td><input type="text" name="addchargeself3" id="addchargeself3" value="#numberformat(addchargeself3,'_.__')#" onKeyUp="dedcal();" onBlur="dedcal();"  size='10' ><input type="checkbox" name="claimadd3" id="claimadd3" value="3" checked style="display:none"></td>
       <td style="background-color:#bgcolor#"><input type="text" name="addchargecust3" id="addchargecust3" value="#numberformat(addchargecust3,'_.__')#" onKeyUp="dedcalcust();" onBlur="dedcalcust();" size='10'></td>
      </tr>
      ...
      <td colspan="3"><input type="text" name="addchargedesp4" id="addchargedesp4" size="50" value="#addchargedesp4#"></td>
       ...
       <td><input type="text" name="addchargeself4" id="addchargeself4" value="#numberformat(addchargeself4,'_.__')#" onKeyUp="calselftotal();" onBlur="calselftotal();" size='10' ><input type="checkbox" name="claimadd4" id="claimadd4" value="4" <cfif claimadd4 eq "Y">checked</cfif>></td>
       <td><input type="text" name="addchargecust4" id="addchargecust4" value="#numberformat(addchargecust4,'_.__')#" onKeyUp="calcusttotal();" onBlur="calcusttotal();" size='10'></td>
       <td colspan="2"></td>
      </tr>
      <tr style="display:none">
      <th>Description 5</th>
      <td colspan="3"><input type="text" name="addchargedesp5" id="addchargedesp5" size="50" value="#addchargedesp5#"></td>
       ...
       <td><input type="text" name="addchargeself5" id="addchargeself5" value="#numberformat(addchargeself5,'_.__')#" onKeyUp="calselftotal();" onBlur="calselftotal();" size='10' ><input type="checkbox" name="claimadd5" id="claimadd5" value="5" <cfif claimadd5 eq "Y">checked</cfif>></td>
       <td><input type="text" name="addchargecust5" id="addchargecust5" value="#numberformat(addchargecust5,'_.__')#" onKeyUp="calcusttotal();" onBlur="calcusttotal();" size='10'></td>
       <td colspan="2"></td>
      </tr>
       <tr style="display:none">
      <th>Description 6</th>
      <td colspan="3"><input type="text" name="addchargedesp6" id="addchargedesp6" size="50" value="#addchargedesp6#"></td>
       ...
       <td><input type="text" name="addchargeself6" id="addchargeself6" value="#numberformat(addchargeself6,'_.__')#" onKeyUp="calselftotal();" onBlur="calselftotal();" size='10' ><input type="checkbox" name="claimadd6" id="claimadd6" value="6" <cfif claimadd6 eq "Y">checked</cfif>></td>
       <td><input type="text" name="addchargecust6" id="addchargecust6" value="#numberformat(addchargecust6,'_.__')#" onKeyUp="calcusttotal();" onBlur="calcusttotal();" size='10'></td>
       <td colspan="2"></td>
      </tr>
       <tr style="display:none">
      <th colspan="6"><div align="right">Total Addtional Charges</div></th>

       <td><input type="text" name="addchargeselftotal" id="addchargeselftotal" value="" onKeyUp="calselftotal();" onBlur="calselftotal();" size='10'></td>
       <td><input type="text" name="addchargecusttotal" id="addchargecusttotal" value="" onKeyUp="calcusttotal();" onBlur="calcusttotal();" size='10'></td>
      </tr>
      <tr style="display:none">
      <th colspan="6">
      <div align="right">
     ...
     <td></td>
     <td></td>
     <td style="background-color:#bgcolor#"><input type="text" size='10' name="rebate" id="rebate" value="#numberformat(rebate,'_.__')#" onkeyup="dedcalcust();" onBlur="dedcalcust();"></td>
     </tr> 
     <tr style="display:none">
     <td>&nbsp;</td>
     </tr>
     ...
     <th colspan="5"><div align="left">Deductions (Do Not Enter "-")</div></th>
      ...
      <td><input type="text" name="ded1" id="ded1" value="#numberformat(ded1,'_.__')#" onKeyUp="dedcal();" onBlur="dedcal();" size='10' ></td>
        <td style="background-color:#bgcolor#"><input type="text" name="dedcust1" id="dedcust1" value="#numberformat(dedcust1,'_.__')#" onkeyup="dedcalcust();" onBlur="dedcalcust();" size='10' ></td>
      </tr>
      <tr>
      <td colspan="4"><input type="text" name="ded2desp" id="ded2desp" size="65" value="#ded2desp#"></td>
      <td><input type="text" name="ded2" id="ded2" value="#numberformat(ded2,'_.__')#" onKeyUp="dedcal();" onBlur="dedcal();" size='10' ></td>
       <td style="background-color:#bgcolor#"><input type="text" name="dedcust2" id="dedcust2" value="#numberformat(dedcust2,'_.__')#"  onkeyup="dedcalcust();" onBlur="dedcalcust();" size='10' ></td>
      </tr>
      <tr>
      <td colspan="4"><input type="text" name="ded3desp" id="ded3desp" size="65" value="#ded3desp#"></td>
      <td><input type="text" name="ded3" id="ded3" value="#numberformat(ded3,'_.__')#" onKeyUp="dedcal();" onBlur="dedcal();" size='10' ></td>
       <td style="background-color:#bgcolor#"><input type="text" name="dedcust3" id="dedcust3" value="#numberformat(dedcust3,'_.__')#"  onkeyup="dedcalcust();" onBlur="dedcalcust();" size='10' ></td>
      </tr>
      <tr>
      <th colspan="6">
      <div align="right">
...
<input type="hidden" name="tran" id="tran" value="INV"> 
      ...
      <td style="background-color:#bgcolor#">
          <div style="display: none">
              <select name="taxcode" id="taxcode" onchange="document.getElementById('taxper').value=this.options[this.selectedIndex].id;calcusttotal();">
        ...
        &nbsp;<!---<input type="text" name="taxper" id="taxper" value="#taxper#" size="3" onKeyUp="caltax2();calcusttotal();" bindonload="yes"  />--->
        <select name="taxper" id="taxper" onChange="caltax2();calcusttotal();" bindonload="yes">
            <option id="6" value="6" <cfif taxper eq "6">selected</cfif>>6</option>
            <option id="0" value="0" <cfif taxper eq "0">selected</cfif>>0</option>
        </select>
      </td>
      <td style="background-color:#bgcolor#">
  <input type="text" name="taxamt" id="taxamt" value="#numberformat(taxamt,'.__')#" size="10" readOnly/></td>
    ...
    <cfoutput>
    <input type="hidden" name="processtimesheet" id="processtimesheet" value="#url.auto#">
 <!---   <input type="hidden" name="startdate" id="startdate" value="#lsdateformat(url.startdate, 'yyyy-mm-dd', 'en_AU')#">
    <input type="hidden" name="enddate" id="enddate" value="#lsdateformat(url.enddate, 'yyyy-mm-dd', 'en_AU')#">--->
     ...
     <tr> 
      ...
      <input type="button" id="calculateemppay" name="calculateemppay" value="Process TimeSheet" <cfif posted eq "P">disabled </cfif> onClick="<cfif locked eq "Y">alert('This Assignment Has Been Locked')<cfelseif combine eq "Y">alert('This Assignment Has Been Combine')<cfelseif posted neq "P">checkpay()<cfelse>alert('This assignment has been posted!')</cfif>;">
        </cfoutput></td>
    </tr>
    <cfelse>
    <tr> 
      ...
      <input type="button" id="calculateemppay" name="calculateemppay" value="Calculate Employee Pay" <cfif posted eq "P">disabled </cfif> onClick="document.getElementById('submit1').disabled = false;document.getElementById('submit2').disabled = false;<cfif locked eq "Y">alert('This Assignment Has Been Locked')<cfelseif combine eq "Y">alert('This Assignment Has Been Combine')<cfelseif posted neq "P">checkpay()<cfelse>alert('This assignment has been posted!')</cfif>;">&nbsp;&nbsp;&nbsp;&nbsp;
          
        <cfset message = "">
        <cfif posted eq "P">
            <cfset message = "This assignemntslip has posted.">
        <cfelseif locked eq "Y">
            <cfset message = "This assignemntslip has locked.">
        <cfelseif url.type eq "create" or url.type eq "edit">
            <cfset message = "Please calculate before #url.type#.">
        </cfif>
          
        <input id="dsubmit1" name="dsubmit1" type="button" onClick="<cfif url.type eq 'create' or url.type eq 'edit'>if(document.getElementById('submit1').disabled == true){alert('#message#');}else{document.getElementById('submit1').click();}<cfelse>document.getElementById('submit1').click();</cfif>" value="  #button#  ">
          &nbsp;&nbsp;&nbsp;&nbsp;
        <input id="dsubmit2" name="dsubmit2" type="button" onClick="<cfif url.type eq 'create' or url.type eq 'edit'>if(document.getElementById('submit2').disabled == true){alert('#message#');}else{document.getElementById('submit2').click();}<cfelse>document.getElementById('submit2').click();</cfif>" value="#button# and Create New">      
          
        <div style="display: none">
            <input name="submit1" id="submit1" type="button" onClick="this.disabled=true;<cfif combine eq "Y">alert('This Assignment Has Been Combine')<cfelse>var stdate = new Date(); stdate.setFullYear(document.getElementById('startdate').value.substring(6,10),parseInt(document.getElementById('startdate').value.substring(3,5))-1,document.getElementById('startdate').value.substring(0,2));var eddate = new Date();eddate.setFullYear(document.getElementById('completedate').value.substring(6,10),parseInt(document.getElementById('completedate').value.substring(3,5))-1,document.getElementById('completedate').value.substring(0,2));if((stdate-eddate) > 0 ){alert('Please check Job Start Date and Job Completed Date')}else{submitform()}</cfif>;" value="  #button#  " <cfif url.type eq "create" or url.type eq "edit" or posted eq "P">disabled</cfif>>
          
          <input name="submit2" id="submit2" type="button" onClick="this.disabled=true;<cfif combine eq "Y">alert('This Assignment Has Been Combine')<cfelse>var stdate = new Date(); stdate.setFullYear(document.getElementById('startdate').value.substring(6,10),parseInt(document.getElementById('startdate').value.substring(3,5))-1,document.getElementById('startdate').value.substring(0,2));var eddate = new Date();eddate.setFullYear(document.getElementById('completedate').value.substring(6,10),parseInt(document.getElementById('completedate').value.substring(3,5))-1,document.getElementById('completedate').value.substring(0,2));if((stdate-eddate) > 0 ){alert('Please check Job Start Date and Job Completed Date')}else{submitform2()};</cfif>"  value="#button# and Create New"<cfif url.type eq "create" or url.type eq "edit" or posted eq "P">disabled</cfif>>
        </div>
        </cfoutput></td>
    </tr>
    </cfif>
