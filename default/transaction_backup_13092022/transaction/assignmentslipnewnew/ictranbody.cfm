    ...
    <cfif left(form.refno,1) neq "S">
         ...
         <cfif checkassign.trancodestart eq 0 and checkassign.trancodeend eq 0>
            ...
            <cftransaction>
                <cfquery name="getlatesttrancode" datasource="#dts#">
                    SELECT trancodeend FROM assignmentslip 
                    WHERE refno in (select brem6 from ictran where  refno = "#refnofound#" and type = "INV")
                    ORDER BY trancodeend desc
                </cfquery>
                <cfquery name="updateasgn" datasource="#dts#">
                    UPDATE assignmentslip 
                    SET trancodestart = "#val(getlatesttrancode.trancodeend)+21#",
                    trancodeend ="#val(getlatesttrancode.trancodeend)+71#",
                    orirefno = "#form.refno#" 
                    WHERE refno = "#assignmentslipno#"
                </cfquery>
             </cftransaction>
     ...
     <!---Salary---->
     <cfset xqty=1>
    <!---Updated by Nieo 20171213 1140 to add qty and rate in invoice--->
     <!---<cfif form.invdesp2 eq "">--->
        ...
        <cfelse>
            <cfif val(form.workd) neq 0>
                <cfset datestartof = createdate(listlast(form.startdate,'/'),listgetat(form.startdate,'2','/'),listfirst(form.startdate,'/'))>
                <cfset dateendof = createdate(listlast(form.completedate,'/'),listgetat(form.completedate,'2','/'),listfirst(form.completedate,'/'))>
                <cfset monthprorate = ROUND((val(DateDiff("d", datestartof, dateendof) + 1)/val(form.workd))*100000)/100000>
            <cfelse>
                <cfset monthprorate = 1>
            </cfif>
            <cfset xqty=numberformat(monthprorate,'.____')>
         </cfif>
        ...
        <cfset xprice=val(form.custsalary)>
         ...
         <cfelseif form.paymenttype eq 'day' >
            ...
            <cfset xunit ="Day">
        <cfelse>
            <cfset xprice = numberformat(form.custusualpay,'_.__')>
            <cfset xunit ="Month">
         </cfif>
    ...
    <!---Updated by Nieo 20171213 1140 to add qty and rate in invoice--->
     ...
     <cfquery name="insertictran" datasource="#dts#">
        ...
        (
            ...
            '',
            <cfqueryparam cfsqltype="cf_sql_varchar" value="#xqty#">,
            <cfqueryparam cfsqltype="cf_sql_double" value="#numberformat(val(xprice),'.__')#">,
            '#xunit#',
            <cfqueryparam cfsqltype="cf_sql_double" value="#numberformat(val(form.custsalary),'.__')#">,
            0,
            0,
            0,
            0,
	 ...
	 <cfif evaluate('form.allowance#i#') neq "" and val(evaluate('form.awer#i#')) neq 0>
          <cfquery name="insertictran" datasource="#dts#">
            ...
            (
                ...
                '', 
                '',
                <cfqueryparam cfsqltype="cf_sql_double" value="#evaluate('form.awerqty#i#')#">,
                <cfqueryparam cfsqltype="cf_sql_double" value="#evaluate('form.awerrate#i#')#">,
                '#xunit#',
                 <cfqueryparam cfsqltype="cf_sql_double" value="#evaluate('form.awer#i#')#">,
                0,
                0,
                0,
                0,
    ...
    <cfset trancodestart = trancodestart + 1>
     ...
     <cfquery name="insertictran" datasource="#dts#">
        ...
        (
            ...
            '',
            '1',
            #numberformat(val(form.custeis),'.__')#, 
            '#xunit#',
             #numberformat(val(form.custeis),'.__')#,
            0,
            0,
    ...
    <!---Added by Nieo 20170810 for period scenario fix, Updated by Nieo 20171212 1005 to fix invoice period bug--->
    ...
    <!---Added by Nieo 20190417 for period scenario fix for BAE--->
    <cfif form.custno eq '300034085'>
        <cfset checkassignperiod.recordcount = 0>
    </cfif>    
    <!---Added by Nieo 20190417 for period scenario fix for BAE--->
    ...
    <!---Edited by Nieo 20170810 for period scenario fix--->
     ...
     SELECT arrem5,invoiceformat,arrem7 FROM arcust WHERE custno = "#form.custno#"
     </cfquery>
     
     <cfif gettaxprofile.arrem5 neq "">
         
    <cfif trim(gettaxprofile.arrem7) eq "1" and datediff('d',lsdateformat("31/08/2018",'YYYY-MM-DD', 'en_AU'),lsdateformat((form.completedate),'YYYY-MM-DD', 'en_AU')) gt 0>
     ...
     <cfif gettaxmethod.billtaxable eq "Y">
                <cfquery datasource='#dts#' name="updateictran">
                    Update ictran a, 
                    (
                    select refno,taxper from assignmentslip 
                    where refno in (select brem6 from ictran where  refno = "#form.refno#" and type = "INV")
                    ) b
                    set 
                    taxpec1=b.taxper,
                    note_a=<cfqueryparam cfsqltype="cf_sql_varchar" value="#form.taxcode#">,
                    <cfif gettaxprofile.arrem5 eq "GSTBILLRATE">
                        taxamt=(round(coalesce(amt,'0')+0.000001,2) * b.taxper/100)+0.000001,
                        taxamt_bil=(round(coalesce(amt,'0')+0.000001,2) * b.taxper/100)+0.000001
                    <cfelse>
                        taxamt=round((round(coalesce(amt,'0')+0.000001,2) * b.taxper/100)+0.000001,2),
                        taxamt_bil=round((round(coalesce(amt,'0')+0.000001,2) * b.taxper/100)+0.000001,2)
                    </cfif>
                    where a.refno = '#form.refno#' and type='INV'
                    and a.brem6=b.refno
                </cfquery>
     <cfelse>
    ...
    <cfif isdefined('getprevinvoiceno.invoiceno')>
        <cfif getprevinvoiceno.invoiceno neq ''>
            <cfif getprevinvoiceno.invoiceno neq form.refno>
                <cfquery name="checkinvamt" datasource="#dts#">
                    SELECT itemno from ictran 
                    where type='INV'
                ...
                <cfset tran = "INV">
                <cfset url.nexttranno = getprevinvoiceno.invoiceno>
