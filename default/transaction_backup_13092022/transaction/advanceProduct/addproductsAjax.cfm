...
<cfquery name="getcomment" datasource="#dts#">
	SELECT details from comments WHERE code=<cfqueryparam cfsqltype="cf_sql_varchar" value="#URLDecode(url.comment)#" >
</cfquery>
...
<cfif url.unit eq "#selecticitem.unit2#" and val(selecticitem.factor2) neq 0>
<cfset qtyReal = ( val(url.expqty) * val(selecticitem.factor1) ) / val(selecticitem.factor2)>
<cfelseif url.unit eq "#selecticitem.unit3#" and val(selecticitem.factorU3_b) neq 0>
<cfset qtyReal = ( val(url.expqty) * val(selecticitem.factorU3_a) ) / val(selecticitem.factorU3_b)>
<cfelseif url.unit eq "#selecticitem.unit4#" and val(selecticitem.factorU4_b) neq 0>
<cfset qtyReal = ( val(url.expqty) * val(selecticitem.factorU4_a) ) / val(selecticitem.factorU4_b)>
<cfelseif url.unit eq "#selecticitem.unit5#" and val(selecticitem.factorU5_b) neq 0>
<cfset qtyReal = ( val(url.expqty) * val(selecticitem.factorU5_a) ) / val(selecticitem.factorU5_b)>
<cfelseif url.unit eq "#selecticitem.unit6#" and val(selecticitem.factorU6_b) neq 0>
...
<cfquery name="insertictran" datasource="#dts#">
	...
	(
        ...
        <cfif getGsetup.bcurr eq "HKD" or REReplace(URLDecode(url.desp),"925925925925","%","ALL") contains "?">'#selecticitem.desp#'<cfelse>'#REReplace(URLDecode(url.desp),"925925925925","%","ALL")#'</cfif>, 
