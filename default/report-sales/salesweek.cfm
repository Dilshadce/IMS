...
function show_info(rset){
		...
		for(i=0;i<itemnoArray.length;i++){
			...
			document.salesweek.itemfrom.options.add(myoption);
		}
		...
		for(i=0;i<itemnoArray.length;i++){
			...
			myoption.value = itemnoArray[i];
			document.salesweek.itemto.options.add(myoption);
		}
		
	}
function getItem(){
	
		var text = document.salesweek.letter.value;
		
		var w = document.salesweek.searchtype.selectedIndex;
		
		var searchtype = document.salesweek.searchtype.options[w].value;
...
function getItem1(){
		var text = document.salesweek.letter1.value;
		var w = document.salesweek.searchtype1.selectedIndex;
		var searchtype = document.salesweek.searchtype1.options[w].value;
		if(text != ''){
...
<cfquery name="getgeneral" datasource="#dts#">
	select lastaccyear,filterall,lCATEGORY,lGROUP,lSIZE,lMATERIAL,lMODEL,lRATING,lAGENT,lDRIVER,lLOCATION,agentuserid,AGENTLISTUSERID,lTEAM,reportagentfromcust,filteritemreport,ddlitem from gsetup
</cfquery>
<!--- Add On 15-01-2010 --->
...
<table width="80%" border="0" cellspacing="0" cellpadding="3" class="data" align="center">
    ...
    <tr> 
        ...
        <select name="itemfrom">
				...
				<option value="#convertquote(itemno)#">#itemno# - #aitemno# - #desp#</option>
				</cfloop>
			</select>
			<cfif getgeneral.filterall eq "1">
