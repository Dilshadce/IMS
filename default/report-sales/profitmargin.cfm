...
<script type="text/javascript">
	...
	function selectlist(custno,fieldtype){
		...
		for (var idx=0;idx<document.getElementById(fieldtype).options.length;idx++){
			if (custno==document.getElementById(fieldtype).options[idx].value){
				socument.getElementById(fieldtype).options[idx].selected=true;
			}
		} 
	}
	...
	function show_info(rset){
		...
		for(i=0;i<itemnoArray.length;i++){
			...
			document.profitmargin.itemfrom.options.add(myoption);
		}
	}
	...
	function show_info1(rset){
		...
		for(i=0;i<itemnoArray.length;i++){
			...
			myoption.value = itemnoArray[i];
			document.profitmargin.itemto.options.add(myoption);
			}	
	}

	function getItem(){
		
		var text = document.profitmargin.letter.value;
		var w = document.profitmargin.searchtype.selectedIndex;
		var searchtype = document.profitmargin.searchtype.options[w].value;
	...
	function getItem1(){
		...
		var text = document.profitmargin.letter1.value;
		var w = document.profitmargin.searchtype1.selectedIndex;
		var searchtype = document.profitmargin.searchtype1.options[w].value;
...
<cfswitch expression="#url.type#">
    ...
    <cfcase value="groupmargin"><cfset trantype = "PROFIT MARGIN BY GROUP"></cfcase>
</cfswitch>
...
<cfelseif type eq "billmargin" or type eq "billitemmargin">
	<cfquery name="getproject" datasource="#dts#">
		select source,project from #target_project# <cfif lcase(HcomID) eq "mphcranes_i">order by project<cfelse> order by source</cfif>
	</cfquery>
   ...
   <tr> 
     ...
     <tr>
    <th>Filter by</th>
     <td><input type="checkbox" name="cbdiscount" id="cbdiscount" value="1">Include header discount</td>
     </tr>
    <tr>
    <th></th>
...
<form name="profitmargin" action="profitmargin7.cfm?trantype=#trantype#" method="post" target="_blank">
<table width="70%" border="0" cellspacing="0" cellpadding="3" class="data" align="center">
	<tr><input type="hidden" name="tran" id="tran" value="#target_arcust#" /><input type="hidden" name="fromto" id="fromto" value="" />
		<th>Report Format</th>
		<td><input type="radio" name="result" value="HTML" checked>HTML<br/>
