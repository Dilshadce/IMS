...
<cfif isdefined('form.locfrom') and isdefined('form.locto')>
<cfif form.locfrom neq "" and form.locto neq "">
<cfquery name="getalllocation" datasource='#dts#'>
select refno from ictran where location >='#form.locfrom#' and location <= '#form.locto#'
</cfquery>
...
<cfswitch expression="#form.result#">
 ...
 <cfset iDecl_UPrice = getgsetup2.Decl_UPrice>
	...
	<cfquery name="gettotal" datasource="#dts#">
			select sum(invgross) as invgross,sum(roundadj) as roundadj,sum(m_charge1) as m_charge1,sum(discount) as discount,sum(net) as net,sum(tax) as tax,sum(grand) as grand,sum(CS_PM_cash) as CS_PM_cash,sum(CS_PM_crcd)+sum(CS_PM_crc2) as CS_PM_crcd,sum(CS_PM_cheq) as CS_PM_cheq,sum(CS_PM_vouc) as CS_PM_vouc,sum(CS_PM_dbcd) as CS_PM_dbcd,sum(CS_PM_cashcd) as CS_PM_cashcd,sum(deposit) as deposit from artran
