...
<cfelse>
    ...
    <cfif form.userfrom neq "" and form.userto neq "">
     <Row ss:AutoFitHeight="0" ss:Height="20.0625">
     <cfwddx action = "cfml2wddx" input = "#form.userfrom# - #form.userto#" output = "wddxText6">
...
<cfloop query="getagent">
	...
	<cfquery name="getintran" datasource="#dts#">
		...
		where wos_date > #getgeneral.lastaccyear# and fperiod = '#form.periodfrom#' and <cfif isdefined('form.agentbycust')>custno in (select custno from #target_arcust# where agent='#agenno#')<cfelse>agenno = '#agenno#'</cfif> and (void = '' or void is null)
		<cfif isdefined("form.include") and form.include eq "yes">
	...
	<cfif isdefined("form.include") and form.include eq "yes">
		...
		<cfif getagent.agenno eq "">
			<Cell ss:StyleID="s26"><Data ss:Type="String">No - Agent</Data></Cell>
			<Cell ss:StyleID="s26"><Data ss:Type="String">No - Agent<</Data></Cell>
		<cfelse>
			<cfquery name="getagentname" datasource="#dts#">
				select desp from #target_icagent# where agent = '#getagent.agenno#'
			</cfquery>
			<Cell ss:StyleID="s26"><Data ss:Type="String">#getagent.agenno#</Data></Cell>
			<Cell ss:StyleID="s26"><Data ss:Type="String">#getagentname.desp#</Data></Cell>
		</cfif>
		<cfloop index="a" from="1" to="#noweek#">
			<cfif form.label eq "salesqty">
		...
		<cfif form.label eq "salesqty">
			<Cell ss:StyleID="s51"><Data ss:Type="String">#numberformat(arraysum(total),"0")#v
		<cfelse>
