...
<cfif type eq "QUO" or type eq "DO">
	<cfquery name="getheader" datasource="#dts#">
		select refno,refno2,type,custno,name,invgross,wos_date,pono,rem5,rem7,agenno,source,job from artran where type = '#type#' and toinv = '' and (void = '' or void is null)
		<cfif trim(custfrom) neq "" and trim(custto) neq "">
        	and custno >= '#custfrom#' and custno <= '#custto#'
        </cfif>
        <cfif trim(suppfrom) neq "" and trim(suppto) neq "">
        	and a.custno >= '#suppfrom#' and a.custno <= '#suppto#'
        </cfif>
...
<cfelseif url.type eq "4" or url.type eq "6" or url.type eq "7" or url.type eq "8">
    ...
    <cfif form.result eq 'EXCEL'>
		...
		<cfloop query="getheader">
			...
			<cfquery name="getbody" datasource="#dts#">
				select refno,trancode,qty,price,amt,itemno,desp,despa,comment,brem1,brem2,brem4,exported,writeoff,batchcode,expdate,source,dodate from 
  ...
  <Cell ss:StyleID="s32"><Data ss:Type="String">#wddxText10#</Data></Cell>
	...
	<table width="100%" border="0" class="data1" align="center" cellspacing="0" cellpadding="3">
		...
		<cfloop query="getheader">
			...
			<cfquery name="getbody" datasource="#dts#">
				select refno,trancode,qty,disamt,price,amt,itemno,desp,despa,comment,brem1,brem2,brem4,exported,writeoff,batchcode,expdate,source,dodate from 
				<cfif url.type eq '8'>ictranmat<cfelse>ictran</cfif> where refno = '#refno#' and type = '#ttype#' 
                <cfif url.type eq '6'>
                <cfif lcase(HcomID) eq "bestform_i"><cfif isdefined("form.checkbox1")><cfelse>and shipped < qty</cfif></cfif>
                </cfif>
			...
			<cfloop query="getbody">
                ...
                <cfset sno=sno+1>
              				...
              				<td>&nbsp;</td>
            				<td nowrap>
							<cfif cnt eq 0><a href="inforecastreport2e1.cfm?itemno=#urlencodedformat(itemno)#&itembal=0&pf=&pt=&cf=&ct=&gpf=&gpt=&projfr=&projto=">#Itemno# - <cfif lcase(HcomID) neq "bestform_i">#getbody.desp#&nbsp;#getbody.despa#<cfelse><cfif itemno eq '..'>#tostring(getbody.comment)#<cfelse>#getbody.desp#&nbsp;#getbody.despa#</cfif></cfif></a>
							</cfif>						</td>
							<cfif lcase(HcomID) eq "sumiden_i" and (url.type eq "6" or url.type eq "4")>
                            <td></td>
                            <td></td>
...
#geticdata.batchcode# <br>Expire Date: #dateformat(geticdata.expdate,'dd/mm/yyyy')#</div></td>
                ...
                <cfset sno=sno+1>
						...
						<td>&nbsp;</td>
					  	...
					  	<td nowrap><a href="inforecastreport2e1.cfm?itemno=#urlencodedformat(itemno)#&itembal=0&pf=&pt=&cf=&ct=&gpf=&gpt=&projfr=&projto=">#Itemno# - <cfif lcase(HcomID) neq "bestform_i">#getbody.desp#&nbsp;#getbody.despa#<cfelse><cfif itemno eq '..'>#tostring(getbody.comment)#<cfelse>#getbody.desp#&nbsp;#getbody.despa#</cfif></cfif></a></td>
                        <cfif lcase(HcomID) eq "sumiden_i" and (url.type eq "6" or url.type eq "4")>
                            <td></td>
                            <td></td>
                        </cfif>
