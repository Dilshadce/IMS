...
<cfif type eq "QUO" or type eq "DO">
	<cfquery name="getheader" datasource="#dts#">
		select refno,refno2,type,custno,name,invgross,wos_date,pono,rem5,rem7,agenno from artran where type = '#type#' and toinv = '' and (void = '' or void is null)
		<cfif trim(custfrom) neq "" and trim(custto) neq "">and custno >= '#custfrom#' and custno <= '#custto#'</cfif>
		...
		order by refno
            ...
            <cfquery name="getheader" datasource="#dts#">
 select a.refno,a.refno2,a.type,a.custno,a.name,a.agenno,a.invgross,a.wos_date,b.itemno,b.desp,a.pono,a.rem5,a.rem7,a.discount from artran a,ictran b where 
			a.refno = b.refno and a.type = b.type and a.type = '#billtype#' and a.order_cl = '' and (b.shipped+b.writeoff) < qty 
		and (a.void = '' or a.void is null)
            	<cfif trim(custfrom) neq "" and trim(custto) neq "">and a.custno >= '#custfrom#' and a.custno <= '#custto#'</cfif>
		...
		<cfif url.type neq "6">
			<cfif trim(productfrom) neq "" and trim(productto) neq ""> and b.itemno >= '#productfrom#' and b.itemno <= '#productto#' </cfif> 
		</cfif>
		<cfif deldatefrom neq "" and deldateto neq "">
        <cfif lcase(hcomid) eq "dgalleria_i">
        	and a.rem7 between '#ndeldatefrom#' and '#ndeldateto#'
        <cfelse>
			and cast(concat_ws('-',substring(b.brem1,1,2),substring(b.brem1,4,2),substring(b.brem1,7,2)) as date) between '#ndeldatefrom#' and '#ndeldateto#'
		</cfif>
        </cfif>
 ...
 group by b.itemno <cfif isdefined('form.sortbydelivery')>order by a.rem7<cfelse>order by b.itemno</cfif>
	...
	<cfquery name="getheader" datasource="#dts#">
		<cfswitch expression="#url.type#">
			...
			a.refno = b.refno and a.type = b.type and a.type = 'PO' and a.order_cl = '' and (b.shipped+b.writeoff) < qty and (a.void='' or a.void is null)
			</cfcase>
			<cfcase value="4">
			select a.refno,a.refno2,a.type,a.custno,a.name,a.invgross,a.discount,a.wos_date,a.agenno,a.pono,a.rem5,a.rem7,
			b.brem1,b.brem2,a.pono,a.currcode from artran a, ictran b where a.refno = b.refno and 
			a.type = b.type and a.type = 'PO' <cfif isdefined("form.checkbox1")><cfelse>and a.order_cl = '' and (b.shipped+b.writeoff) < qty</cfif> and (a.void='' or a.void is null)
			</cfcase>
			<cfcase value="5">
			...
			b.brem1,b.brem2,b.exported,a.pono from artran a, ictran b where a.refno = b.refno and a.type = b.type and
	...
	<table width="90%" border="0" class="data1" align="center" cellspacing="0" cellpadding="3">
		...
		<cfloop query="getheader">
			...
			<cfquery name="getbody" datasource="#dts#">
      			select qty,price,amt,itemno,desp,expdate,taxamt,disamt from ictran where refno = '#refno#' and type = '#ttype#' 
      		</cfquery>
...
<cfelseif url.type eq "4" or url.type eq "6" or url.type eq "7" or url.type eq "8">
    ...
    <cfif form.result eq 'EXCEL'>
		...
		<cfloop query="getheader">
			...
			<cfquery name="getbody" datasource="#dts#">
				select refno,trancode,qty,price,amt,itemno,desp,comment,brem1,brem2,brem4,exported,writeoff,batchcode,expdate,source,dodate from 
				...
				<cfif url.type eq '6' or url.type eq '7' or url.type eq '8'>
                    ...
                    <cfif lcase(hcomid) eq "dgalleria_i">
       				<cfelse>
					and cast(concat_ws('-',substring(brem1,1,2),substring(brem1,4,2),substring(brem1,7,2)) as date) between '#ndeldatefrom#' and '#ndeldateto#'
					</cfif>
                    </cfif>
				</cfif>
                        ...
                        <cfwddx action = "cfml2wddx" input = "#getbody.exported#" output = "wddxText2">
                         ...
                         <cfwddx action = "cfml2wddx" input = "#getbody.brem4#" output = "wddxText3">#wddxText3#</cfif></Data></Cell>
					  	<Cell ss:StyleID="s32"><Data ss:Type="String"><cfif ttype2 eq 'RC'><b>Request</b> <cfelse><b>Delivery</b> 
                                ...
                                <cfelse>
                                <cfwddx action = "cfml2wddx" input = " #getbody.brem1#" output = "wddxText4">#wddxText4#</cfif>
						  		<b>
								<cfif lcase(HcomID) eq "bestform_i">
                                Material :
                                <cfelse>
  ...
  <Cell ss:StyleID="s32"><Data ss:Type="String">#wddxText10#</Data></Cell>
	...
	<table width="100%" border="0" class="data1" align="center" cellspacing="0" cellpadding="3">
		...
		<cfloop query="getheader">
			...
			<cfquery name="getbody" datasource="#dts#">
				select refno,trancode,qty,disamt,price,amt,itemno,desp,comment,brem1,brem2,brem4,exported,writeoff,batchcode,expdate,source,dodate from 
				<cfif url.type eq '8'>ictranmat<cfelse>ictran</cfif> where refno = '#refno#' and type = '#ttype#' 
                <cfif url.type eq '6'>
                <cfif lcase(HcomID) eq "bestform_i"><cfif isdefined("form.checkbox1")><cfelse>and shipped < qty</cfif></cfif>
                </cfif>
			...
			<cfloop query="getbody">
                ...
                <cfset sno=sno+1>
              				...
              				<td>&nbsp;</td>
            				<td nowrap>
							<cfif cnt eq 0><a href="inforecastreport2e1.cfm?itemno=#urlencodedformat(itemno)#&itembal=0&pf=&pt=&cf=&ct=&gpf=&gpt=&projfr=&projto=">#Itemno# - <cfif lcase(HcomID) neq "bestform_i">#getbody.desp#<cfelse><cfif itemno eq '..'>#tostring(getbody.comment)#<cfelse>#getbody.desp#</cfif></cfif></a>
							</cfif>						</td>
							<cfif lcase(HcomID) eq "sumiden_i" and (url.type eq "6" or url.type eq "4")>
                            <td></td>
                            <td></td>
...
#geticdata.batchcode# <br>Expire Date: #dateformat(geticdata.expdate,'dd/mm/yyyy')#</div></td>
                ...
                <cfset sno=sno+1>
						...
						<td>&nbsp;</td>
					  	...
					  	<td nowrap><a href="inforecastreport2e1.cfm?itemno=#urlencodedformat(itemno)#&itembal=0&pf=&pt=&cf=&ct=&gpf=&gpt=&projfr=&projto=">#Itemno# - <cfif lcase(HcomID) neq "bestform_i">#getbody.desp#<cfelse><cfif itemno eq '..'>#tostring(getbody.comment)#<cfelse>#getbody.desp#</cfif></cfif></a></td>
                        <cfif lcase(HcomID) eq "sumiden_i" and (url.type eq "6" or url.type eq "4")>
                            <td></td>
                            <td></td>
                        </cfif>
						<cfif url.type eq "4" and (lcase(HcomID) eq "ideal_i" or lcase(HcomID) eq "idealb_i")><td>&nbsp;</td></cfif>
					  	...
					  	<td nowrap>
								...
								<cfelse>
						  		...
						  		<b>Date :</b><cfif lcase(HcomID) eq "bestform_i">#dateformat(getbody.dodate,'DD/MM/YYYY')#<cfelseif lcase(HcomID) eq "sumiden_i">#getheader.rem5#<cfelse>#getbody.brem1#</cfif><br>
...
<cfelseif type eq "QUO">
	<table width="80%" border="0" class="data1" align="center" cellspacing="0" cellpadding="3">
		...
		<cfloop query="getheader">
			...
			<cfquery name="getbody" datasource="#dts#">
      			select refno,qty,price,amt,itemno,trancode,desp,pono from ictran where refno = '#refno#' and type = '#ttype#' 
      		</cfquery>
...
<cfif form.result eq 'EXCEL'>
		...
		<cfloop query="getheader">
			...
			<cfquery name="getbody" datasource="#dts#">
      			select refno,qty,desp,price,amt,itemno,brem1,trancode,writeoff,requiredate,replydate from ictran where refno = '#refno#' and type = '#ttype#' 
      			<cfif trim(productfrom) neq "" and trim(productto) neq "">
        			and itemno >= '#productfrom#' and itemno <= '#productto#'
      			</cfif>
      			<cfif deldatefrom neq "" and deldateto neq "">
...
<cfelseif url.type eq "5">
    ...
    truncate outreporttemp
    </cfquery>
    
    </cfif>
    <cfif lcase(hcomid) eq "viva_i" or type eq 'SO'>
	<table width="110%" border="0" class="data1" align="center" cellspacing="0" cellpadding="3">
    <cfelse>
    <table width="85%" border="0" class="data1" align="center" cellspacing="0" cellpadding="3">
		...
		<cfloop query="getheader">
            ...
            insert into outreporttemp (type,wos_date,refno,pono,customer,qtyorder,deliverydate,writeoff,qtyoutstand,price,amount) values ('#type#','#dateformat(wos_date,"dd/mm/yy")#','#refno#','#pono#','#custno# - #name#','','','','','','')
            </cfquery>
        	...
        	<tr> 
                ...
                <td nowrap>#agenno#</td>
         	 	...
         	 	<td nowrap>#pono#</td>
          		<td colspan="2">#custno# - #name#</td>
          		<td><cfif lcase(hcomid) eq "dgalleria_i"><cfif rem7 neq '' and rem7 neq '0000-00-00' and isdate(rem7)>#dateformat(rem7,'DD/MM/YYYY')#</cfif><cfelse>&nbsp;</cfif></td>
          		<td>&nbsp;</td>
          		<td>&nbsp;</td>
				<td>&nbsp;</td>
        	</tr>
			...
			<cfquery name="getbody" datasource="#dts#">
      			select refno,qty,desp,price,price_bil,amt,itemno,brem1,brem2,brem4,exported,trancode,writeoff,deliverydate,disamt from 
			...
			<cfloop query="getbody">
			  ...
			  <cfif getrcqty.qty1 neq "">
                ...
                <cfif result eq 'PDF'>
            <cfquery name="insert" datasource="#dts#">
            insert into outreporttemp (type,wos_date,refno,pono,customer,qtyorder,deliverydate,writeoff,qtyoutstand,price,disamt,amount) values ('','#itemno#','#brem4#',' ','','#orderqty#','#rcqty#','#val(getbody.writeoff)#','#outstand#','#xprice#','#disamt#','#numberformat(rightamt,",_.__")#')
            </cfquery>
          		...
          		<tr> 
            		<td>&nbsp;</td>
           		  ...
           		  <td>#Itemno#<cfif lcase(HcomID) eq "kjcpl_i" or lcase(HcomID) eq "mlpl_i" or lcase(HcomID) eq "viva_i"> - #desp#</cfif></td>
                    <cfquery name="getaitemno" datasource="#dts#">
                    select aitemno from icitem where itemno='#getbody.itemno#'
                    </cfquery> 
                      <cfif lcase(hcomid) eq "vsolutionspteltd_i">
                        <cfquery name="getcurrencycode" datasource="#dts#">
    select currcode from #target_arcust# where custno='#getheader.custno#'
    </cfquery>
                    <td><div align="left">#getaitemno.aitemno#</div></td>
                  </cfif>
                  <td></td>
					<td nowrap>#brem1#</td>
					<cfif lcase(hcomid) eq "net_i">
...
<td></td>
					...
					<td><div align="center">#outstand#</div></td>
                    <cfif lcase(hcomid) eq "vsolutionspteltd_i">
                    <td>#getcurrencycode.currcode#</td>
                    <td><div align="center">#rightamt_bil/qty#</div></td>
                    <td><div align="right">#numberformat(rightamt_bil,",_.__")#</div></td>
                    </cfif>
			...
			<cfoutput> 
                ...
                <cfif result eq 'PDF'>
            <cfquery name="insert" datasource="#dts#">
            insert into outreporttemp (type,wos_date,refno,pono,customer,qtyorder,deliverydate,writeoff,qtyoutstand,price,disamt,amount,counter_1) values ('','','','','','#ttorderqty#','#ttrcqty#','#ttwriteoff#','#ttoutstand#','','#ttdisamt#','#numberformat(ttrightamt,",_.__")#','1')
            </cfquery>
				...
				<tr bgcolor="##83B8ED"> 
                    ...
                    <td>&nbsp;</td>
				  	...
				  	<td><div align="center">#ttoutstand#</div></td>
                    <td><div align="right"></div></td>
                    <td><div align="right"><cfif hcomid eq 'net_i'>#numberformat(ttdisamt+getheader.discount,',_.__')#<cfelse>#ttdisamt#</cfif></div></td>
				  	<td><div align="right"><cfif hcomid eq 'net_i'>#numberformat(rightamt-getheader.discount,',_.__')#<cfelse>#numberformat(rightamt,",_.__")#</cfif></div></td>
        		</tr>
        		<tr> 
          			<td colspan="80%"><hr></td>
        		</tr>
                <cfif hcomid eq 'net_i'>
                <cfset totalrightamt = (totalrightamt + (rightamt-getheader.discount))>
                <cfset totaldisamt = totaldisamt + ttdisamt+getheader.discount>
                <cfelse>
        ...
        <cfif result eq 'PDF'>
            <cfquery name="insert" datasource="#dts#">
            insert into outreporttemp (type,wos_date,refno,pono,customer,qtyorder,deliverydate,writeoff,qtyoutstand,price,disamt,amount,counter_1) values ('','','','Total','','#totalorderqty#','#totalrcqty#','#totwriteoff#','#totaloutstand#','','#totaldisamt#','#numberformat(totalrightamt,",_.__")#','1')
            </cfquery>
    ...
    <cfif result eq 'PDF'>
     ...
     <cfquery name="MyQuery" datasource="#dts#">
            select * from outreporttemp
            </cfquery>
    
    <cfreport template="outreport.cfr" format="PDF" query="MyQuery"><!--- or "FlashPaper" or "Excel" or "RTF" --->
