...
<cfquery name="getitem" datasource="#dts#">
	select itemno, desp, qtybf,price,wos_group from icitem where
	itemno <> ''
...
<table width="100%" border="0" align="center" cellspacing="0">
  ...
  <tr>
    ...
    <td width="10%"><div align="right"><font size="2" face="Times New Roman, Times, serif"><cfif lcase(HcomID) eq "kjcpl_i" or lcase(HcomID) eq "mlpl_i" or lcase(HcomID) eq "viva_i">PO+OH-SO-QUO<cfelse>PO+OH-SO</cfif></font></div></td>
  ...
  <cfloop query="getgroup">
    ...
    <cfoutput query="getdata">
      ...
      <cfquery name="getin" datasource="#dts#">
		...
		where itemno = '#itemno#' and (type = 'RC' or type = 'CN' or type = 'OAI' or type = 'TRIN') 
		and (void = '' or void is null) 
      ...
      <cfquery name="getout" datasource="#dts#">
      	select sum(qty)as sumqty from ictran where itemno = '#itemno#' 
      ...
      <cfquery name="getso" datasource="#dts#">
      	select sum(qty-shipped-writeoff)as sumqty from ictran where type = 'SO' 
		and itemno = '#itemno#' and (void = '' or void is null) <cfif lcase(HcomID) eq "kjcpl_i" or lcase(HcomID) eq "mlpl_i" or lcase(HcomID) eq "viva_i"><cfelse>and wos_date > #getgeneral.lastaccyear#</cfif>
		<cfif trim(form.projectfrom) neq "" and trim(form.projectto) neq "">
			and source >= '#form.projectfrom#' and source <= '#form.projectto#'
		</cfif>
      </cfquery>
      ...
      <cfif getdata.qtybf neq "">
        <cfset itembal=val(getdata.qtybf)>
      </cfif>
      <cfquery name="getquo" datasource="#dts#">
      	select sum(qty-shipped-writeoff)as sumqty from ictran where type = 'QUO' 
		and itemno = '#itemno#' and (void = '' or void is null) <cfif lcase(HcomID) eq "kjcpl_i" or lcase(HcomID) eq "mlpl_i" or lcase(HcomID) eq "viva_i"><cfelse>and wos_date > #getgeneral.lastaccyear#</cfif>
		<cfif trim(form.projectfrom) neq "" and trim(form.projectto) neq "">
			and source >= '#form.projectfrom#' and source <= '#form.projectto#'
		</cfif>
      ...
      <cfif getquo.sumqty neq "">
        <cfset quoqty=val(getquo.sumqty)>
      </cfif>
      <cfset stockin=inqty>
