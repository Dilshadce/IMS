...
<cfif hcomid eq "pnp_i">
  View Location Stock Card Details Menu
  <cfelse>
  View
...
<cfquery name="getgeneral" datasource="#dts#">
	select lastaccyear,filterall,lCATEGORY,lGROUP,lSIZE,lMATERIAL,lMODEL,lRATING,lAGENT,lDRIVER,lLOCATION,singlelocation,lbrand from gsetup
</cfquery>
...
<cfquery name="getitem" datasource="#dts#">
    ...
    where (itemtype <> "SV" or itemtype is null) order by <cfif getdealer_menu.productSortBy neq "">#getdealer_menu.productSortBy#<cfelse>itemno</cfif>
</cfquery>
...
<h3> <a href="location_listingmenu.cfm"><cfoutput>#getgeneral.lLOCATION#</cfoutput> Reports Menu</a> >> <a><font size="2">View <cfoutput>#getgeneral.lLOCATION#</cfoutput> Stock Card
    <cfif hcomid eq "pnp_i">
      Details
    </cfif>
    </font></a> </h3>
<cfform action="locationstockcheck2.cfm" name="form" method="post" target="_blank">
  ...
  <table width="65%" border="0" align="center" class="data">
    ...
    <tr>
      <td nowrap><input type="checkbox" name="include0" id="1" value="yes">
        <label for="include0">Include 0 Figure</label></td>
      <td nowrap colspan="2"><input type="checkbox" name="groupitem" id="1" value="yes"  <cfif (lcase(hcomid) eq "swisspost_i"  or lcase(hcomid) eq "swisspostdemo_i") and husergrpid eq "luser">style="visibility:hidden" checked </cfif>>
        <label <cfif (lcase(hcomid) eq "swisspost_i"  or lcase(hcomid) eq "swisspostdemo_i") and husergrpid eq "luser">style="visibility:hidden"</cfif> for="groupitem">Group Location</label>
        &nbsp;&nbsp;&nbsp;
        <input type="checkbox" name="dodate" id="dodate" value="yes" checked>
        <label for="include0">According to DO Date</label>
        &nbsp;&nbsp;&nbsp;
        <input type="checkbox" name="reserve" id="reserve" value="yes">
        <label for="include0">Include reserved Qty</label>
        &nbsp;&nbsp;&nbsp;
        <input type="checkbox" name="sortByGroup" id="sortByGroup" value="yes">
        <label for="include0">Sort By Group</label></td>
    </tr>
    <tr>
    ...
    <tr>
      ...
      <td><div align="center">To</div></td>
      <td colspan="2" nowrap><select name="groupto">
          <option value="">Choose a <cfoutput>#getgeneral.lGROUP#</cfoutput></option>
          <cfoutput query="getgroup">
            <option value="#getgroup.wos_group#">#getgroup.wos_group# - #getgroup.desp#</option>
          </cfoutput>
    ...
    <tr>
      <th><cfoutput>#getgeneral.lbrand#</cfoutput></th>
      <td><div align="center">From</div></td>
    ...
    <cfif lcase(hcomid) neq "ecraft_i" and lcase(hcomid) neq "ovas_i">
      ...
      <tr>
        ...
        <td colspan="2"><cfinput type="text" name="dateto" maxlength="10" validate="eurodate" size="10">
          (DD/MM/YYYY)</td>
      </tr>
    ...
    <cfif getgeneral.singlelocation eq 'Y'>
      <tr>
        ...
        <td><div align="center">From</div></td>
        <td colspan="2"><select name="locfrom">
