    ...
    <cfif lcase(hcomid) neq "acht_i">
    <td><a onMouseOver="JavaScript:this.style.cursor='hand';" onClick="document.getElementById('expressservicelist').value = unescape('#URLENCODEDFORMAT(getitem1.itemno)#');ajaxFunction(window.document.getElementById('itemDetail'),'/default/report-location/locationexpressadjustment/addItemAjax.cfm?itemno=#URLENCODEDFORMAT(getitem1.itemno)#'+'&location='+document.getElementById('location').value+'&date='+document.getElementById('wos_date').value);setTimeout('updateVal();',750);document.getElementById('expressservicelist').focus();ColdFusion.Window.hide('searchitem');" >SELECT</a></td>
    </cfif>
    </tr>
    </cfloop>
