<cfif isdefined("form.boxes")>
	<cfset dts1 = replace(LCASE(dts),"_i","_a","all")>
 	...
 	<cfif form.postType eq "3">
    	<cfquery datasource="#dts1#" name="truncate">
        	truncate artran
        </cfquery>
        <cfquery datasource="#dts1#" name="truncate">
        	truncate ictran
	...
	<cfloop index="entry1" list="#form.boxes#">
		<cfset entry1a = replace(entry1,"|",",","all")>
	    ...
	    <cfset count = 0>
		<cfloop index="entry2" list="#entry1a#">
		    <cfset count = count +1>
		    <cfif count eq 1>
				<cfset arefno = entry2>
                <cfif arefno eq "YHFTOKCF">
					<cfset arefno = "">
				</cfif>
		    <cfelseif count eq 2>
				<cfset atype = entry2>
		    <cfelseif count eq 3>
				<cfset acustno = entry2>
		    </cfif>
		</cfloop>
	   
	    <cfquery datasource="#dts#" name="update">
	    	update artran set IRAS_POSTED='P' WHERE refno='#arefno#' and type='#atype#' and custno='#acustno#'
	    </cfquery>
	    
	    <cfset checkdup = 0>
	    <cfif form.postType eq "1">
	    	<cfquery datasource="#dts1#" name="delete">
	        	delete from artran WHERE refno='#arefno#' and type='#atype#' and custno='#acustno#'
	        </cfquery>
            <cfquery datasource="#dts1#" name="delete2">
	        	delete from ictran WHERE refno='#arefno#' and type='#atype#' and custno='#acustno#'
	        </cfquery>
	    <cfelseif form.postType eq "2">
	    	<cfquery datasource="#dts1#" name="update2">
	        	select * from artran WHERE refno='#arefno#' and type='#atype#' and custno='#acustno#'
	        </cfquery>
	        <cfif update2.recordcount gt 0>
	        	<cfset checkdup = 1>
	        </cfif>
        <cfelseif form.postType eq "4">    
          	<cfquery datasource="#dts#" name="update">
	    	update artran set IRAS_POSTED='' WHERE refno='#arefno#' and type='#atype#' and custno='#acustno#'
	    	</cfquery> 
			<cfset checkdup = 1> 
            <cfquery datasource="#dts1#" name="delete">
	        	delete from artran WHERE refno='#arefno#' and type='#atype#' and custno='#acustno#'
	    ...
	    <cfif checkdup eq 1>
	    <cfelse>
	    	<cfquery datasource="#dts#" name="select">
	 			...
	 			from artran 
	 			where refno='#arefno#' and type='#atype#' and custno='#acustno#'
			</cfquery>
			<cfquery datasource="#dts#" name="select_ictran">
	 		select 	TYPE,REFNO,REFNO2,TRANCODE,CUSTNO,FPERIOD,WOS_DATE,CURRRATE,ITEMCOUNT,LINECODE,ITEMNO,DESP,DESPA,AGENNO,LOCATION,SOURCE,JOB,SIGN,QTY_BIL,PRICE_BIL,UNIT_BIL,AMT1_BIL,DISPEC1,DISPEC2,DISPEC3,DISAMT_BIL,AMT_BIL,TAXPEC1,TAXPEC2,TAXPEC3,TAXAMT_BIL,IMPSTAGE,QTY,PRICE,UNIT,AMT1,DISAMT,AMT,TAXAMT,FACTOR1,FACTOR2,DONO,DODATE,SODATE,BREM1,BREM2,BREM3,BREM4,PACKING,NOTE1,NOTE2,GLTRADAC,UPDCOST,GST_ITEM,TOTALUP,WITHSN,NODISPLAY,GRADE,PUR_PRICE,QTY1,QTY2,QTY3,QTY4,QTY5,QTY6,QTY7,QTY_RET,TEMPFIGI,SERCOST,M_CHARGE1,M_CHARGE2,ADTCOST1,ADTCOST2,IT_COS,AV_COST,BATCHCODE,EXPDATE,POINT,INV_DISC,INV_TAX,SUPP,EDI_COU1,WRITEOFF,TOSHIP,SHIPPED,NAME,DEL_BY,VAN,GENERATED,UD_QTY,TOINV,EXPORTED,EXPORTED1,EXPORTED2,EXPORTED3,BRK_TO,SV_PART,LAST_YEAR,VOID,SONO,MC1_BIL,MC2_BIL,USERID,DAMT,OLDBILL,WOS_GROUP,CATEGORY,AREA,SHELF,TEMP,TEMP1,BODY,TOTALGROUP,MARK,TYPE_SEQ,PROMOTER,TABLENO,MEMBER,TOURGROUP,TRDATETIME,TIME,BOMNO,DEFECTIVE,M_CHARGE3,M_CHARGE4,M_CHARGE5,M_CHARGE6,M_CHARGE7,MC3_BIL,MC4_BIL,MC5_BIL,MC6_BIL,MC7_BIL,TITLE_ID,TITLE_DESP,NOTE_A
	   		...
	   		<cfif select.recordcount neq 0>
                ...
                <cfinvoke component="cfc.date" method="getFormatedDateTime" inputDate="#select.UPDATED_ON#" returnvariable="select.UPDATED_ON"/>    
	   			...
	   			<cfquery datasource="#dts1#" name="insertData">
	    			INSERT INTO artran 
            ...
            <cfif select_ictran.recordcount neq 0>
               ...
               <cfinvoke component="cfc.date" method="getFormatedDate" inputDate="#select_ictran.WOS_DATE#" returnvariable="select_ictran.WOS_DATE"/>  
                ...
                <cfquery datasource="#dts1#" name="insertData">
                        ...
                        ('#select_ictran.TYPE#','#select_ictran.REFNO#','#select_ictran.REFNO2#','#select_ictran.TRANCODE#','#select_ictran.CUSTNO#','#select_ictran.FPERIOD#','#select_ictran.WOS_DATE#','#select_ictran.CURRRATE#','#select_ictran.ITEMCOUNT#','#select_ictran.LINECODE#','#select_ictran.ITEMNO#','#select_ictran.DESP#','#select_ictran.DESPA#','#select_ictran.AGENNO#','#select_ictran.LOCATION#','#select_ictran.SOURCE#','#select_ictran.JOB#','#select_ictran.SIGN#','#select_ictran.QTY_BIL#','#select_ictran.PRICE_BIL#','#select_ictran.UNIT_BIL#','#select_ictran.AMT1_BIL#','#select_ictran.DISPEC1#','#select_ictran.DISPEC2#','#select_ictran.DISPEC3#','#select_ictran.DISAMT_BIL#','#select_ictran.AMT_BIL#','#select_ictran.TAXPEC1#','#select_ictran.TAXPEC2#','#select_ictran.TAXPEC3#','#select_ictran.TAXAMT_BIL#','#select_ictran.IMPSTAGE#','#select_ictran.QTY#','#select_ictran.PRICE#','#select_ictran.UNIT#','#select_ictran.AMT1#','#select_ictran.DISAMT#','#select_ictran.AMT#','#select_ictran.TAXAMT#','#select_ictran.FACTOR1#','#select_ictran.FACTOR2#','#select_ictran.DONO#','#select_ictran.DODATE#','#select_ictran.SODATE#','#select_ictran.BREM1#','#select_ictran.BREM2#','#select_ictran.BREM3#','#select_ictran.BREM4#','#select_ictran.PACKING#','#select_ictran.NOTE1#','#select_ictran.NOTE2#','#select_ictran.GLTRADAC#','#select_ictran.UPDCOST#',
...
<script>
parent.document.getElementById('radio').checked='true';
parent.document.getElementById('radio10').checked='true';
parent.document.getElementById('radio11').checked='true';
</script>


<cfif form.postType eq "1">
<font color="#FF0000">The bills is successfully post.</font>
<cfelseif form.postType eq "2">
<font color="#FF0000">The bills is successfully post.</font>
<cfelseif form.postType eq "3">
<font color="#FF0000">The bills is successfully post.</font>
<cfelseif form.postType eq "4">
<font color="#FF0000">The bills is successfully unpost.</font>
</cfif>