<cfquery name="getData" datasource="#dts#">
	SELECT	custno,name,name2,comUEN,
...
<cfscript>
	...
	row = sheet.createRow(javacast("int",count));	
		...
		cell.setCellValue(javacast("string","PURCHASE CODE (12)"));
		cell.setCellStyle(sBody);
		cell = row.createCell(javacast("int",29));
		cell.setCellValue(javacast("string","PURCHASE RETURN CODE (12)"));
		cell.setCellStyle(sBody);
	...
	FOR(i=1;i LTE getData.recordcount;i=i+1){			
		...
		row = sheet.createRow(javacast("int",count));
			...
			cell.setCellStyle(sBody);
			cell = row.createCell(javacast("int",49));
			cell.setCellValue(javacast("string",getData["status"][i]));
			cell.setCellStyle(sBody);				
	}
