...
<cfif isdefined('url.month')>
  <cfif val(startdate) neq 0>
    ...
    <!---Updated by Nieo 20180108 1139 --->
    created_on > #createdate(getmonth.myear,1,7)#
    <!---Updated by Nieo 20180108 1139 --->
    and payrollperiod = "#url.month#"<!---month(assignmentslipdate) = "#url.month#"--->
    ...
    <input type="checkbox" id="checkAll" value="checkAll" onChange="checkall(this.value)"> Select All <br />
    <input type="checkbox" id="checkApproved" value="checkApproved" onChange="checkall(this.value)"> Select All Approved<br />
    <input type="checkbox" id="checkSubmitted" value="checkSubmitted" onChange="checkall(this.value)"> Select All Submitted<br />
    <hr />
    <cfloop query="getbatch">
<!---    <input type="checkbox" name="batches" id="batches" value="#getbatch.batches#">&nbsp;&nbsp;#getbatch.batches#-#dateformat(getbatch.giropaydate,'dd/mm/yyyy')#<cfif dts eq "bepstest_i"><cfif listfindnocase(submitedlist,getbatch.batches) neq 0>&nbsp;(Approved)</cfif><cfif listfindnocase(submitedlist2,getbatch.batches) neq 0>&nbsp;(Submitted)</cfif></cfif>--->
