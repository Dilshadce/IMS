	...
	<cfif post eq "post">
		...
		<cfif type eq "RC" or type eq "CN"<!---  or type eq "PR" --->>
			<cfquery name="insertpost1" datasource="#dts#">
				...
				values('#type#','#get_credit_card2_code.creditcardaccount2#','#ceiling(fperiod)#','#dateformat(wos_date,"dd/mm/yy")#','#billno#','#refno2#',
				...
				'#numberformat(val(getartran.cs_pm_crc2),".__")#','-#numberformat(val(getartran.cs_pm_crc2_bil),".__")#','#currrate#','<!--- #note# --->','#getartran.rem6#','#dateformat(wos_date,"dd/mm/yy")#')
			</cfquery>
