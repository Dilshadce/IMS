...
<cfquery name="gettemp" datasource="#dts#">
	...
	from temptrx
	where trxdate<>'' 
	group by accno<cfif wpitemtax eq "Y">,GSTTYPE</cfif><!--- <cfif lcase(HcomID) eq "probulk_i"> --->,SOURCE,JOB<!--- </cfif> --->
	order by trxbillno,accno<cfif wpitemtax eq "Y">,GSTTYPE</cfif><!--- <cfif lcase(HcomID) eq "probulk_i"> --->,SOURCE,JOB<!--- </cfif> --->;
</cfquery>
...
<cfloop query="gettemp">
	<tr <cfif problembatch eq 1>style="background-color:##FF0000" </cfif>>
		...
		<cfelse>
			<cfset acctype = "Cr">
			<td></td>		
			<td><div align="right">#numberformat(val(gettemp.amtt)-val(gst_item_value)+val(check_misc.sum_without_misc),".__")#</div></td>
		</cfif>
