...
<cfset payrollyear = getpayroll.myear>
    ...
    <cfelse>
		...
		<cffile action="WRITE" file="#HRootPath#\Excel_Report\MPpostingCNDN#dateformat(now(),'ddmmyyyy')#.csv"
        ...
        <cffile action="APPEND" file="#HRootPath#\Excel_Report\MPpostingCNDN#dateformat(now(),'ddmmyyyy')#.csv" 
    	...
    	<cfloop query="getartran">
            ...
            <cfquery name="getictran" datasource="#dts#">
                SELECT ic.desp, ic.amt_bil, ic.taxamt_bil, ic.brem5, ic.refno2, aslip.placementno, pmt.location, pmt.jobpostype
                FROM ictran as ic
                LEFT JOIN assignmentslip as aslip
                ON ic.refno2 = aslip.invoiceno
			...
			<cfif #getartran.fperiod# GT 12>
            	<cffile action="APPEND" file="#HRootPath#\Excel_Report\MPpostingCNDN#dateformat(now(),'ddmmyyyy')#.csv" 
   				output="1,#getbatches2.batches#,#counter#,#getartran.custno#,#getartran.refno#,3,AP-MONTH=#evaluate('getartran.fperiod MOD 12')#,#dateformat(getdate.wos_date,'yyyymmdd')#,0,#dateformat(getdate.duedate,'yyyymmdd')#,1,1,GST,3,#NumberFormat(getartran.tax_bil, '.__')#,#dateformat(getdate.wos_date,'yyyy')#,#evaluate('getartran.fperiod MOD 12')#,"
  				addnewline="Yes">
            <cfelse>
            	<cffile action="APPEND" file="#HRootPath#\Excel_Report\MPpostingCNDN#dateformat(now(),'ddmmyyyy')#.csv" 
   				output="1,#getbatches2.batches#,#counter#,#getartran.custno#,#getartran.refno#,3,AP-MONTH=#getartran.fperiod#,#dateformat(getdate.wos_date,'yyyymmdd')#,0,#dateformat(getdate.duedate,'yyyymmdd')#,1,1,GST,3,#NumberFormat(getartran.tax_bil, '.__')#,#dateformat(getdate.wos_date,'yyyy')#,#getartran.fperiod#,"
  				addnewline="Yes">
            </cfif>
		...
		<cfset filename = "MPpostingCNDN#dateformat(now(),'ddmmyyyy')#.csv">

		<cfheader name="Content-Disposition" value="inline; filename=#filename#">		
		<cfcontent type="application/vnd.ms-excel" deletefile="yes" file="#HRootPath#\Excel_Report\MPpostingCNDN#dateformat(now(),'ddmmyyyy')#.csv">--->
	</cfif>
</cfoutput>