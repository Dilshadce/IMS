	...
	<cfif post eq "post">
		...
		<cfif type eq "RC" or type eq "CN"<!---  or type eq "PR" --->>
			<cfquery name="insertpost1" datasource="#dts#">
				...
				values('#type#','#get_deposit_code.depositaccount#','#ceiling(fperiod)#','#dateformat(wos_date,"dd/mm/yy")#','#billno#','#refno2#',
				...
				'#numberformat(val(getartran.deposit),".__")#','-#numberformat(val(getartran.deposit_bil),".__")#','#currrate#','<!--- #note# --->','#getartran.rem6#','#dateformat(wos_date,"dd/mm/yy")#')
			</cfquery>
