...
<cfif gettran.currentrow mod 10 eq 0>
<cfquery name="updatepost" datasource="#dts#">
UPDATE postlog SET actiontype = "#gettran.currentrow#" WHERE postid = "#getuuid#"
</cfquery>
</cfif>
</cfif>
...
<cfquery name="getartran" datasource="#dts#">
	...
	(round(invgross+0.000001,2)-round(discount+0.000001,2)+round(tax+0.000001,2)) as incltax_amtt,
...
<cfset problembatch = 0 >
<cfif Hlinkams eq "Y" and isdefined('url.ubs') eq false>

<cfloop query="getartran">
<cfquery name="checkbatchlok" dbtype="query">
SELECT reference FROM getlocktran WHERE UPPER(reference) = <cfqueryparam cfsqltype="cf_sql_varchar" value="#UCASE(gettran.refno)#"> 
AND UPPER(acc_code) = <cfqueryparam cfsqltype="cf_sql_varchar" value="#UCASE(gettran.type)#"> 
</cfquery>
<cfif checkbatchlok.recordcount neq 0>
<cfset problembatch = 1>
<script type="text/javascript">
document.getElementById('unpost').disabled = true;
document.getElementById('message').innerText = "Locked Batch Transaction Found";
</script>
</cfif>
</cfloop>
</cfif>
...
<cfloop query="getartran">
	...
	<cfelse>
		<tr <cfif problembatch eq 1>style="background-color:##FF0000" </cfif>>
			<td>#getartran.type#</td>
			<td>#getartran.refno#</td>
			<td>#dateformat(getartran.wos_date,"dd/mm/yyyy")#</td>
