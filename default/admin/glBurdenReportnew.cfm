...
<cfsetting showdebugoutput="no" requesttimeout="0">
...
<cfset billformdate = "#dateformat(createdate(val(payrollyear),val(form.monthselected),28),'yyyy-mm-dd')#">
...
<cfset leftjoinplacement = "LEFT JOIN #dts#.placement c ON b.placementno = c.placementno">
		...
		<cfquery name="getpayout" datasource="#replace(dts, '_i', '_p')#">
            SELECT * FROM
            (
                SELECT a.empno, a.socsocc, a.epfcc, a.eiscc, b.assignmentslipdate, b.batches, b.placementno, c.jobpostype, c.location
                FROM 
                ...
                AND (a.socsocc != 0 OR a.epfcc != 0 OR a.eiscc != 0)
                AND a.entity = "#url.entity#"
                AND b.branch = "#url.entity#"
                AND b.payrollperiod = "#form.monthselected#"
                AND YEAR(b.assignmentslipdate) IN ("#payrollyear#"<cfif "#form.monthselected#" EQ "12">, "#val(payrollyear) + 1#"</cfif>)
                    
                <cfif #IsDefined('url.billdate')#>
                    <cfif "#url.billdate#" EQ '28'>
                        AND assignmentslipdate <= "#DateFormat(CreateDate(val(payrollyear),val(form.monthselected), url.billdate), 'yyyy-mm-dd')#"
                    <cfelse>
                        <!---AND assignmentslipdate <= "#DateFormat(DateAdd('m', 1, DateFormat(CreateDate(val(payrollyear),val(form.monthselected), url.billdate), 'yyyy-mm-dd')), 'yyyy-mm-dd')#"
                        AND assignmentslipdate > "#DateFormat(CreateDate(val(payrollyear),val(form.monthselected), 28), 'yyyy-mm-dd')#"--->
                        AND assignmentslipdate > "#DateFormat(CreateDate(val(payrollyear),val(form.monthselected), 28), 'yyyy-mm-dd')#"
                        AND a.empno NOT IN
                        (SELECT empno FROM #dts#.assignmentslip
                         WHERE assignmentslipdate <= "#DateFormat(CreateDate(val(payrollyear),val(form.monthselected), 28), 'yyyy-mm-dd')#" 
                         AND payrollperiod = "#form.monthselected#"
                         AND YEAR(assignmentslipdate) IN ("#payrollyear#"<cfif "#form.monthselected#" EQ "12">, "#val(payrollyear) + 1#"</cfif>)
                         AND branch = '#url.entity#'
                         AND empno = a.empno)
                    </cfif>
                </cfif>
            
                <cfif #IsDefined('url.paydate')#>
                    <cfif "#url.paydate#" EQ '28'>
                        AND giropaydate <= "#DateFormat(CreateDate(val(payrollyear),val(form.monthselected), url.paydate), 'yyyy-mm-dd')#"
                    <cfelse>
                        <!---AND giropaydate <= "#DateFormat(DateAdd('m', 1, DateFormat(CreateDate(val(payrollyear),val(form.monthselected), url.paydate), 'yyyy-mm-dd')), 'yyyy-mm-dd')#"--->
                        AND giropaydate > "#DateFormat(CreateDate(val(payrollyear),val(form.monthselected), 28), 'yyyy-mm-dd')#"
                        AND a.empno NOT IN
                        (SELECT empno FROM #dts#.assignmentslip
                         WHERE giropaydate <= "#DateFormat(CreateDate(val(payrollyear),val(form.monthselected), 28), 'yyyy-mm-dd')#" 
                         AND payrollperiod = "#form.monthselected#"
                         AND YEAR(giropaydate) IN ("#payrollyear#"<cfif "#form.monthselected#" EQ "12">, "#val(payrollyear) + 1#"</cfif>)
                         AND branch = '#url.entity#'
                         AND empno = a.empno)
                    </cfif>
                </cfif>
                
                ORDER BY <!---b.refno ASC--->b.empno, b.refno, c.placementno
            ) AS sort
            GROUP BY sort.empno
        </cfquery>
        <!---<cfabort>--->
        ...
        <cfquery name="getchartofaccount" datasource="#dts#">
            SELECT type1, type1_id, type2, c_credit, c_creditbranch, c_debit, c_debitbranch
            FROM chartofaccount
            WHERE type2 IN ('EPF', 'Socso', 'EIS')
        </cfquery>
                    ...
                    addnewline="Yes">
                        ...
                        <cfif #getpayout.epfcc# neq 0>
                            ...
                            <cfquery name="chartofaccount" dbtype="query">
                                ...
                                WHERE CAST(type1_id AS VARCHAR) = '#getpayout.jobpostype#'
                                AND
                                type2 = '#type2#'
                        ...
                        <cfif #getpayout.eiscc# neq 0>
                            ...
                            <cfif #chartofaccount.c_debitbranch# eq 'Y'>
                                <cffile action="APPEND" file="#HRootPath#\Excel_Report\MPpostingBUR_#url.entity#_#timenow#.csv"
                                output="2,#getpayout.batches#,#counter#,#itemcounter#,#chartofaccount.c_debit#-#getpayout.location#,#NumberFormat(getpayout.eiscc,".__")#,/TS## /Job###getpayout.placementno#,#dateformat(getpayout.assignmentslipdate,'yyyymmdd')#,GL,JE,/Batch ###getpayout.batches# /TSheet ## /Burden ##1 /Item: #type2#,#type2#"
                                    addnewline="yes">
                                <cfelse>
                                <cffile action="APPEND" file="#HRootPath#\Excel_Report\MPpostingBUR_#url.entity#_#timenow#.csv"
                                output="2,#getpayout.batches#,#counter#,#itemcounter#,#chartofaccount.c_debit#,#NumberFormat(getpayout.eiscc,".__")#,/TS## /Job###getpayout.placementno#,#dateformat(getpayout.assignmentslipdate,'yyyymmdd')#,GL,JE,/Batch ###getpayout.batches# /TSheet ## /Burden ##1 /Item: #type2#,#type2#"
                                    addnewline="yes">
                            </cfif>
                            <cfset itemcounter += 1>
                            <cfif #chartofaccount.c_creditbranch# eq 'Y'>
                                <cffile action="APPEND" file="#HRootPath#\Excel_Report\MPpostingBUR_#url.entity#_#timenow#.csv"
                                output="2,#getpayout.batches#,#counter#,#itemcounter#,#chartofaccount.c_credit#-#getpayout.location#,#val(NumberFormat(getpayout.eiscc,".__"))*-1#,/TS## /Job###getpayout.placementno#,#dateformat(getpayout.assignmentslipdate,'yyyymmdd')#,GL,JE,/Batch ###getpayout.batches# /TSheet ## /Burden ##2 /Item: #type2#,#type2#"
                                    addnewline="yes">
                                <cfelse>
                                <cffile action="APPEND" file="#HRootPath#\Excel_Report\MPpostingBUR_#url.entity#_#timenow#.csv"
                                output="2,#getpayout.batches#,#counter#,#itemcounter#,#chartofaccount.c_credit#,#val(NumberFormat(getpayout.eiscc,".__"))*-1#,/TS## /Job###getpayout.placementno#,#dateformat(getpayout.assignmentslipdate,'yyyymmdd')#,GL,JE,/Batch ###getpayout.batches# /TSheet ## /Burden ##2 /Item: #type2#,#type2#"
                                    addnewline="yes">
                            </cfif>
                            <cfset itemcounter += 1>
                        </cfif>
