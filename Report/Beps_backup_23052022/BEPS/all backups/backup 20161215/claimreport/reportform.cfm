<title>Claim Capping Report</title>
...
<h3>
	<a><font size="2">Claim Capping Report</font></a>
</h3>
...
<input type="button" size="10" value="Ajax Search" onClick="document.getElementById('fromto').value='to';ColdFusion.Window.show('findcustomer');" /></td>
  ...
  <tr> 
        ...
        SELECT placementno FROM placement WHERE empno <> "" GROUP BY empno ORDER BY placementno
        </cfquery>
  <tr> 
        	<th>Placement From</th>
            <td>:</td>
            <td>
            	<select name="placementfrom" id="placementfrom"  onChange="document.getElementById('placementto').selectedIndex=this.selectedIndex;">
