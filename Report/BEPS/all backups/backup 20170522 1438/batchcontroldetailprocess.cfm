...
,"#dateformat(approvedbydateold,'yyyy-mm-dd')#"
					...
					<cfif isdefined('form.batches')>
						and batchno in (<cfqueryparam cfsqltype="cf_sql_varchar" value="#form.batches#" separator="," list="yes">)
					</cfif>
...
WHERE UPPER(EMPNO) = <cfqueryparam cfsqltype="cf_sql_varchar" value="#UCASE(getassignment.empno)#">
						...
						<td  nowrap="nowrap" align="right">
							#numberformat(val(checkma.totalded)+val(getdbded.dbded114)+val(getdbded.dbded113)+val(getdbded.dbded109),',.__')#
						</td>                        
						...
						<cfif checkapproval.recordcount	neq 0>
							<cfset totalnet = numberformat(val(getassignment.selftotal),'.__')-numberformat(val(checkma.totalded),'.__')-numberformat(val(getintax.funddd),'.__')>
						<cfelse>
					...
					<cfif getassignment.giropaydate eq ''>
						<cfset getassignment.giropaydate = dateformat(now(),'yyyy-mm-dd')>
					</cfif>
					<!---<cfif form.getfrom eq "" and form.getto eq "" and form.agentfrom eq "" and form.agentto eq "" and form.areafrom eq "" and form.areato eq "" and form.billfrom eq "" and form.billto eq "" and isdefined('form.batches') and form.createdfrm eq "" and form.createdto eq "">--->
...
<cfqueryparam cfsqltype="cf_sql_varchar" value="#dateformat(getassignment.giropaydate,'yyyy-mm-dd')#">,
