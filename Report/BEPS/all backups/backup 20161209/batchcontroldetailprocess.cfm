...
<cfset subpreimb = 0>
<cfset subpded = 0>
<cfset subpeecpf = val(getassignment.selfcpf)>
<cfset subpfund = 0>
<cfset subpnetpay = 0>
<cfset subpercpf = val(getassignment.custcpf)>
<cfset subpsdf = val(getassignment.custsdf)>
<cfset subpinvless = 0 >
<cfset subma1list = "">
<cfset subma2list = "">
...
<th align="right">#numberformat(subpsdf,',.__')#</th>
...
SELECT count(refno) as rc, sum(if(claimadd1 = 'Y',coalesce(addchargeself,0),0)+if(claimadd2 = 'Y',coalesce(addchargeself2,0),0)+if(claimadd3 = 'Y',coalesce(addchargeself3,0),0)+if(claimadd4 = 'Y',coalesce(addchargeself4,0),0)+if(claimadd5 = 'Y',coalesce(addchargeself5,0),0)+if(claimadd6 = 'Y',coalesce(addchargeself6,0),0)) as totalamt, sum(coalesce(ded1,0)+coalesce(ded2,0)+coalesce(ded3,0)) as totalded,selfcpf, selfsdf, custcpf,custsdf FROM assignmentslip WHERE empno = <cfqueryparam cfsqltype="cf_sql_varchar" value="#getassignment.empno#"> and month(assignmentslipdate) = "#form.month#"
and year(assignmentslipdate) = "#payrollyear#"
...
<cfset getpayroll.epfww = val(checkma.selfcpf)>
<cfset getpayroll.tded = 0>
<cfset getpayroll.epfcc = val(checkma.custcpf)>
...
<td  nowrap="nowrap" align="right">#numberformat(val(getpayroll.grosspay)-val(getpayroll.ded101),',.__')#</td>
<cfset peegross = peegross + numberformat(val(getpayroll.grosspay)-val(getpayroll.ded101),'.__')>
<cfset subpeegross = subpeegross + numberformat(val(getpayroll.grosspay)-val(getpayroll.ded101),'.__')>
...
<td  nowrap="nowrap" align="right">#numberformat(val(getpayroll.tded)-val(getpayroll.ded101),',.__')#</td>
...
<cfset totalnet = numberformat(val(getpayroll.netpay),'.__') + numberformat(val(checkma.totalamt),'.__')-numberformat(val(checkma.totalded),'.__')>
...
<td  nowrap="nowrap" align="right">#numberformat(val(getpayroll.epfcc),',.__')#</td>
<cfset psdf = psdf + numberformat(val(levysd.levy_sd),'.__')>
<cfset subpsdf = subpsdf + numberformat(val(levysd.levy_sd),'.__')>
<td  nowrap="nowrap" align="right">#numberformat(val(levysd.levy_sd),',.__')#</td>
<cfset pnetpay = pnetpay + numberformat(val(totalnet),'.__')>
<cfset subpnetpay = subpnetpay + numberformat(val(totalnet),'.__')>
<cfset invlesspay = numberformat(val(getassignment.custtotalgross),'.__') - (numberformat(val(getpayroll.grosspay)-val(getpayroll.ded101),'.__')+ numberformat(val(checkma.totalamt),'.__')+numberformat(val(getpayroll.epfcc),'.__')+numberformat(val(levysd.levy_sd),'.__')+numberformat(val(levysd.levy_fw_w),'.__'))>
<td  nowrap="nowrap" align="right">#numberformat(invlesspay,',.__')#</td>
...
<cfqueryparam cfsqltype="cf_sql_double" value="#numberformat(val(getpayroll.grosspay)-val(getpayroll.ded101),'.__')#">,
...
<th align="right">#numberformat(psdf,',.__')#</th>
