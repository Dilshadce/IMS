...
<cfloop query="getpay_qry">
    ...
    <cfset rcd16 = getpay_qry.bankaccno>
    ...
    <cfelseif len(getpay_qry.name) gte 41 and len(getpay_qry.name) lte 80>
        ...
        <cfset rcd21 = mid(getpay_qry.name,41,80)>
    <cfelseif len(getpay_qry.name) gte 81 and len(getpay_qry.name) lte 120>
        <cfset rcd20 = left(getpay_qry.name,40)>
        <cfset rcd21 = mid(getpay_qry.name,41,80)>
        <cfset rcd22 = mid(getpay_qry.name,81,120)>
    </cfif>
    <cfset rcd23 = "">
    <cfset rcd24 = "">
    ...
    <cfif len(tempadd) gte 0 and len(tempadd) lte 30>
        <cfset rcd29 = getpay_qry.add1>
        <cfset rcd30 = getpay_qry.add2>
        <cfset rcd31 = "">
    <cfelseif len(tempadd) gte 31 and len(tempadd) lte 60>
        <cfset rcd29 = left(tempadd,30)>
        <cfset rcd30 = mid(tempadd,31,60)>
        <cfset rcd31 = "">  
    <cfelseif len(tempadd) gte 61 and len(tempadd) lte 90>
        <cfset rcd29 = left(tempadd,30)>
        <cfset rcd30 = mid(tempadd,31,60)>
        <cfset rcd31 = mid(tempadd,61,90)>
    </cfif>    
    <cfset rcd32 = "">
    <cfset rcd33 = "">
    <cfset rcd34 = "">
...
<cfif error eq "">	
    <cfset filename="#pir_ref#"&"-#replace(form.batch,',','-','all')#">
