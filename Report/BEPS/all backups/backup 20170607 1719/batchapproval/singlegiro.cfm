...
SELECT * FROM address a where category="#url.type#" and org_type="BANK"
		...
		<cfquery name="getempno" datasource="#dts#">
						SELECT empno,empname from manpower_i.assignmentslip WHERE
    ...
    and payrollperiod = "#gs_qry.mmonth#"
		...
		<cfquery name="getbatches" datasource="manpower_i">
						 SELECT batches,giropaydate FROM assignmentslip WHERE 1=1
    and year(assignmentslipdate) = "#gs_qry.myear#"
    and (payrollperiod = "#gs_qry.mmonth#" or payrollperiod = "#val(gs_qry.mmonth)-1#")
    and batches <> "" and batches is not null
    Group By Batches
    order by batches
						</cfquery>
		<cfoutput>
			<form name="tgForm" action="/Report/BEPS/batchapproval/generateSingleGiroBankFile.cfm" method="post" target="BLANK">
				<table class="form">
					<tr>
						<th colspan="2">
							Generate ASCII File
