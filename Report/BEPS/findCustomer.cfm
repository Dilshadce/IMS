...
<cfset nametype = "cust">
	...
	<cfquery name="getcustsupp" datasource="#dts#">
   		select custno as xcustno,name from #tran#
        where custno in (select custno from #dts#.assignmentslip)
        order by name
        limit 15
	</cfquery>
    ...
    <td><a onMouseOver="JavaScript:this.style.cursor='hand';" onClick="selectlist('#getcustsupp.xcustno#','get#url.fromto#');document.getElementById('monthtotag').style.visibility='visible';document.getElementById('monthfromtag').style.visibility='visible';ColdFusion.Window.hide('findCustomer');"><u>SELECT</u></a></td>
    </tr>
    </cfloop>
