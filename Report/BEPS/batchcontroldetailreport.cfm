...
<!--- (function($){
	$(document).ready(function(){
		...
		$("#checkSubmitted").change(function(){
			if($(this).is(":checked") == true){
...
<!---jquery to load batch on document ready--->
$(document).ready(function(){
...
{
	ajaxFunction(document.getElementById('ajaxfield'),'getbatch.cfm?month='+mon);
}
...
function searchSel(fieldid,textid) {
  ...
  for(var i=0;i<output.length;i++) {
    if(output[i].text.toLowerCase().indexOf(input)>=0 && i != 0){
      ...
      if(fieldid == 'month')
	  {
		  document.getElementById('monthto').options[i].selected=true;
	  }
...
function selectlist2(data,fieldtype){
	document.getElementById(fieldtype).value = data;
	if(fieldtype == 'billfrom')
			{
				document.getElementById('billto').value = data;
			}
    if(fieldtype == 'getempfrom')
			{
				document.getElementById('getempto').value = data;
			}
}
...
function checkempfilter(){
    if( (document.getElementById('getempfrom').value =='' || document.getElementById('getempto').value =='') && 
      (document.getElementById('getfrom').value =='' || document.getElementById('getto').value =='') ){
        document.getElementById('monthtotag').style.visibility='hidden';
        document.getElementById('monthfromtag').style.visibility='hidden';
        document.getElementById('monthto').value=document.getElementById('month').value;
    }
}
    
function empfilter(){
    if( document.getElementById('permonly').checked == true){
       document.getElementById('monthtotag').style.visibility='visible';
    }else{
        document.getElementById('monthtotag').style.visibility='hidden';
    }
}
...
Select userid from main.users where (userbranch = "manpower_i" or userbranch = "#dts#")
and usergrpid <> "super"
ORDER BY userid
 </cfquery>
...
<cfset assigndate =  dateformat(createdate(val(getmonth.myear),1,1),'yyyy-mm-dd')>
	<cfquery datasource="#dts#" name="getbill">
		select refno,assignmentslipdate from assignmentslip WHERE year(assignmentslipdate) >= "#getmonth.myear#" order by refno desc
	</cfquery>
...
<body>
<cfform action="batchcontroldetailprocess.cfm" method="post" name="form123" id="form123" target="_blank">

<!--- <h2>Print <cfoutput>#trantype#</cfoutput> Listing Report</h2> --->
<cfoutput>
<h3>
	<a><font size="2">Batch Control Detail Report</font></a>
</h3>
...
<input type="hidden" name="fromto" id="fromto" value="" />
    ...
    <tr>
      	...
      	<td colspan="2"><input type="text" name="billfrom" id="billfrom" value=""><!---<select name="billfrom" id="billfrom" onChange="document.getElementById('billto').selectedIndex=this.selectedIndex;">
			...
			</select>--->&nbsp;<input type="button" size="10" value="Ajax Search" onClick="document.getElementById('fromto').value='from';ColdFusion.Window.show('findAssign');" /><!---<input type="text" name="searchbillfrom" id="searchbillfrom" onKeyUp="searchSel('billfrom','searchbillfrom')">---></td>
    </tr>
    <tr>
      	<th>Ref No</th>
      	<td><div align="center">To</div></td>
      	<td colspan="2"><input type="text" name="billto" id="billto" value=""><!---<select name="billto" id="billto">
          		<option value="">Select a ref no</option>
          		<cfloop query="getbill">
            		<option value="#getbill.refno#">#getbill.refno# - #dateformat(getbill.assignmentslipdate,'YYYY-MM-DD')#</option>
          		</cfloop>
			</select>--->&nbsp;<input type="button" size="10" value="Ajax Search" onClick="document.getElementById('fromto').value='to';ColdFusion.Window.show('findAssign');" /><!---<input type="text" name="searchbillto" id="searchbillto" onKeyUp="searchSel('billto','searchbillto')">---></td>
    </tr>
	<tr>
      	<td colspan="5"><hr></td>
    </tr>
    <tr>
      	...
      	<td colspan="2">
			<input type="text" name="getfrom" id="getfrom" onkeyup="checkempfilter()">
    ...
    <tr>
      	...
      	<td colspan="2">
			<input type="text" name="getto" id="getto" onkeyup="checkempfilter()">
			<cfif getgeneral.filterall eq "1">
				<cfoutput>
<input type="button" size="10" value="Ajax Search" onClick="document.getElementById('fromto').value='to';ColdFusion.Window.show('findCustomer');" /><!---&nbsp;<input type="text" id="searchsuppto" name="searchsuppto" onKeyUp="getSupp('getto','#title#');">---></cfoutput>
    ...
    <tr>
      	...
      	<td width="5%"></td>
      	<td colspan="2">
			<input type="text" name="getcust" id="getcust" onkeyup="if(this.value != ''){document.getElementById('monthtotag').style.visibility='visible';}else{document.getElementById('monthtotag').style.visibility='hidden';}" onblur="if(this.value != ''){document.getElementById('monthtotag').style.visibility='visible';}else{document.getElementById('monthtotag').style.visibility='hidden';}">
		</td>
    </tr>
    <tr>
      	<td colspan="5"><hr></td>
     ...
     <tr>
      	...
      	<td width="5%"> <div align="center">From</div></td>
      	<td colspan="2">
			<input type="text" name="getempfrom" id="getempfrom" onkeyup="checkempfilter()">
			<cfif getgeneral.filterall eq "1">
				<cfoutput><input type="button" size="10" value="Ajax Search" onClick="document.getElementById('fromto').value='from';ColdFusion.Window.show('findEmployee');" /><!---&nbsp;<input type="text" id="searchsuppfr" name="searchsuppfr" onKeyUp="getSupp('getfrom','#title#');">---></cfoutput>
			</cfif>
		</td>
    </tr>
    <tr>
      	...
      	<td width="5%"> <div align="center">To</div></td>
      	<td colspan="2">
			<input type="text" name="getempto" id="getempto" onkeyup="checkempfilter()">
			<cfif getgeneral.filterall eq "1">
				<cfoutput>
<input type="button" size="10" value="Ajax Search" onClick="document.getElementById('fromto').value='to';ColdFusion.Window.show('findEmployee');" /><!---&nbsp;<input type="text" id="searchsuppto" name="searchsuppto" onKeyUp="getSupp('getto','#title#');">---></cfoutput>
			</cfif>
		</td>
    </tr>
     <tr>
...
<option value="#userlist.userid#">#userlist.userid#</option>
    ...
    <tr>
      	...
      	<td width="5%"> <span id="monthfromtag" style="visibility:hidden">From</span></td>
      	<td colspan="2">
        <select name="month" id="month" onChange="updatebatch(this.value);document.getElementById('monthto').selectedIndex = this.selectedIndex;">
    ...
    <tr id="monthtotag" style="visibility:hidden">
        <th width="16%">Month</th>
        <td width="5%"> To</td>
        <td colspan="2">
    ...
    <tr>
      	...
      	<td colspan="2">
         <div id="ajaxfield">
    <!---commented out, document ready will get value from getbatch.cfm--->
    <!---
	...
	<input type="checkbox" id="checkApproved" > Select All Approved<br />
	<input type="checkbox" id="checkSubmitted" > Select All Submitted<br />
<hr />
    ...
    <option value="batches" selected>Batch No</option>
    <option value="payrollperiod">Period</option>
    ...
    <tr>
        <th>Show System Item No.</th>
        <td></td>
        <td>
        <input type="checkbox" name="showitemno" id="showitemno" value="" >
        </td>
    </tr>
    <tr >
      	<td colspan="5"><hr></td>
    ...
    <tr>
    <th colspan="3" style="text-align: left">Download Only For the Below: </th>
    </tr>
    <tr>
    <th>PERM</th>
    <td></td>
    <td>
    <input type="checkbox" name="permonly" id="permonly" value="" onchange="empfilter()" >
    </td>
    </tr>
    <tr>
    <th>CN DN</th>
    <td></td>
    <td>
    <input type="checkbox" name="cndnonly" id="cndnonly" value="" >
    </td>
    </tr>
    <tr>
    <th>Assignmentslips</th>
    <td></td>
    <td>
    <input type="checkbox" name="assignonly" id="assignonly" value="" <cfif not(left(getauthuser(),5) eq "ultra" or getauthuser() eq "kangwei.tay@manpower.com.my" or getauthuser() eq "nantha.shanmugan@manpower.com.my" or getauthuser() eq "kelly.lim@manpower.com.my" or getauthuser() eq "muhammadrizal.rafles@manpower.com.my")>checked</cfif>>
    </td>
    </tr>
    <tr>
        <td>&nbsp;</td>
    ...
    <tr>
        <td colspan="4" style="color: red">&nbsp;&nbsp;Note: To download all, please left all untick.</td>
    </tr>
    <tr>
    <tr >
      	<td colspan="5"><hr></td>
    </tr>
    <th>Include StaffCode</th>
    <td></td>
    <td>
    <input type="checkbox" name="StaffCode" id="StaffCode" value="">
    </td>
    </tr>
	<tr>
    </tr>
    ...
    <td colspan="100%" align="center">
		<input type="Submit" name="Submit" value="View Report" onClick="document.form123.action = 'batchcontroldetailprocess.cfm';">
		<!--- &nbsp;&nbsp;&nbsp;<input type="Submit" name="Submit" value="Pay Bill Audit Report" onClick="document.form123.action = 'batchcontroldetailprocess2.cfm';"> --->
		&nbsp;&nbsp;&nbsp;
	<!---<input type="Submit" name="Submit" value="Pay Bill Audit Report Excel" onClick="document.form123.action = 'batchcontroldetailprocess2excel.cfm';">
	&nbsp;&nbsp;&nbsp;--->
	<input type="Submit" name="Submit" value="Pay Bill Audit Report" onClick="document.form123.action = 'batchcontroldetailprocess2xlsx.cfm';">
	&nbsp;&nbsp;&nbsp;
	<input type="Submit" name="Submit" value="Payroll summary report" onClick="document.form123.action = 'payrollsummary.cfm';">
        &nbsp;&nbsp;&nbsp;
        <input type="Submit" name="Submit" value="Admin Fee Details Report" onClick="document.form123.action = 'adminfeereport.cfm';">
        <!---<input type="Submit" name="Submit" value="Pay Bill Audit report (with CN DN)" onClick="document.form123.action = 'batchcontroldetailprocess2exceltest.cfm';">--->
	</td>
    </tr>
    <tr>
...
<cfwindow center="true" width="600" height="400" name="findAssign" refreshOnShow="true"
        title="Find Assignmentslip" initshow="false"
        source="findAssign.cfm?type=assignmentslip&fromto={fromto}" />
<cfwindow center="true" width="600" height="400" name="findEmployee" refreshOnShow="true"
        title="Find Employee" initshow="false"
        source="findEmployee.cfm?type=pmast&fromto={fromto}" />
