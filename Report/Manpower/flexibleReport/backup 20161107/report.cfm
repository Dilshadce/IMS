...
<cfset exceptionColumn = "Employer Total Monthly Billable Amount,Employer Total Reimbursement,Employer Total Deduction,Employee Total Reimbursement,Employee Total Deduction">
...
<cfif getColumn.recordcount gt 0 and (getreport.fcustomer eq 'y' or getreport.fplacement eq 'y' or getreport.fassignment eq 'y' or getreport.finvoice eq 'y' or getreport.femployee eq 'y')>
    ...
    <cfif getreport.femployee eq 'y'>
    	<cfset table = table & " beps_p.pmast e">
    </cfif>          
 	...
 	<cfif getColumn.recordcount gt 0 and (getreport.fcustomer eq 'y' or getreport.fplacement eq 'y' or getreport.fassignment eq 'y' or getreport.finvoice eq 'y' or getreport.femployee eq 'y')>
        ...
        <tr>
            <cfloop query="getColumn">
                ...
                <cfset "grandtotal_#replacelist(REReplace(getColumn.column,'[^0-9A-Za-z ]','','all'), ' ', '_', 'all')#" = "">
                <cfset "grouptotal_#replace(REReplace(getColumn.column,'[^0-9A-Za-z ]','','all'), ' ', '_', 'all')#" = "">
                                
                <cfif find("double", listgetat(dbtype, getcolumn.currentrow, ';')) neq 0 or find("integer", listgetat(dbtype, getcolumn.currentrow, ';')) neq 0 or findnocase("Date", getcolumn.column) neq 0 or findnocase("Age", getcolumn.column) neq 0>
        ...
        <cfif getreport.groupby neq ''>
        	<cfloop query="getgroup">
            	<cfloop query="getColumn">
                    <cfset "grouptotal_#replace(getColumn.column, ' ', '_', 'all')#" = "">
                </cfloop>                
                <tr>
                    <td colspan="100%">
                    	<strong>
                        	<u> 
                                ...
                                <cfif getreport.groupby eq 'Employee'>
                                	<cfquery name="getempname" datasource="#dts#">
                                    	select empname from beps_p.pmast where empno = "#evaluate('getgroup.#listgetat(realgroupby, 2, '.')#')#"
                                    </cfquery>
                ...
                <tr>
                	<cfloop query="getcolumn"> 
                    	<cfif isnumeric(evaluate("grouptotal_#replace(getColumn.column, ' ', '_', 'all')#"))>
                            <td><div align="right">#numberformat(evaluate("grouptotal_#replace(getColumn.column, ' ', '_', 'all')#"), ',.__')#</div></td>
                        <cfelse>
                            <td>#evaluate("grouptotal_#replace(getColumn.column, ' ', '_', 'all')#")#</td>
                        </cfif> 
                    </cfloop>
                </tr>
                <tr>
        ...
        <tr>
            <cfloop query="getcolumn"> 
                <cfif isnumeric(evaluate("grandtotal_#replace(getColumn.column, ' ', '_', 'all')#"))>
                    <td><div align="right">#numberformat(evaluate("grandtotal_#replace(getColumn.column, ' ', '_', 'all')#"), ',.__')#</div></td>
                <cfelse>
                    <td>#evaluate("grandtotal_#replace(getColumn.column, ' ', '_', 'all')#")#</td>
                </cfif> 
            </cfloop>
        </tr>
        <tr>
