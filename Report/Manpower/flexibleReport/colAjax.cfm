...
<cfoutput>
	<cfif url.type eq 'add'>
        ...
        <cfif checkExist.recordcount eq 0>
            <cfquery name="add" datasource="#dts#">
                ...
                values ('#url.uuid#', <cfqueryparam cfsqltype="varchar" value="#url.selcol#">, '#newPosition#')
            </cfquery>
        </cfif>
    <cfelseif url.type eq 'addall'>  
        <cfset newPosition = 1>
            
        <cfset entity = ''>
            
        <cfif url.fplacement eq 'true' and url.fassignment eq 'true' and url.fcustomer eq 'true' and url.femployee eq 'true'>
        <cfelse>
            <cfif url.fplacement eq 'true'>
                <cfset entity = entity&',placement'>
            </cfif>
            <cfif url.fassignment eq 'true'>
                <cfset entity = entity&',assignment'>
            </cfif>
            <cfif url.fcustomer eq 'true'>
                <cfset entity = entity&',customer'>
            </cfif>
            <cfif url.femployee eq 'true'>
                <cfset entity = entity&',employee'>
            </cfif>
        </cfif>
            
        <cfquery name="getavacolumn" datasource="#dts#">
            select * from avacolumn where 1=1
            <cfif entity neq ''>
                and entity in (<cfqueryparam cfsqltype='varchar' list="true" separator=',' value='#entity#'>)            
            </cfif>
        </cfquery>  
        
        <cfloop query="getavacolumn">
            <cfset url.selcol = getavacolumn.column>
            <cftry>
            <cfquery name="getlastpos" datasource="#dts#">
                select position from selcolumn where uuid = "#url.uuid#" order by position*1 desc limit 1
            </cfquery>
    ...
    <cfelseif url.type eq 'removeall'>                
    	<cfquery name="removeall" datasource="#dts#">
        	delete from selcolumn where uuid = "#url.uuid#"
        </cfquery>
