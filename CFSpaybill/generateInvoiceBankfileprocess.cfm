	...
	<cfif isdefined('url.type') and isdefined('url.id')>
		...
		<cfelseif lcase(url.type) eq 'edit'>
			...
			<cfquery name="getemplist" datasource="#dts#">
				select g.id as rowID, c.id as id,c.name as name FROM geninvbankfile g LEFT JOIN CFSEmp c ON g.empno = c.id
			...
			<cfloop query="getemplist">
				...
				<cfquery datasource="#dts#">
					...
					workdays =  <cfif evaluate('form.workdays#getemplist.id#') eq ''>0.00<cfelse><cfqueryparam cfsqltype='cf_sql_integer' value="#evaluate('form.workdays#getemplist.id#')#" ></cfif>,
					payamt = <cfif evaluate('form.totalpayamt#getemplist.id#') eq ''>0.00<cfelse><cfqueryparam cfsqltype='cf_sql_double' value="#evaluate('form.totalpayamt#getemplist.id#')#" ></cfif>,
					billamt =<cfif evaluate('form.totalbillamt#getemplist.id#') eq ''>0.00<cfelse><cfqueryparam cfsqltype='cf_sql_double' value="#evaluate('form.totalbillamt#getemplist.id#')#" ></cfif>,
					adminfeeamt = <cfif evaluate('form.adminfeeamt#getemplist.id#') eq ''>0.00<cfelse><cfqueryparam cfsqltype="double" value="#evaluate('form.adminfeeamt#getemplist.id#')#"></cfif>,
...
WHERE profileid = <cfqueryparam cfsqltype="cf_sql_varchar" value="#url.profileid#">
		...
		<cfquery name="getemplist" datasource="#dts#">
SELECT * FROM cfsemp
WHERE id=<cfqueryparam cfsqltype="cf_sql_varchar" list="yes" separator="," value="#getprofilelist.empno#">
</cfquery>
		<cfif trim(form.submit) eq "Generate Bankfile Now">
...
<cfqueryparam cfsqltype="cf_sql_varchar" value="#getemplist.id#">,
