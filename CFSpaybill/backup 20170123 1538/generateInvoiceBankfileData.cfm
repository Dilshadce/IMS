...
SELECT profilename,ratetype,payrate,billrate,adminfee,adminFeeCap,custno FROM paybillprofile
...
/*function recalcx(id){
	...
	<cfif isdefined('url.type') and isdefined('url.id') AND url.type eq "Edit">
		<cfquery name="gettran" datasource="#dts#">
				select g.*,name,c.id as empId FROM geninvbankfile g LEFT JOIN CFSEmp c ON g.icno = c.icno
