...
component accessors=true output=false persistent=false {
	...
	public function setTypeFlag(flag){
		this.typeFlag = flag;
	}
	...
	public function addLineType(lineType, rowStyleID = ""){
		if(rowStyleID != ""){
			ArrayAppend(this.rowStyles,rowStyleID);
		}else{
			ArrayAppend(this.rowStyles,this.DEFAULT_STYLE);
		...
		ArrayAppend(this.excelDataType, lineType);
	}
	...
	public function output(){
			...
			var outputString = "";
			var simpleString = complexString = "";
			...
			if(this.customHeader != ""){
 			...
 			for( var i = 1; i <= ArrayLen(this.excelData); i++){
 				...
 				for(var j = 1; j <= ArrayLen(this.excelData[i]); j++){
 					...
 					IF(this.typeFlag eq True)
 					{
 						dataType = this.excelDataType[i][j];
 					}
 					ELSE
 					{
	...
	public function outputPBRequest(){
			...
			var filepath = this.Root & "output/" & TimeFormat(Now()) &"_file.xlsx";
 			 FileWrite(filepath,toString(outputString),"utf-8");

			include "assets/fileOutputPBrequest.cfm";
