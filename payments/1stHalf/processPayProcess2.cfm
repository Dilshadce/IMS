...
SELECT p.empno,pm.epfcat FROM paytra1 p, pmast pm
WHERE p.empno = pm.empno AND p.payyes="Y" and confid >= #hpin#
and pm.paystatus = "A"
<cfif #form.empno# neq "">
...
<!--- PROCESS PAY PAYTRA1 --->
<cfinvoke component="cfc.processPay1" method="updatePay" empno="#selectList.empno#" db="#dts#" 
...
<cfif name eq "success" AND update eq "update" and sdl_cfc eq "success">
	<cfwindow name="success" title="Progress Complete" modal="true" closable="false" width="350" height="260" center="true" initShow="true" >
	<p>Progess Completed! </p>
	<p><cfoutput>#selectList.recordcount#</cfoutput> Employees pay have been process</p>
