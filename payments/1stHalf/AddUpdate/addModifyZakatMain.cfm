...
FROM paytra1 p, pmast pm
WHERE p.empno = pm.empno and pm.paystatus = "A" and confid >= #hpin# 
ORDER BY length(pm.empno),pm.empno
</cfquery>

<cfoutput>
<form name="aForm" action="/payments/1stHalf/addUpdate/addModifyZakatMain_process.cfm" method="post">
...
<tr onClick="showFoot('#zakat_qry.currentrow#');">
	...
	<td><input type="text" name="zakat_bf_#zakat_qry.currentrow#" value="#zakat_qry.zakat_bf#" size="18" maxlength="10" /></td>
</tr>
