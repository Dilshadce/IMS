...
<!--- <cfset currentFile="http://localhost/payments/1stHalf/AddUpdate/normalPayMain.cfm"> --->  
...
<cfquery name="getData" datasource="#dts#">
	...
	(
		...
		WHERE confid >= #hpin# 
        and paystatus = "A" 
        <cfif general_qry.t1 eq 0>
        <cfif general_qry.bp_payment eq "2">
		...
		<cfif sFor neq "">
        and (#variables.sType# like '%#URLDecode(variables.sFor)#' 
...
function confirmDelete(empno,type) {
	var answer = confirm("Confirm Delete?")
	if (answer){
		//window.location = "/personnel/employee/act.cfm?type="+type+ "&empno="+empno;
	}
	else{
...
<body>
		<div class="mainTitle">Normal Pay</div>
		<div class="ttype3">
			<cfoutput>
			...
			<span class="ttype">Search By :
				<select name="sType">
                  ...
                  <option value="nricn">IC No.</option>
			    </select>
				Search for : 
			...
			<table width="" class="data">
				...
				<cfif getData.recordcount>
					...
					<tr onMouseOver="javascript:this.style.backgroundColor='99FF00';" onMouseOut="javascript:this.style.backgroundColor='';">
						...
						<td>
							#getData.empno#
							<input type="hidden" name="empno" value="#getData.empno#">
						</td>
						<td>
							#getData.name#
						</td>
						...
						<td>
	           			...
	           			<td width="">
		           			<a href="/payments/1stHalf/AddUpdate/normalPayEditForm.cfm?empno=#URLEncodedFormat(getData.empno)#"><img height="18px" width="18px" src="/images/edit.ICO" alt="Update" border="0">PAY</a>
		           			<!--- <a href="##" onclick="confirmDelete('#empno#','del')"><img height="18px" width="18px" src="/images/delete.ICO" alt="Delete" border="0">Delete</a> --->
						</td>
					</tr>
					</cfoutput>
				<cfelse>
		...
		<div><br /><a href="/payments/1stHalf/addUpdate/addUpdateList.cfm"><strong>< Back</strong></a></div>
