...
<!--- 	document.getElementById(abc).innerHTML ="<select name='sa2' size='5'  ondblclick=getsa(this.options[this.selectedIndex].value,<cfoutput>#entryno#</cfoutput>);document.getElementById('"+abc+"').style.display = 'none';><cfquery name='get_pc_code' datasource='#dts#'>
	...
	function appendRow()
	{
		...
		//<img src="/images/down.png" border="0" onClick="showsa1('+row+');" width="15" height="15"/><br /><div style="display : none ; position: absolute;" id=r_'+row+'>
		newCel1.innerHTML = '<input type="text" name="ncode__r'+row+'" size="18" maxlength="12" /></div>';
...
WHERE empno='#url.empno#' and confid >= #hpin#
</cfquery>
...
<div class="tabber">
	...
	<form name="eForm" action="/payments/1stHalf/AddUpdate/TipAndPieceWorkedMaintenance/prwMain_addprocess.cfm?empno=<cfoutput>#url.empno#</cfoutput>" method="post">
