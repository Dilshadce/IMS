...
<tr>
<td></td>
<td><input type="submit" name="GO" value="GO" onclick="ajaxFunction(document.getElementById('ajaxField3'),'searchEmployeeAjax.cfm?keyword='+document.getElementById('search').value );"/></td>
</tr>
</table>
...
SELECT * FROM pmast WHERE paystatus = "A"  and confid >= #hpin# <cfif gs_qry.t1 eq 0>
        <cfif gs_qry.bp_payment eq "2">
        and (nppm = "2" or nppm = "0") 
		<cfelse>
        and nppm = "2"
		</cfif>
		</cfif> order by length(empno), empno
</cfquery>
...
<td>#searchKey.name#</td><td>#searchKey.nricn#</td>
