...
<cfif isdefined ("url.type")>
	...
	<cfquery name="select_emp_list" datasource="#dts#">
		select empno from paytra1 <cfif isdefined("form.allEMP") eq false>WHERE empno = "#form.empno#"</cfif>
	</cfquery>
	
		<cfloop query="select_emp_list" >
			<cfset ndate = createdate(right(form.date_p,4),mid(form.date_p,4,2),left(form.date_p,2))>
			<cfquery name="add_pj_qry" datasource="#dts#">
				Insert into proj_rcd_1
		...
		<cflocation url="/payments/1stHalf/AddUpdate/normalPayEditForm.cfm?empno=#URLEncodedFormat(empno)#">
	
	
	<cfelseif url.type eq "update">
		
		<cfset qString="select * FROM proj_rcd_1 where entryno = #form.entryno_u# and empno='#form.empno#'">
		...
		<cfinvoke component="cfc.auditTrail" method="createAuditFrmUpdate" dts="#dts#" act="update" 
			user="#Huserid#" module="PYM" pointer="proj_rcd_1[entryno = #form.entryno_u#
				and empno = <cfqueryparam cfsqltype='cf_sql_varchar' value='#form.empno#'>]" oldArray="#array_ov#" 
			qStr="#qString#" comment="1st half project site 1">>
		
		<cflocation url="/payments/1stHalf/AddUpdate/normalPayEditForm.cfm?empno=#URLEncodedFormat(form.empno)#">
		
	<cfelseif url.type eq "del">
		<cfset delentryno= url.entryno>
		<cfset empno= url.empno>
		<cfquery datasource="#dts#" name="getAudit1">
			select * FROM proj_rcd_1
			where entryno = #delentryno# and empno='#empno#'
		</cfquery>
		
		
		<cfset qString="select * FROM proj_rcd_1 where entryno = #delentryno# and empno='#empno#'">
		...
		<cfquery name="del_prj_qry" datasource="#dts#">
			Delete FROM proj_rcd_1 where entryno=#delentryno#
		</cfquery>
		<cfinvoke component="cfc.auditTrail" method="createAuditFrmDelete" dts="#dts#" act="delete" user="#huserid#" module="PYM" pointer="proj_rcd_1[entryno='#delentryno#']" 
					dataQr="#getAudit1#" comment="1st half project site 1">
...
<html>
<head>
	<title>1st Half - Add Daily Record</title>
</head>
<body>
...
<cfif isdefined ("url.act")>
	...
	<cfquery name="prj_rcd_qry" datasource="#dts#">
		SELECT * FROM proj_rcd_1 where entryno=#entry_no#
	</cfquery>
...
<cfoutput>
	...
	<tr>
		<th>Date</th>
		<td><input type="text" name="date_p" value="#date_p#" size="10" maxlength="10"/>
			<img src="/images/cal.gif" width=17 height=15 border=0 onClick="showCalendarControl(date_p);">(dd/mm/yyyy)
		</td>
	</tr>
	<tr>
		<th>DW</th>
