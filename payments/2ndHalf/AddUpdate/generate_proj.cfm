...
<cfif isdefined("url.type")>
	...
	<cfquery name="emp_list" datasource="#dts#">
		SELECT empno FROM proj_rcd 
		where 0=0
		<cfif form.empno neq "" > 
			and empno >= "#form.empno#"
		</cfif>
		<cfif form.empno2 neq "" > 
			and empno <= "#form.empno2#"
		</cfif>
		group by empno
	</cfquery>
	
	<cfloop query="emp_list">
		<cfoutput>#emp_list.empno#</cfoutput>
		<cfquery name="prj_rcd_qry" datasource ="#dts#">
			SELECT sum(coalesce(dw_p,0)) as DW_P, sum(coalesce(MC_P,0)) as mc_p, sum(coalesce(npl_p,0)) as npl_p,
	 		...
	 		FROM proj_rcd WHERE empno = <cfqueryparam cfsqltype="cf_sql_varchar" value="#emp_list.empno#">
		</cfquery>
		<cfquery name="update_proj_rcd" datasource="#dts#">
			Update proj_rcd set payyes = "Y", userid="#HUserName#" 
...
<cflocation url="/payments/2ndHalf/AddUpdate/normalPayEditForm.cfm?empno=#URLEncodedFormat(form.empno_ori)#">
</cfif>

<html>
<head>
	<title>2nd Half - Generate Record</title>
</head>
<body>
<cfquery name="proj_tbl_qry" datasource="#dts#">
	SELECT p.empno, name FROM proj_rcd as p left join pmast as pm on pm.empno=p.empno group by p.empno ;
</cfquery>
<cfoutput>
