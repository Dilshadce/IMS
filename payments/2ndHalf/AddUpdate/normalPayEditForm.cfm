...
<html>
<head>
	<title>2nd Half - Normal Pay</title>
	...
	<script src="/javascripts/test.js" language="javascript"> </script>
	...
	function saveonly()
	{
	document.eForm.action = "/payments/2ndHalf/AddUpdate/normalPay_process.cfm?type=normal<cfoutput>&placement=#url.placement#&invoiceno=#url.invoiceno#</cfoutput>";
	}
	
	function savenext()
	{
	document.eForm.action = "/payments/2ndHalf/AddUpdate/normalPay_process.cfm?type=normal&page=next<cfoutput>&placement=#url.placement#&invoiceno=#url.invoiceno#</cfoutput>";
	}
	
	function all_add()
	{
	document.eForm.action = "/payments/2ndHalf/AddUpdate/normalPay_process.cfm?type=all_add<cfoutput>&placement=#url.placement#&invoiceno=#url.invoiceno#</cfoutput>";
	}
	...
	{
		var listobj = document.getElementById('code');
		document.getElementById('desp_relif').value = listobj.options[listobj.selectedIndex].id;
		document.getElementById('capping').value = listobj.options[listobj.selectedIndex].title;
	}
...
<cfquery name="pay1_qry" datasource="#dts#">
SELECT mc,al FROM paytra1
WHERE EMPNO = "#URLDecode(url.empno)#"
</cfquery>
...
SELECT * FROM proj_rcd WHERE EMPNO = "#URLDecode(url.empno)#"
</cfquery>
...
<!--- Prompt Table Start --->
	...
	<cfoutput>
	<div class="mainTitle">2nd Half Payroll - Normal Pay 
	...
	<cfform name="eForm" id="eForm" action="/payments/2ndHalf/AddUpdate/normalPay_process.cfm?type=normal<cfoutput>&placement=#url.placement#&invoiceno=#url.invoiceno#</cfoutput>" method="post">
	<table class="form">
	<tr>
		<td>Employee No.</td>
		...
		<td ><input type="text" name="name" value="#pmast_qry.NAME#" size="40" readonly></td>
         <td><input type="button" name="goprocess" value="Process Pay" onClick="window.location.href='/payments/2ndHalf/processpaymain.cfm?empno=#pmast_qry.EMPNO#'" /></td>
	</tr>
	<tr>
		<td>Line No.</td>
	...
	<div class="tabber" >
		<div  class="tabbertab tabbertabdefault" style="height:390px;" id="mytab1">
			...
			<tr>
				...
				<tr>
					...
					<td colspan="2" width="150">
						...
						<cfif pay_qry.Brate neq 0>
	                    	<cfoutput>value="#numberformat(pay_qry.BRATE,'.__')#"</cfoutput>
	                    <cfelse>
	                    	value="#numberformat(pmast_qry.BRATE,'.__')#"
						</cfif> 
							size="22" onBlur="rdate();"> --->
					...
					<td width="180" colspan="3">
						<label class="llabel" name="payrtype">
							<cfif #pmast_qry.PAYRTYPE# eq "M">
								Monthly
							<cfelseif #pmast_qry.PAYRTYPE# eq "D">
							Daily
							<cfelseif #pmast_qry.PAYRTYPE# eq "H">
							Hourly
							</cfif>
						</label>
					</td>
				...
				<tr>
					...
					<td width="">
						...
						empno="#URLDecode(url.empno)#" emp_pay_time="#pmast_qry.nppm#"	
							comp_pay_time="#gs_qry.bp_payment#"	db="#dts#" emp_grp="#pmast_qry.wrking_grp#">
					...
					<td align="right"><input type="button" name="cal" value="Cal." <cfif #pmast_qry.mcomm# neq "#gs_qry.mmonth#">disabled<cfelse></cfif> onClick="window.open('/payments/2ndHalf/AddUpdate/normalPay_cal.cfm?empno=#URLEncodedFormat(pmast_qry.empno)#', 'windowname', 'width=150, height=150, left=100, top=100')"></td>
					<td><input type="button" name="" value="More" onClick="window.open('/payments/2ndHalf/AddUpdate/normalPay_more.cfm?empno=#URLEncodedFormat(pmast_qry.empno)#', 'windowname', 'width=350, height=280, left=100, top=100')"></td>
				</tr>
				<tr>
			...
			<!--- start AL --->
				...
				<cfloop query="leave_data2">
					...
					<cfif leave_date lt today AND leave_data2.LVE_TYPE eq "AL">  
						<cfset tLVE_DAY_a= tLVE_DAY_a + val(leave_data2.LVE_DAY)>
					</cfif>
					<!-- sum of the leave in this month -->
					<cfif leave_date eq today AND leave_data2.LVE_TYPE eq "AL">  
				...
				<cfif leave_data.AL gte tLVE_DAY_a >
					<cfset bal_AL = total_AL_entitled -(val(leave_data.AL)+val(tLVE_DAY_b)+val(pay1_qry.al)) >
					<input type="hidden" name="total_AL_entitled" id="total_AL_entitled" value="#total_AL_entitled#">
					<input type="hidden" name="past_month_leave" id="past_month_leave" value="#leave_data.AL+val(pay1_qry.al)#" >
					<input type="hidden" name="bal_AL2" id="bal_AL2" value="#bal_AL#" >
				<cfelse>
					<cfset bal_AL = total_AL_entitled -(val(tLVE_DAY_a)+val(tLVE_DAY_b)+val(pay1_qry.al))>
					<input type="hidden" name="total_AL_entitled" id="total_AL_entitled" value="#total_AL_entitled#">
					<input type="hidden" name="past_month_leave" id="past_month_leave" value="#tLVE_DAY_a+val(pay1_qry.al)#" >
					<input type="hidden" name="bal_AL2" id="bal_AL2" value="#bal_AL#" >
				</cfif>
			...
			<!--- start MC --->
				...
				<cfloop query="leave_data2">
					<cfset leave_date="#dateformat(leave_data2.LVE_DATE,'MM')#">
					<cfif leave_date lt today AND leave_data2.LVE_TYPE eq "MC">  
						<cfset tLVE_DAY_MC_a= tLVE_DAY_MC_a + val(leave_data2.LVE_DAY)>
					</cfif>
					
					<cfif leave_date eq today AND leave_data2.LVE_TYPE eq "MC">  
					<cfset tLVE_DAY_MC_b= tLVE_DAY_MC_b + val(leave_data2.LVE_DAY)>
					</cfif>
				</cfloop>
				
				<cfif leave_data.MC gte #tLVE_DAY_MC_a# >
					<cfset bal_MC = val(emp_data.mcall)-(val(leave_data.MC)+val(tLVE_DAY_MC_b)+val(pay1_qry.mc)) >
					<input type="hidden" name="total_mc_entitled" id="total_mc_entitled" value="#emp_data.mcall#">
					<input type="hidden" name="past_month_mc" id="past_month_mc" value="#leave_data.MC+val(pay1_qry.mc)#" >
					<input type="hidden" name="bal_MC2" id="bal_MC2" value="#bal_MC#" >
				<cfelse>
					<cfset bal_MC = val(emp_data.mcall)-(val(tLVE_DAY_MC_a)+val(tLVE_DAY_MC_b)+val(pay1_qry.mc))>
					<input type="hidden" name="total_mc_entitled" id="total_mc_entitled" value="#emp_data.mcall#">
					<input type="hidden" name="past_month_mc" id="past_month_mc" value="#tLVE_DAY_MC_a+val(pay1_qry.mc)#" >
					<input type="hidden" name="bal_MC2" id="bal_MC2" value="#bal_MC#" >
				</cfif>
				...
				<cfloop query="leave_data2">
					<cfset leave_date="#dateformat(leave_data2.LVE_DATE,'MM')#">
					<cfif leave_date lt today AND leave_data2.LVE_TYPE eq "CC">  
						<cfset tLVE_DAY_CC_a= tLVE_DAY_CC_a + val(leave_data2.LVE_DAY)>
					</cfif>
					
					<cfif leave_date eq today AND leave_data2.LVE_TYPE eq "CC">  
				...
				<tr>
					...
					<tr>
						...
						<td><cfif emp_poj_qry.payyes eq "Y">
		...
		<div class="tabbertab" style="height:390px;" id="mytab2" >
			<h3>Allowance</h3>
			<table class="form" border="0">
			<tr>
				<td>
				...
				<cfloop query="at1_qry">
					<tr>
						...
						<!--- <cfif #evaluate('pmast_qry.DBAW#j#')# eq 0> --->
	                    
						<cfoutput> value="#numberformat(evaluate('pay_qry.AW#j#'),'.__')#" </cfoutput>
						<!--- <cfelse><cfoutput>value="#numberformat(evaluate('pmast_qry.DBAW#j#'),'.__')#" readonly="readonly"</cfoutput>
						</cfif> ---> 
	                    size="14"></td>
					</tr>
					<cfset i=i+1>
				...
				<tr>
					<td colspan="2"></td>
					<td align="right"><input type="button" name="moreAW" value="More.." onClick="window.open('/payments/2ndHalf/AddUpdate/normalPay_moreAW.cfm?empno=#URLEncodedFormat(pmast_qry.empno)#', 'windowname1', 'width=400, height=500, left=100, top=100')"></td>
				</tr>
				<tr>
					<td>&nbsp;</td>
				</tr>
		...
		<div class="tabbertab" style="height:390px;" id="mytab3">
			<h3>Deduction</h3>
			<table class="form" border="0">
			<tr>
				<td>
				...
				<tr>
					...
					<cfelse>
						<cfoutput>value="#numberformat(evaluate('pmast_qry.DBDED#j#'),'.__')#" readonly="readonly"</cfoutput>
					</cfif> 
                    size="14"></td>
				</tr>
				<cfset i=i+1>
				...
				<tr>
					<td colspan="3" align="right"><input type="button" name="more" value="More.." onClick="window.open('/payments/2ndHalf/AddUpdate/normalPay_moreDED.cfm?empno=#URLEncodedFormat(pmast_qry.empno)#', 'windowname1', 'width=400, height=500, left=100, top=100')"></td>
				</tr>
				<tr>
		...
		<div class="tabbertab" style="height:390px;" id="mytab4">
			<h3>Other</h3>
			<table class="form" border="0">
			<tr>
			...
			<tr>
				...
				<td width="80">
					...
					<select name="project">
						...
						<option value="#pro_qry.project#" <cfif #pro_qry.project# eq #pay_qry.PROJECT# >selected</cfif> >#pro_qry.project# | #pro_qry.desp#</option>
						</cfloop>
					</select>
				</td>
			</tr>
		...
		<div class="tabbertab" style="height:390px;" id="mytab5">
		...
		<div class="tabbertab" style="height:390px;" id="mytab6">
			<h3>Project Site</h3>
			...
			<table class="form" width="500px" border="0">
				...
				<tr>
					...
					<td><input name="proj_code" id="proj_code" size="20" value="#emp_poj_qry.project_code#-#prj_name.desp#" readonly/></td>
					<cfset pdate = dateformat(emp_poj_qry.date_p,'dd-mm-yyyy')>
					<td><input name="date_p" id="date_p" value="#pdate#" size="20" readonly/></td>
				...
				SELECT sum(coalesce(dw_p,0))as totdw_p,sum(coalesce(mc_p,0))as totmc_p, 
					   ...
					   sum(coalesce(ot6_p,0))as totot6_p FROM proj_rcd p 
				WHERE EMPNO = "#URLDecode(url.empno)#";
				</cfquery> 
				<cfquery name="proj_pay_qry" datasource="#dts#">
					select sum(tot_dw) as tot_dw from proj_pay where empno = "#URLDecode(url.empno)#" 
				</cfquery>
				<cfset tottotot = "0">	
				<cfset tottotot = #val(sum_prj_rcd.totdw_p)# + 
#val(sum_prj_rcd.totmc_p)# + 
#val(sum_prj_rcd.totnpl_p)# + 
#val(sum_prj_rcd.totot1_p)# +
#val(sum_prj_rcd.totot2_p)# + 
#val(sum_prj_rcd.totot3_p)# + 
#val(sum_prj_rcd.totot4_p)# + 
#val(sum_prj_rcd.totot5_p)# + 
#val(sum_prj_rcd.totot6_p)# > 
				<tr>
					...
					<td><input name="totot" id="tottotot" value="#tottotot#" size="5" readonly/></td>
					<td></td>
				</tr>
			<cfif #HcomID# eq "demo">	
				<cfset date_now1 = #dateformat(now(),'dd-mm-yyyy')# >
				<cfset date_all="">
				<cfset all_dw ="">
				<cfset all_mc ="">
				<cfset all_npl ="">
				...
				<cfset all_ot5 ="">
				<cfset all_ot6 ="">
				<cfquery name="prj_name1" datasource="#dts#">
						SELECT project,desp FROM project ;
					</cfquery>
				
				<br/><br/>	
				<tr><td>For all Employee</td></tr>		
				<tr>
					<th>PROJECT</th>
					<th>DATE</th>
					<th>DW</th>
				...
				<select name="projecttype" id="prjectytpe">
					<cfloop query="prj_name1">	
   					<option "value="#prj_name1.project#">#prj_name1.project#</option>
 					</cfloop>
 				</select>
			
				</td>	
					<td><input type="text" size="13" maxlength="10" name="date_all" id="date_all" value="#date_now1#"/>
					<img src="/images/cal.gif" width=17 height=15 border=0 onClick="showCalendarControl(document.getElementById('date_all'));"></td>
					<td><input name="all_dw" id="all_dw" value="#all_dw#" size="5" ></td>
					<td><input name="all_mc" id="all_mc" value="#all_mc#" size="5" ></td>
					<td><input name="all_npl" id="all_npl" value="#all_npl#" size="5" ></td>
					<td><input name="all_ot1" id="all_ot1" value="#all_ot1#" size="5" ></td>
					<td><input name="all_ot2" id="all_ot2" value="#all_ot2#" size="5" ></td>
					<td><input name="all_ot3" id="all_ot3" value="#all_ot3#" size="5" ></td>
					<td><input name="all_ot4" id="all_ot4" value="#all_ot4#" size="5" ></td>
					<td><input name="all_ot5" id="all_ot5" value="#all_ot5#" size="5" ></td>
					<td><input name="all_ot6" id="all_ot6" value="#all_ot6#" size="5" ></td>
					<td><input type="submit" name="sbt_all" id="sbt_all" value="Add to all" onClick="javascript:all_add();"></td>		
				</tr>
			</cfif>
				<tr>
				...
				<tr>
					...
					<input type="button" name="add" value="Add" size="5" onClick="javascript:ColdFusion.Window.show('Add_proj')" >
		...
		<div class="tabbertab" style="height:390px;" id="mytab7">
			<h3>Relief and Rebate</h3>
				<cfquery name="emprelief_qry" datasource="#dts#">
					SELECT * FROM emprelief where empno = <cfqueryparam cfsqltype="cf_sql_varchar" value="#URLDecode(url.empno)#">
				</cfquery>
				<table>
					<tr>
						<th width="100px">Relief Code</th>
						<th width="300px">Description</th>
						<th width="100px">Relief Date</th>
						<th width="100px">Relief Amount</th>
						<th width="100px">Action</th>			
					</tr>
					<cfloop query="emprelief_qry">
						<cfquery name="tax_relief_qry" datasource="#dts#">
							SELECT description from tax_relief where code = <cfqueryparam cfsqltype="cf_sql_varchar" value="#emprelief_qry.relief_code#">					
						</cfquery>
						<tr>
							<td><input type="type" name="relief_code" id="relief_code" value="#emprelief_qry.TRANID#" size="5" readonly></td>
							<td><input type="type" name="description" id="description" value="#tax_relief_qry.description#" size="5" readonly></td>
							<td><input type="type" name="date_relief" id="date_relief" value="#tax_relief_qry.daterelief#" size="5" readonly></td>
							<td><input type="type" name="relief_amt" id="relief_amt" value="#tax_relief_qry.reliefamt#" size="5" readonly></td>
							<td><a href="##"><img height="18px" width="18px" src="/images/edit.ICO" alt="Update" border="0">edit</a></td>			
						</tr>
					</cfloop>
					<tr>
						<th colspan="5"> Rebate(Zakat/Levy)Through TP1 From Declaration</th>
					</tr>
					<tr>
						<td> Zakat(TP1 Form)</td>
						<td><input type="text" name="tp1zakat" id="tp1zakat" size ="5" value="#numberformat(pay_qry.TP1zakat,'.__')#"></td>
						<td> Levy (TP1 Form)</td>
						<td><input type="text" name="tp1levy" id="tp1levy" size ="5" value="#numberformat(pay_qry.TP1levy,'.__')#"></td>
					</tr>
					<tr>
						<td>
						<input type="button" name="add" value="Add" size="5" onClick="javascript:ColdFusion.Window.show('Add_relief')" >
						</td>
					</tr>
				</table>
	...
	<center>
	    ...
	    <cfquery name="nex_list" datasource="#dts#">
	    	...
	    	and paystatus = "A" and confid >= #hpin# order by empno
	    </cfquery>
	    <cfquery name="prev_list" datasource="#dts#">
	   		SELECT * FROM pmast WHERE empno < "#URLDecode(url.empno)#" and paystatus = "A" and confid >= #hpin# ORDER BY empno DESC
	    </cfquery>
        ...
        source="searchEmployee.cfm"  modal="true"/>
		...
		<cfwindow x="210" y="100" width="570" height="480" name="Add_relief"
  		 minHeight="400" minWidth="400" 
   		 title="Add Tax Relief" initshow="false"
   		 source="Add_relief.cfm?empno=#URLEncodedFormat(pay_qry.empno)#"/>
