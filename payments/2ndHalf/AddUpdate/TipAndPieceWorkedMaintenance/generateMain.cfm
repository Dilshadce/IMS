...
<body>
    ...
    SELECT * FROM paytran Where tippoint > 0
    </cfquery>
   ...
   <cfset claimable_amount = #val(tip_amt[#i#])# * #val(claimable_ratio)# >
    ...
    SELECT SUM(tippoint) AS TipTotal FROM paytran
    </cfquery>
...
UPDATE paytran SET CLTIPOINT=#evaluate(tip_emp_amt_temp)# , TIPRATE = #form.tip_rate# WHERE empno = #evaluate(tip_emp_list_temp)#
</cfquery>
<cfset tip_amt = #evaluate(tip_emp_amt_temp)# * #val(form.tip_rate)# >
<cfquery name="update_tip" datasource="#dts#">
UPDATE paytran SET TIPAMT=#val(tip_amt)# , AW112 = #val(tip_amt)# WHERE empno = #evaluate(tip_emp_list_temp)#
</cfquery>
</cfloop>
