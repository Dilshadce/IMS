...
<cfcomponent>
    <cffunction name="listDetails" access="remote" returntype="struct"> 
        ...
        <cfquery name="getFilteredDataSet" datasource="#dts#">
            ...
            FROM
            (	SELECT name, add1, add2, dbirth, sex, race, Country_code_address, phone, email, workemail, mstatus, nricn, passprt_to, passport, edu, national, econtact, etelno, eadd1, eadd2,
            			itaxno, itaxbran, sname, sdisble, numchild, num_child, child_edu_m, child_edu_f, child_disable, child_edu_disable,
                        epfno, pbholiday, countryserve, wpermit, wp_from, wp_to, bankcode, bankaccno, bankbefname
            	FROM pmast
                WHERE empno = #empno#
            ) AS aa
