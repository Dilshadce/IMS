...
$(document).ready(function(e) {
	...
	var resultTable=$('#resultTable')
		.dataTable({
				...
				},
				{
					...
					'mRender':function(data,type,row){
						return 	'<span class="glyphicon glyphicon-pencil btn btn-link" '+
								'onclick="window.open(\'\/menulist\/maintenance\/target.cfm?target='+target+'&action=update&menuID='+escape(encodeURIComponent(menuID))+'&custno='+escape(encodeURIComponent(data))+'\',\'_self\');"></span>'+
								'<span class="glyphicon glyphicon-remove btn btn-link" '+
								'onclick="if(confirm(\'Are you sure you wish to delete this?\')){window.open(\'\/menulist\/maintenance\/'+redirect+'?action=delete&menuID='+escape(encodeURIComponent(menuID))+'&custno='+escape(encodeURIComponent(data))+'\',\'_self\');}"></span>';
					}
				}
        	],
			'bAutoWidth':false,
			...
			'sAjaxSource':'/menulist/maintenance/custSuppProfile.cfc',
			'sServerMethod':'POST',
        	'sScrollX':'100%'
		})
		.fadeIn();
