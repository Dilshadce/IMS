...
$(document).ready(function(e) {
	...
	$("#taxCode").select2({ width: '100%' });	
	$("#currcode").select2({ width: '100%' });			
	$("#enduser").select2({ width: '100%' });	
	$("#rating").select2({ width: '100%' });			
	$("#agent").select2({ width: '100%' });	
	$("#terms").select2({ width: '100%' });			
	$("#area").select2({ width: '100%' });	
	$("#business").select2({ width: '100%' });
...
function custnoExist(){
	$.ajax({
		'dataType':'json',
		'type':'POST',
		'url':'/menulist/maintenance/customer.cfc',
		'data':{
