	...
	var resultTable=$('#resultTable')
		.dataTable({
			...
			'aoColumnDefs':[
				...
				{'aTargets':[10],'sTitle':'BUSINESS TYPE','mData':'business','bSortable':true,'sWidth':'15%'},
				{
					'aTargets':[11],
					'sTitle':'ACTION',
					'mData':'custno',
					...
					'mRender':function(data,type,row){
						/*return '<span class="glyphicon glyphicon-pencil btn btn-link" '+
								'onclick="window.open(\'\/account\/account.cfm?&action=update&custno='+escape(encodeURIComponent(data))+'\',\'_self\');"></span>'+
								'<span class="glyphicon glyphicon-remove btn btn-link" '+
								'onclick="if(confirm(\'Are you sure you wish to delete this?\')){window.open(\'\/account\/customerProcess.cfm?action=delete&custno='+escape(encodeURIComponent(data))+'\',\'_self\');}"></span>';*/
							
							if(Dedit =='T' && Ddelete =='T'){
								return '<span class="glyphicon glyphicon-pencil btn btn-link" title="EDIT" '+
								'onclick="window.open(\'\/account\/customer.cfm?&action=update&custno='+escape(encodeURIComponent(data))+'\',\'_self\');"></span>'+
			...
			'fnServerParams':function(aoData){
				aoData.push(
					...
					{"name":"H1008","value":H1008},
					{"name":"acccreate","value":acccreate},
					{"name":"accagent","value":accagent}
				);
        	},
			'sAjaxSource':'/account/accountProfile.cfc',
			'sServerMethod':'POST',
        	'sScrollX':'100%'
		})
		.fadeIn();
