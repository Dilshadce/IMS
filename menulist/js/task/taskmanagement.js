	...
	var resultTable=$('#resultTable')
		.dataTable({
			...
			'aoColumnDefs':[
				...
				{'aTargets':[7],'sTitle':'STATUS','mData':'astatus','bSortable':false,'bSearchable':false,'sWidth':'10%'},
				{
					...
					'mRender':function(data,type,row){
						return 	'<span class="glyphicon glyphicon-flag btn btn-link" title="FINISH" '+
								'onclick="if(confirm(\'Are you sure you wish to finish this?\')){window.open(\'\/task\/taskProcess.cfm?action=update&taskid='+escape(encodeURIComponent(data))+'\',\'_self\');}"></span>';
					}
				}
        	],
			'bAutoWidth':false,
			...
			'sAjaxSource':'/task/taskmanagement.cfc',
			'sServerMethod':'POST',
        	'sScrollX':'100%'
		})
		.fadeIn();
