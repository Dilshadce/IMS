...
$(document).ready(function(e) {
	var resultTable=$('#resultTable')
		.dataTable({
				...
				{'aTargets':[6],'sTitle':'CREATED BY','mData':'created_by','bSortable':true,'sWidth':'11%'},
				{
					...
					'mRender':function(data,type,row){
						return 	'<span class="glyphicon glyphicon-pencil btn btn-link" '+
								'onclick="window.open(\'\/menulist\/generalSetup\/userMaintenance\/user.cfm?action=update&userid='+escape(encodeURIComponent(data))+'&companyID='+escape(encodeURIComponent(row.userbranch))+'\',\'_self\');"></span>'+
								'<span class="glyphicon glyphicon-remove btn btn-link" '+
								'onclick="if(confirm(\'Are you sure you wish to delete this user?\')){window.open(\'\/menulist\/generalSetup\/userMaintenance\/userProcess.cfm?action=delete&userid='+escape(encodeURIComponent(data))+'&companyID='+escape(encodeURIComponent(row.userbranch))+'\',\'_self\');}"></span>';
					}
				}
			...
			'sAjaxSource':'/menulist/generalSetup/userMaintenance/userAdministration2.cfc',
			'sServerMethod':'POST',
        	'sScrollX':'100%'
		})
		.fadeIn();
