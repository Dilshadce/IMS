...
<cfoutput>
    ...
    <!---<cfquery name="getreportto" datasource="#dts#">
        SELECT supervisor FROM placement
        WHERE placementno in (SELECT placementno FROM assignmentslip WHERE invoiceno=<cfqueryparam cfsqltype="cf_sql_varchar" value="#trim(getHeaderInfo.refno)#" >)
        GROUP BY supervisor
    </cfquery>

    <cfloop query="getreportto">
        <cfif reporttolist eq "">
            <cfset reporttolist = getreportto.supervisor>   
        <cfelse>
            <cfset reporttolist = reporttolist&", "&getreportto.supervisor>   
        </cfif>
    </cfloop>--->
