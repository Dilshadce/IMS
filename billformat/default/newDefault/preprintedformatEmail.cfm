...
<cfquery name="getHeaderInfo" datasource='#dts#' >
	SELECT 	a.type,a.custno,a.refno,a.refno2,a.wos_date,a.desp,a.despa,a.term,a.pono,a.dono,a.sono,
            ...
            a.phonea,a.d_phone2,a.postalcode,a.e_mail,a.d_email,   
	...
	WHERE a.type = '#tran#' 
    AND <cfif IsDefined('url.nexttranno')> 
    		a.refno = '#url.nexttranno#' 
        <cfelse> 
        	a.refno IN (<cfqueryparam cfsqltype="cf_sql_varchar" value="#form.sono#" list="yes">)
        </cfif>
</cfquery>
...
<cfelse>
    <cfquery name="updatePrinted" datasource="#dts#">
        ...
        AND type =<cfqueryparam cfsqltype="cf_sql_varchar" value="#url.tran#" >l
    </cfquery>
</cfif>
<cfset j = 1>
<cfset k = 0>
...
<cfloop query="getHeaderInfo">
	...
	<cfelse>
        ...
        <cfset getDeliveryAdd.postalcode = getHeaderInfo.postalcode>
	...
	<cfquery name="InsertICBil_M" datasource="#dts#">
	  	INSERT INTO r_icbil_m ( B_Name, B_Name2, B_Add1, B_Add2, B_Add3, B_Add4, B_Add5, B_Attn, B_Tel, B_HP, B_Fax,
        						D_Name, D_Name2, D_Add1, D_Add2, D_Add3, D_Add4, D_Add5, D_Attn, D_Tel, D_HP, D_Fax,
	  	...
	  	VALUES (
                    ...
                    <cfqueryparam cfsqltype="cf_sql_varchar" value="#getHeaderInfo.rem13#">,
                    ...
                    <cfqueryparam cfsqltype="cf_sql_varchar" value="#getHeaderInfo.rem14#">,
	...
	<cfquery name="getBodyInfo" datasource="#dts#">
		SELECT  a.refno,a.itemno,a.desp,a.despa,a.comment,
        		a.unit_bil,a.qty_bil,a.price_bil,a.amt_bil,a.dispec1,a.dispec2,a.dispec3,a.disamt_bil,a.taxpec1,a.taxpec2,a.taxpec3,
                a.price,a.amt,
                a.brem1,a.brem2,a.brem3,a.brem4,
	...
	<cfloop query="getBodyInfo">
		...
		<cfquery name="getserial" datasource="#dts#">
            ...
            and type = '#getBodyInfo.type#' 
	    	and itemno = '#getBodyInfo.itemno#' 
            and trancode = '#getBodyInfo.trancode#';
	  	</cfquery>
		...
		<cfquery name="InsertICBil_S" datasource='#dts#'>
			INSERT INTO r_icbil_s (	no, sRefNo, itemNo, aitemno, desp, despa, sn_no, comment, 
									unit, qty, price, amt, dispec1, dispec2, dispec3, itemdis_bil, taxpec1, taxpec2, taxpec3,
									sgd_price, sgd_amt,   
...
<cfelseif IsDefined('url.tax')>
	<cfset fname="Template_2/"&tran&".cfr">
<cfelse>
	<cfset fname="Template_1/"&tran&".cfr">    
</cfif>

<cfset filename1="#dts#_"&getheaderinfo.refno&".pdf">

<cfreport template="#fname#" format="PDF" query="MyQuery" filename="#filename1#" overwrite="yes">    	
...
<cfoutput>
    ...
    <cfquery name="customerSupplierInfo" datasource="#dts#">
        SELECT a.custno, b.name, b.e_mail
        FROM artran AS a
        LEFT JOIN #targetTable# AS b ON a.custno = b.custno
        WHERE a.refno = '#getheaderinfo.refno#'
        AND a.type = '#tran#'
    </cfquery>
    
    <cfquery name="userEmail" datasource="#dts#">
        SELECT userEmail
        FROM main.users
        WHERE userID = '#HuserID#'
    </cfquery>
    
    <cfset clientList = customerSupplierInfo.e_mail>
    <cfset userList = userEmail.userEmail>
    <cfif IsDefined('form.emailSubject')>
   		<cfset emailSubject = form.emailSubject>
    <cfelse>
    	<cfset emailSubject = ''>
    </cfif>
    <cfif IsDefined('form.emailDetail')>
    	<cfset emailDetail = form.emailDetail>
    <cfelse>
    	<cfset emailDetail = ''>
    </cfif>    

    <!---Abort the email if no email found --->
    <cfif clientList EQ ''>
        <script>
            alert('Customer/Supplier email not found!')
            window.history.back();
        </script>
        <cfabort>	
    </cfif>
    ...
    <cfmail 
        ...
        <cfmailparam file = "\billformat\default\newDefault\#filename1#" > 
        #emailDetail#
    </cfmail> 
</cfoutput>
