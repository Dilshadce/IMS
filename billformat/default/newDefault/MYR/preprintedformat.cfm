...
<cfquery name="getHeaderInfo" datasource='#dts#' >
	SELECT 	a.type,a.custno,a.refno,a.refno2,a.wos_date,a.desp,a.despa,a.term,a.pono,a.dono,a.sono,a.quono,a.depositno,
            ...
            a.rem30,a.rem31,a.rem32,a.rem33,a.rem34,a.rem35,a.rem36,a.rem37,a.rem38,a.rem39,a.dono,
            ...
            a.taxp1,a.tax,a.tax_bil,a.tax1_bil,a.tax2_bil,a.tax3_bil,a.discount,
            a.invgross,a.gross_bil,a.net,a.net_bil,a.grand,a.grand_bil,a.roundadj+mc1_bil as roundadj,a.currrate,
            a.phonea,a.d_phone2,a.e_mail,a.d_email,   
            a.name,a.userid,a.username,a.created_by,a.consignment,a.returndate,a.returnbillno,a.returnreason,
            ...
            dr.driverno AS driverNo,dr.name AS driverName,dr.photo AS driverSignature,dr.photo as driverphoto
    ...
    LEFT JOIN (SELECT * FROM #target_project# WHERE porj="P") as proj ON a.source = proj.source
    LEFT JOIN (SELECT * FROM #target_project# WHERE porj="J") as job ON a.job = proj.source
...
<cfloop query="getHeaderInfo">
		...
		<cfif getHeaderInfo.rem0 EQ "Profile">
            <cfquery name="getCustAdd" datasource='#dts#'>
                SELECT 	name,name2,add1,add2,add3,add4,country,postalcode,
                        attn,phone,phonea,fax,e_mail,gstno 
                FROM #ptype# 
		...
		<cfelse>
			<cfquery name="getCustAdd" datasource='#dts#'>
				SELECT 	a.name,'' AS name2,a.add1,a.add2,a.add3,a.add4,a.country,a.postalcode,
                		a.attn,a.phone,a.phonea,a.fax,b.e_mail,a.gstno
                FROM address a			
		...
		<cfif getHeaderInfo.rem1 eq "Profile">
            <cfquery name="getDeliveryAdd" datasource='#dts#'>
                SELECT  name,name2,daddr1 AS add1,daddr2 AS add2,daddr3 AS add3,daddr4 AS add4,d_country AS country,d_postalcode AS postalcode,
                        dattn AS attn,dphone AS phone,contact AS phonea,dfax AS fax,d_email,gstno
                FROM #ptype#
                WHERE custno = '#getHeaderInfo.custno#';
            </cfquery>
		<cfelse>
			<cfquery name="getDeliveryAdd" datasource='#dts#'>
				SELECT 	a.name,'' as name2,a.add1,a.add2,a.add3,a.add4,a.country,a.postalcode,
                		a.attn,a.phone,b.contact AS phonea,a.fax,b.d_email,a.gstno
				FROM address a			
				LEFT JOIN #ptype# AS b ON a.custno = b.custno
    ...
    <cfif getCustAdd.GSTno EQ '' OR getDeliveryAdd.GSTno EQ ''>
    ...
    <cfif getHeaderInfo.driverSignature NEQ "">
		<cfset driverSign = '/billformat/#dts#/#getHeaderInfo.driverSignature#'>
	<cfelse>
        <cfset driverSign = ''>
	</cfif>
	...
	<cfquery name="InsertICBil_M" datasource="#dts#">
	  	INSERT INTO r_icbil_m ( B_Name, B_Name2, B_Add1, B_Add2, B_Add3, B_Add4, B_Country, B_PostalCode, B_Attn, B_Tel, B_HP, B_Fax, B_Email, B_GSTno,
                                ...
                                UserName, EndUser, EndUserSignature, TermsCondition,
	  	...
	  	VALUES (
                    ...
                    <cfqueryparam cfsqltype="cf_sql_varchar" value="#driverSign#">,
    ...
    <cfelse>
		...
		<cfif tran eq "INV" and getserial.recordcount eq 0>
		<cfquery name="getserial" datasource="#dts#">
                    SELECT * 
                    FROM iserial 
                    WHERE refno = '#getBodyInfo.dono#' 
                    AND type = 'DO' 
                    AND itemno = '#getBodyInfo.itemno#' 
                </cfquery>
		</cfif>
                ...
                <!--- Used: By 3rd Template --->
                <!--- Function: Allow users to show TITLE --->
                <!--- Input: Under Item > Title Desp Box --->
                <cfif getBodyInfo.title_desp NEQ ''>
                    <cfquery name="InsertICBil_S" datasource='#dts#'>
                        INSERT IGNORE INTO r_icbil_s (	no, sRefNo, itemNo, titledesp, 
                        ...
                        VALUES ( 	<cfqueryparam cfsqltype="cf_sql_decimal" value="#j#">,
                                    ...
                                    <cfqueryparam cfsqltype="cf_sql_varchar" value="#getBodyInfo.title_desp#">,
...
<cfoutput>
    ...
    <cfif getheaderinfo.driverphoto neq "">
		<cfset driverlogo='/billformat/#dts#/#getheaderinfo.driverphoto#'>
        <cfelse>
        <cfset driverlogo=''>
	</cfif>
...
<cfif IsDefined('url.tax')>
    <cfif trim(url.tax) EQ '2'>
...
<cfif IsDefined('url.GST')>
	<cfset GST = url.GST>    
</cfif>

<cfif GST eq "N" and (tran eq "INV" or tran eq "CS" or tran eq "QUO")>
	<cfif trim(url.tax) EQ '2'>
    	<cfset fname = "Template_"&templateID&"/Template_ItemTax/"&tran&"nongst.cfr">   
   	<cfelse>
    	<cfset fname = "Template_"&templateID&"/Template_BillTax/"&tran&"nongst.cfr">  	    
    </cfif>  
</cfif>

<!--- Kastam Diraja Malaysia Required Fields --->
<cfquery name="getGSTsummary" datasource="#dts#">
	SELECT note_a,taxpec1,
		   ROUND(SUM(if(taxincl = 'T',amt-taxamt,amt)),2) AS sumAmt,
		   ROUND(SUM(taxamt),2)
...
<cfif getheaderinfo.returnbillno neq "">
    <cfset getheaderinfo.rem49="#getHeaderInfo.returnbillno#">
</cfif>

<cftry>
<cfif getheaderinfo.rem49 eq "">
<cfset getheaderinfo.rem49="#getbodyinfo.invlinklist#">
</cfif>
<cfcatch>
</cfcatch>
</cftry>
...
<cfif getfunctioncontrol.billlocationadd eq "Y" and lcase(huserloc) neq "all_loc">

<cfquery name="getlocationadd" datasource="#dts#">
	SELECT * FROM iclocation where location=<cfqueryparam cfsqltype="cf_sql_varchar" value="#huserloc#"> 
</cfquery>

<cfset getGsetup.compro=getlocationadd.desp>
<cfset getGsetup.compro2=getlocationadd.addr1>
<cfset getGsetup.compro3=getlocationadd.addr2>
<cfset getGsetup.compro4=getlocationadd.addr3>
<cfset getGsetup.compro5=getlocationadd.addr4>
...
<cfreport template="#HRootPath#/billformat/default/newdefault/MYR/#fname#" format="PDF" query="MyQuery">    	
    ...
    <cfif getheaderinfo.currrate eq 1>
    <cfreportparam name="currencyCodeControl" value="MYR">
    <cfelse>
    ...
    <cfif getheaderinfo.returnreason neq "">
    ...
    <cfif getheaderinfo.returndate neq "" and isdate(getheaderinfo.returndate)>
    <cftry>
    ...
    <cfcatch>
    	<cfif Find(".", getheaderinfo.returndate) EQ 0>
	...
	<cfreportparam name="localGross" value="#getHeaderInfo.invgross#">
    <cfreportparam name="localDisamt" value="#getHeaderInfo.discount#">
    <cfreportparam name="localNet" value="#getHeaderInfo.net#">
    <cfreportparam name="localTaxamt" value="#getHeaderInfo.tax#">
    <cfreportparam name="localTotal" value="#getHeaderInfo.grand#">
    <cfreportparam name="custSuppGSTno" value="#getCustSuppGSTinfo.gstno#">
    <!---<cfreportparam name="custSuppGSTapprovalNo" value="#getCustSuppGSTinfo.gstappno#">
    <cfif IsDate(getCustSuppGSTinfo.gst_app_datefrom)>
        <cfreportparam name="custgstdate" value="#DateFormat(getCustSuppGSTinfo.gst_app_datefrom,'dd/mm/yyyy')#">
    <cfelse>
        <cfreportparam name="custgstdate" value="#getCustSuppGSTinfo.gst_app_datefrom#">
    </cfif>
    <cfif IsDate(getCustSuppGSTinfo.gst_app_dateto)>
        <cfreportparam name="custgstdate" value="#DateFormat(getCustSuppGSTinfo.gst_app_dateto,'dd/mm/yyyy')#">
    <cfelse>
        <cfreportparam name="custgstdate" value="#getCustSuppGSTinfo.gst_app_dateto#">
    </cfif>    --->   
    <cfreportparam name="GSTcode" value="#ValueList(getGSTsummary.note_a)#">
    <cfreportparam name="GSTtaxPercentage" value="#ValueList(getGSTsummary.taxpec1)#">
    <cfreportparam name="GSTamt" value="#ValueList(getGSTsummary.sumAmt)#">
    <cfreportparam name="GSTtaxAmt" value="#ValueList(getGSTsummary.taxAmt)#">
    <cfreportparam name="GST" value="#GST#">
    <cfreportparam name="consignment" value="#getheaderinfo.consignment#">
    <cfreportparam name="fH" value="30px">
    <cfreportparam name="driverlogo" value="#driverlogo#">
