<!---<cfif Huserid EQ "ultrakhsiung">
	<cfoutput>#url.nexttranno#</cfoutput>
    <cfabort>
</cfif>--->
...
<cfif getGsetup.wpitemtax NEQ 1>
	<cftry>
        ...
        <cfquery name="mySelect" datasource="#dts#">
            SELECT type, refno, taxincl, net_bil, taxp1, grand_bil, tax1_bil, tax_bil 
            FROM artran 
            WHERE type = '#tran#' 
                  AND <cfif IsDefined('url.nexttranno')> 
                  		 refno='#url.nexttranno#'; 
        ...
        <cfloop query="mySelect">
            <cfif mySelect.taxincl EQ "T">
                <cfset grandBil = mySelect.net_bil>
                <cfset tax1Bil = mySelect.net_bil * mySelect.taxp1/(100+mySelect.taxp1)> 
                <cfset taxBil = mySelect.net_bil * mySelect.taxp1/(100+mySelect.taxp1)>
            <cfelse>
                <cfset grandBil = mySelect.net_bil + (mySelect.net_bil * mySelect.taxp1/100)>
                <cfset tax1Bil = mySelect.net_bil * mySelect.taxp1/100> 
                <cfset taxBil = mySelect.net_bil * mySelect.taxp1/100>
            </cfif>
            
            <cfquery name="updateGrand2_artran" datasource="#dts#" result="test">
                Update artran 
                SET grand_bil = if(taxincl = "T",#grandBil#,round(#grandBil#,#getdecimal.Decl_totalamt#)),
                    tax1_bil = if(taxincl = "T",round(#tax1Bil#,#getdecimal.Decl_totalamt#), round(#tax1Bil#,#getdecimal.Decl_Uprice#)),
                    tax_bil = if(taxincl = "T",round(#taxBil#,#getdecimal.Decl_totalamt#), round(#taxBil#,#getdecimal.Decl_totalamt#))
                WHERE type = '#mySelect.type#' and refno='#mySelect.refno#'
            </cfquery>
        </cfloop>
...
<cfquery name="getGsetup2" datasource='#dts#'>
  	SELECT CONCAT('.',REPEAT('_',Decl_Uprice)) AS Decl_Uprice,CONCAT('.',REPEAT('_',Decl_Discount)) AS Decl_Discount, CONCAT('.',REPEAT('_',Decl_totalamt)) AS Decl_Totalamt  
    FROM gsetup2;
</cfquery>
...
<cfloop query="getHeaderInfo">
	...
	<cfquery name="InsertICBil_M" datasource="#dts#">
	  	...
	  	VALUES (
                    ...
                    <cfif LCase(Hcomid) EQ "emjay_i" OR LCase(Hcomid) EQ "emjaytest_i">
						<cfif Val(getHeaderInfo.net_bil) EQ Val(getHeaderInfo.grand_bil) AND getHeaderInfo.taxIncl EQ "F"> 
                            <cfqueryparam cfsqltype="cf_sql_double" value="#val(getHeaderInfo.net_bil)-val(getHeaderInfo.tax_bil)#">,
                        <cfelse>
...
<!---<cfif Huserid EQ "ultraikmei">
    <cfoutput>#fname#</cfoutput>
    <cfabort>
</cfif>
--->
<cfreport template="#HRootPath#/billformat/default/newdefault/#fname#" format="PDF" query="MyQuery">    	
