...
<cfquery name="getPB" datASource="#dts#">
	...
	ON a.empno = b.empno
	WHERE date_format(a.requested_on, '%Y-%m-%d')  = date_format(date_add(now(), interval -1 day),'%Y-%m-%d')
	...
	where date_format(a.requested_on, '%Y-%m-%d') >= date_format(date_add(now(), interval -5 day),'%Y-%m-%d')
