		...
		<cfquery name="updateintopaytm" datasource="#db#">
        	UPDATE pay_tm SET
		      <!---  HRD_PAY = #val(hrd_pay)# ,--->
				...
				SOCSOWW = IF(#val(ps.SOCSOWW)+val(ps1.SOCSOWW)# >= 19.75, 19.75, #val(ps.SOCSOWW)+val(ps1.SOCSOWW)# ),
				SOCSOCC = IF(#val(ps.SOCSOCC)+val(ps1.SOCSOCC)# >= 69.05 or #val(ps.SOCSOWW)+val(ps1.SOCSOWW)# >= 19.75 , 69.05, #val(ps.SOCSOCC)+val(ps1.SOCSOCC)# ),
                EISWW = IF(#val(ps.EISWW)+val(ps1.EISWW)# >= 7.90, 7.90, #val(ps.EISWW)+val(ps1.EISWW)# ),
				EISCC = IF(#val(ps.EISCC)+val(ps1.EISCC)# >= 7.90, 7.90, #val(ps.EISCC)+val(ps1.EISCC)# ),
