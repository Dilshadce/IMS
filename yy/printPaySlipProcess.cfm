...
<cfquery name="getEntity" datasource="#dts#">
	...
	<cfscript>
		...
		totalEarnings = 0;
		 for(i = 1; i <= ArrayLen(earnings_amt); i++){
		 	earnings_amt[i] = numberformat(earnings_amt[i],'9.99');
		 	totalEarnings += earnings_amt[i];
		 }

		 totalDeductions = 0;
		 for(i=1; i <= ArrayLen(deductions_amt); i++){
		 	earnings_amt[i] = numberformat(deductions_amt[i],'9.99');
	...
	<cfreport template="payslip.cfr" format="PDF" userpassword="test">
